...
/// the `grow/tool` `_meta` object. Equivalent tools across toolsets emit the
...
mod tests {
    ...
    fn canonical_omits_absent_options_not_null() {
        let grow = parse(serde_json::json!({"variant":"ReadFile","target_file":"/a"}));
        let g = canonical_input(&grow).unwrap();
