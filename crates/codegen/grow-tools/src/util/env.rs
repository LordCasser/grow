...
//! heavyweight `grow-tools` dependency. This module re-exports the public
...
/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${GROW_PLUGIN_ROOT}`
/// and `${CLAUDE_PLUGIN_DATA}` / `${GROW_PLUGIN_DATA}`) in `s`. Each pair is expanded
...
mod tests {
    ...
    const ALL_TOKENS: &str = "${CLAUDE_PLUGIN_ROOT}/a ${GROW_PLUGIN_ROOT}/b ${CLAUDE_PLUGIN_DATA}/c ${GROW_PLUGIN_DATA}/d";
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${GROW_PLUGIN_DATA}/d"
        );
    }
