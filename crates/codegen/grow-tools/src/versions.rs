...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"Grow:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"Grow:run_terminal_cmd"`).
...
/// between namespaces (e.g. `Grow:run_terminal_cmd` vs.
/// `GrowConcise:run_terminal_cmd`).
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("Grow:run_terminal_cmd", "legacy-0.4.10"),
            ("Grow:read_file", "legacy-0.4.10"),
            ("Grow:search_replace", "legacy-0.4.10"),
            ("Grow:get_task_output", "legacy-0.4.10"),
            ("Grow:kill_task", "legacy-0.4.10"),
            ("Grow:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("Grow:run_terminal_cmd", "current"),
            ("Grow:read_file", "current"),
            ("Grow:search_replace", "current"),
            ("Grow:list_dir", "current"),
            ("Grow:grep", "current"),
            ("Grow:kill_task", "current"),
            ("Grow:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "Grow:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "Grow:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "Grow:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn per_tool_override_wins() {
        let v = resolve_version("current", "Grow:run_terminal_cmd", Some("legacy-0.4.10")).unwrap();
        assert_eq!(v, Some("legacy-0.4.10".to_string()));
    }
    ...
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "Grow:run_terminal_cmd", None).unwrap_err();
        assert!(err.contains("unknown behavior_preset"));
    }

    #[test]
    fn unknown_override_version_errors() {
        let err = resolve_version("current", "Grow:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // GrowConcise tools should NOT be version-managed.
        assert!(!is_version_managed("GrowConcise:run_terminal_cmd"));
        let v = resolve_version("current", "GrowConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("Grow:run_terminal_cmd"));
        assert!(is_version_managed("Grow:read_file"));
        assert!(is_version_managed("Grow:search_replace"));
        assert!(is_version_managed("Grow:list_dir"));
        assert!(is_version_managed("Grow:grep"));
        assert!(is_version_managed("Grow:kill_task"));
        // Not managed:
        assert!(!is_version_managed("Grow:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res = resolve_version_with_warnings("current", "Grow:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "Grow:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) = validate_and_resolve("Grow:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("Grow:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "Grow:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
