...
//!
//! `MemoryBackend` is defined in `grow-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `grow-tools`.
/// Mirrors `grow_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "grow_memory";
