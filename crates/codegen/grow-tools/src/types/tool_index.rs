...
//!
//! `ToolSearchIndex` is defined in `grow-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `grow-shell`
...
/// in `grow-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `grow-shell` after MCP initialization.
#[derive(Clone)]
