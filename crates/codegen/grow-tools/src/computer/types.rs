...
///
/// Serialized over the wire in the `grow/task/kill` ext response
/// (`grow-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (grow-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in grow-tools, spawns processes)
/// - `AcpTerminalBackend` (in grow-shell, calls ACP protocol)
#[async_trait::async_trait]
