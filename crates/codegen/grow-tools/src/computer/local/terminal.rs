...
impl LocalTerminalActor {
    ...
    async fn spawn_static_command(
        ...
        #[cfg(target_os = "linux")]
        if grow_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| grow_sandbox::child_net::install_child_network_filter());
            }
        }
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::grow_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
#[cfg(unix)]
const ENV_LOGIN_ENV: &str = "GROW_LOGIN_ENV";
...
/// active policy filters out and those already set in grow's own environment.
#[cfg(unix)]
fn layer_login_env_vars(
    ...
    if let Some(login) = login_env {
        for (key, value) in login {
            // `var_os` reads grow's own process env (not the possibly cleared
            // child env): a login var already present in grow's environment is
...
/// policy base, login-shell capture, grow control vars, request env, pager
...
/// name cannot re-enter; grow's own control vars, login `PATH`, and the marker
...
/// path must inherit grow's environment untouched (including non-UTF-8 vars).
...
fn apply_child_env(
    ...
    // 2. Login-shell capture (filtered). 3. Grow control vars. 4. Request env
    ...
    crate::util::apply_grow_agent_marker(cmd);
}
...
fn spawn_shell_command(
    ...
    let mut cmd = {
        ...
        // The parent (grow) process retains network for the LLM API.
    ...
    let mut build_cmd = |with_breakaway: bool| {
        ...
        let inv = grow_config::shell::shell_command_argv(command);
        ...
        // `inv.env` is grow's trusted shell setup, so it is not filtered.
        ...
        cmd.envs(crate::util::pager_env());
        crate::util::apply_grow_agent_marker(&mut cmd);
        ...
        //     have JOB_OBJECT_LIMIT_BREAKAWAY_OK set — common when grow-agent
...
mod tests {
    ...
    fn apply_child_env_layers_in_fixed_order() {
        ...
        let policy = ShellEnvironmentPolicy {
            ...
            set: HashMap::from([("GROW_TEST_BASE".to_string(), "1".to_string())]),
            ..Default::default()
        };
        let login = HashMap::from([
            ("GROW_TEST_LOGIN".to_string(), "l".to_string()),
            ("PATH".to_string(), "/login/bin".to_string()),
        ]);
        let request = HashMap::from([
            ("GROW_TEST_REQ".to_string(), "r".to_string()),
            ("PATH".to_string(), "/req/bin".to_string()),
            ("GROW_TEST_SECRET".to_string(), "s".to_string()),
        ]);
        ...
        assert_eq!(env.get("GROW_TEST_BASE").map(String::as_str), Some("1"));
        assert_eq!(env.get("GROW_TEST_LOGIN").map(String::as_str), Some("l"));
        assert_eq!(env.get("GROW_TEST_REQ").map(String::as_str), Some("r"));
        // Request env is filtered by the policy.
        assert!(!env.contains_key("GROW_TEST_SECRET"));
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export GROW_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $GROW_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/grow-sentinel-tty".to_string());
