...
//! config helper `grow-shell::util::config::resolve_search_tools_enabled`
...
//! (release builds, self-extracted to `~/.grow/vendor/<name>-<ver>-<target>`) →
//! `~/.grow/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
//! function (which tags itself with a `__grow_shadow_{name}` marker) or a
...
// Binaries embedded by build.rs when `GROW_TOOLS_BUNDLE_{BFS,UGREP}_PATH` is set
// (release pipeline). Self-extracted to `~/.grow/vendor` on first use, mirroring
// the ripgrep bundling in `grow_build::grep::ripgrep`.
#[cfg(bundle_bfs)]
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("GROW_TOOLS_BFS_VER"),
    "-",
    env!("GROW_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("GROW_TOOLS_UGREP_VER"),
    "-",
    env!("GROW_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
/// OS binary again. Gated on the `__grow_shadow_{name}` marker that
...
fn restore_command(name: &str) -> String {
    format!(
        "if [ -n \"${{__grow_shadow_{name}-}}\" ]; then \
           unalias {name} 2>/dev/null || true; \
           unset -f {name} 2>/dev/null || true; \
           unset __grow_shadow_{name} 2>/dev/null || true; \
         fi"
    )
}
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "GROW_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "GROW_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}

/// Write embedded `bytes` to `~/.grow/vendor/<versioned_name>` (chmod 755) on
...
fn extract_bundled(versioned_name: &str, bytes: &[u8]) -> std::io::Result<PathBuf> {
    ...
    let dir = crate::util::grow_home().join("vendor");
...
fn resolve_tool(bin_name: &str, env_override: &str, bundled: Option<PathBuf>) -> Option<PathBuf> {
    resolve_tool_from(
        ...
        bundled,
        crate::util::grow_home().join("vendor").join(bin_name),
        bin_name,
    )
}

/// Resolution order: explicit env path → bundled (self-extracted) →
/// `~/.grow/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
/// `__grow_shadow_{name}=1` marks this as a harness shadow so `restore_command`
...
fn shell_function(
    ...
    // `local __grow_bin` is re-resolved every call. The host hint is trusted
    ...
    // binary. `|| __grow_bin=''` keeps the lookup `set -e`-safe (a failed
    ...
    format!(
        "unalias {name} 2>/dev/null || true; \
         {name}() {{ \
           local __grow_bin={qpref}; \
           [ -x \"$__grow_bin\" ] || __grow_bin=$(command -v {bin_name} 2>/dev/null) || __grow_bin=''; \
           if [ -z \"$__grow_bin\" ]; then command {name} \"$@\"; return; fi; \
           if [[ -z ${{ZSH_VERSION-}} ]] && (( BASH_SUBSHELL > 0 )); then \
             exec -a {name} \"$__grow_bin\" {prepend}\"$@\"; \
           else \
             (exec -a {name} \"$__grow_bin\" {prepend}\"$@\"); \
           fi; \
         }}; \
         __grow_shadow_{name}=1"
    )
}
...
mod tests {
    ...
    fn shell_function_shape() {
        ...
        // Preferred path is the fast-path hint; the shadow execs `$__grow_bin`.
        assert!(fn_body.contains("local __grow_bin=/tmp/bfs"));
        assert!(fn_body.contains("exec -a find \"$__grow_bin\" \"$@\""));
        // Hint is trusted only when executable (`[ -x ]`, not `[ -f ]`), so a
        // non-exec hint falls through instead of hard-failing exec.
        assert!(fn_body.contains("[ -x \"$__grow_bin\" ]"));
        assert!(!fn_body.contains("[ -f \"$__grow_bin\" ]"));
        ...
        assert!(fn_body.contains("__grow_shadow_find=1"));
    ...
    fn shell_function_unresolved_uses_empty_hint() {
        ...
        assert!(fn_body.contains("local __grow_bin=''"));
        assert!(fn_body.contains("command -v bfs"));
        assert!(fn_body.contains("command find \"$@\""));
    }
    ...
    fn grep_prepends_ugrep_defaults() {
        ...
        assert!(fn_body.contains("local __grow_bin=/tmp/ugrep"));
        assert!(fn_body.contains("\"$__grow_bin\" -G --ignore-files --hidden -I"));
    ...
    fn bash_safe_quote_escapes_metacharacters() {
        ...
        assert!(body.contains("local __grow_bin='/tmp/evil$(id)'"), "{body}");
    ...
    fn restore_command_is_marker_gated() {
        ...
        assert!(r.contains("if [ -n \"${__grow_shadow_find-}\" ]"));
        assert!(r.contains("unalias find"));
        assert!(r.contains("unset -f find"));
        assert!(r.contains("unset __grow_shadow_find"));
    }
    ...
    fn build_injection_off_emits_marker_gated_restore_not_function() {
        ...
        assert!(inject.contains("if [ -n \"${__grow_shadow_find-}\" ]"));
        assert!(inject.contains("if [ -n \"${__grow_shadow_grep-}\" ]"));
    ...
    fn build_injection_on_shadows_resolved_tools() {
        ...
        assert!(inject.contains("__grow_shadow_find=1"));
    }
    ...
    fn resolve_tool_precedence() {
        ...
        let dir = std::env::temp_dir().join(format!("grow-resolve-{}-{:?}", std::process::id(), {
    ...
    /// `GROW_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.grow/vendor` and the extracted `bfs` runs.
    #[cfg(all(bundle_bfs, bundle_ugrep))]
    #[test]
    fn bundled_binaries_extract_and_run() {
        let vendor = crate::util::grow_home().join("vendor");
    ...
    /// mode-0644 `GROW_TOOLS_*_PATH` / vendor copy) must fall through to the OS
