...
pub use grow_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__GROW_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    grow_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = grow_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias grow_alias_probe='echo ALIAS_OK'\ngrow_fn_probe() { echo FN_OK; }\n",
            "grow_alias_probe && grow_fn_probe",
        )
        .await;
