...
crate::register_resource!("grow_build", "TaskWakeSuppressed", TaskWakeSuppressed);
...
/// events from the structured `grow/monitor_event` notification, never by
/// parsing this text.
///
...
/// - `grow-shell`'s `SessionActor` to sweep matching synthetic
            ...
            let surface_reminders = !goal_loop_active
                && res
                    .get::<crate::types::resources::Params<
                        crate::implementations::grow_build::bash::BashParams,
                    >>()
...
mod tests {
    ...
    async fn completions_suppressed_when_goal_loop_active() {
        ...
        shared
            .lock()
            .await
            .insert(crate::implementations::grow_build::task::types::GoalLoopActive(false));
        let second = reminder.collect_reminders(shared, &output).await;
        assert!(
            second.is_empty(),
    ...
    fn format_monitor_events_single_vs_batched() {
        ...
        let bare = crate::implementations::grow_build::monitor::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::grow_build::monitor::event::wrap_monitor_event;
