//! Conversion from the gRPC wire config types (`grow-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`grow_tools_api::config_validation`] so every consumer (this
...
pub use grow_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`grow_tools_api::ToolConfigEntry`] to a runtime
/// [`ToolConfig`].
///
...
pub fn tool_config_from_entry(
    index: usize,
    entry: grow_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let grow_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params =
        grow_tools_api::config_validation::parse_params_json(index, &id, params_json.as_deref())?;
    grow_tools_api::config_validation::validate_name_override(
        index,
        &id,
...
pub fn tool_server_config_from_entries(
    entries: Vec<grow_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> grow_tools_api::ToolConfigEntry {
        grow_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
    ...
    fn minimal_entry_converts_with_defaults() {
        let cfg = tool_config_from_entry(0, entry("Grow:read_file")).unwrap();
        assert_eq!(cfg.id, "Grow:read_file");
    ...
    fn name_override_valid_tool_id_charset_is_accepted() {
        for name in ["search", "Grow:grep", "a-b_C9"] {
            let mut e = entry("Grow:grep");
    ...
    fn name_override_outside_tool_id_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "emoji✨", "dot.name"] {
            ...
            assert_eq!(err.tool_id, "Grow:grep");
    ...
    fn server_config_conversion_preserves_valid_name_overrides() {
        let mut a = entry("Grow:grep");
        a.name_override = Some("search".to_owned());
        let cfg = tool_server_config_from_entries(vec![a, entry("Grow:bash")]).unwrap();
