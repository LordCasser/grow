...
use grow_config::shell::AmpersandSemantics;
use regex::Regex;
...
pub struct BashParams {
    ...
    /// grow-build opts up to 10h via grow-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "grow_build.Bash";
...
// particular production grow-build sets it to 10h in grow-shell's
...
/// [`grow_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `GROW_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("GROW_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in grow_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the grow_build
...
impl BashTool {
    ...
    /// (xai-grow-cursor) uses this ceiling to report the true FG wait in
    ...
    ///   the terminal backend default applies (`GROW_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `GROW_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = grow_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = grow_config::shell::ampersand_semantics();
        ...
        if input.is_background {
            ...
            // We tag it for statistics (grow_build backend) and can surface an educational
...
mod tests {
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // grow-shell opts up to 10h via max_timeout_secs. Background unaffected.
