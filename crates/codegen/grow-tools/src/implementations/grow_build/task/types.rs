...
/// defense-in-depth check in `grow-shell`'s subagent coordinator.
...
///
/// Set by grow-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by grow-shell at turn start.
