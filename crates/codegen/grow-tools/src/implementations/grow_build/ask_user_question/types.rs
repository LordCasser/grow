...
//!
//! - **`grow-tools`** — tool blocks on a oneshot, formats the result.
//! - **`grow-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`grow-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `grow-tools`.
...
register_resource!("grow_build", "UserQuestionSender", UserQuestionSender);
