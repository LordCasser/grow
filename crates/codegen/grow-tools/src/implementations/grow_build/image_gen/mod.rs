//! `image_gen` tool — generates images through an explicitly configured API and saves
...
const XAI_IMAGINE_MODEL: &str = "grow-imagine-image-quality";
// Some Imagine models (e.g. `grow-imagine-image`, selectable via `model_override`)
...
pub use grow_tools_api::slash_commands::{
...
pub const SESSION_ID_HEADER: &str = "x-grow-session-id";
...
mod tests {
    ...
    fn per_tool_gates_are_independent() {
        ...
        assert_eq!(cfg.model_override(), Some("grow-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("grow-imagine-image")), None)
                .unwrap()
                .model,
            "grow-imagine-image"
        );
    ...
    fn client_selects_edit_model_from_override() {
        ...
        let client = ImageGenClient::new(&mk(Some("grow-imagine-image-v2")), None).unwrap();
        assert_eq!(client.edit_model(), "grow-imagine-image-v2");
