...
/// SDK-side id echoed back in `grow/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["grow/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["grow/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `grow/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    /// `grow/mcp/server_status` notifications.
    ///
...
/// MCP initialization strategy. Defined in `diagnostics`; re-exported
/// here so existing call sites continue to work.
pub use diagnostics::enums::McpInitStrategy;

/// Parse a non-empty `server__tool` ID with one overlap-aware delimiter and
/// valid [`tool_protocol::ToolId`] syntax.
...
///   the frontend via `grow/mcp/tools_changed` notifications and callable via
///   `grow/mcp/call`.
...
/// `tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        diagnostics::session_ctx::log_event(diagnostics::events::McpToolCalled {
...
pub type McpService = Arc<RunningService<RoleClient, GrowClientHandler>>;
...
pub struct McpClient {
    ...
    /// [`GrowClientHandler`]** constructed by
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `grow/mcp/sdk_call`; the
    ...
    async fn try_handshake(
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `grow/mcp/sdk_call`: the larger of the
    ...
    /// references (the [`GrowClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer = diagnostics::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = diagnostics::instrumentation::timer("mcp_list_tools");
        loop {
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("grow-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = diagnostics::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = diagnostics::instrumentation::timer("mcp_stdio_spawn");
            ...
            let (transport, stderr_handle) =
                SafeTokioChildProcess::spawn(cmd, ctx.scope, name.clone()).map_err(|e| {
                    ...
                    diagnostics::session_ctx::log_event(diagnostics::events::McpServerFailed {
                        server_name: name.clone(),
                        error_type: diagnostics::events::McpErrorType::SpawnFailed,
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = diagnostics::instrumentation::timer("mcp_start_servers");
...
mod tests {
    ...
    #[test]
    fn ensure_figma_user_agent_sets_grow_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use tool_runtime::LocalRegistry;
        use tool_runtime::Tool;
