...
//!
//! ```
//! use grow_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `GrowDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `GrowNight`, `TokyoNight`).
    Dark,
}
