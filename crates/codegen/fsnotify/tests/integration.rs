...
use fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource};
...
///
/// ```bash
/// cargo test -p fsnotify --test integration \
...
async fn shared_watcher_scaling_demo() {
    ...
    for _ in 0..SESSIONS {
        let Ok(src) = fsnotify::shared(path.clone(), FsConfig::default()) else {
            eprintln!("skipping: OS watcher unavailable (resource limit?)");
            return;
        };
        handles.push(src);
