...
//! startup latency, under either strategy (`GROW_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
//! # Generate a synthetic tree, then measure both strategies against it:
//! cargo run --release -p fsnotify --example watch_stats -- gen js /tmp/js-repo
//! GROW_FSNOTIFY_PER_DIR=0 cargo run --release -p fsnotify --example watch_stats -- run /tmp/js-repo 5
//! GROW_FSNOTIFY_PER_DIR=1 cargo run --release -p fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "grow holds 55k inotify watches" report).
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("GROW_FSNOTIFY_PER_DIR").unwrap_or_default();
