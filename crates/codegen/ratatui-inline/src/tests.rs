...
mod links {
    ...
    fn emits_osc8_around_linked_cells() {
        let mut t = term(20, 3);
        let out = frame(&mut t, "AB", &[span(0, 2, "https://example.com", None)]);
        assert!(
    ...
    fn link_removed_next_frame_rewrites_cells_without_osc8() {
        let mut t = term(20, 3);
        let _ = frame(&mut t, "AB", &[span(0, 2, "https://example.com", None)]);
    ...
    fn emit_id_param_included() {
        let mut t = term(20, 3);
        let out = frame(&mut t, "AB", &[span(0, 2, "https://example.com", Some(7))]);
        assert!(
            out.contains("\x1b]8;id=7;https://example.com\x07"),
    ...
    fn wide_char_under_link_wraps_lead_cell_only() {
        ...
        // the OSC 8 wraps it.
        let out = frame(&mut t, "世", &[span(0, 2, "https://example.com", None)]);
        assert!(
            out.contains("\x1b]8;;https://example.com\x07世\x1b]8;;\x07"),
            "wide-char run: {out:?}"
        );
    }
    ...
    fn nonzero_origin_viewport_maps_links() {
        ...
        assert!(
            out.contains("\x1b]8;;https://example.com\x07AB\x1b]8;;\x07"),
