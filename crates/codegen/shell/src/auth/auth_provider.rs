...
/// grandchildren -- and the `GROW_AUTH_PROVIDER_*` credentials in their env --
...
impl AuthProviderRef {
    ...
    /// rejected key.
    pub(crate) fn bearer_resolver(&self) -> sampler::SharedBearerResolver {
        #[derive(Debug)]
        struct Resolver(AuthProviderRef);
        impl sampler::BearerResolver for Resolver {
