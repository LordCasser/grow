//! Shim — see `::diagnostics::instrumentation` for the implementation.
//!
...
///
/// Stays in shell so callers can keep calling `shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `shell::instrumentation_timer!(...)`.
