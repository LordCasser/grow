//! ACP extension handler for bulk session updates (`grow/session/updates`).
//!
...
/// Send updates as chunked `_grow/session/updates/chunk` notifications.
...
mod tests {
    ...
    fn dummy_gateway() -> acp_transport::AcpAgentGatewaySender {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("grow/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_grow/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    fn capturing_gateway() -> (
        acp_transport::AcpAgentGatewaySender,
        tokio::sync::mpsc::UnboundedReceiver<acp_transport::AcpClientMessage>,
    ) {
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        (acp_transport::AcpAgentGatewaySender::new(tx), rx)
    }
