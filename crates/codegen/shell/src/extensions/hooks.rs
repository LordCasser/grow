...
pub fn hook_spec_to_info(spec: &::hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: ::hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[::hooks::event::HookEventName] = &[
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern) if event.traits().matcher == ::hooks::event::MatcherPolicy::Ignored => {
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "grow/hooks/action" => {
            let req: extension_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use ::hooks::event::{HookEventEnvelope, HookPayload};
