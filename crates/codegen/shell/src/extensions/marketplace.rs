...
fn load_filtered_marketplace_sources() -> Vec<plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                plugin_marketplace::SourceKind::Local { path } => path.to_str().unwrap_or("?"),
            };
            format!("{}={}", s.name, url)
        })
        .collect();
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            {
                Ok(outcome) => outcome,
                Err(e) => extension_types::ActionOutcome {
                    status: extension_types::OutcomeStatus::InternalError,
...
fn refresh_sources(
    sources: &[plugin_marketplace::MarketplaceSource],
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = plugin_marketplace::git::default_cache_root();
            if let Err(e) = plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
    ...
    extension_types::ActionOutcome {
        status: extension_types::OutcomeStatus::Success,
        message: msg,
        requires_reload: false,
        requires_restart: false,
    }
...
async fn handle_update(
    ...
    use extension_types::{ActionOutcome, OutcomeStatus};
    use plugin_marketplace::installer;

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    ...
    let plugin_path = match plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path)
    {
        Ok(path) => path,
        Err(e) => {
            return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        plugin_marketplace::SourceKind::Local { path } => {
            marketplace_lease = None;
            path.clone()
        }
        plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = plugin_marketplace::git::default_cache_root();
            match plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, extension_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
...
async fn handle_install(
    ...
    if let Some((remote_url, remote_ref, remote_sha, remote_subdir)) = remote_entry {
        ...
        let marketplace_root = match &source.kind {
            ...
            plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
        ...
        let plugin_path =
            match plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
                Ok(path) => path,
                Err(e) => {
                    return ActionOutcome {
                        status: OutcomeStatus::ValidationError,
...
fn scan_source(source: &plugin_marketplace::MarketplaceSource) -> (MarketplaceScanResult, bool) {
    let lease;
    ...
    let scan = plugin_marketplace::scan_marketplace(&root);
    ...
    let plugins = discovered
        ...
        .map(|p| {
            let (install_status, installed_version) =
                match plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: plugin_marketplace::MarketplaceEntry,
...
async fn handle_add_source(url: &str) -> extension_types::ActionOutcome {
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
    ...
    let config_path = config::grow_home().join("config.toml");
...
async fn handle_remove_source(source_url_or_path: &str) -> extension_types::ActionOutcome {
...
fn remove_source_locked(source_url_or_path: &str) -> extension_types::ActionOutcome {
    use crate::plugin;
    use extension_types::{ActionOutcome, OutcomeStatus};
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            ...
            components: Some(extension_types::PluginComponents {
                skills: vec![extension_types::ComponentItem::new(
                    "code-review",
