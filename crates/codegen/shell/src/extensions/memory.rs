//! `grow/memory/flush`, `grow/memory/rewrite`, and `grow/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("grow/compact_conversation") => handle_compact(agent, args).await,
        "grow/memory/flush" => handle_flush(agent, args).await,
        "grow/memory/rewrite" => handle_rewrite(agent, args).await,
