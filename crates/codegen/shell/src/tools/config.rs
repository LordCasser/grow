...
/// Production grow-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production grow-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `GROW_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `GROW_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> tools::implementations::grow_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("GROW_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| config::env_bool("GROW_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `Grow` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `GrowHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "GrowHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "GrowHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"Grow:read_file"));
        assert!(ids.contains(&"Grow:search_replace"));
        assert!(ids.contains(&"Grow:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"GrowHashline:hashline_read"));
        assert!(ids.contains(&"GrowHashline:hashline_edit"));
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [agent::config::AgentDefinition::explore()] {
            let name = def.name.clone();
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "Grow:search_replace"),
            ...
            assert!(
                ids.contains(&"GrowHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"Grow:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"GrowHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production grow-build must set the 10h foreground ceiling"
        );
    }
