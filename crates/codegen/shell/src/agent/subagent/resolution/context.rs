...
//!
//! Extracted from `shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `chat-state::compaction_utils::strip_system_tags` which
        ...
        ConversationItem::User(u) => {
            let text: String = u
                ...
                .filter_map(|p| match p {
                    sampling_types::conversation::ContentPart::Text { text } => Some(text.as_ref()),
                    _ => None,
                })
                .collect::<Vec<_>>()
                .join("\n");
...
mod tests {
    use super::*;
    use sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(sampling_types::synthesized_reasoning_item(text))
    }
