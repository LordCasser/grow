...
impl tools::implementations::grow_build::task::coordinator::ChildRunner for ShellChildRunner {
    ...
    fn run(
        ...
        Box::pin(async move {
            ...
            let Some(mut ctx) = this.try_build_subagent_spawn_context(&parent_sid) else {
                ...
                return tools::implementations::grow_build::task::coordinator::ChildRunOutput {
    ...
    fn on_completed(
        &self,
        completion: tools::implementations::grow_build::task::coordinator::ChildCompletion<
            Self::CompletionData,
        >,
    ) {
...
impl MvpAgent {
    ...
    /// [`SubagentCoordinator`](tools::implementations::grow_build::task::coordinator::SubagentCoordinator)
                ...
                ps.map(|h| h.tool_context.fs.inner().clone())
                    .unwrap_or_else(|| {
                        let cwd = ps
                            ...
                            .unwrap_or_default();
                        std::sync::Arc::new(workspace::file_system::LocalFs::new(cwd))
                    }),
