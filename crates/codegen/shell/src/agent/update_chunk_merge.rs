        ...
        match (prev, new) {
            ...
            (Some(SessionNotification::Grow(prev)), SessionNotification::Grow(new)) => {
                ...
                (
                    ...
                    second.map(|s| SessionNotification::Grow(Box::new(s))),
                )
            }
...
fn estimate_payload_bytes(n: &SessionNotification) -> u64 {
    match n {
        ...
        SessionNotification::Grow(n) => match &n.update {
...
mod tests {
    ...
    #[test]
    fn grow_same_id_deltas_merge_args_and_preserve_name() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        let n = match flushed {
            SessionNotification::Grow(n) => *n,
            _ => panic!("expected Grow extension update"),
        };
        match n.update {
            crate::extensions::notification::SessionUpdate::ToolCallDeltaChunk {
                tool_call_id,
    ...
    #[test]
    fn grow_different_tool_call_id_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn grow_tool_call_delta_is_bufferable() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn grow_after_acp_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        let acp_chunk = msg_chunk("s", 1, "thinking...");
        let grow_chunk = delta_chunk("s", Some("call_1"), 0, Some("bash"), Some("args"));

        assert!(buf.consume_chunk(acp_chunk).is_none());
        let (flushed, rest) = buf.consume_chunk(grow_chunk).expect("should force-flush");
