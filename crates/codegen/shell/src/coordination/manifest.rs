...
fn secure_directory(path: &Path) -> io::Result<()> {
    ...
    #[cfg(windows)]
    {
        shell_base::util::secure_file::set_windows_secure_permissions(path)
    }
...
pub(crate) fn write_manifest(path: &Path, manifest: &PeerManifest) -> io::Result<()> {
    ...
    shell_base::util::secure_file::write_secure_file(&temp, &bytes)?;
...
fn ensure_private_manifest(path: &Path) -> io::Result<()> {
    #[cfg(unix)]
    {
    ...
    #[cfg(windows)]
    {
        // Tighten restored or hand-copied manifests before trusting their token.
        shell_base::util::secure_file::ensure_owner_only_permissions(path)
    }
    #[cfg(not(any(unix, windows)))]
    {
        let _ = path;
...
pub(crate) fn read_all_manifests(grow_home: &Path) -> Vec<PeerManifest> {
    let Ok(entries) = std::fs::read_dir(peers_dir(grow_home)) else {
        return Vec::new();
    };
    entries
        .filter_map(Result::ok)
        .filter(|entry| entry.path().extension().and_then(|value| value.to_str()) == Some("json"))
        .filter_map(|entry| match read_manifest(&entry.path()) {
            Ok(manifest) => Some(manifest),
            Err(error) => {
                tracing::warn!(
                    path = %entry.path().display(),
                    error = %error,
                    "ignoring invalid coordination manifest"
                );
                None
            }
        })
