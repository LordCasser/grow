//! grow-build's L5 wiring onto the shared full-replace engine
//! (`compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](compaction::sample_full_replace_summary);
...
//! [`FullReplaceError`](compaction::FullReplaceError).
...
/// [`CompactionSampler`] for grow-build's full-replace pass.
///
...
/// else the structured grow-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the grow-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured grow-build), ignoring the shared
