...
// outcomes (Unknown / Disabled / Unavailable) get
...
/// Fail-open — the `agent_type` exists but is disabled
...
/// Bridge `GoalPauseReason` to its diagnostic representation. The exhaustive
/// `match` ensures the compiler catches drift if either side adds a variant.
