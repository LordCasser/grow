...
pub(crate) fn format_hook_name(spec: &::hooks::config::HookSpec) -> String {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &::hooks::config::HookSpec) -> Self {
        Self {
    ...
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names =
            agent::prompt::agents_md::read_agents_config_with_paths(&self.cwd)
                ...
                .collect();
        let skill_names = agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: ::diagnostics::context::collect_git_context(&self.cwd).is_git_repo,
