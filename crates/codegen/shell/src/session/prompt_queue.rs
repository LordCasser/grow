...
//!
//! Canonical definitions live in `prompt_queue`; this re-export keeps every existing
//! `crate::session::prompt_queue::*` and cross-crate `shell::session::prompt_queue::*`
...
mod tests {
    ...
    fn queue_entry_wire_round_trips_last_editor() {
        let entry = QueueEntryWire {
            ...
            version: 3,
            owner: Some("grow-tui".to_string()),
            last_editor: Some("grow-vscode".to_string()),
            kind: "prompt".to_string(),
            text: "hello".to_string(),
            position: 0,
            combined_texts: None,
        ...
        assert_eq!(json["lastEditor"], "grow-vscode");
