//! ACP conversion functions for `tools`'s `ToolOutput`.
//!
...
///
/// This converts `tools`' TodoItem (which has `id`, `content: Option<String>`,
...
mod tests {
    ...
    fn test_acp_tool_update_todo_returns_completed() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            ...
            todos: vec![],
            state: tools::implementations::grow_build::todo::TodoState::default(),
        }));
        let update = acp_tool_update(&output, "call-1", None, None).unwrap();
        assert_eq!(update.fields.status, Some(acp::ToolCallStatus::Completed));
    }
    ...
    fn test_turn_end_plan_cleanup_preserves_semantics_and_priority() {
        use crate::tools::todo::plan_entry_from_todo_item;
        use tools::implementations::grow_build::todo::{TodoItem, TodoPriority, TodoStatus};
    ...
    fn test_acp_plan_update_todo() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            summary_for_prompt: "tasks".to_string(),
            todos: vec![tools::implementations::grow_build::todo::TodoItem {
                content: "Task 1".to_string(),
                priority: tools::implementations::grow_build::todo::TodoPriority::Medium,
                status: tools::implementations::grow_build::todo::TodoStatus::Completed,
                meta: None,
            }],
            state: tools::implementations::grow_build::todo::TodoState::default(),
        }));
