...
fn api_error_with_context_window(context_window: u64) -> sampler::SamplingErrorInfo {
...
async fn test_compact_on_error_triggers_when_tokens_exceed_new_window() {
    ...
    local
        .run_until(async {
            let (gateway_tx, _) = mpsc::unbounded_channel::<acp_transport::AcpClientMessage>();
...
async fn test_compact_on_error_noop_without_model_metadata() {
    ...
    local
        .run_until(async {
            ...
            let err = sampler::SamplingErrorInfo {
                kind: sampler::SamplingErrorKind::Api,
                status_code: Some(400),
                message: "prompt is too long".into(),
                is_retryable: false,
...
async fn compaction_at_tokens_header_flips_after_compaction() {
    use sampling_types::CompactionAtTokens;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _g) = mpsc::unbounded_channel();
