        ...
        .spawn(|| {
            ...
            local
                .run_until(async {
                    ...
                    let sampling_client = crate::sampling::Client::new(sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(sampling_types::UserItem {
                    content: vec![sampling_types::ContentPart::Text { text: "hi".into() }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
