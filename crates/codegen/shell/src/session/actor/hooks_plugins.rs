...
impl SessionActor {
    ...
    pub(super) async fn handle_hooks_action(
        ...
        use extension_types::{ActionOutcome, HooksAction, OutcomeStatus};
        ...
        match action {
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        ::hooks::trust::disable_hook(name).is_ok()
                    } else {
                        ::hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
    ...
    pub(super) async fn handle_plugins_action(
        ...
        use extension_types::{ActionOutcome, OutcomeStatus, PluginsAction};
    ...
    pub(super) async fn reload_plugins_impl(
        ...
        ::diagnostics::unified_log::info("reload_plugins_impl: start", Some(sid), None);
