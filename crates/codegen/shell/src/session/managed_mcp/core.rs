...
///
/// Single source of truth lives in `workspace` (which matches policy
/// `serverName`s against it); re-exported here so the two never drift.
pub use workspace::permission::resolution::MANAGED_MCP_PREFIX;
...
pub fn to_managed_name(display_name: &str) -> String {
    use workspace::permission::resolution::{MANAGED_MCP_NAME_MAX_CHARS, normalize_managed_name};
    let raw = format!(
        "{MANAGED_MCP_PREFIX}{}",
        normalize_managed_name(display_name)
    );
    shell_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
