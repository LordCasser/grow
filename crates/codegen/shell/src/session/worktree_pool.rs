...
//!   under `~/.grow/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.grow/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.grow/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.grow/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.grow/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.grow/`.
...
/// `CleanAll` mode. Placed under `~/.grow/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || fast_worktree::remove_worktree(&path_owned)).await;
