...
pub enum Event {
    ...
    /// discriminator constants in `shell::session::events`.
    ...
    /// `shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Diagnostic mirror of `shell`'s `GoalPauseReason`. The two types
