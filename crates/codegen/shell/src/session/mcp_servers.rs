...
pub use ::mcp::servers::{
    AcpServerEntry, HttpConfig, McpClient, McpClientTimeoutOverrides, McpConfigDiff, McpError,
    McpInitStrategy, McpMetaConfigMap, McpServerMetaConfig, McpServerName, McpService, McpSpawnCtx,
    McpState, McpTool, McpToolRegistration, SharedMcpPool, mcp_server_name, mcp_target_str,
...
pub async fn start_mcp_server(
    ...
    inner::start_mcp_server(mcp_server, overrides.as_ref(), meta_config, ctx).await
}
...
pub async fn build_pending_clients(
    ...
    let mut results = start_mcp_servers(configs_to_start, cwd, meta_config_map, ctx).await;
...
pub async fn start_mcp_servers(
    ...
    let overrides_map: HashMap<String, inner::McpClientTimeoutOverrides> = mcp_servers
        ...
        .collect();
    inner::start_mcp_servers(mcp_servers, &overrides_map, meta_config_map, ctx).await
}
