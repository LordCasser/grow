...
impl RelocationView {
    pub(crate) fn load(grow_home: &Path) -> Result<Self> {
        Self::load_for_sessions_root(&grow_home.join("sessions"))
    }
    pub(crate) fn journal_ids(grow_home: &Path) -> Result<Vec<String>> {
        Ok(load_journals(grow_home)?.into_keys().collect())
    }
    pub(crate) fn load_for_sessions_root(sessions_root: &Path) -> Result<Self> {
        let grow_home = sessions_root
            .parent()
            .ok_or_else(|| RelocationError::Inconsistent("sessions root has no parent".into()))?;
        let journals = load_journals(grow_home)?;
        let (all_candidates, persisted_candidates) = load_candidates(sessions_root)?;
        Ok(Self {
            grow_home: grow_home.into(),
    ...
    pub(crate) fn protects_cwd_dir(&self, cwd_dir: &Path) -> bool {
        self.journals.values().any(|journal| {
            [&journal.source_cwd, &journal.target_cwd]
                .into_iter()
                .any(|cwd| self.sessions_root.join(config::encode_cwd_dirname(cwd)) == cwd_dir)
        })
    }
    ...
    fn select(
        ...
        let selected = if let Some(relocation) = self.journals.get(session_id) {
            let expected =
                journal::session_dir_at(&self.grow_home, authoritative_cwd(relocation), session_id);
            let path = self
                .all_candidates
                .get(session_id)
            ...
            RelocationStorage::new(self.grow_home.clone())
...
fn load_journals(grow_home: &Path) -> Result<HashMap<String, RelocationJournal>> {
    let dir = journal::relocation_dir(grow_home);
    ...
    for entry in entries {
        ...
        journals.insert(session_id.to_owned(), journal::read(grow_home, session_id)?);
    }
    Ok(journals)
}
