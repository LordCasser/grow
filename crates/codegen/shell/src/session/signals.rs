...
pub use ::diagnostics::enums::PrCreationSource;
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
...
mod tests {
    ...
    async fn test_model_tracking() {
        ...
        // Set primary model
        handle.set_primary_model("grow-3");

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.primary_model_id, Some("grow-3".to_string()));
        assert_eq!(snapshot.models_used, vec!["grow-3".to_string()]);

        // Record additional model usage
        handle.record_model_usage("grow-4");
        handle.record_model_usage("grow-3"); // Duplicate

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.models_used.len(), 2);
        assert!(snapshot.models_used.contains(&"grow-3".to_string()));
        assert!(snapshot.models_used.contains(&"grow-4".to_string()));
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        handle.seed_counts(
            ...
            vec!["grow-3".to_string(), "grow-4".to_string()],
        );
        ...
        handle.record_model_usage("grow-3"); // existing model
        handle.record_model_usage("grow-4.5"); // new model
        ...
        assert_eq!(snapshot.models_used.len(), 3); // grow-3 not duplicated, grow-5 added
        assert!(snapshot.models_used.contains(&"grow-4.5".to_string()));
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        handle1.record_model_usage("grow-3");
        ...
        handle1.record_assistant_message();
        handle1.record_model_usage("grow-4");
        ...
        assert!(restored.models_used.contains(&"grow-3".to_string()));
        assert!(restored.models_used.contains(&"grow-4".to_string()));
        ...
        // ITL stats must survive the restore (regression test for grow-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the grow-critique bug describes: the
        ...
        handle2.record_model_usage("grow-3"); // existing model (should dedup)
        ...
        assert_eq!(after_turn.models_used.len(), 2); // grow-3 not duplicated
