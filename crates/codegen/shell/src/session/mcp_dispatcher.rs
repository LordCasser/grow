...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`grow_managed_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `grow_managed_` prefix)
    /// or `local` (user `.grow/config.toml`).
...
mod tests {
    ...
    fn classify_source_uses_managed_prefix() {
        assert_eq!(
            classify_source("grow_managed_linear"),
            McpServerSource::Managed
        );
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("grow_managed_slack"), // managed
