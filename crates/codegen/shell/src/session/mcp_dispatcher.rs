...
use crate::extensions::mcp::McpToolEntry;
...
pub const SERVER_STATUS_METHOD: &str = "grow/mcp/server_status";
...
pub struct McpServerStatusPayload {
    ...
    /// Full tool inventory when the producer already has it. A config-change
    /// delta may leave this absent, telling the client to refresh the catalog.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub enum McpServerStatus {
    /// Client is in [`::mcp::servers::ClientStateKind::Ready`]
...
/// [`::mcp::servers::McpClientEventKind`]). Used as the
...
///   `ConfigRemoved` keys,
/// - emit one ACP `grow/mcp/server_status` push per surviving
///   buffer entry, via the provided gateway.
///
/// `gateway` is a [`acp_transport::AcpAgentGatewaySender`] (forwarded
...
/// 3. `flush_window` — emit ACP `grow/mcp/server_status` per
///    surviving entry.
...
mod tests {
    ...
    /// shape the project-scoped catalog reload produces
    /// downstream.
    #[test]
    ...
    ///
    /// Locks the camelCase field naming, lowercase enum values, and omission
    /// of an unavailable tool inventory.
    #[test]
    ...
    /// `recoverable_http_servers` keeps only enabled HTTP/SSE entries — the
    /// same predicate as the recovery gate.
    #[test]
    fn recoverable_http_servers_excludes_stdio_and_disabled() {
        ...
        let want: HashSet<String> = ["http-mcp-server".to_string(), "slack".to_string()]
            .into_iter()
            .collect();
        assert_eq!(got, want);
    }
    ...
    fn discard_gateway() -> acp_transport::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        acp_transport::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = acp_transport::AcpAgentGatewaySender::new(gw_tx);
