...
// The foundation utilities live in `shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `shell::util::…` users compile
// unchanged.
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn grow_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_grow_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
