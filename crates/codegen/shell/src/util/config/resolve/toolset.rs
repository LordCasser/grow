...
/// [`config::env_bool`] parser (`GROW_TOOLS_FIND_BFS` /
...
/// [`tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            tools::implementations::grow_build::ask_user_question::RESPONSE_TIMEOUT.as_secs(),
        )
}
...
mod ask_user_question_timeout_tests {
    ...
    fn timeout_secs_tier_precedence() {
        let d = tools::implementations::grow_build::ask_user_question::RESPONSE_TIMEOUT.as_secs();
