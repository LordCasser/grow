...
pub(crate) const ENV_COMPACTION_TOOL_CHOICE: &str = "GROW_COMPACTION_TOOL_CHOICE";
...
///      (populated from `grow_build_models[i].auto_compact_threshold_percent`;
...
const ENV_COMPACTION_WALL_CLOCK_BUDGET_SECS: &str = "GROW_COMPACTION_WALL_CLOCK_SECS";

/// Resolve the per-compaction wall-clock budget (seconds). Precedence: env
/// `GROW_COMPACTION_WALL_CLOCK_SECS` > remote settings global
...
mod compaction_wall_clock_budget_tests {
    ...
    // Assumes GROW_COMPACTION_WALL_CLOCK_SECS is unset in the test env.
    #[test]
