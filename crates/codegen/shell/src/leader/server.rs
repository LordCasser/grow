...
use sampler::OutputDelivery as SamplingOutputDelivery;
...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "grow-tui", "grow-code-extension").
...
#[derive(Debug, Clone)]
struct SamplingAttemptSignal {
    key: SamplingCandidateKey,
    boundary: Option<SamplingAttemptBoundary>,
    output_delivery: Option<SamplingOutputDelivery>,
}

/// Classify a sampling-attempt boundary or a provisional output update. The
/// leader uses only the explicit request/attempt metadata; ordinary ACP and
/// Grow notifications remain on their existing route.
fn sampling_attempt_signal(json: &serde_json::Value) -> Option<SamplingAttemptSignal> {
    let session_id = extract_session_id(json)?;
    let params = json.get("params")?;
    let meta = params
        .get("_meta")
        .or_else(|| params.get("params").and_then(|p| p.get("_meta")));
    let output_delivery =
        |meta: Option<&serde_json::Value>| match meta?.get("samplingOutputDelivery")?.as_str()? {
            "retractable" => Some(SamplingOutputDelivery::Retractable),
            "irreversible" => Some(SamplingOutputDelivery::Irreversible),
            "buffered" => Some(SamplingOutputDelivery::Buffered),
            _ => None,
        };
    let preview_key = || {
        let meta = meta?;
        Some(SamplingCandidateKey {
    ...
    let update = params
        .get("update")
        .or_else(|| params.get("params").and_then(|p| p.get("update")));
    if let Some(update) = update
        && update.get("sessionUpdate").and_then(|v| v.as_str()) == Some("sampling_attempt")
    {
        let request_id = update
            ...
            .as_str()?
            .to_owned();
        let attempt = u32::try_from(update.get("attempt")?.as_u64()?).ok()?;
        let state = match update.get("state")?.as_str()? {
            "started" => SamplingAttemptBoundary::Started,
            "discarded" => SamplingAttemptBoundary::Discarded,
            "accepted" => SamplingAttemptBoundary::Accepted,
            _ => return None,
        };
        return Some(SamplingAttemptSignal {
            key: SamplingCandidateKey {
                session_id,
                request_id,
                attempt,
            ...
            output_delivery: output_delivery(
                update.get("_meta").or_else(|| update.get("meta")).or(meta),
            ),
        });
    }
    preview_key().map(|key| SamplingAttemptSignal {
        key,
        boundary: None,
        output_delivery: output_delivery(meta),
    })
}
...
fn fanout_sampling_payloads(
    ...
    load_live_buffer: &mut HashMap<(ClientId, String), Vec<BufferedLive>>,
) {
    let Some(subscribers) = session_subscribers.get(&key.session_id) else {
        return;
    };
    for &client_id in subscribers {
        ...
        if client.capabilities.sampling_attempt_lifecycle {
            continue;
        }
        let load_key = load_live_buffer.keys().find_map(|(cid, root)| {
            (*cid == client_id
                && (root == &key.session_id
...
/// `params._meta["grow/leaderClientId"]`.
///
...
/// Replay notifications are already ordered by session storage. They must
/// never enter the live sampling candidate reducer, even when they are not
/// targeted to a loading client.
fn is_replay_notification(json: &serde_json::Value) -> bool {
    let params = json.get("params");
    let meta = params.and_then(|params| params.get("_meta")).or_else(|| {
        params
            .and_then(|params| params.get("params"))
            .and_then(|params| params.get("_meta"))
    });
    meta.and_then(|meta| meta.get("isReplay"))
...
///
/// - `grow/sessions/changed` — roster delta; every open dashboard must stay
///   in sync.
/// - `grow/models/update` — the model catalog changed (config.toml
...
///   (`grow/settings/update` stays non-broadcast — it carries auth/gate state.)
///
/// Matched via [`method_of`], NOT the raw top-level `method`: agent ext
/// notifications arrive `_`-prefixed on the wire (`_grow/sessions/changed`),
...
fn inject_capabilities_into_session_new(
    ...
    if let Some(params) = json.get_mut("params").and_then(|p| p.as_object_mut()) {
        ...
        if let Some(meta_obj) = meta.as_object_mut() {
            ...
            if !meta_obj.contains_key("grow/leaderClientId") {
                meta_obj.insert(
                    "grow/leaderClientId".to_string(),
            ...
            meta_obj.insert(
                "samplingAttemptLifecycle".to_string(),
                serde_json::json!(capabilities.sampling_attempt_lifecycle),
            );
        }
    }
    mutated
...
fn make_version_mismatch_notification(
    ...
    Some(
        serde_json::json!({
            "jsonrpc": "2.0",
            "method": "grow/leader/version_mismatch",
            "params": {
                "clientVersion": client_version,
                "leaderVersion": leader_version,
                "message": format!(
                    "Client version {client_version} differs from leader version \
                     {leader_version}. Restart the grow binary to use the same version."
                )
            }
        })
        .to_string(),
...
pub async fn run_leader_server(
    ...
    // and its terminal watermark for each session. Durable session/load replay
    // owns history and therefore must not be supplemented by late backfills.
    let mut sampling_candidates: HashMap<String, SamplingCandidate> = HashMap::new();
    ...
    loop {
        ...
        match poll {
                ...
                ServerEvent::Disconnected(id) => {
                    ...
                    for sid in viewed {
                        ...
                        if now_empty {
                            ...
                            sampling_candidates.remove(&sid);
                    ...
                    if !detached_sessions.is_empty() {
                        let evict_notification = serde_json::json!({
                            "jsonrpc": "2.0",
                            "method": "grow/internal/evict_sessions",
            ...
            LeaderServerPoll::Response(payload) => {
                ...
                let json = json;
                // Targeted session/load replay is already ordered and
                // unicast below. Keep it out of the live candidate reducer;
                // replayed sampling metadata must not look like a late live
                // preview or enter another observer's transient buffer.
                let targeted_replay = json
                    .as_ref()
                    .is_some_and(|json| extract_target_client_id(json).is_some());
                let sampling_live_notification = json.as_ref().is_some_and(|json| {
                    json.get("id").is_none()
                        && extract_session_id(json).is_some()
                        && !is_machine_wide_broadcast_notification(json)
                        && !targeted_replay
                        && !is_replay_notification(json)
                });
                let sampling_signal = sampling_live_notification
                    ...
                    match signal.boundary {
                            ...
                            let stale_same_request =
                                candidate.key.as_ref().is_some_and(|current| {
                                    current.request_id == signal.key.request_id
                                        && current.attempt > signal.key.attempt
                                });
                            if stale_same_request || candidate.key.as_ref() == Some(&signal.key) {
                                // Duplicate/older boundaries are not new
                                // lifecycle events. In particular, do not
                                // leak a duplicate Started to a capable
                        ...
                        None => {
                                ...
                                && candidate.terminal.is_none()
                            {
                                ...
                                // boundary (or for an older request) belongs
                                // to no live candidate and must not leak to
                                // even lifecycle-capable subscribers.
                                drop_sampling_preview = true;
                            }
                        }
                    }
...
mod tests {
    ...
    async fn connect_and_register_with_capabilities(
        sock_path: &std::path::Path,
        client_type: &str,
        capabilities: ClientCapabilities,
    ) -> (
    ...
    async fn initialize_gets_client_identifier_injected() {
        ...
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "grow-tui",
    ...
    async fn initialize_preserves_existing_client_identifier() {
        ...
        .unwrap();
        let _: ServerMessage = read_message(&mut reader).await.unwrap();
        let payload = r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"grow-web"}}}"#;
        write_message(
            &mut writer,
            &ClientMessage::Acp {
                payload: payload.into(),
        ...
        let json: serde_json::Value = serde_json::from_str(&received).unwrap();
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "grow-web",
    ...
    fn is_interaction_request_detects_only_interaction_methods() {
        ...
        assert!(!is_interaction_request(&pv(
            r#"{"jsonrpc":"2.0","method":"grow/sessions/changed","params":{}}"#
        )));
    }
    #[test]
    fn extract_interaction_tool_call_id_handles_direct_and_nested() {
        assert_eq!(
            extract_interaction_tool_call_id(&pv(
                r#"{"id":1,"method":"grow/ask_user_question","params":{"sessionId":"s","toolCallId":"tc-q"}}"#
            ))
            .as_deref(),
            Some("tc-q")
        );
        ...
        assert_eq!(
            extract_interaction_tool_call_id(&pv(
                r#"{"id":1,"method":"_grow/ask_user_question","params":{"method":"grow/ask_user_question","params":{"sessionId":"s","toolCallId":"tc-w"}}}"#
            ))
            .as_deref(),
            Some("tc-w")
        );
    ...
    fn extract_interaction_resolved_tool_call_id_matches_only_resolved() {
        assert_eq!(
            extract_interaction_resolved_tool_call_id(&pv(
                r#"{"method":"grow/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-r"}}}"#
            ))
            .as_deref(),
            Some("tc-r")
        );
        assert_eq!(
            extract_interaction_resolved_tool_call_id(&pv(
                r#"{"method":"_grow/session_notification","params":{"method":"grow/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-rw"}}}}"#
            ))
            .as_deref(),
            Some("tc-rw")
        );
        assert_eq!(
            extract_interaction_resolved_tool_call_id(&pv(
                r#"{"method":"grow/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"pending_interaction","tool_call_id":"tc-r","kind":"permission"}}}"#
            )),
            None
        );
    }
    ...
    fn event_seq_of_parses_acp_and_ext_and_handles_missing() {
        ...
        let ext = pv(
            r#"{"params":{"method":"grow/session/update","params":{"sessionId":"019e-aa","_meta":{"eventId":"019e-aa-7"}}}}"#,
        );
    ...
    fn patch_initialize_response_noop_when_no_default_model() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"modelState":{"currentModelId":"grow-3"}}}}"#,
        );
        let before = json.clone();
    ...
    fn extract_session_id_from_nested_params_works() {
        let payload = r#"{"jsonrpc":"2.0","method":"_grow/session_notification","params":{"method":"grow/session_notification","params":{"sessionId":"sess-nested"}}}"#;
        assert_eq!(
            extract_session_id(&pv(payload)),
    ...
    fn extract_child_session_event_spawned() {
        let payload = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-1"}}}"#;
    ...
    fn extract_child_session_event_finished() {
        let payload = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-2"}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_nested_ext_notification() {
        let payload = r#"{"jsonrpc":"2.0","method":"_grow/session_notification","params":{"method":"grow/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-3"}}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_nested_ext_notification_finished() {
        let payload = r#"{"jsonrpc":"2.0","method":"_grow/session_notification","params":{"method":"grow/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-4"}}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_none_for_other_updates() {
        let payload = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#;
        assert!(extract_child_session_event(&pv(payload)).is_none());
    }
    #[test]
    fn extract_child_session_event_none_without_child_id() {
        let payload = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned"}}}"#;
        assert!(extract_child_session_event(&pv(payload)).is_none());
    }
    #[test]
    ...
    #[test]
    fn sampling_attempt_signal_keeps_request_and_attempt_boundaries_separate() {
        let started = serde_json::json!({
            "method": "grow/session_notification",
            "params": {
                "sessionId": "s",
                "update": {
                    "sessionUpdate": "sampling_attempt",
                    "request_id": "r",
                    "attempt": 1,
                    "state": "started"
                }
        ...
        let preview = serde_json::json!({
            ...
            "params": {
                "sessionId": "s",
                "_meta": {"samplingRequestId": "r", "samplingAttempt": 1}
            }
        });
        let second_attempt = serde_json::json!({
            "method": "session/update",
            "params": {
                "sessionId": "s",
                "_meta": {"samplingRequestId": "r", "samplingAttempt": 2}
            }
        });
        assert_eq!(
            sampling_attempt_signal(&started).unwrap().boundary,
            Some(SamplingAttemptBoundary::Started)
        );
        assert_eq!(sampling_attempt_signal(&preview).unwrap().key.attempt, 1);
        assert_ne!(
            sampling_attempt_signal(&preview).unwrap().key,
            sampling_attempt_signal(&second_attempt).unwrap().key
        );
    }

    #[tokio::test]
    async fn sampling_attempt_mixed_clients_buffer_unknown_until_accept_and_drop_discard() {
        let temp = TempDir::new().unwrap();
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut capable_reader, mut capable_writer) = connect_and_register_with_capabilities(
            &sock_path,
            "capable",
            ClientCapabilities {
                sampling_attempt_lifecycle: true,
                ..Default::default()
            },
        )
        .await;
        let (mut buffered_reader, mut buffered_writer) =
            connect_and_register(&sock_path, "buffered").await;
        for writer in [&mut capable_writer, &mut buffered_writer] {
            write_message(
                writer,
                &ClientMessage::Acp {
                    payload: r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"sampling-session"}}"#.into(),
                },
            )
            .await
            .unwrap();
        ...
        for _ in 0..2 {
            tokio::time::timeout(Duration::from_secs(1), acp_rx.recv())
                .await
                .expect("timed out waiting for session subscription")
                .expect("agent channel closed");
        }
        ...
        assert!(
            tokio::time::timeout(
                ...
                read_message::<_, ServerMessage>(&mut buffered_reader)
            )
            .await
            .is_err()
        );
        assert!(
            next_acp_payload(&mut capable_reader)
                .await
                .is_some_and(|payload| payload.contains("started"))
        );
        response_tx.send(preview.into()).unwrap();
        assert!(
            tokio::time::timeout(
                Duration::from_millis(200),
                read_message::<_, ServerMessage>(&mut buffered_reader)
        ...
        // A duplicate terminal Started and an older attempt boundary are
        // suppressed for lifecycle-capable clients as well.
        response_tx.send(started.into()).unwrap();
        response_tx
            .send(started.replace("\"attempt\":1", "\"attempt\":0"))
            .unwrap();
        assert!(
            tokio::time::timeout(
                Duration::from_millis(300),
                read_message::<_, ServerMessage>(&mut capable_reader)
            )
            .await
            .is_err()
        );

        let discarded_started = started.replace("req-1", "req-2");
        let discarded_preview = preview.replace("req-1", "req-2");
        let discarded = accepted
            .replace("req-1", "req-2")
            .replace("accepted", "discarded");
        response_tx.send(discarded_started).unwrap();
        response_tx.send(discarded_preview).unwrap();
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"sampling-session","_meta":{"eventId":"sampling-session-40"},"value":"discard-independent"}}"#
                    .into(),
            )
            .unwrap();
        response_tx.send(discarded).unwrap();
        assert!(
            next_acp_payload(&mut buffered_reader)
                .await
                .is_some_and(|payload| payload.contains("discard-independent"))
        );
        assert!(
            next_acp_payload(&mut buffered_reader)
                .await
                .is_none_or(|payload| !payload.contains("preview-1"))
        );

        // A headless/irreversible attempt must stay live for every client;
        // capability=false alone is not a reason to delay its output.
        let irreversible_started = started
            .replace("req-1", "req-3")
            .replace("retractable", "irreversible");
        let irreversible_preview = preview.replace("req-1", "req-3");
        let irreversible_accepted = accepted.replace("req-1", "req-3");
        response_tx.send(irreversible_started.into()).unwrap();
        assert!(
            next_acp_payload(&mut buffered_reader)
                .await
                .is_some_and(|payload| payload.contains("started"))
        );
        response_tx.send(irreversible_preview.into()).unwrap();
        assert!(
            next_acp_payload(&mut buffered_reader)
                .await
                .is_some_and(|payload| payload.contains("preview-1"))
        );
        response_tx.send(irreversible_accepted.into()).unwrap();
        assert!(
            next_acp_payload(&mut buffered_reader)
                .await
                .is_some_and(|payload| payload.contains("accepted"))
    ...
    #[tokio::test]
    async fn accepted_sampling_candidate_flushes_pending_load_once() {
        let temp = TempDir::new().unwrap();
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut reader, mut writer) = connect_and_register(&sock_path, "buffered").await;
        load_session(&mut writer, "sampling-load").await;
        let forwarded = tokio::time::timeout(Duration::from_secs(1), acp_rx.recv())
            .await
            .expect("timed out waiting for forwarded sampling load")
            .expect("agent channel closed");
        let load_id = serde_json::from_str::<serde_json::Value>(&forwarded)
            ...
            .cloned()
            .unwrap();
        let started = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sampling-load","_meta":{"samplingOutputDelivery":"retractable"},"update":{"sessionUpdate":"sampling_attempt","request_id":"req-load","attempt":1,"state":"started"}}}"#;
        let preview = r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"sampling-load","_meta":{"samplingRequestId":"req-load","samplingAttempt":1},"value":"load-preview"}}"#;
        let accepted = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sampling-load","_meta":{"eventId":"sampling-load-10"},"update":{"sessionUpdate":"sampling_attempt","request_id":"req-load","attempt":1,"state":"accepted"}}}"#;
        response_tx.send(started.into()).unwrap();
        response_tx.send(preview.into()).unwrap();
        response_tx.send(accepted.into()).unwrap();
        let target_client = serde_json::from_str::<serde_json::Value>(&forwarded)
            .unwrap()["params"]["_meta"]["grow/leaderClientId"]
            .clone();
        response_tx
            .send(
                serde_json::json!({
                    ...
                    "method": "grow/session_notification",
                    "params": {
                        "sessionId": "sampling-load",
                        "_meta": {
                            "grow/leaderClientId": target_client,
                            "eventId": "sampling-load-10"
                        },
                        "update": {
                            "sessionUpdate": "sampling_attempt",
                            "request_id": "req-load",
        ...
        for _ in 0..4 {
            flushed.push(
                next_acp_payload(&mut reader)
                    .await
                    .expect("load flush frame"),
            );
        }
        assert_eq!(
            flushed
                .iter()
                .filter(|payload| payload.contains("accepted"))
                .count(),
            1,
            "accepted replay and buffered live copy must overlap by event sequence only once"
        );
        assert!(flushed.iter().any(|payload| payload.contains("started")));
        assert!(
            flushed
                .iter()
                .any(|payload| payload.contains("load-preview"))
        );

        load_session(&mut writer, "sampling-load").await;
        complete_load(&mut acp_rx, &response_tx).await;
        assert!(
            next_acp_payload_matching(&mut reader, "load-preview")
                .await
                .is_none()
        );
        cancel.cancel();
    ...
    #[test]
    fn inject_capabilities_includes_sampling_attempt_lifecycle_policy() {
        let caps = ClientCapabilities {
            sampling_attempt_lifecycle: true,
            ..Default::default()
        };
        ...
        assert_eq!(
            json["params"]["_meta"]["samplingAttemptLifecycle"],
    ...
    fn inject_capabilities_does_not_override_existing_client_identifier() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "grow-tui", ClientId(1));
    ...
    fn inject_capabilities_adds_leader_client_id_to_session_load() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "grow-tui", ClientId(42));
        assert_eq!(
            json["params"]["_meta"]["grow/leaderClientId"].as_u64(),
            Some(42)
        );
    }
    #[test]
    fn inject_capabilities_does_not_override_existing_leader_client_id() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-1","_meta":{{"grow/leaderClientId":7}}}}}}"#,
            AGENT_METHOD_NAMES.session_load
        );
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
    ...
    fn extract_target_client_id_some_when_meta_present() {
        let direct = r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"sess-1","_meta":{"grow/leaderClientId":9}}}"#;
        assert_eq!(extract_target_client_id(&pv(direct)), Some(ClientId(9)));
        let nested = r#"{"jsonrpc":"2.0","method":"_grow/session/update","params":{"params":{"sessionId":"sess-1","_meta":{"grow/leaderClientId":11}}}}"#;
    ...
    fn inject_client_identity_adds_identifier_to_initialize() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "grow-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated, "should have injected the identifier");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "grow-tui");
    }
    #[test]
    fn inject_client_identity_does_not_override_existing() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"grow-web"}}}"#,
        );
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "grow-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(!mutated, "existing identifier means nothing was injected");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "grow-web");
    }
    #[test]
    ...
    fn inject_client_identity_preserves_existing_meta() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "grow-code-extension");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated);
        assert_eq!(json["params"]["_meta"]["foo"], "bar");
        assert_eq!(
    ...
    fn version_mismatch_notification_contains_correct_fields() {
        ...
        assert_eq!(json["method"], "grow/leader/version_mismatch");
    ...
    /// A session-local model switch must not change the next session's global default.
    #[tokio::test]
    async fn set_model_does_not_update_default_for_next_session_new() {
        ...
        let json: serde_json::Value = serde_json::from_str(&forwarded).unwrap();
        assert_eq!(
            json["params"]["_meta"]["modelId"], "provider/default",
            "Leader must seed new sessions from the registered global default"
        );
        cancel.cancel();
    }
    #[tokio::test]
    ...
    async fn ext_notification_with_nested_session_id_routes_correctly() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"_grow/session_notification","params":{"method":"grow/session_notification","params":{"sessionId":"sess-A","update":{"sessionUpdate":"retry_state","attempt":1,"maxRetries":3,"reason":"transient"}}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
        ...
        match msg {
            ServerMessage::Acp { payload } => {
                let json: serde_json::Value = serde_json::from_str(&payload).unwrap();
                assert_eq!(json["method"], "_grow/session_notification");
            }
            other => panic!("Expected Acp message, got {:?}", other),
        }
    ...
    /// sends an `grow/internal/evict_sessions` notification through acp_tx
    ...
    #[tokio::test]
    async fn driver_only_request_during_sampling_candidate_is_not_buffered() {
        let temp = TempDir::new().unwrap();
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut driver_reader, mut driver_writer) =
            connect_and_register(&sock_path, "driver").await;
        load_session(&mut driver_writer, "sess-driver-sampling").await;
        complete_load(&mut acp_rx, &response_tx).await;
        let _ = next_acp_payload(&mut driver_reader).await;
        let (mut observer_reader, mut observer_writer) =
            connect_and_register(&sock_path, "observer").await;
        load_session(&mut observer_writer, "sess-driver-sampling").await;
        complete_load(&mut acp_rx, &response_tx).await;
        let _ = next_acp_payload(&mut observer_reader).await;

        let started = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-driver-sampling","_meta":{"samplingOutputDelivery":"retractable"},"update":{"sessionUpdate":"sampling_attempt","request_id":"req-driver","attempt":1,"state":"started"}}}"#;
        response_tx.send(started.into()).unwrap();
        let request = r#"{"jsonrpc":"2.0","id":42,"method":"fs/read_text_file","params":{"sessionId":"sess-driver-sampling","path":"/tmp/x"}}"#;
        response_tx.send(request.into()).unwrap();
        assert!(
            next_acp_payload(&mut driver_reader)
                .await
                .is_some_and(|payload| payload.contains("read_text_file"))
        );
        assert!(
            next_acp_payload(&mut observer_reader)
                .await
                .is_none_or(|payload| !payload.contains("read_text_file"))
        );

        let accepted = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-driver-sampling","update":{"sessionUpdate":"sampling_attempt","request_id":"req-driver","attempt":1,"state":"accepted"}}}"#;
        response_tx.send(accepted.into()).unwrap();
        for _ in 0..2 {
            assert!(
                next_acp_payload(&mut observer_reader)
    ...
    async fn interaction_request_broadcasts_to_all_subscribers() {
        ...
        let _ = next_acp_payload(&mut reader_b).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":501,"method":"_grow/ask_user_question","params":{"method":"grow/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-q","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
    ...
    async fn pending_interaction_replayed_to_late_joiner() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":601,"method":"_grow/ask_user_question","params":{"method":"grow/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-late","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
    ...
    async fn reattached_client_backfilled_into_child_routes() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-sub","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub"}}}"#;
        ...
        let child_reverse = r#"{"jsonrpc":"2.0","id":777,"method":"grow/child_thing","params":{"sessionId":"child-sub"}}"#;
    ...
    async fn replayed_spawn_registers_child_route_for_loading_client() {
        ...
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-fresh","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-fresh"}}}}}}"#,
            a_id.0
        );
    ...
    async fn late_attacher_backfilled_into_existing_child_routes() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-sub2","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub2"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let (mut reader_b, mut writer_b) = connect_and_register(&sock_path, "client-b").await;
    ...
    async fn replayed_finished_does_not_tear_down_live_child_route() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-tear","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-tear"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-tear","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-tear"}}}}}}"#,
            b_id.0
        );
    ...
    async fn backfill_covers_nested_children() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_child = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-nest"}}}"#;
        response_tx.send(spawned_child.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let spawned_grandchild = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"child-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-nest"}}}"#;
    ...
    async fn intermediate_finish_reparents_live_grandchild_for_root_backfill() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_a = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-a"}}}"#;
        response_tx.send(spawned_a.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "child-a").await;
        let spawned_b = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"child-a","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-b"}}}"#;
        response_tx.send(spawned_b.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "grandchild-b").await;
        let finished_a = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-a"}}}"#;
    ...
    async fn live_finished_prunes_index_so_reattach_skips_dead_child() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-dead"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let finished_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-dead"}}}"#;
    ...
    async fn detached_live_finished_prunes_index_so_reattach_skips_dead_child() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-detach"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        drop(reader_a);
        drop(writer_a);
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-detach"}}}"#;
        response_tx.send(finished_live.to_string()).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
        let (mut reader_a2, mut writer_a2) = connect_and_register(&sock_path, "client-a").await;
    ...
    async fn mid_burst_disconnect_still_prunes_dead_child_route() {
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-leak"}}}}}}"#,
            a_id.0
        );
        response_tx.send(spawned_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-leak"}}}}}}"#,
            a_id.0
        );
        response_tx.send(finished_replay).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
    ...
    async fn orphaned_replayed_finished_leaves_held_route_untouched() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-hold","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-hold"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-hold","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-hold"}}}}}}"#,
            b_id.0
        );
        response_tx.send(finished_replay).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
    ...
    async fn replayed_spawn_unions_into_existing_live_route() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-union","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let (mut reader_b, mut writer_b, b_id) =
            connect_register_get_id(&sock_path, "client-b").await;
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-union","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}}}}"#,
            b_id.0
        );
        response_tx.send(spawned_replay).unwrap();
    ...
    async fn replayed_finished_last_subscriber_prunes_dead_child() {
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-last"}}}}}}"#,
            a_id.0
        );
        response_tx.send(spawned_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"grow/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-last"}}}}}}"#,
            a_id.0
        );
        response_tx.send(finished_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_finished").await;
    ...
    async fn mid_load_child_delta_reaches_loader_via_request_side_backfill() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-midload","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-midload"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        drop(reader_a);
        drop(writer_a);
    ...
    async fn pending_interaction_survives_disconnect_and_replays_on_reconnect() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":801,"method":"_grow/ask_user_question","params":{"method":"grow/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-reconnect","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "ask_user_question").await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        drop(reader_a);
    ...
    async fn interaction_raised_with_no_subscriber_is_cached_and_replayed_on_first_attach() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let req = r#"{"jsonrpc":"2.0","id":901,"method":"_grow/ask_user_question","params":{"method":"grow/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-nosub","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
    ...
    async fn resolved_interaction_not_replayed_to_late_joiner() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":701,"method":"_grow/ask_user_question","params":{"method":"grow/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-ev","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "ask_user_question").await;
        let resolved = r#"{"method":"_grow/session_notification","params":{"method":"grow/session_notification","params":{"sessionId":"sess-int","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-ev"}}}}"#;
    ...
    /// `grow/sessions/changed` is a machine-wide roster notification with no
    ...
    async fn models_update_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_grow/models/update","params":{"currentModelId":"grow-new","availableModels":[{"modelId":"grow-new","name":"Grow New"}]}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a.as_deref().is_some_and(|p| p.contains("grow-new")),
            "client A must receive the models broadcast, got {got_a:?}"
        );
        assert!(
            got_b.as_deref().is_some_and(|p| p.contains("grow-new")),
    ...
    fn machine_wide_broadcast_classifier_matches_both_wire_forms() {
        ...
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"_grow/sessions/changed","params":{}}"#
        )));
        assert!(is_machine_wide_broadcast_notification(&pv(
        ...
        assert!(!is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"grow/settings/update","params":{}}"#
        )));
    }
    ...
    fn inject_capabilities_sets_code_nav_enabled_true() {
        ...
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "grow-web", ClientId(1));
        assert_eq!(
    ...
    fn inject_capabilities_sets_code_nav_enabled_false() {
        ...
        let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"grow-tui"}}}"#;
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "grow-tui", ClientId(1));
        assert_eq!(
            json["params"]["_meta"]["codeNavEnabled"],
    ...
    fn inject_capabilities_two_clients_stay_isolated() {
        ...
        inject_capabilities_into_session_new(&mut web_json, &web_caps, "grow-web", ClientId(1));
        let mut tui_json = pv(session_new);
        inject_capabilities_into_session_new(&mut tui_json, &tui_caps, "grow-tui", ClientId(2));
        assert_eq!(
    ...
    async fn subagent_child_session_routed_after_spawned() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-123"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"child-123","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
    ...
    async fn subagent_child_session_cleaned_up_on_finished() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-456"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-456"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"child-456","update":{"sessionUpdate":"message_delta"}}}"#
                    .into(),
            )
            .unwrap();
        let timeout_result: Result<Result<ServerMessage, _>, _> =
    ...
    async fn subagent_child_session_not_leaked_to_other_client() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"_grow/session_notification","params":{"method":"grow/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-789"}}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"grow/session_notification","params":{"sessionId":"child-789","update":{"sessionUpdate":"message_delta"}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
    ...
    async fn leader_client_id_unicasts_to_target_only() {
        ...
        response_tx
            .send(
                format!(
                r#"{{"jsonrpc":"2.0","method":"session/update","params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"agent_message_chunk"}},"_meta":{{"grow/leaderClientId":{}}}}}}}"#,
                id_a
            ),
            )
            .unwrap();
    ...
    async fn leader_client_id_dropped_when_target_disconnected() {
        ...
        response_tx
            .send(
                format!(
                r#"{{"jsonrpc":"2.0","method":"session/update","params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"agent_message_chunk"}},"_meta":{{"isReplay":true,"grow/leaderClientId":{}}}}}}}"#,
                id_a
            ),
            )
            .unwrap();
        response_tx
            .send(
                format!(
                r#"{{"jsonrpc":"2.0","method":"_grow/session/update","params":{{"params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"hook_annotation","message":"m"}},"_meta":{{"isReplay":true,"grow/leaderClientId":{}}}}}}}}}"#,
                id_a
            ),
            )
            .unwrap();
