// Re-export all types from the standalone sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `sampler` crate can reference them without a circular
// dep on `shell`.

// Tests for the types now live in sampling-types crate.
...
///
/// Lives in the shell (rather than `sampling-types` or
/// `tools`) so neither of those crates needs to depend on
