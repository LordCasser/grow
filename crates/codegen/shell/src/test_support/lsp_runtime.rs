...
use std::sync::Arc;
use tokio::sync::mpsc;
pub(crate) type GatewayOut = <acp::AgentSide as acp_transport::AcpSide>::OutMessage;
...
pub(crate) fn ctx_with_toggle(toggle: HashMap<String, bool>) -> SubagentSpawnContext {
    ...
    SubagentSpawnContext {
        ...
        fs: Arc::new(workspace::file_system::LocalFs::new(PathBuf::from("/tmp"))),
        ...
        task_output_tool_name: Some("get_task_output".to_string()),
...
impl tools::implementations::lsp::LspBackend for DummyLspDispatch {
    ...
    async fn dispatch(
        &self,
        _input: &tools::implementations::lsp::LspToolInput,
    ) -> tools::implementations::lsp::LspToolResult {
        tools::implementations::lsp::LspToolResult {
            text: String::new(),
            is_error: false,
        }
    ...
    ) -> Option<tools::implementations::lsp::DiagnosticsSummary> {
        None
    }
    ...
    ) -> Vec<tools::implementations::lsp::FileDiagnosticEntry> {
        vec![]
    }
}
