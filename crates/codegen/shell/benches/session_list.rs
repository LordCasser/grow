...
//!
//! Run: `cargo bench -p shell --bench session_list`
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/grow-org/session-list-benchmark.git",
    )
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:grow-org/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    assert_eq!(shell::util::grow_home::grow_home(), home.path());
