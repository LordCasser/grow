...
Usage:
    ...
    python3 crates/codegen/shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    GROW_BINARY=/path/to/pager python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a grow agent stdio subprocess."""
...
class IsolatedEnv:
    ...
    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="grow-memtest-")
        ...
        # Create .grow dirs in fake home
        self.grow_home = os.path.join(self.fake_home, ".grow")
        self.memory_dir = os.path.join(self.grow_home, "memory")
    ...
    def write_config(self, toml_str):
        ...
        with open(os.path.join(self.grow_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("GROW_BINARY", "grow")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}grow binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
...
def _default_config() -> str:
    embed_model = os.environ.get("GROW_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"GROW_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """GROW_MEMORY=1 env var enables memory even without config."""
    section("13. GROW_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"GROW_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with GROW_MEMORY=1")
        else:
            fail("no index despite GROW_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.grow/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("GROW_BINARY", "grow")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["GROW_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["GROW_NO_LEADER"] = "1"
...
def build_binary():
    """Build pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "pager")
    ...
    print(f"{B}Building pager (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "GROW_BINARY" in os.environ:
        binary = os.environ["GROW_BINARY"]
        print(f"Binary: {binary} (from $GROW_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["GROW_BINARY"] = binary
