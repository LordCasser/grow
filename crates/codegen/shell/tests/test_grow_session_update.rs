...
use shell::session::info::Info as SessionInfo;
use shell::session::persistence::default_model_id;
use shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};
...
/// must survive `updates.jsonl` and reload as a replayable `_grow/session/update`.
#[tokio::test]
