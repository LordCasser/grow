//! Built-binary end-to-end tests for the grow (pager) binary.
//!
...
//! ```bash
//! cargo test -p shell --test test_built_binary_e2e -- --ignored
//! ```
//!
...
async fn test_headless_streaming_messages_json_carries_per_response_metadata() {
    use serde_json::json;
    use test_support::scripted::{ScriptedResponse, SseEvent};
...
async fn test_headless_json_reports_server_cost() {
    use test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        test_support::scripted::ScriptedResponse::sse(vec![
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        test_support::scripted::ScriptedResponse::sse(
            test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
