...
//!
//! With the fs-watch machinery on (`grow/hunkTracker` + `grow/gitHeadChanged`
...
//!
//! Knobs: GROW_PERF_GIT_FILES (default 300), GROW_PERF_GIT_PICKS (default 6).
...
use hunk_tracker::{REFRESH_SCAN_LOG_PREFIX, REFRESH_SKIP_LOG_PREFIX};
use test_utils::tracing_capture::MessagePrefixCounter;
...
fn install_global_scan_counter() -> ScanCounter {
    // GROW_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("GROW_E2E_LOG").ok();
...
fn build_repo(files: usize, picks: usize) -> (TempDir, String) {
    ...
    let base = test_utils::git::make_feature_branch(wd, picks);
    (dir, base)
}
...
async fn run_storm(
    ...
    let files = env_usize("GROW_PERF_GIT_FILES", 300);
    let picks = env_usize("GROW_PERF_GIT_PICKS", 6);
    ...
    local
        .run_until(async move {
            ...
            let method = init
                ...
                .unwrap_or_else(|| panic!("{label}: provider.api_key auth method not advertised"));
            client_conn
                .authenticate(
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let server = mock_rt
        ...
        .expect("mock server");
    let grow_home = TempDir::new().expect("grow home");
    ...
    let on = agent_rt.block_on(run_storm(
        &server,
        Some(json!({
            "grow/hunkTracker": { "mode": "agent_only" },
