...
//! - `GROW_BINARY_LEADER` — the binary that elects the initial leader.
//! - `GROW_BINARY_CLIENT` — the second client.
//!
...
use shell::leader::{ClientCapabilities, ClientMode, ControlCommand, ControlPayload, LeaderClient};
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: GROW_BINARY_LEADER/GROW_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set GROW_BINARY_LEADER/GROW_BINARY_CLIENT and run with --ignored"]
            ...
            common::leader::run_with_cleanup(&fixture, &mut clients, |fixture, clients| {
                Box::pin(async move {
                    ...
                    let control = LeaderClient::connect(
                        sandbox.home().join(".grow").join("leader.sock"),
                        "grow-pager-update",
                        ClientMode::Stdio,
