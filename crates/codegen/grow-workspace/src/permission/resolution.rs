//! Permission resolution engine: merges native `.grow/config.toml`,
...
/// (fail-safe) with a warn + skip record for `grow inspect`.
...
/// user `~/.grow` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    grow_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native Grow TOML config files:
///
///   * `~/.grow/config.toml` (lowest priority)
///   * Each `.grow/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.grow/config.toml` first (lowest priority within this layer).
    // Gated on user_grow_home() so a project's .grow/config.toml is never read as
    // global permissions when neither GROW_HOME nor a home dir resolves.
    if let Some(global_path) = grow_config::user_grow_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match grow_config::load_config_file(&global_path) {
    ...
    // `.grow/config.toml` (same gate as project `.claude/settings.json`).
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match grow_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[grow_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Grow and Claude sources.
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `grow inspect`.
...
struct ResolveInputs<'a> {
    ...
    /// `.claude/settings.json` / `.grow/config.toml` permission rules are dropped
...
impl ResolveInputs<'static> {
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(&grow_config::managed_config_layers()),
            project_trusted,
        }
    }
}
...
/// bypass is pinned off via grow `requirements.toml`
...
///
/// `project_trusted` gates project-tier Claude settings and `.grow/config.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `grow inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `grow inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `grow inspect` via the outer resolver.
...
/// [`grow_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = grow_config::claude_managed_settings_path() else {
...
/// not consulted: grow must not inherit a host-wide always-approve lockdown from
/// that file. grow still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in grow use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = grow_config::requirements_layers();
...
/// without `~/.grow`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for managed (service.example.com-injected) MCP server names. Defined
/// here (shell depends on workspace) and re-exported by shell's `to_managed_name`
/// so the prefix and policy matching never drift.
pub const MANAGED_MCP_PREFIX: &str = "grow_managed_";

/// Max `char` length of a managed runtime name (`grow_managed_` + normalized display
...
///
/// Both sides reduce to one key (strip `grow_managed_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `GROW_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // GROW_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("GROW_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_GROW_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate GROW_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate GROW_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "grow_managed_foo"));
        assert!(mcp_name_matches("grow_managed_foo", "foo"));
        assert!(mcp_name_matches("grow_managed_foo", "grow_managed_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "grow_managed_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "grow_managed_slack"));
        assert!(mcp_name_matches("My Server", "grow_managed_my_server"));
        assert!(mcp_name_matches("grow_managed_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("grow_managed_", "grow_managed_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        let managed = http_named("grow_managed_foo", "https://foo.example.com/mcp");
        assert!(al.is_server_denied(&managed));
        assert!(!al.is_server_allowed(&managed));

        // Name match is transport-agnostic.
        let stdio = stdio_named("grow_managed_foo", "npx");
        ...
        for unrelated in ["foobar", "grow_managed_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&stdio_named("grow_managed_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("grow_managed_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "grow_managed_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("grow_managed_foo", "https://x.example.com/mcp")));
    ...
    fn untrusted_project_claude_permissions_are_not_honored() {
        ...
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("HOME", home.path());
        let _grow_guard = EnvVarGuard::set("GROW_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_GROW_CLAUDE_MARKER_OVERRIDE");
    ...
    /// Untrusted clone must not contribute project `.grow/config.toml` [permission].
    ///
    /// Sync + `block_on` so `ENV_LOCK` is not held across `.await` (clippy
    /// `await_holding_lock`). Does not assert exact global rule counts:
    /// `grow_config::grow_home()` is a process-wide `OnceLock`, so under
    /// single-process `cargo test` an earlier test may have already pinned
    /// `GROW_HOME`. Project-rule filtering is independent of that; global
    /// survival is checked only when our temp home is the live `user_grow_home()`.
    #[test]
    fn untrusted_project_config_toml_permissions_are_not_honored() {
        ...
        // Global allow (survives untrusted project when GROW_HOME resolves here).
        std::fs::write(
...
"#,
        ...
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .expect("test runtime");
        // Untrusted may be None when no global rules load (GROW_HOME OnceLock
        ...
        let global_live = grow_config::user_grow_home()
            .is_some_and(|g| g == home.path() || g.starts_with(home.path()));
        if global_live {
            let untrusted = untrusted.expect("global rules present when GROW_HOME is live");
            assert!(
                untrusted
                    .config
                    .rules
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/grow/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/grow/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.grow/requirements.toml` is not, despite its path.
        ...
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.grow/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/grow/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/grow.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for grow inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers = grow_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            grow_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
