...
/// Canonical definition lives in `grow_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `grow-mcp` can
...
/// reached `grow_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for GrowTUI - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For GrowTUI clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::GrowPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::GrowWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::GrowPager, true);
