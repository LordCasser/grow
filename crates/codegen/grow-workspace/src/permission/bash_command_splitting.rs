...
///
/// ```bash
/// GROW_API_KEY='xai-some-key' cargo run --bin grow-pager
/// ```
///
...
///   prefix: ["GROW_API_KEY=xai-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "grow-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "GROW_API_KEY='xai-some-key' cargo run --bin grow-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["GROW_API_KEY=xai-some-key".to_owned()],
        ...
        let redirection_command = "cargo build --bin grow-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package grow-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
