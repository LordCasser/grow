...
impl WorkspaceIdentity {
    ...
    /// `ProviderAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `grow-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$GROW_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("GROW_SANDBOX_PROFILE").ok(),
