...
use grow_agent::repo::RepoDirChain;
...
fn is_user_grow_config_file(config_path: &Path) -> bool {
    let Some(user_home) = grow_config::user_grow_home() else {
        return false;
    };
...
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        ...
        .filter(|config_path| config_path.is_file() && !is_user_grow_config_file(config_path))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn find_project_configs_excludes_user_grow_config_file() {
        let Some(user_home) = grow_config::user_grow_home() else {
            return;
        };
        let user_config = user_home.join("config.toml");
        if user_config.is_file() {
            ...
            assert!(
                !from_home.iter().any(|p| is_user_grow_config_file(p)),
                "user config leaked into project configs: {from_home:?}"
            );
            assert!(is_user_grow_config_file(&user_config));
        }
        ...
        assert!(!is_user_grow_config_file(&found[0]));
    }
}
