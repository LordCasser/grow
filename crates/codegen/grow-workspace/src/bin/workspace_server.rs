...
//!
//! Reads OIDC credentials from `~/.grow/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use grow_workspace::config::WorkspaceServerMetadata;
use grow_workspace::daemonize;
use grow_workspace::diag_server::{self, DiagHandle, ErrorClass};
use grow_workspace::error::WorkspaceError;
use grow_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_grow_workspace";
...
#[derive(Parser)]
#[command(name = "grow-workspace-server")]
...
struct Args {
    ...
    /// Trust project-scoped LSP servers from `<repo>/.grow/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "GROW_WORKSPACE_PROJECT_LSP_TRUSTED",
        default_value_t = false,
        action = clap::ArgAction::Set,
    )]
    project_lsp_trusted: bool,
    /// Confine `grow/fs/*` resolution to the workspace root (reject `..`,
    ...
    /// Override with `GROW_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "GROW_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/grow-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("GROW_WORKSPACE_OTLP_ENDPOINT") {
    ...
    {
        use grow_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("GROW_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized GROW_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if grow_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via GROW_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = grow_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = grow_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = grow_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let session_id = std::env::var("GROW_SESSION_ID").ok();
    ...
    grow_workspace::init_metrics();
    let ws_handle = match grow_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = grow_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(grace_budget, grow_workspace::handle::DrainReason::Sigterm)
        .await;
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("GROW_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        let args = Args::try_parse_from([
            ...
            "https://service.example.com/preview-auth",
        ...
        assert_eq!(
            ...
            Some("https://service.example.com/preview-auth")
        );
