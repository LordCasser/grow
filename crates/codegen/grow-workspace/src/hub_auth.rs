//! Hub [`AuthProvider`] from `~/.grow/auth.json` for the standalone
...
//!
//! The in-leader `grow workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let grow = grow_config::user_grow_home()
        .ok_or_else(|| anyhow::anyhow!("no user grow home (set $GROW_HOME or $HOME)"))?;
    Ok(grow.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `grow login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `grow login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.grow/auth.json`).
pub fn provider(
    hub_url: &Url,
...
mod tests {
    ...
    fn build_oidc_provider_requires_refresh_token() {
        let entry = AuthEntry {
            ...
            user_id: "u1".into(),
            refresh_token: None,
            oidc_issuer: Some("https://login.example.com".into()),
    ...
    fn write_refreshed_token_updates_jwt_entry() {
        ...
        let path = write_auth_json(
            ...
            "oidc": { "key": "eyJ.old", "user_id": "u2", "refresh_token": "rt-old", "oidc_issuer": "https://login.example.com" }
        }"#,
        );
    ...
    fn write_refreshed_token_targets_exact_scope_key() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "zzz": { "key": "eyJ.z", "refresh_token": "rt-z", "oidc_issuer": "https://login.example.com" },
            "aaa": { "key": "eyJ.a", "refresh_token": "rt-a", "oidc_issuer": "https://login.example.com" }
        }"#,
        );
    ...
    fn write_refreshed_token_preserves_existing_rt_when_not_rotated() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "oidc": { "key": "eyJ.old", "user_id": "u1", "refresh_token": "rt-keep", "oidc_issuer": "https://login.example.com" }
        }"#,
        );
    ...
    fn provider_loopback_uses_bearer() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{ "oidc": { "key": "eyJ.tok", "user_id": "u1", "refresh_token": "rt", "oidc_issuer": "https://login.example.com", "oidc_client_id": "c1" } }"#,
        );
