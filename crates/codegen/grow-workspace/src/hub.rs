...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(grow_version::VERSION)
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        tool = %self.name(),
                        session = %session_id,
                        "GROW_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
...
mod tests {
    ...
    use grow_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("Grow:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use grow_tools::notification::types::{BashExecutionBackgrounded, BashNotificationBase};
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use grow_tools::computer::types::{TaskKind, TaskSnapshot};
