...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<grow_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn grow_tools::computer::types::TerminalBackend>,
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            grow_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_web_fetch_config() -> grow_tools::implementations::grow_build::web_fetch::WebFetchConfig {
    use grow_tools::implementations::grow_build::web_fetch::{WebFetchConfig, WebFetchParams};
    ...
    if grow_config::env_bool("GROW_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
...
mod tests {
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use grow_tools::types::resources::{State, WebCitationCounter};
