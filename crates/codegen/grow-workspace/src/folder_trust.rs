...
//! `project_scope_allowed`, the loader filters) lives in `grow-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `grow-shell`
//! concern, documented there.)

use std::io::IsTerminal;
use std::path::Path;

use grow_config_types::{BoolFlag, RemoteSettings};
...
///
/// Kept local (not in `grow-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    ...
    if std::env::var(grow_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    let user = grow_config::load_from_disk().ok();
    let managed = grow_config::load_managed_config().ok();
...
fn first_repo_config_kind(cwd: &Path) -> Option<&'static str> {
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = grow_config::load_config_file(&path) else {
            continue;
        };
    ...
    if !grow_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    ...
    if !grow_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_grow_config_mcp_servers() {
        let tmp = repo_tmp();
    ...
    #[test]
    fn repo_configs_present_detects_grow_config_plugins_paths() {
    ...
    #[test]
    fn repo_configs_present_detects_grow_config_permission() {
    ...
    // tests (the workspace crate has no `serial_test`/`grow-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(grow_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn is_local_build_honors_test_version_override() {
        ...
        {
            let _sim = EnvVarGuard::set(grow_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
        ...
        let _unset = EnvVarGuard::unset(grow_version::TEST_VERSION_ENV);
