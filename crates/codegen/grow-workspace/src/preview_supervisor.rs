...
//! spawns the unchanged `/usr/local/bin/grow-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/grow-preview-proxy";
...
/// `grow_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "grow_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `grow-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`GROW_SERVER_KEY` /
/// `GROW_SESSION_ID`) reach the proxy by env inheritance — never argv.
...
/// `grow-preview-proxy`'s `/__control/activity` route).
...
/// the default in `grow-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://service.example.com/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match grow-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "GROW_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
