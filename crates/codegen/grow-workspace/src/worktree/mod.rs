...
// Canonical in grow-workspace-types; re-exported for existing paths.
...
/// `worktrees.db` (`xai_fast_worktree::resolve_grow_home`), so checkout dirs and
/// the metadata DB always live under the same `.grow` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `grow_config::grow_home()`,
/// so worktree paths also agree with trust/hooks and other grow-home paths.
fn grow_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_grow_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &grow_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use grow_config_types::WorktreeKindMaxAge;
...
fn resolve_worktree_auto_gc_local() -> xai_fast_worktree::ResolvedWorktreeAutoGc {
    use grow_config_types::WorktreeAutoGcSettings;
