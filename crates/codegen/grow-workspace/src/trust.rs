...
//!
//! The store is rooted at [`grow_config::user_grow_home`] — the **Option**
...
impl TrustStore {
    ...
    ///
    /// Resolves via [`grow_config::user_grow_home`], never
    /// [`grow_config::grow_home`], so it never falls back to a cwd-relative
    ...
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(grow_config::user_grow_home())
    }
...
///
/// The legacy `~/.grow/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = grow_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global grow-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match grow_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_sources_from_user_grow_home() {
        ...
        assert_eq!(
            ...
            grow_config::user_grow_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
