...
/// [`grow_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> grow_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => grow_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => grow_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => grow_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => grow_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => grow_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](grow_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<grow_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    grow_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    ...
    grow_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(grow_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(grow_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
            files_indexed: count,
        });
    }
}
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: grow_workspace_types::FsEventKind,
) -> xai_codebase_graph::FileEvent {
    use xai_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        grow_workspace_types::FsEventKind::Created => FileEventKind::Created,
        grow_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        grow_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        grow_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.grow/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use grow_workspace_types::FsEventKind as WsKind;
        assert_eq!(
