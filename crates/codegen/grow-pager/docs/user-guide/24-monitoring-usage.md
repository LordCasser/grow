...
> **Status: alpha.** The schema below is versioned (`grow_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

Grow CLI can export usage **metrics** and **events** to your organization's
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `GROW_TELEMETRY_ENABLED` |
| `/privacy` | `/privacy opt-in` / `/privacy opt-out`, or Settings |
| Trace upload | `[telemetry] trace_upload` / `GROW_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `GROW_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
```bash
export GROW_EXTERNAL_OTEL=1                  # master switch
...
`GROW_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `GROW_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `GROW_TELEMETRY_ENABLED`, which controls SpaceXAI-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `grow_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `GROW_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`GROW_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `grow setup` managed config /
...
## Metrics (meter scope `ai.grow`)

| Metric | Unit | Attributes |
|---|---|---|
| `grow_code.session.count` | `{session}` | base attrs only |
| `grow_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `grow_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `grow_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `grow_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `grow_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `grow_code.token.usage` with your own
...
|---|---|
| `grow_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `grow_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `grow_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `grow_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `grow_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `grow_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `grow_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `grow_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `grow_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `grow_code.permission_mode_changed` | `to_mode`, `trigger` |
| `grow_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `grow_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `grow_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `grow_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `grow_code.auth` | `auth_method` |
| `grow_code.internal_error` | `error_type` (class only — no message, no location) |
| `grow_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(grow_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(grow_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(grow_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(grow_code_tool_decision_total[1h]))
```
