...
Agent mode runs Grow as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `grow -p` instead ([headless mode](14-headless-mode.md)).
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) defines how clients talk to coding agents over JSON-RPC. With Grow it covers:
...
| -------------------------- | -------------------- | ------------------------------------------------ |
| **Filesystem**             | `grow/fs/*`          | `list`, `exists`, `read_file`, `write_file`      |
| **Git**                    | `grow/git/*`         | `status`, `stage`, `commit`, `diffs`, `discard`  |
| **Git Worktree**           | `grow/git/worktree/*`| `create`, `remove`, `apply`, `list`, `gc`        |
...
| -------------------------- | ------------------------------------ |
| `grow/search/fuzzy/status` | Fuzzy search results update          |
| `grow/git/worktree/status` | Worktree creation progress           |
| `grow/fs_notify`           | Filesystem change notification       |
| `grow/fs/index`            | Full file index update               |
| `grow/fs/index/delta`      | Incremental file index update        |
| `grow/session_notification`| Session-specific updates (diff review, retry state, auto-compact) |
| `grow/session/update`      | Session update (tool calls, content) |
...
// Usage
const client = await new GrowACPChat(".").init();
