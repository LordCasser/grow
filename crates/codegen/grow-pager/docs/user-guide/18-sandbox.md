...
```bash
# Run with workspace sandbox (read everywhere, write to CWD + temp dirs + ~/.grow/)
grow --sandbox workspace

# Read-only mode (read everywhere, write only to ~/.grow/ + temp dirs)
grow --sandbox read-only

# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.grow/, no child network)
grow --sandbox strict
```
...
| `workspace`           | Everywhere         | CWD + `~/.grow/` + `/tmp` + `/var/tmp`         | Allowed       | Normal development                |
| `devbox`              | Everywhere         | All top-level dirs except `/data`              | Allowed       | Disposable dev VMs                |
| `read-only`           | Everywhere         | `~/.grow/` + `/tmp` + `/var/tmp`               | Blocked¹      | Exploration, code review          |
| `strict`              | CWD + system paths | CWD + `~/.grow/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**read-only** -- Use when you want the agent to analyze code without modifying your project files. The agent can read everything but can only write to `~/.grow/` (needed for session persistence) and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.grow/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

### Direct global hook write protection

Under `workspace`, `read-only`, and `strict` (and custom profiles that extend those bases), the Grow state directory remains writable for session/runtime files, but the kernel **write-denies** the Grow-owned direct disk paths used as user-global hook sources (they stay readable):

- `~/.grow/hooks/` (hook directory)
...
Create custom sandbox profiles in `~/.grow/sandbox.toml` (global) or `.grow/sandbox.toml` (per-project):
...
If the user and project files define the same custom profile differently, Grow uses the user profile and shows a startup warning. Run `/doctor` to see both file locations and how to resolve the conflict. Identical definitions do not produce a warning.
...
> `bubblewrap`: if it is missing (or any single deny path can't be bound), Grow
...
> metacharacter, or one that is malformed, makes Grow **refuse to start** (fail
...
>   so matching files are denied **even if created after Grow starts**.
...
>   too many files, or whose tree is too deep/broad to walk, makes Grow **refuse
...
The sandbox is applied to the **entire grow process** at startup using kernel primitives -- not per-command wrapping. This means all tool operations are covered:
...
When a non-`off` sandbox profile is **requested** (CLI, `GROW_SANDBOX`, config, or a managed requirement):
...
for the life of the session**. When you resume it (`grow --resume <id>`,
`grow --continue`, or `grow -r`), Grow restores that same profile automatically —
...
1. An explicit `--sandbox <profile>` flag or `GROW_SANDBOX` environment variable
...
If the sandbox cannot be applied (e.g., unsupported kernel, missing entitlements), Grow logs a warning and continues without enforcement. The exception is an explicitly-requested **custom profile**: on **both macOS and Linux**, if it cannot be applied (unknown profile, malformed `sandbox.toml`, or — on Linux — `bubblewrap` unavailable for a non-empty `deny`), Grow refuses to start rather than run with its denied paths exposed.
...
Sandbox events are logged to `~/.grow/sandbox-events.jsonl` for debugging. Events include:
