...
Commands come from two places: **shell builtins**, handled by the agent backend (grow-shell), and **pager builtins**, handled by the TUI frontend (grow-pager). Both show up in the same menu, and any enabled skill with `user-invocable: true` appears there too.
