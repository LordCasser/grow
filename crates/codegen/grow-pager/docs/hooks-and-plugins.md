...
Grow supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.grow/hooks/`
- **Project hooks** — from `.grow/hooks/` in your repo
...
Plugin hooks automatically receive `GROW_PLUGIN_ROOT` and `GROW_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
Press `a` on the Marketplace tab (or run `grow plugin marketplace add <source>`)
...
Sources land in `~/.grow/config.toml`:
...
- [Hook Examples](../../../grow-hooks/examples/README.md) — ready-to-use sample hooks
