...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    grow_pager::views::settings_modal::render_settings_modal(&mut buf, area, &mut s, false, None);
...
fn defaults_round_trip_through_registry() {
    use grow_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    grow_pager::appearance::cache::set_keep_text_selection(
        grow_pager::appearance::TextSelection::Flash,
    );
    grow_pager::appearance::cache::set_show_thinking_blocks(true);
    grow_pager::appearance::cache::set_prompt_suggestions(true);
    grow_pager::appearance::cache::set_group_tool_verbs(true);
    grow_pager::appearance::cache::set_page_flip_on_send(true);
    grow_pager::appearance::cache::set_combine_queued_prompts(false);
    grow_pager::appearance::cache::set_scroll_mode(grow_pager::appearance::ScrollMode::Auto);
    grow_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    grow_pager::appearance::cache::set_scroll_lines(3);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("grownight"),
    ...
    for meta in reg.all() {
        ...
        let default_value = grow_pager::settings::default_value_for(meta);
...
fn d_key_emits_open_reset_confirm_for_every_setting() {
    ...
    for meta in reg.all() {
        ...
        let mut s = make_state();
        // Skip settings with no visible row; their reset path is covered by the
...
// `crates/codegen/grow-pager/src/app/dispatch.rs::tests` (next to
...
fn reset_overlay_dims_all_rows_except_target() {
    use grow_pager::views::settings_modal::ResetConfirmOverlay;
    use ratatui::buffer::Buffer;
    ...
    grow_pager::views::settings_modal::render_settings_modal(
        &mut buf,
        area,
        &mut s,
        false,
...
/// "Ask Grow" tip footer at the bottom of the content area in
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask Grow"),
            "[{fixture_label}] docs footer (`Ask Grow`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change theme to growday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    grow_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_not_expanded_and_not_edited() {
    ...
    // default is `false` (matches the snapshot's None → false fallback
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use grow_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use grow_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use grow_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use grow_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use grow_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        grow_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        grow_pager::settings::default_value_for(mt_meta),
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use grow_pager::appearance::RenderMermaid;
...
fn pr13_space_on_show_tips_dispatches_typed_setter() {
    ...
    navigate_to(&mut s, "show_tips");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Char(' ')));
    assert_set_bool_action(outcome, "show_tips", true);
}
...
fn pr13_cli_batch_defaults_roundtrip_via_current_value_for() {
    ...
    let ui = UiConfig::default();
    let pager = PagerLocalSnapshot::default();
    for (key, expected) in [("show_tips", false), ("auto_update", true)] {
...
fn pr14_string_settings_use_known_model_validator() {
    use grow_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use grow_pager::appearance::TextSelection;
    ...
    grow_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    grow_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    grow_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    grow_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    grow_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    grow_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    grow_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    grow_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
