...
const PROMO_URL: &str = "https://example.com/zz-promo-cta";
...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    let overrides: Vec<(String, String)> = vec![(
        "GROW_ANNOUNCEMENTS_OVERRIDE".into(),
...
/// safe-open path — observed via the `GROW_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("GROW_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `GROW_ANNOUNCEMENTS_OVERRIDE`.
...
/// `GROW_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
fn spawn_with_announcements_and_env(
    ...
    let binary = pager_binary().expect("resolve pager binary");
    let announcement = ("GROW_ANNOUNCEMENTS_OVERRIDE", override_json);
