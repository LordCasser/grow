...
/// Minimal mode surfaces the Ctrl+R session mode in the one-line info bar
/// directly under the prompt. The mode cycle itself is shared with the full TUI,
/// but minimal had no persistent indicator, so pressing Ctrl+R "did nothing
...
#[ignore]
async fn minimal_ctrl_r_shows_mode_in_info_bar() {
    ...
    // Ctrl+R. First press cycles Normal → Plan.
    harness.inject_keys(b"\x12").expect("inject Ctrl+R");
    harness
        .wait_for_text("plan", Duration::from_secs(10))
        .expect("plan flag in the info bar under the prompt after Ctrl+R");
