...
/// turn and the turn-end `grow/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    ...
    let overrides = [("GROW_PROMPT_SUGGESTIONS".to_owned(), "true".to_owned())];
