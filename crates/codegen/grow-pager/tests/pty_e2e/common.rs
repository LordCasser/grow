...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("GROW_CONTEXTUAL_HINTS", "1")];
...
/// the harness's default `GROW_API_KEY` (ApiKey/BYOK mode, no auth.json entry)
/// would never fetch `/v1/settings`. Spawns WITHOUT `GROW_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    overrides.push(EnvOp::set("GROW_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// Explicit overrides for a folder-trust run. A self-built grow auto-trusts,
/// so `GROW_TEST_VERSION` simulates a release; the gate is pinned both ways.
pub(crate) fn trust_env(feature_on: bool) -> [(&'static str, &'static str); 2] {
    [
        ("GROW_TEST_VERSION", "0.0.0-sim"),
        ("GROW_FOLDER_TRUST", if feature_on { "1" } else { "0" }),
    ]
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".grow")
        .join(grow_workspace::trust::TRUST_FILE_NAME);
    let store = grow_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&grow_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `grow_pager_pty_harness::flows`
// (re-exported above).
...
/// Budget for session creation plus the `grow/mcp/list` round-trip.
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&grow_home).expect("create fake GROW_HOME");
    let config = format!(
...
// unit tests in grow-hooks::discovery::tests. The PTY E2E test requires
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("GROW_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = grow_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// Run `grow wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `GROW_HOME`, returning the exit code (`None` only while still running at
...
pub(crate) fn run_wrap_driving(
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill grow wrap after timeout");
            None
        }
        Ok(PtyExitPoll::PendingStatus) => {
            panic!("grow wrap exited but portable status remained unavailable for {WRAP_TIMEOUT:?}")
        }
        Err(error) => panic!("poll grow wrap exit: {error:#}"),
    };
...
/// `grow wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$GROW_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("GROW_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use grow_pager_pty_harness::host_clipboard::{
...
pub(crate) use grow_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
