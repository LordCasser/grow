...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party xAI OAuth (`is_service_auth()`:
    // AuthMode::Oidc + xAI issuer); the harness's GROW_API_KEY is ApiKey mode
    ...
    let overrides = [
        ...
        EnvOp::set("GROW_TRACE_UPLOAD", "true"),
        EnvOp::set("GROW_TELEMETRY_TRACE_UPLOAD", "true"),
        EnvOp::set("GROW_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"),
    ];
    ...
    //   not fire inside this 3s window; with `GROW_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`
