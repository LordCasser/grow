...
/// backend, often with the service.example.com proxy blocked. A TCP listener that accepts
...
async fn embedded_mode_boots_without_hanging_on_blocked_backend() {
    ...
    let grow_home = home.path().join(".grow");
    std::fs::create_dir_all(&grow_home).unwrap();
    let env = [
        ("HOME", home.path().to_str().unwrap()),
        ("GROW_HOME", grow_home.to_str().unwrap()),
