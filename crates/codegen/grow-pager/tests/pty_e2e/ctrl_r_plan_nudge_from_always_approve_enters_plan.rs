...
/// Always-Approve + plan nudge: one Ctrl+R enters Plan (not Normal).
/// The tip advertises `ctrl+r` → plan mode; with the nudge up that chord
/// must jump from Always-Approve rather than taking the next ring step.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore]
async fn ctrl_r_plan_nudge_from_always_approve_enters_plan() {
    ...
    harness
        ...
        .unwrap_or_else(|e| {
            panic!(
                "nudge + Always-Approve Ctrl+R must enter Plan; {e}\nscreen:\n{}",
                harness.screen_contents()
            )
        });
