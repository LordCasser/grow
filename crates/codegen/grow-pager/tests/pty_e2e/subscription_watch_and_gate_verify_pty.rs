...
const PAID_TIER: &str = "Provider PlanPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "Provider Plan Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `provider_plan_high` = live
/// `/user` string `Provider PlanPro`. Must match
...
/// Like [`seed_fake_oauth`], but under the `GROW_LOCAL_AUTH` dev issuer
/// (`http://localhost:22255`). Two reasons: `is_TEST_OAUTH2_ISSUER` accepts
...
/// call to login.example.com (hermetic, no CI-network flake). Pair with
/// `GROW_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
...
fn spawn_subscription_pager(
    ...
    overrides.push(EnvOp::set("GROW_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let content = ContentController::start_with_models(vec![MockModel::new("grow-3")])
        .await
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[EnvOp::set("GROW_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://service.example.com/provider_plan?referrer=grow-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[EnvOp::set("GROW_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
