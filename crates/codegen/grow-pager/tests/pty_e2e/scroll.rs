...
use grow_pager_pty_harness::PtyHarness;
pub(crate) use grow_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`grow-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `grow-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `GROW_SCROLL_SPEED`) appended to the controller's env.
