...
//!
//! These tests exercise the real `grow-pager` binary through a PTY using
...
/// enterprise deploy report: with `GROW_GOAL=1`, `/goal` must
...
/// explicitly off (`GROW_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// keyword shows the "Planning? Check out plan mode via ctrl+r" banner.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "scripted scenario; run with cargo test -- --ignored"]
...
/// also pins `GROW_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
