...
//!
//! Uses `grow-pager-pty-harness` (`PtyHarness`) + Ctrl+R to cycle
//! Normal → Plan → Auto
//! and assert the mode banner / status shows Auto without conflating
//! Always-Approve.
//!
//! Auth: seeds `HOME/.grow/auth.json` from `GROW_AUTH_JSON` (path) or the
//! developer's `~/.grow/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p grow-pager --test pty_permission_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use grow_pager_pty_harness::{PtyHarness, pager_binary};
use grow_test_support::TestSandbox;
...
/// Ctrl+R — pager binds this to CycleMode while the prompt is focused.
const CTRL_R: &[u8] = b"\x12";

/// Prefer explicit path, else the user's real `~/.grow/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("GROW_AUTH_JSON") {
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    sandbox.remove_env("GROW_API_KEY");

    let home = sandbox.home();
    let grow = sandbox.grow_home();
    let _ = std::fs::create_dir_all(grow);
    if let Some(src) = auth_json_source() {
        let dest = grow.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.grow/auth.json — may hit device login");
    }
    ...
    // of the developer's shell. `GROW_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (grow_config::
    ...
    env.push((
        "GROW_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("GROW_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_ctrl_r_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (grow-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "GROW_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
        ...
        eprintln!(
            "pty_permission_mode: login/device-auth screen blocked Ctrl+R cycle \
    ...
    // Normal → Plan
    harness.inject_keys(CTRL_R).expect("inject Ctrl+R (Plan)");
    let _ = harness.wait_for_text("Plan", Duration::from_secs(8));

    // Plan → Auto
    harness.inject_keys(CTRL_R).expect("inject Ctrl+R (Auto)");
...
/// Gate OFF (the shipped default): the Ctrl+R ring must SKIP Auto entirely
...
#[test]
#[ignore = "spawns real pager PTY; run with cargo test -- --ignored"]
fn pty_ctrl_r_skips_auto_when_gate_off() {
    let binary = match pager_binary() {
        Ok(b) => b,
        Err(e) => panic!("resolve pager binary via harness env: {e:#}"),
    };
    ...
    for _ in 0..4 {
        harness.inject_keys(CTRL_R).expect("inject Ctrl+R");
    ...
    assert!(
        !saw_auto,
        "gate OFF: Ctrl+R ring must skip Auto, but the Auto banner appeared"
    );
    assert!(
        saw_plan && saw_always,
