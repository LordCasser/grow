//! `grow plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
use grow_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use grow_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use grow_plugin_marketplace::SourceKind;
use grow_plugin_marketplace::git::SourceCacheLease;
use grow_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `grow plugin list --json`. The `status` field acts as a
...
/// Typed entry for `grow plugin marketplace list --json`.
#[derive(Serialize)]
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  grow plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `grow plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(&str, &grow_agent::plugins::install_registry::InstalledRepo)],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = grow_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = grow_plugin_marketplace::load_sources(&config);
    sources.extend(grow_plugin_marketplace::load_extra_sources_from_settings(
        &sources,
    ));
    ...
    for source in &sources {
        ...
        for plugin in grow_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                grow_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &grow_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = grow_plugin_marketplace::git::default_cache_root();
            grow_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = grow_plugin_marketplace::install_resolve::parse_marketplace_ref(source) {
    ...
    if !trust {
        use grow_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &grow_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `grow plugin update {}` to update it.",
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               grow plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `grow plugin list` to see installed plugins."
        );
    }
    if let Err(e) = grow_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    grow_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = grow_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    grow_shell::config::add_disabled_plugin(name)
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `grow plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use grow_shell::plugin::MarketplaceAddInput;
    ...
    if !force && let MarketplaceAddInput::GitUrl(git_url) = &input {
        grow_plugin_marketplace::git::probe_git_remote(git_url).map_err(|e| {
            anyhow::anyhow!(
...
fn find_removal_source<'a>(
    sources: &'a [grow_plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a grow_plugin_marketplace::MarketplaceSource, String> {
    ...
    let local_input = match plugin::classify_marketplace_add_input(input, cwd) {
        grow_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &grow_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match grow_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use grow_plugin_marketplace::MarketplaceSource;
    ...
    fn removal_fixture() -> Vec<MarketplaceSource> {
        vec![
            ...
            MarketplaceSource {
                ...
                featured: true,
                kind: SourceKind::Git {
                    url: "https://github.com/example-org/plugin-marketplace.git".into(),
                    branch: None,
                },
            },
            MarketplaceSource {
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        let msg = trust_prompt(
            "\"sentry\" from marketplace \"Featured Marketplace\"",
            "sentry@example-org/plugin-marketplace",
        );
        assert!(
            msg.starts_with(
                "Installing \"sentry\" from marketplace \"Featured Marketplace\" requires confirmation."
            ),
            "{msg}"
        );
        assert!(msg.contains("hooks, MCP servers, and skills"));
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  grow plugin install sentry@example-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  grow plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir =
            grow_plugin_marketplace::git::sync_source_cache(&url, Some("main"), cache_root.path())
                .unwrap();
    ...
    fn current_head(repo: &Path) -> String {
        let output = grow_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
