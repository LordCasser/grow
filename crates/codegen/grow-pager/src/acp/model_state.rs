...
impl ModelState {
    ...
    /// correct today (all current Grow models accept images, so nothing is
    ...
    /// not offer (e.g. `none` on grow-4.5) are rejected so we fail in the TUI
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        ...
        state
            .available
            .insert(id.clone(), model_with_effort("grow-build", "Grow", "high"));
        ...
        refreshed.insert(id.clone(), model_with_effort("grow-build", "Grow", "high"));
