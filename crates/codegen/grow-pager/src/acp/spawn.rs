...
//!
//! Simplified to only support GrowShell (in-process) mode.
...
///
/// Hold one from every site that calls [`spawn_grow_shell`] (headless, the TUI,
...
/// Spawn a GrowShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_grow_shell(
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        grow_agent::prompt::workspace_user::optional_workspace_user_dir();
                    grow_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    grow_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    grow_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
