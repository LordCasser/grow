...
impl HeadlessEmitter {
    ...
    /// Render an `grow/*` lifecycle notification for the active format.
...
async fn fork_then_open(
    ...
    let req = acp::ExtRequest::new("grow/session/fork", fork_params.into());
    ...
    let outcome: Result<()> = match prompt_result {
        ...
        Some(Err(err)) => {
            ...
            let stop_reason_override =
                (grow_shell::sampling::error::stop_reason_for_turn_error(&err) == "MaxTokens")
