//! Headless single-turn mode (`grow -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_grow_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<grow_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use grow_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(json: &str) -> anyhow::Result<Vec<grow_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = grow_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut grow_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    grow_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &grow_tools::types::compat::CompatConfig::default());
            ...
            acp::LoadSessionRequest::new(acp::SessionId::new(sid.to_string()), cwd.to_path_buf())
                ...
                .meta({
                    ...
                    if let Some(true) = restore_code {
                        m.insert("grow/restore_code".into(), serde_json::Value::Bool(true));
                    }
                    Some(m)
                }),
            acp_tx,
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'grow models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    grow_shell::http::set_process_client_mode_headless();
    ...
    agent_config.cli_agent_overrides = grow_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        grow_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_grow_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("GROW_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = grow_shell::active_sessions::register(grow_shell::active_sessions::ActiveSession {
    ...
    // Pending background work: bash/monitor via grow/task_backgrounded +
    // task_completed; background subagents via SubagentSpawned + SubagentFinished
    // on grow/session_notification (prefixed `subagent:{id}` in pending_bg).
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.grow can't hang exit.
        let _ = grow_shell::active_sessions::try_unregister(&session_id);
    }
    ...
    match prompt_result {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = grow_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    ...
    if method == "grow/task_backgrounded" {
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "grow/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "grow/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use grow_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            grow_workspace::permission::types::PatternMode::Domain
        );
    ...
    fn headless_task_backgrounded_parses_task_id() {
        ...
        // the inner update object (matching the real `grow/task_backgrounded`
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let notif = xai_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("grow/other", raw.into()),
            response_tx: tx,
        }
        .boxed();
        assert!(matches!(
