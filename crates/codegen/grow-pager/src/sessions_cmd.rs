...
use grow_shell::auth::{AuthManager, try_ensure_fresh_auth};
use grow_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom inference_base_url.
    // If the user has previously run the interactive `grow` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.auth).await;

    let auth_manager =
        std::sync::Arc::new(AuthManager::new(&grow_home(), agent_config.auth.clone()));

    let client = grow_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions =
                grow_shell::session::merge::fetch_merged(Some(&client), cwd.to_str(), None, limit)
                    .await;
            print_sessions_grouped(&sessions);
        }
        SessionsCommand::Search { query, limit } => {
            use grow_shell::session::merge::REMOTE_TIMEOUT;
            use grow_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = grow_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
