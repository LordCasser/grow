...
/// Canonical model-facing tool name from the `grow/tool` `_meta` envelope, else
/// the display title, else kind, else `"tool"`. The shell stamps the wire name
/// (`bash`, `x_search`, `read_file`) under `grow/tool.name`; without this the
...
/// `grow/tool.kind`; without this every client tool reports `other`.
