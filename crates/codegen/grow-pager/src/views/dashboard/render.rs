...
pub fn render_dashboard(
    ...
    // Promo announcement CTA to paint in the header after the location label
    // (`None` = no CTA); field meanings live on [`HeaderPromoCta`].
    promo_cta: Option<HeaderPromoCta<'_>>,
) -> Option<(u16, u16)> {
    ...
    state.pinned_promo_cta_live = promo_cta.is_some_and(|cta| cta.pinned);
    ...
    // Header.
    render_header(buf, layout.header, &theme, &rows, state, promo_cta);
...
fn render_header(
    ...
    let label_budget = full_label_budget.saturating_sub(promo_reserve);
    ...
    if let Some(HeaderPromoCta { label, .. }) = promo_cta {
        ...
        if avail > 1 {
            ...
            let painted = crate::views::announcements::render_cta_button(
                ...
                label,
                promo_caption,
                state.promo_cta_hit.hovered,
            );
            state.promo_cta_hit.set(painted);
        }
    }
}
...
mod tests {
    ...
    fn header_location_renders_from_staged_cwd() {
        ...
        state.cwd = std::path::PathBuf::from("/grow-staged-cwd-marker");
        ...
        let top_row: String = (0..area.width)
            ...
            .collect();
        assert!(
            top_row.contains("/grow-staged-cwd-marker"),
    ...
    fn render_location_picker_shows_worktree_toggle_in_repo() {
        ...
        let repo = std::env::temp_dir().join("grow-loc-wt-toggle-repo-test");
    ...
    fn render_location_picker_hides_worktree_toggle_outside_repo() {
        ...
        let mut modal = LocationPickerState::new(
            vec![],
            std::path::PathBuf::from("/grow-not-a-repo-xyz-12345"),
            std::collections::HashMap::new(),
        );
        let mut buf = Buffer::empty(area);
        render_location_picker(&mut buf, area, &theme, &mut modal);
