...
/// `~/.grow/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<grow_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
...
/// Load the full persisted dashboard from `~/.grow/config.toml`.
///
...
/// `~/.grow/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no grow home"))?;
...
mod tests {
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/grow");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/grow");
    }
    ...
    fn env_var_force_disables() {
        ...
        // same `GROW_AGENT_DASHBOARD` key reads it concurrently.
