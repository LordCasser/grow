...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("Grow Test", 4); // cursor between "Grow" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
/// Settings → Theme row → Enter into chooser: Esc returns to Browse.
#[test]
fn browse_enter_picker_esc_returns_to_browse() {
    ...
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Action(Action::PreviewTheme(_))),
        "browse-path Esc must revert the preview (Action), got {outcome:?}"
    );
    assert!(
        matches!(s.mode(), SettingsModalMode::Browse),
...
fn deep_link_commit_closes_modal() {
    ...
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
    match outcome {
        SettingsKeyOutcome::ActionThenClose(Action::SetTheme(_)) => {}
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
    ...
    assert!(
        ...
        "validation_error must be Some for unknown model 'a' \
         (catalog has 'Grow 4 Fast' only)",
    );

    // Enter on a buffer that fails the KnownModel validator
    // (catalog has 'Grow 4 Fast'; "a" doesn't match) is
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("Grow Tes", "Grow Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("Grow Test"));
...
//    the "Tip · Ask Grow…" docs footer and the keybindings hints.
//
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("Grow Tst", "Grow T".len());
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask Grow to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("growday"),
        "width=40 must NOT render LONG path (no `growday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask Grow") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"grownight"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"grownight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `grownight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `grownight`; entering the picker
            // captures `original_value = current value = grownight`,
...
/// misleading: on dark themes (GrowNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the GrowDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::GrowNight => crate::theme::Theme::grownight(),
