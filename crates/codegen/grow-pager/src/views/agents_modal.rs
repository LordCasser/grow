...
use grow_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use grow_shell::agent::config::AgentSelectionConfig;
use grow_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.grow` vs `{cwd}/.grow`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup Agent name from the global selection chain.
...
impl AgentsModalState {
    ...
    pub fn new(
        ...
        let default_agent = resolve_default_agent_name(cwd);
        Self {
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = grow_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.grow/personas` and `{cwd}/.grow/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = grow_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".grow").join("personas")),
        (ConfigFileScope::User, grow_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match grow_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => grow_config::grow_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".grow").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.grow/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.grow/{subdir}` or `{cwd}/.grow/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let grow_home = grow_config::grow_home();
    let in_user = dunce::canonicalize(grow_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".grow").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    grow_shell::config::load_effective_config()
        .ok()
        .and_then(|root| grow_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in grow-shell.
pub fn resolve_default_agent_name(cwd: &Path) -> String {
    let agent_config = load_agent_selection_config();
    grow_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
mod tests {
    use super::*;
    use grow_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".grow").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".grow").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.grow/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".grow").join("personas").join("gone.toml");
