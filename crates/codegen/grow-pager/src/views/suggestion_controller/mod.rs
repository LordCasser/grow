...
//! `grow/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `grow/suggest` response.
#[derive(Debug, Clone)]
...
// (e.g. grow-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Wire `limit` for `grow/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `GROW_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `GROW_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `grow/suggest` request.
    /// Resolved at construction from `GROW_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: grow_config::env_bool("GROW_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: grow_config::env_bool("GROW_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("GROW_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `grow/suggest` response arrives, with the text and
