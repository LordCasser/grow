#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/grow-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/grow-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/grow-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/grow-test-image.png"),
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod grow-4");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> grow_workspace::file_system::FuzzyMatchResult {
        grow_workspace::file_system::FuzzyMatchResult {
    ...
    /// after a turn ends with `grow/suggestPrompt` resolved.
