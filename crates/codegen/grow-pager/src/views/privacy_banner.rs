...
const PRIVACY_BANNER_LEGAL: &str = "Review Grow and provider privacy documentation.";

/// Click target for the legal line links.
pub(crate) const PRIVACY_BANNER_LEGAL_URL: &str = "https://github.com/LordCasser/grow";
...
pub(crate) fn render(
    ...
    let title = Span::styled("Help improve Grow", Style::default().fg(theme.text_primary));
    ...
    let line1 = if left.width as usize >= "Help improve Grow  ".len() + desc.len() {
        Line::from(vec![
            title,
            Span::raw("  "),
