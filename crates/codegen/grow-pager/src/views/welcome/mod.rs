...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Grow** VERSION+channel **Beta** (right-aligned).
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub gate: Option<&'a grow_shell::auth::GateInfo>,
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("This feature is unavailable for the configured provider");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
        ...
        if centered.height > 2
            && let Some(gate_link) = p.gate.and_then(|g| g.url.as_deref())
        {
            let url_area = Rect {
                y: centered.y + 2,
                height: 1,
