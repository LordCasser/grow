...
pub const SSH_WRAP_FIX_COMMAND: &str = "grow doctor fix terminal.ssh-wrap";
pub const SSH_WRAP_ONE_OFF: &str = "grow wrap ssh <host>";

const MANAGED_NAMESPACE: &str = "grow doctor";
const SSH_WRAP_ALIAS_POSIX: &str = "alias ssh='grow wrap ssh'";
const SSH_WRAP_ALIAS_FISH: &str = "alias ssh 'grow wrap ssh'";
const TMUX_SCANNER_CAVEAT: &str = "Grow checks this file for direct global assignments of this option. Review sourced files, conditionals, plugins, and generated tmux setup yourself.";
...
pub enum FixError {
    ...
    Managed(grow_config::managed_text::ManagedConfigError),
    TmuxManaged(grow_config::managed_text::ManagedConfigError),
    PostconditionFailed,
...
impl std::fmt::Display for FixError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::HomeUnavailable => formatter.write_str("Grow could not find your home directory."),
            ...
            Self::UnsafeDirectory { label, path } => write!(
                formatter,
                "Grow refused unsafe {label} `{}`. Use a non-root absolute directory without control characters, `~`, `.` or `..` components.",
                path.display()
            ),
                    ...
                    || detail.contains("`ssh` fish function") =>
            {
                write!(
                    formatter,
                    "Grow found an existing SSH alias or function in {} and did not change it: {detail}",
                    path.display()
                )
            }
            Self::ExistingCustomization { path, detail } => write!(
                formatter,
                "Grow found an existing customization in {} and did not change it: {detail}",
                path.display()
            ),
            ...
            Self::PostconditionFailed => formatter
                .write_str("The configuration changed, but Grow could not verify the SSH alias."),
            Self::TmuxPostconditionFailed => formatter.write_str(
                "The configuration changed, but Grow could not verify the managed tmux option.",
            ),
        }
    }
}
...
impl From<grow_config::managed_text::ManagedConfigError> for FixError {
    fn from(error: grow_config::managed_text::ManagedConfigError) -> Self {
        Self::Managed(error)
    }
}
...
const FIX_REGISTRY: &[FixSpec] = &[
    ...
    FixSpec {
        ...
        command: "grow doctor fix terminal.tmux-clipboard",
    ...
    FixSpec {
        ...
        command: "grow doctor fix terminal.dcs-passthrough",
    ...
    FixSpec {
        ...
        command: "grow doctor fix terminal.tmux-extended-keys",
...
pub(crate) fn human_fix_command(id: DiagnosticId) -> Option<String> {
    fix_spec(id).map(|spec| format!("grow doctor fix {}", spec.handle))
}
...
pub(crate) fn format_fix_preview(plan: &FixPlan) -> String {
    ...
    match &change.backup_path_hint {
        Some(path) => {
            let _ = writeln!(
                output,
                "\nBackup will be saved to: {}\nIf that file exists, Grow will choose a unique name.",
                preview_path(path)
            );
        }
    ...
    match &plan.payload {
        FixPayload::SshWrap(_) => {
            output.push_str(
                "\nWhat this changes:\n  In new interactive shells, `ssh ...` runs as `grow wrap ssh ...`.\n",
            );
            let _ = writeln!(
                output,
...
fn plan_ssh_wrap(
    ...
    Ok(FixPlan {
        ...
        caveats: vec![
            ...
            "`grow wrap` starts the SSH process directly, so the alias does not loop.",
            "Grow checks this file for direct SSH aliases and functions. Review sourced files, plugins, and generated shell setup yourself.",
        ],
