...
mod tests {
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let grownight = items
                .iter()
                .find(|i| i.insert_text == "grownight")
                .expect("grownight should be in list");
            assert!(
                grownight.description.contains("(active)"),
                "explicit theme should show (active), got: {}",
                grownight.description
            );
        });
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "grownight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "grownight");
                }
                other => panic!("expected Action::SetTheme(\"grownight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] = GrowNight; next is available[1].
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for GrowNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "grownight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"grownight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // Default auto config maps Light -> GrowDay.
            assert_eq!(Theme::current_kind(), ThemeKind::GrowDay);
        });
    }
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::GrowDay);
            let cmd = ThemeCommand;
