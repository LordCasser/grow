...
///
/// Delegates to the shared implementation in `grow_tools::reminders`.
...
fn format_cron_prompt(prompt: &str, task_id: &str, human_schedule: &str) -> String {
    grow_tools::reminders::format_scheduled_task_prompt(prompt, task_id, human_schedule)
}
...
mod tests {
    ...
    fn drain_scroll_honors_page_flip_setting() {
        ...
        crate::appearance::cache::set_page_flip_on_send(
            grow_shell::agent::config::UiConfig::PAGE_FLIP_ON_SEND_DEFAULT,
        );
    }
    ...
    /// `grow/follow_ups` for a PRIOR turn's response stays rejected (its
