...
/// Whether the configured service reports a highest subscription tier.
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    t.eq_ignore_ascii_case("max") || t.eq_ignore_ascii_case("highest")
}

/// Grow never supplies a billing URL. A configured backend may attach one to
/// its gate response.
...
///
/// **`max_tier = true`** (positively identified as Provider Plan High):
...
pub(super) fn open_credit_limit_upsell(
    ...
    use grow_tools::implementations::grow_build::ask_user_question::{Question, QuestionOption};
...
/// `SubscriptionUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_provider_plan_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the Provider Plan upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_provider_plan_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}
...
fn open_provider_plan_upsell(
    ...
    let options = vec![
        QuestionOption {
            label: "Review provider plan".into(),
            description: "Check quota and feature availability with the configured provider".into(),
            preview: None,
            id: None,
        },
        QuestionOption {
            label: "Dismiss".into(),
            description: "Return to the session".into(),
            preview: None,
            // No Heavy-specific URL exists; the /provider_plan page lists
...
/// `grow/auth/check_subscription` completed. Meta is authoritative
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<grow_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<grow_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_provider_plan_url(app: &mut AppView) -> Vec<Effect> {
    log_event(SubscriptionUpsellClicked {
        source: SubscriptionUpsell::WelcomeScreen,
        auth_method: app.login_method_id.as_ref().map(|id| id.0.to_string()),
    });
    let Some(url) = app.gate.as_ref().and_then(|g| g.url.clone()) else {
        app.show_toast("No provider billing URL is configured");
        return vec![];
    };
    // Funnel attribution: tag CLI-originated Provider Plan upsell clicks
    // with `referrer=grow-build`, matching the OAuth consent flow and
    // the OAuth consent flow. Applied even when the URL came from
