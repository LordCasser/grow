...
/// clear every agent's per-session auto flag, so the UI / Ctrl+R cycle /
...
pub(super) fn set_permission_mode(
    ...
    // the Ctrl+R cycle uses) keeps the settings modal and the cycle in
...
pub(super) fn dispatch_cycle_mode(app: &mut AppView) -> Vec<Effect> {
    ...
    // Tip copy promises one Ctrl+R → Plan; collapse Auto/Always-Approve to
...
/// The permission-mode cycle body shared by the agent view and dashboard peek:
...
/// a Ctrl+R that turns plan mode on while the nudge shows as an acceptance.
...
/// the user presses Ctrl+R faster than the ACP round-trip.
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    // The permission cycle skips Auto entirely (Normal→Plan→Always-Approve→
    ...
    let in_auto = agent.session.is_auto();
    let Some(session_id) = agent.session.session_id.clone() else {
        // No session yet (Ctrl+R forwarded from the welcome screen or a
        ...
        let persist_canonical: Option<&'static str> = match (in_plan, in_auto, in_yolo) {
            ...
            (false, false, false) => {
                agent.plan_mode_pending = Some(true);
                agent.deferred_session_mode = Some(grow_tools::types::SessionMode::Plan);
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(grow_tools::types::SessionMode::Plan.as_id()),
            }]
        }
