...
use super::*;
use grow_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("max")));
    assert!(is_max_tier(Some("MAX")));
    assert!(is_max_tier(Some("highest")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("provider_plan")));
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("provider_plan_heav")));
    assert!(!is_max_tier(Some("provider_plan_high_plus")));
...
fn upsell_non_max_shows_qa_with_two_options() {
    ...
    assert_eq!(q.options[0].id, None);
    assert_eq!(q.options[1].label, "Pay as you go");
    assert_eq!(q.options[1].id, None);
}
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): Grow usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: grow_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
fn manage_billing_gates_on_consumer_billing_surface() {
    let out = std::env::temp_dir().join(format!("grow-manage-billing-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&out);
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("GROW_TEST_OPEN_URL_FILE", &out) };
    let mut app = test_app_with_agent();
    app.usage_billing_redirect_url = Some("https://provider.example/account".into());
    dispatch(Action::ManageBilling, &mut app);
    let opened = std::fs::read_to_string(&out).unwrap_or_default();
    assert!(
        opened.contains("https://provider.example/account"),
        "got: {opened}"
    );
    let _ = std::fs::remove_file(&out);
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    let usage = grow_shell::extensions::notification::PromptUsage {
        totals: grow_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        grow_shell::extensions::notification::PromptUsage {
            totals: grow_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("provider_plan_high".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("provider_plan_high"));
}
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use grow_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens a provider-neutral review modal
/// and neither runs the command nor leaks the text to the model.
#[test]
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert_eq!(
        q.question,
        "This feature is unavailable on the current provider plan."
    );
    assert_eq!(q.options.len(), 2);
    assert_eq!(q.options[0].label, "Review provider plan");
    assert_eq!(q.options[0].id, None);
    assert_eq!(q.options[1].label, "Dismiss");
    assert_eq!(q.options[1].id, None);
}
...
/// `GROW_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "grow-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("GROW_TEST_OPEN_URL_FILE", &bad) };

    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    let url = "https://provider.example/account";
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("grow-open-url-ok-{}.txt", std::process::id()));
    ...
    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    let url = "https://provider.example/credits";
