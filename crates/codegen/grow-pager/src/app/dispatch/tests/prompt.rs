...
fn multiple_queued_prompts_drain_one_per_turn() {
    ...
    // Turn end with an empty queue has no follow-up effect.
    let effects = dispatch(end_turn(), &mut app);
    assert!(effects.is_empty());
    assert!(app.agents[&id].session.state.is_idle());
}
...
/// Turn end with prompt suggestions enabled fires the `grow/suggestPrompt`
/// fetch, and the loaded suggestion routes back
...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `grow-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `grow-build-0.1`
...
/// `grow/queue/changed` rebroadcast — not client-side prediction — updates
...
/// The always-on pipeline end to end, with `GROW_SUGGESTIONS` semantics
