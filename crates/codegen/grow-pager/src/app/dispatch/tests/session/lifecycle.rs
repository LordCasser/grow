...
fn worktree_session_preserves_subdirectory_offset() {
    ...
    app.cwd = PathBuf::from("/repo/crates/codegen/grow-pager");
    ...
    let session_cwd = worktree_root.join("crates/codegen/grow-pager");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
...
fn bg_task_killed_no_op_for_unknown_session() {
    ...
    let effects = dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            ...
            outcome: Some(grow_tools::types::KillOutcome::AlreadyExited),
        }),
        &mut app,
    );
    assert!(effects.is_empty());
...
/// Pre-session (welcome screen / fresh tab): Ctrl+R must cycle the
