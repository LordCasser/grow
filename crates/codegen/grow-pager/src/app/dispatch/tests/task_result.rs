...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    app.gate = Some(grow_shell::auth::GateInfo {
        message: "Provider Plan subscription required".into(),
        url: Some("https://service.example.com/provider_plan".into()),
        label: Some("Subscribe".into()),
    });
    assert!(!app.has_access());

    // Server-side settings now show no gate (user purchased subscription).
    let settings = grow_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = grow_shell::util::config::RemoteSettings {
...
fn test_gate() -> grow_shell::auth::GateInfo {
    grow_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(grow_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(grow_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
