...
pub(super) fn dispatch_send_feedback(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    agent.scrollback.push_block(RenderBlock::system(
        "Thanks for the feedback! The Grow team is on it.".to_string(),
    ));
...
/// Send a raw remember note for LLM-powered rewriting via `grow/memory/rewrite`.
...
/// agent is mid-turn. Fires the `grow/recap` ext method; the recap arrives
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    // entirely when the feature is off so we never hit `grow/recap`.
