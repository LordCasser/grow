...
pub enum Action {
    ...
    /// merges its text into the in-flight turn. Routed as `grow/queue/interject`;
    /// the `grow/session/interjection` + `grow/queue/changed` rebroadcasts are
    ...
    /// `GROW_SCROLL_LOG=1` enables it from startup).
    ToggleScrollLog,
    ...
    /// Add or update an MCP server via grow/mcp/upsert.
    UpsertMcpServer {
        name: String,
        config: Box<grow_shell::util::config::McpServerConfig>,
    },
    /// Delete an MCP server via grow/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via grow/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    DemoteToBackground,
    /// Request current bundle cache status via `grow/bundle/status`.
    RequestBundleStatus,
    ...
    /// Commit the active theme (canonical name, e.g. `"grownight"`, `"auto"`).
    SetTheme(String),
...
pub enum Effect {
    ...
    /// `grow/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `grow/session/list` — the non-leader fallback for the
    ...
    /// Cancel a subagent via `grow/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.grow/config.toml`.
    ...
    /// Resolve the running agent name for a session (`grow/session/info`).
    ...
    /// Fetch hooks list from the shell (grow/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (grow/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Fetch skills list from the shell (grow/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Toggle a skill via grow/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    /// Install a plugin from the inline CTA via `grow/marketplace/action`,
    ...
    /// Reload plugins after a CTA install via `grow/plugins/action`
    ...
    /// Read the MCP server list after a CTA install via `grow/mcp/list`,
    ...
    /// the same `grow/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via
    ...
    /// Upsert an MCP server via grow/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via grow/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via grow/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Send raw note to grow/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via grow/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
        question: String,
    ...
    /// Request a session recap via the grow/recap ext method. Fire-and-forget:
    ...
    /// Call `grow/session/fork` to create a peer session that resumes
    ...
    /// Send an ACP `grow/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `grow/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`grow-build-0.1` when the
...
pub enum TaskResult {
    ...
    /// Leader session roster loaded via `grow/sessions/list`.
    RosterLoaded {
    ...
    CancelComplete,
    /// Response to `grow/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<grow_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<grow_shell::util::changelog::ChangelogEntry>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<grow_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<grow_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<grow_shell::extensions::notification::PromptUsage>,
    },
    ...
    /// `grow/session/fork` failed. The placeholder agent stays in
    ...
    /// Shell suggestions loaded from ACP `grow/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `grow/suggestPrompt`.
