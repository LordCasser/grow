...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `grow-pager-minimal`
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&grow_shell::util::config::RemoteSettings>,
    eligible: bool,
...
fn validate_llm_config(raw: &toml::Value) -> Result<(), String> {
    grow_shell::agent::config::Config::new_from_toml_cfg(raw)?.validate_llm_configuration()
}

fn ensure_llm_configured(mut raw: toml::Value) -> anyhow::Result<toml::Value> {
    let Err(reason) = validate_llm_config(&raw) else {
        return Ok(raw);
    };
    let path = grow_tools::util::grow_home::grow_home().join("config.toml");
    eprintln!("No LLM is configured: {reason}");
    eprintln!(
        "Configure one in {}:\n\n{LLM_CONFIG_TEMPLATE}",
        path.display()
    );
    if !io::stdin().is_terminal() {
        anyhow::bail!(
            "No LLM is configured. Update {} with a [provider.<id>] model and [models].default before connecting.",
            path.display()
        );
    }

    eprint!("Press Enter to open the configuration file, or Ctrl-C to cancel: ");
    io::stderr().flush()?;
    ...
    raw = grow_shell::config::load_effective_config()
        .map_err(|error| anyhow::anyhow!("Failed to reload config: {error}"))?;
    validate_llm_config(&raw).map_err(|error| {
        anyhow::anyhow!(
            "LLM configuration is still incomplete in {}: {error}",
            path.display()
        )
    })?;
    Ok(raw)
...
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<grow_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ...
    let raw_config = ensure_llm_configured(raw_config)?;
    ...
    let env_hunk_tracker_mode = std::env::var("GROW_HUNK_TRACKER").ok();
    ...
    let launch_auto = grow_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  grow --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  grow --resume {}", info.session_id);
    }
}
...
fn init_terminal(
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        if mode.is_fullscreen() {
            grow_shell::util::with_locked_stderr(|stderr| execute!(stderr, EnterAlternateScreen))?;
        }
        #[cfg(windows)]
        if want_minimal {
...
/// BEL/ESC (titles can arrive from service.example.com conversation metadata) would
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - grow"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "grow");
        assert_eq!(terminal_title_string(""), "grow");
        assert_eq!(terminal_title_string("My chat"), "My chat - grow");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["grow-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["grow-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["grow-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["grow-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["grow-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["grow-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = grow_shell::util::config::RemoteSettings {
    ...
    fn cli_resume_parses_session_id() {
        let args = try_parse_pager(&["grow-pager", "--resume", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    fn cli_short_r_parses_session_id() {
        let args = try_parse_pager(&["grow-pager", "-r", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    ...
    fn cli_continue_short_c_parses() {
        let args = try_parse_pager(&["grow-pager", "-c"]).unwrap();
        assert!(args.continue_last_session);
    }
    #[test]
    fn cli_resume_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["grow-pager", "--resume"]).unwrap();
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["grow-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["grow-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["grow-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["grow-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["grow-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["grow-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["grow-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["grow-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["grow-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["grow-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["grow-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["grow-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_grow() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "grow");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "Grow TUI",
                "",
                "Usage: grow [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["grow-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["grow-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  grow --resume sess-abc\n"));
    }
    #[test]
