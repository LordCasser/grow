...
use grow_shell::sampling::error::http_status_from_error;
use grow_shell::session::{ExtMethodResult, SessionInfoResponse};
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = grow_shell::active_sessions::register(grow_shell::active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let mcp_servers = grow_shell::util::config::load_mcp_servers(
                &session_cwd,
                &compat,
            );
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = grow_shell::util::config::worktree_type();
        ...
        Effect::ScanForeignSessions { cwd, compat, grow_home, coordinator, seq } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            grow_workspace::foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &grow_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || grow_workspace::foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = grow_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use grow_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    let params = grow_shell::extensions::task::KillTaskRequest {
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            grow_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            grow_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    grow_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = grow_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            serde_json::from_value::<
                                Vec<
                                    grow_tools::implementations::skills::types::SkillInfo,
                                >,
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: grow_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text } => {
            use grow_shell::session::ClientType;
            use grow_shell::session::acp_types::ClientFeedbackInput;
            ...
            tasks
                .spawn(async move {
                    let input = ClientFeedbackInput {
                        ...
                        turn_number: None,
                        request_id: None,
                        client_version: Some(grow_version::VERSION.to_string()),
                        metadata: None,
                        terminal_info,
                    };
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = grow_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    grow_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            grow_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionTitleFromDisk { agent_id, session_id, cwd } => {
            tasks
                .spawn(async move {
                    ...
                    let path = grow_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    let title = tokio::task::spawn_blocking(move || -> Option<
                            (String, bool),
                        > {
                            let raw = std::fs::read_to_string(path).ok()?;
                            let summary: grow_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent } => {
            ...
            tasks
                .spawn(async move {
                    use grow_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "grow/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !grow_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let grow_home = grow_shell::util::grow_home::grow_home();
                            let store = grow_shell::auth::read_auth_json(
                                    &grow_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = grow_shell::auth::ServiceAuthConfig::default()
                                .auth_scope();
                            let auth = grow_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    grow_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            grow_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<grow_shell::extensions::notification::PromptUsage, String> {
    let request = acp::ExtRequest::new(
        "grow/session/usage",
        serde_json::value::to_raw_value(
                &serde_json::json!({
    ...
    let parsed: grow_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
async fn lookup_session_title(session_id: &acp::SessionId) -> Option<String> {
    let summaries = grow_shell::session::persistence::list_summaries(None)
        .await
        .ok()?;
    summaries
        .into_iter()
...
fn format_session_info(
    ...
    let model_display = grow_shell::session::model_display_name(
    ...
    let model_hash_line = if grow_shell::session::should_show_model_fingerprint(
    ...
    let backend_line = info
        ...
        .map(|b| format!("\n  API Backend: {b}"))
        .unwrap_or_default();
    let sandbox_line = grow_sandbox::profile_name()
        .map(|profile| format!("\n  Sandbox: {profile}"))
        .unwrap_or_default();
