...
use super::*;
use grow_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade to a Grow subscription for higher limits: https://service.example.com/provider_plan";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(&rpm, false).contains("service.example.com/provider_plan"));
...
fn parse_kill_outcome_reads_result_envelope() {
    use grow_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use grow_workspace::session::git::RestoreDegree;
...
/// Redirect `GROW_HOME` to a tempdir for test isolation.
fn setup_grow_home_in_tempdir() -> tempfile::TempDir {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use grow_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            grow_shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: grow_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            grow_home: PathBuf::from("/path/that/must/not/be-read"),
...
/// browse opts into `allowRelax` and parses `grow/listScope`; all
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "grow/session/list");
    ...
    match tasks.join_next().await.expect("task").expect("no panic") {
        TaskResult::SessionListLoaded { scope, seq, query, .. } => {
            ...
            assert!(
                    scope.is_relaxed(),
                    "_meta[\"grow/listScope\"] must parse into the task result"
                );
        }
        other => panic!("expected SessionListLoaded, got {other:?}"),
    }
...
async fn fetch_workflows_list_sends_session_id() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "grow/workflows/list");
                let params: serde_json::Value = serde_json::from_str(
                        args.request.params.get(),
                    )
                    .expect("params JSON");
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> grow_shell::session::SessionInfoResponse {
    use grow_shell::session::acp_types::{ContextInfo, SessionInfoData};
    grow_shell::session::SessionInfoResponse {
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    assert_eq!(roster.model_id.as_deref(), Some("grow-4"));
