...
pub enum Command {
    /// Run Grow without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Grow discovers for this directory
    Inspect {
...
Examples:
  grow wrap docker exec -it my-container bash
  grow wrap kubectl exec -it my-pod -- bash
...
")]
    ...
    /// `~/.grow/config.toml` or when the `GROW_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "grow: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("grow: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub struct PagerArgs {
    ...
    /// Use a custom leader socket path instead of the default `~/.grow/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            grow_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Session-scoped only — does not write config. To default plain `grow` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.grow/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `grow "fix the bug"` or `grow --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`GROW_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                grow_shell::session::persistence::resumed_session_sandbox_profile(Some(&id), cwd)
            }
            ResumeTarget::MostRecentForCwd => {
                grow_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<grow_sandbox::ProfileName>().ok()
                    != s.parse::<grow_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `grow "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["grow"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["grow", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["grow", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["grow", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json =
            PagerArgs::try_parse_from(["grow", "doctor", "--json"]).expect("doctor --json parses");
        assert!(matches!(
            json.command,
            Some(Command::Doctor(crate::doctor_cmd::DoctorArgs {
                json: true,
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["grow", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["grow", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["grow", "doctor", "all"],
            vec!["grow", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["grow", "doctor", "fix", "--yes"],
            vec!["grow", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["grow"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["grow", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["grow", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["grow", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["grow", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["grow", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["grow", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["grow", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["grow", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["grow", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["grow", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["grow"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["grow", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["grow"]).expect("bare grow parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["grow", "leader", "list", "--json"])
            .expect("grow leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["grow", "leader", "info", "--pid", "42"])
            .expect("grow leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["grow", "leader", "kill"]).expect("grow leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["grow", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["grow", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["grow", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args =
            PagerArgs::try_parse_from(["grow", "fix the bug"]).expect("positional prompt parses");
        assert_eq!(args.initial_prompt(), Some("fix the bug"));
        assert!(args.command.is_none());
        assert!(args.single.is_none());
    }
    #[test]
    fn bare_grow_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["grow"]).expect("bare grow parses");
        assert_eq!(args.initial_prompt(), None);
    }
    #[test]
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args = PagerArgs::try_parse_from(["grow", "  spaced  "]).expect("padded prompt parses");
        assert_eq!(args.initial_prompt(), Some("spaced"));
        let blank = PagerArgs::try_parse_from(["grow", "   "]).expect("blank prompt parses");
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["grow", "-w", "x"]).expect("-w x parses");
