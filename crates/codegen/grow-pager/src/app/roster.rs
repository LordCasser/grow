...
//!
//! - Request/response `grow/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `grow/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `grow/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `grow/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `grow/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `grow-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `grow/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `grow/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> grow_shell::agent::roster::RosterEntry {
        use grow_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `grow/sessions/list`.
    ...
    /// `grow/sessions/changed` round-trip: serialize the agent's `RosterChanged`
