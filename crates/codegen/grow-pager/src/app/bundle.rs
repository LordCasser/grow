...
//!
//! Pager-side cache of what `grow-shell` reports from
//! `grow/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `grow/bundle/status` ACP responses.
...
/// Deserialized response from `grow/bundle/status`.
...
/// Deserialized response from `grow/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
