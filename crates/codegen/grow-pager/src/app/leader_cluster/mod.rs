...
//!
//! ```bash
//! cargo test -p grow-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
struct PagerLeaderCluster {
    ...
    /// agent on the same GROW_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    ...
    async fn start() -> Self {
        ...
        let sock_path = grow_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("GROW_HOME", grow_home.path()),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        // grow-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: grow-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same GROW_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `GROW_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        if !self.authenticated {
            let _: acp::AuthenticateResponse = bounded(
                "authenticate",
                acp_send(
                    acp::AuthenticateRequest::new(acp::AuthMethodId::new("provider.api_key"))
