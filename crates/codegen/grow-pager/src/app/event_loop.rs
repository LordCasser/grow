...
/// [`decide`](grow_workspace::folder_trust::decide) precedence.
///
...
fn seed_trust_state(app: &mut AppView, remote: Option<&grow_shell::util::config::RemoteSettings>) {
    use grow_workspace::folder_trust::{
        TrustOutcome, decide, decide_inputs_with_interactive, feature_enabled,
    };
    use grow_workspace::trust::workspace_key;
...
pub(crate) async fn run(
    ...
    let launch_effective_ui = grow_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(grow_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = grow_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match grow_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.session_picker_grouped = std::env::var("GROW_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            grow_shell::config::load_effective_config()
                .ok()
    ...
    let requirements = grow_shell::config::load_merged_requirements();
    let user_config = grow_shell::config::load_from_disk().ok();
    let managed_config = grow_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match grow_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        grow_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        grow_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        grow_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    app.scheduler_background_loops_seed =
        grow_shell::util::config::resolve_scheduler_background_loops(
            remote_settings
                .as_ref()
...

    {
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = grow_shell::util::tips::pick_and_advance(&app.tips, &grow_home);
        }
    ...
    let hints = grow_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.grow/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        grow_shell::util::config::clamped_display_permission_mode(
            grow_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        grow_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = grow_shell::util::config::resolve_contextual_hints(
    ...
    // GROW_SCROLL_SPEED/_MODE/_LINES + GROW_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    // `grow dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `GROW_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("GROW_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.grow/config.toml [ui]`
                // rather than `~/.grow/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> grow_shell::agent::config::UiConfig {
    use grow_shell::agent::config::UiConfig;
    let Ok(root) = grow_shell::config::load_effective_config() else {
