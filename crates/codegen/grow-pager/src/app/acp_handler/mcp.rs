...
/// `GROW_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        grow_shell::util::config::resolve_mcp_push_server_status(
...
/// Handle `grow/mcp/tools_changed` and `grow/mcp_initialized`.
///
/// Routing rules (verified against the four shell emit sites in
/// `grow-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`grow_shell::extensions::mcp::McpToolsChanged`] struct), and
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    // Both `grow/mcp_initialized` and (newer shell)
    // `grow/mcp/tools_changed` carry `sessionId`. Route by it so a
    ...
    if method == "grow/mcp_initialized"
...
/// ([`grow_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`grow_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use grow_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "grow/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
...
/// Handle `grow/mcp/servers_updated`.
///
/// Emitted by the shell from `MvpAgent` on managed-config resolve and
/// on config reload (`crates/codegen/grow-shell/src/agent/mvp_agent.rs`
