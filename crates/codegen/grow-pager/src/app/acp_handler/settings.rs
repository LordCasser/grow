...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(campaigns) = update.campaigns.clone() {
        ...
        grow_shell::util::config::set_remote_campaigns_from_settings(Some(&rs));
    }
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = grow_shell::config::load_effective_config().ok();
    ...
    // Always recompute API-key state from the tier so a later provider-plan
    // stamp does not leave the billing surface hidden.
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(grow_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        grow_shell::config::load_merged_requirements(),
        grow_shell::config::load_from_disk().ok(),
        grow_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = grow_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = grow_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = grow_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use grow_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use grow_shell::util::config::resolve_slash_command_tags;
        let effective_config = grow_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = grow_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        grow_shell::util::config::resolved_display_permission_mode(effective_ui, remote).to_string()
    });
}
...
/// `grow-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `grow-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
...
pub(super) struct PagerSettingsUpdate {
    ...
    #[serde(default)]
    campaigns: Option<Vec<grow_shell::util::config::CampaignOverride>>,
    #[serde(default)]
