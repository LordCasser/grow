...
pub static USER_GUIDE: &[Doc] = &[
    ...
    guide!(
        ...
        "ACP stdio/WebSocket server transports and SDK integration"
    ),
    guide!(
        "16-subagents.md",
...
pub fn extract_user_guide_docs(grow_home: &std::path::Path) {
    let docs_dir = grow_home.join("docs").join("user-guide");
