...
//! the generic [`CodeBlockSpan`](grow_markdown::CodeBlockSpan) API) and
...
use grow_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](grow_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](grow_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a grow_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `GrowDay` is the only light theme; every other concrete theme (and the
/// `GrowNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `grow_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::GrowDay)
}
...
mod tests {
    ...
    use grow_markdown::StreamingMarkdownRenderer;
    ...
    #[test]
    fn theme_is_dark_maps_growday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::GrowDay),
            "GrowDay is the light theme"
        );
        for dark in [
            ThemeKind::GrowNight,
