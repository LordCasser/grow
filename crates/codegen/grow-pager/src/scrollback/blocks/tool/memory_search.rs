...
fn shorten_path(path: &str) -> &str {
    let memory_root = grow_config::grow_home().join("memory");
...
**File:** /root/.grow/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.grow/memory/MEMORY.md");
...
**File:** /root/.grow/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.grow/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured grow memory root keep one trailing segment group.
        let memory_root = grow_config::grow_home().join("memory");
        let session = memory_root.join("grow-50aa78f0/sessions/2026-05-01.md");
