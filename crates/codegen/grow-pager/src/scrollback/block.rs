...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: grow_shell::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(&self, f: impl FnOnce(&[grow_markdown::HyperlinkTarget]) -> R) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    fn context_info_indexes_model_only() {
        ...
        let block = RenderBlock::context_info(snapshot, "grow-4.5");
        // Only the model name is source text; the rest is a numeric breakdown.
        assert_eq!(block.searchable_text().as_deref(), Some("grow-4.5"));
    }
