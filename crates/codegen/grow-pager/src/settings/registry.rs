...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"grownight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"Grow Night"`).
...
mod tests {
        ...
        for meta in reg.all() {
            ...
            match (meta.key, &meta.kind) {
                ...
                ("show_tips", SettingKind::Bool { default }) => {
                    assert!(!*default, "show_tips registry default must be false");
                }
                ("auto_update", SettingKind::Bool { default }) => {
                    assert!(*default, "auto_update registry default must be true");
                }
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in grow-tools"
                    );
                }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: grow_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
