...
//! primitive consumed cross-crate by `grow-workspace`, not agent-definition
//! discovery.
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.grow`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `grow-workspace`, which depends on THIS crate) to keep the dep edge
