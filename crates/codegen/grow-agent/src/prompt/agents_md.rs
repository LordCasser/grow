...
async fn read_agents_config_with_options(
    ...
    read_agents_config_with_roots(
        ...
        workspace_user_dir,
        compat,
        grow_tools::util::grow_home::grow_home(),
        dirs::home_dir(),
    )
    .await
}
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                grow_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `grow-shell` to structurally detect legacy untagged AGENTS.md
