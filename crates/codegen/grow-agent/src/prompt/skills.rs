...
//! live in `grow_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use grow_tools::implementations::skills::types::skill_name_from_path;
pub use grow_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `grow_tools` directly.
pub use grow_tools::types::compat::CompatConfig;

use grow_tools::implementations::skills::discovery::{
...
/// (e.g. `Repo` for `.grow/plugins/`). Native skills always win bare-name
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_roots(
        working_directory,
...
async fn list_skills_with_roots(
    working_directory: Option<&str>,
    workspace_user_dir: Option<&Path>,
    user_roots: &[PathBuf],
    compat: CompatConfig,
    ...
    for root in user_roots {
        collect_discovered_paths(
            find_skill_paths(&root.join("bundled")),
            SkillScope::Bundled,
            &mut seen_canonical_paths,
            &mut skill_files,
        );
...
/// Ordered roots used by user-level Skill discovery and file watching.
pub fn user_skill_roots() -> Vec<PathBuf> {
    crate::resource_roots::user_roots(dirs::home_dir().as_deref())
}
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(grow_tools::types::config_source::ConfigSource::ConfigToml {
...
fn stamp_plugin_fields(skills: &mut [SkillInfo], plugin: &crate::plugins::LoadedPlugin) {
    ...
    for skill in skills.iter_mut() {
        ...
        skill.config_source = Some(grow_tools::types::config_source::ConfigSource::Plugin {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill
        ...
        .as_ref()
        .filter(|b| !b.is_empty())
        .map(|body| grow_tools::implementations::skills::skill::build_skill_message(skill, body))
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || grow_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match grow_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    async fn server_skills_discovered_and_shadowed_by_local() {
        ...
        write_skill_md(&cwd.path().join(".grow").join("skills").join("dup"), "dup");
    ...
    fn find_skill_paths_flat_layout() {
        ...
        write_skill_md(&grow_dir.join("skills").join("alpha"), "alpha");
        write_skill_md(&grow_dir.join("skills").join("beta"), "beta");

        let paths = find_skill_paths(&grow_dir);
    ...
    fn find_skill_paths_nested_layout() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let grow_dir = tmp.path().join(".grow");
        let skills = grow_dir.join("skills");
    ...
    fn find_skill_paths_no_skills_dir() {
        // .grow exists but no skills/ subdirectory
        let tmp = tempfile::tempdir().unwrap();
        let grow_dir = tmp.path().join(".grow");
    ...
    async fn list_skills_includes_workspace_user_skills() {
        ...
        write_skill_md(
            &user_dir.join(".grow").join("skills").join("my-tool"),
            "my-tool",
        );
    ...
    async fn list_skills_workspace_user_dedup_when_cwd_inside_user_dir() {
        ...
        let user_dir = repo_root.join("x").join("testuser");
        write_skill_md(
            &user_dir.join(".grow").join("skills").join("dedup-skill"),
            "dedup-skill",
        );
    ...
    async fn list_skills_no_workspace_user_dir_no_extra_skills() {
        ...
        write_skill_md(
            &user_dir.join(".grow").join("skills").join("ghost-skill"),
            "ghost-skill",
        );
    ...
    async fn list_skills_workspace_user_dir_with_recursive_skills() {
        ...
        let skills_base = user_dir.join(".grow").join("skills");
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(grow_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    fn stamp_plugin_fields_sets_root_and_data() {
        ...
        let plugin = LoadedPlugin {
            ...
            origin: crate::plugins::PluginOrigin::UserGrow,
            trusted: true,
            enabled: true,
    ...
    async fn list_skills_deduplicates_auto_and_config_overlap() {
        ...
        let auto_dir = repo_root.join(".grow").join("skills").join("dup-skill");
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        let auto_dir = repo_root.join(".grow").join("skills").join("overlap-skill");
        ...
        assert!(
            matches!(
                ...
                Some(grow_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    fn dedupe_skills_name_collision_does_not_propagate_config_source() {
        ...
        loser.config_source = Some(grow_tools::types::config_source::ConfigSource::ConfigToml {
    ...
    async fn list_skills_ignore_allows_lower_priority_same_name_fallback() {
        ...
        write_skill_md(&cwd.join(".grow").join("skills").join("same"), "same");
        let repo_skill_dir = repo_root.join(".grow").join("skills").join("same");
    ...
    // these functions now live in grow-tools::implementations::skills::discovery
    // and grow-tools::types::skill_discovery_tracker, tested there.
    ...
    async fn disabled_config_marks_skill_enabled_false() {
        ...
        write_skill_md(
            &repo_root.join(".grow").join("skills").join("commit"),
            "commit",
        );
        write_skill_md(
            &repo_root.join(".grow").join("skills").join("review"),
            "review",
        );
    ...
    async fn disabled_config_empty_leaves_all_enabled() {
        ...
        write_skill_md(
            &repo_root.join(".grow").join("skills").join("deploy"),
            "deploy",
        );
    ...
    fn min_plugin(name: &str) -> crate::plugins::LoadedPlugin {
        ...
        crate::plugins::LoadedPlugin {
            ...
            origin: crate::plugins::PluginOrigin::ProjectGrow,
            trusted: true,
            enabled: true,
            version: Some("1.0.0".to_string()),
            description: None,
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        for name in [".grow", ".agents", ".claude", ".cursor"] {
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.grow/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.grow/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.grow/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        assert_eq!(out.len(), 1);
        assert!(out[0].path.contains(".grow"));
    }
    ...
    async fn copied_skill_dir_with_stale_frontmatter_name_surfaces_both() {
        ...
        let skills_dir = repo_root.join(".grow").join("skills");
