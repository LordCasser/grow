...
//! - `Default`: the legacy Grow prefix (built by the shell layer).
...
use grow_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
///
/// Built-in variants use Grow's default first-message rendering behavior.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum UserMessageTemplate {
    /// Legacy Grow prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .grow/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `grow_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.grow/, ~/.claude/).
    ...
    /// (`~/.grow/projects/<encoded-cwd>/mcps`). Surfaced in
