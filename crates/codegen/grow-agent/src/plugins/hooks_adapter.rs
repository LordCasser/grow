...
//! `grow-hooks`' parser and inject plugin env vars. Not a second engine.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use grow_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        spec.layer = grow_hooks::config::HookProvenance::Plugin;
        spec.name = format!(
            "{}{}/{}",
            grow_hooks::config::PLUGIN_HOOK_PREFIX,
            plugin_name,
            spec.name
        );
        ...
        if let Some(cmd) = &spec.command {
            ...
            let expanded = grow_config::expand_env_vars_in_string(&substituted);
            if expanded != cmd_str {
...
mod tests {
    ...
    fn parse_plugin_hooks_from_file() {
        ...
        assert_eq!(
            specs[0].extra_env.get("GROW_PLUGIN_ROOT").unwrap(),
            "/path/to/plugin"
        );
        assert_eq!(
