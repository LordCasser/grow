...
//!
//! **Trust storage**: `~/.grow/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.grow/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.grow/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_grow_home() so a project's `.grow/trusted-plugins` is never
        // read as the user trust store when neither GROW_HOME nor a home dir resolves.
        let Some(grow) = grow_config::user_grow_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = grow.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.grow/trusted-plugins`.
    ...
    /// rewrites `~/.grow/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.grow/trusted-plugins`.
