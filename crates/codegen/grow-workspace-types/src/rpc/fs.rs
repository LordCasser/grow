...
//! the shell's `grow/fs/*` ACP methods.
...
// workspace server (`grow-workspace`) and the service.example.com backend
...
// `workspace.fs_*` ops above already serve the shell's `grow/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `grow/fs/list`
/// node `type` strings.
...
/// `grow/fs/list` plus `offset` pagination).
