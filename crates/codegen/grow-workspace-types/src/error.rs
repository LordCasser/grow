...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] grow_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `grow-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `grow_tools::ToolError`) happens
