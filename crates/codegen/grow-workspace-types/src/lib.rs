//! Wire types for the `grow-workspace` API.
//!
...
//! `grow-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `grow-workspace-types` (instead of `grow-workspace` or
/// `grow-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `grow_workspace::permission`
