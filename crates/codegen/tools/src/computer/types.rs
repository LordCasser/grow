...
/// (`shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in tools, spawns processes)
/// - `AcpTerminalBackend` (in shell, calls ACP protocol)
#[async_trait::async_trait]
