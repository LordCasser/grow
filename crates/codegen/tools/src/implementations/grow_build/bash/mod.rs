...
pub struct BashParams {
    ...
    /// grow-build opts up to 10h via shell's `BashToolConfig`.
    ///
    /// When set:
...
// particular production grow-build sets it to 10h in shell's
...
impl BashTool {
    ...
    /// cursor `Shell` adapter uses this ceiling to report the true FG wait in
    ...
    async fn run(
        ...
        let (
            ...
            (
                ...
                res.get::<crate::implementations::grow_build::task::types::CurrentSubagentOwnerResource>()
                    .and_then(|owner| owner.0.goal_definition_revision()),
            )
        };
...
mod tests {
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // shell opts up to 10h via max_timeout_secs. Background unaffected.
