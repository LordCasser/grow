...
mod tests {
    ...
    ) -> crate::implementations::grow_build::task::types::SubagentSnapshot {
        crate::implementations::grow_build::task::types::SubagentSnapshot {
    ...
    async fn failed_spawn_resets_chain_state() {
        ...
        let _ = request.result_tx.send(
            crate::implementations::grow_build::task::types::SubagentResult {
                success: false,
    ...
    async fn durable_expiry_persists_before_ack_and_commits_version() {
...

        {
            ...
            assert_eq!(
                snapshot["state"]["grow_build.Scheduler"]["tasks"],
                serde_json::json!([])
            );
            assert!(notifications.try_recv().is_err());
