...
impl tool_runtime::Tool for SearchTool {
    ...
    fn id(&self) -> tool_protocol::ToolId {
        tool_protocol::ToolId::new("search_tool").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let tool_index = resources.lock().await.get::<ToolIndex>().cloned();
...
mod tests {
    ...
    async fn search_tool_groups_gateway_result_by_connector_name() {
        ...
        assert_eq!(json["results"][0]["access"], "call_bound");
        assert_eq!(json["results"][1]["server"], "Slack");
        assert_eq!(json["results"][1]["access"], "call_bound");
        assert_eq!(
    ...
    fn hash_value_pinned_output() {
        ...
        let h = hash_value(&"mcp-fingerprint-stability-test");
        assert_eq!(h, hash_value(&"mcp-fingerprint-stability-test"));
