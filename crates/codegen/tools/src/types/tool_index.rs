...
//!
//! `ToolSearchIndex` is defined in `tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `shell`
...
/// in `shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `shell` after MCP initialization.
#[derive(Clone)]
