...
/// `"namespace.Name"` (e.g., `"grow_build.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"grow_build.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("grow_build", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "grow_build.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"grow_build.ReadFile"`).
    id: String,
...
pub use tool_types::ModelImageInputKey;
...
/// depending on `mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("grow_build", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("grow_build", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("grow_build.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "grow_build.ReadFile".to_string(),
            serde_json::json!({"files_read": ["loaded.rs"]}),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "grow_build.Edit".to_string(),
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "grow_build.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "grow_build.Edit").is_none());
    }
    #[test]
