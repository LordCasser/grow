...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (grow build harness) ─────────────
...
///
/// `"/path/.grow/skills/my-skill/SKILL.md"` -> `"/path/.grow/skills/"`
...
/// Used by the budgeted (grow build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Grow build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the grow
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // grow-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: grow build harness (budgeted XML) ───────────
