//! Canonical external-settings tool name ↔ Grow tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`hooks`) needs the
//! Grow tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`agent`) needs the [`ToolKind`] a `tools:`
...
/// One Claude tool's correspondence to Grow, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Grow [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Grow tool names this Claude tool maps to (empty when there is no direct
    /// Grow tool — the entry then only contributes a `kind`).
    grow: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, grow: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        grow,
...
const fn match_only(claude: &'static str, grow: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        grow,
...
/// The Grow [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Grow tool names a Claude matcher term fires on.
pub fn grow_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.grow)
...
/// The Claude names that map to `grow_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(grow_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.grow.contains(&grow_name))
        .map(|t| t.claude)
}

/// Every distinct Grow name the table references, for the `agent` drift-check
/// test that asserts each is a real client tool name.
pub fn grow_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.grow.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Grow name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.grow.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
