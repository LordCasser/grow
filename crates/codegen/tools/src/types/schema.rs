...
///
/// Use with `#[schemars(with = "GrowIntegerSchema")]` on `Option<usize>` fields.
pub struct GrowIntegerSchema;
impl schemars::JsonSchema for GrowIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "grow_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `tool-types`), re-exported so
