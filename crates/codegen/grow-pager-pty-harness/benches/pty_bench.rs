//! `pty-bench` — PTY benchmark CLI for `grow-pager`.
//!
...
//! ```bash
//! cargo bench -p grow-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/grow-${VERSION}-linux-x86_64 \
//!   cargo bench -p grow-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for grow-pager",
    long_about = None,
)]
struct Cli {
