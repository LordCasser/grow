//! `paste-latency` — clipboard paste latency benchmark for `grow-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p grow-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p grow-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/grow-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for grow-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
