//! Unified PTY harness for grow-pager.
//!
...
//!    `grow-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `GROW_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
pub use flows::{
    inference_request_count, submit_turn, wait_for_labels_absent, wait_for_model_via_new_sessions,
};
...
impl PtyHarness {
    ...
    /// (XTVERSION probes and grow wrap). Content-backed launches must use
    /// [`Self::new_in_sandbox`].
    ...
    /// Spawn from a canonical [`grow_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &grow_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use grow_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through grow's stdout. Used to reproduce the doubled-line class of bugs
    /// where grow's diff renderer never re-asserts a region it didn't write
