...
//! [`MockInferenceServer`] from `grow-test-support` and provides the
...
use grow_test_support::{MockInferenceServer, TestSandbox};
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        sandbox.set_env("GROW_PROMPT_SUGGESTIONS", "false");
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.grow
...
mod tests {
    ...
    async fn read_foreground(url: String, endpoint: InferenceEndpoint) -> String {
        ...
        reqwest::Client::new()
            ...
            .header("x-grow-req-id", format!("direct-{endpoint:?}"))
            .json(&body)
            .send()
            .await
