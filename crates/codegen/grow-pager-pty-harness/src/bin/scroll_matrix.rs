//! `scroll-matrix` — scroll validation matrix sweep for grow-pager.
//!
...
use grow_pager_pty_harness::pager_binary;
use grow_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against grow-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_grow-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves GROW_SCROLL_LOG against ITS cwd (the
