...
//! `ProviderAuth` → auth meta → `AppView::coding_data_sharing_lock()` →
...
//! `GROW_PRIVACY_NOTICE_ROLLOUT=1` — asserted on the authenticated welcome
//! screen before opening settings. Row/input details are unit-tested in
//! `grow-pager` (`views/settings_modal/tests.rs`, `locked_coding_*`);
//! this suite covers the auth-to-render pipeline.
//!
//! ```bash
//! cargo test -p grow-pager-pty-harness --test settings_locked_row_e2e \
//!   -- --ignored --nocapture
//! ```
...
const ROWS: u16 = 50;
const COLS: u16 = 120;
const BANNER_TITLE: &str = "Help improve Grow";
...
/// the sandbox's fake `GROW_API_KEY` removed so the seeded team OAuth entry is
...
fn locked_row_env_ops() -> [EnvOp<'static>; 3] {
    [
        EnvOp::set("GROW_PRIVACY_NOTICE_ROLLOUT", "1"),
        EnvOp::set("GROW_ZDR_ACCESS_ENABLED", "1"),
        EnvOp::remove("GROW_API_KEY"),
    ]
}
