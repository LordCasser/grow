...
//! as the active auth (`GROW_API_KEY` removed) and the rollout forced on via
//! `GROW_PRIVACY_NOTICE_ROLLOUT=1`.
//!
//! ```bash
//! cargo test -p grow-pager-pty-harness --test privacy_banner_e2e \
//!   -- --ignored --nocapture
//! ```
...
/// sandbox's fake `GROW_API_KEY` removed so the seeded opted-out OAuth entry
...
/// Poll `<home>/.grow/config.toml` for the async `privacy_banner_acked`
...
fn wait_for_ack_on_disk(pager: &mut PtyHarness, home: &Path, timeout: Duration) -> Result<()> {
    let path = home.join(".grow").join("config.toml");
