//! Grow-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$GROW_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$GROW_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$GROW_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked GROW_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedGrowHome { path: PathBuf },
    ...
    #[error("Grow hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Grow hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Grow hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Grow hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$GROW_HOME/hooks` dir + `hooks-paths` file (create if missing).
/// Race-resistant create (`create_dir` / `create_new`+`O_NOFOLLOW`); never
/// truncates an existing registry; rejects symlinks/wrong types.
pub fn ensure_grow_hook_slots(grow_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(grow_home) {
        return Err(GlobalHookSourceError::SymlinkedGrowHome {
            path: grow_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(grow_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(grow_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: grow_home.to_path_buf(),
                    source,
                }
    ...
    let grow_meta = std::fs::symlink_metadata(grow_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: grow_home.to_path_buf(),
            source,
        }
    })?;
    if grow_meta.file_type().is_symlink() || !grow_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedGrowHome {
            path: grow_home.to_path_buf(),
        });
    }

    let hooks = grow_home.join("hooks");
    ...
    let registry = grow_home.join("hooks-paths");
...
/// Resolve Grow-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(grow) = grow_home {
        if reject_symlinks && path_has_symlink_component(grow) {
            return Err(GlobalHookSourceError::SymlinkedGrowHome {
                path: grow.to_path_buf(),
            });
        }

        let hooks = grow.join("hooks");
        let hooks_paths = grow.join("hooks-paths");
        if reject_symlinks {
