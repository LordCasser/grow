...
/// A hook's origin (held by `hooks::HookSpec::layer`). Defined here, not
/// in `hooks`, since the dep direction is `hooks -> config`;
...
/// (`shell::util::config::load_effective_config`) when remote campaigns
