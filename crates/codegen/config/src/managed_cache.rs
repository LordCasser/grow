...
//! OS-protected layers (root-owned `/etc/grow`, MDM).
...
pub fn is_managed_config_stale_for(identity: &ServingIdentity) -> bool {
    managed_config_stale_at(user_grow_home().as_deref(), identity)
}
...
pub fn mark_managed_config_synced(marker: SyncMarker<'_>) {
    if let Some(home) = user_grow_home() {
...
pub fn managed_policy_compromised_for(identity: &ServingIdentity) -> bool {
    user_grow_home().is_some_and(|home| managed_policy_compromised_for_at(&home, identity))
}
...
/// Override with `GROW_DEPLOYMENT_CONFIG_CACHE_TTL_SECS` for testing.
fn managed_config_stale_threshold() -> std::time::Duration {
    if let Ok(s) = std::env::var("GROW_DEPLOYMENT_CONFIG_CACHE_TTL_SECS")
