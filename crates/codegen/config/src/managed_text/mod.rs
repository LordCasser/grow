...
impl ManagedConfig {
    pub fn plan(request: ManagedConfigRequest) -> Result<ManagedConfigPlan, ManagedConfigError> {
        ...
        let backup_path_hint = (changes && original.bytes.is_some())
            .then(|| transaction::artifact_hint(&target_path, "grow-backup"));
        let temp_path_hint = changes.then(|| transaction::artifact_hint(&target_path, "grow-tmp"));
        let lock_path = transaction::sibling_artifact(&target_path, "grow.lock");
