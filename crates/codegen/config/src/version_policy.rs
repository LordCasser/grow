...
fn version_candidates(layers: &crate::ConfigLayers, key: &str) -> Vec<String> {
...
impl VersionPolicy {
    ...
    pub fn resolve() -> Self {
        let layers = crate::ConfigLayers::load().unwrap_or_else(|e| {
            tracing::warn!(error = %e, "version policy: config layers failed to load; using env overrides only");
            crate::ConfigLayers::default()
        });
        Self::from_layers(&layers, &env_version)
    }

    fn from_layers<E: Fn(&str) -> Option<String>>(layers: &crate::ConfigLayers, env: &E) -> Self {
...
mod tests {
    ...
    fn layers(user: &str) -> crate::ConfigLayers {
        let parse = |s: &str| {
            if s.is_empty() {
