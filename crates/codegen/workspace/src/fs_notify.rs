...
//! FsNotify adapter functions bridging [`fsnotify`] events to
...
//! sent to the [`IndexManagerHandle`](codebase_graph::IndexManagerHandle).
...
/// Convert [`fsnotify::FsEventKind`] to the wire-type
/// [`workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => workspace_types::FsEventKind::Created,
        FsEventKind::Modified => workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: workspace_types::FsEventKind,
) -> codebase_graph::FileEvent {
    use codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        workspace_types::FsEventKind::Created => FileEventKind::Created,
        workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn workspace_event_kind_round_trip() {
        use workspace_types::FsEventKind as WsKind;
        assert_eq!(
