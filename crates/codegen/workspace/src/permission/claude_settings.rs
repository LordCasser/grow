...
// permissions), consider extracting to a shared helper (e.g., in hooks
...
// =============================================================================
//
// `shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in shell (inaccessible from here at runtime). They also
    ...
    let Some(config_path) = config::user_grow_home().map(|g| g.join("config.toml")) else {
        return false;
    };
