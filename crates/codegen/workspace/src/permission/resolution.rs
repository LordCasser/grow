...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    if let Some(global_path) = config::user_grow_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
...
/// [`config::claude_managed_settings_path`] (platform path such as
...
mod tests {
    ...
    /// `config::grow_home()` is a process-wide `OnceLock`, so under
...
"#,
        ...
        let global_live = config::user_grow_home()
