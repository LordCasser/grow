...
pub(crate) fn edit_target_protection(path: &Path) -> Option<ProtectedEditReason> {
    ...
    let lexical = paths::normalize_lexically(path);
...
fn protected_edit_reason(path: &Path) -> Option<ProtectedEditReason> {
    ...
    if let Some(reason) = protected_config_file(path, &string_components) {
        return Some(reason);
    }
...
fn protected_config_file(path: &Path, components: &[&str]) -> Option<ProtectedEditReason> {
    protected_config_file_with_home(path, components, config::user_grow_home().as_deref())
}

fn protected_config_file_with_home(
    path: &Path,
    components: &[&str],
...
fn grow_home_matches(home: Option<&Path>, pred: impl Fn(&Path) -> bool) -> bool {
    home.is_some_and(|home| {
        let lexical = paths::normalize_lexically(home);
        pred(&lexical)
...
fn path_is_under_user_grow_hook_root(path: &Path, grow_home: &Path) -> bool {
    path.starts_with(grow_home.join("hooks")) || path == grow_home.join("hooks-paths")
}
...
fn protected_grow_hook_root(path: &Path, components: &[&str]) -> bool {
    components.windows(2).any(|pair| pair == [".grow", "hooks"])
        ...
        || grow_home_matches(config::user_grow_home().as_deref(), |home| {
...
mod tests {
    ...
    #[test]
    fn path_is_under_user_grow_hook_root_matches_relocated_home() {
        let home = Path::new("/custom/grow-home");
        for path in [
            "/custom/grow-home/hooks/x.json",
            "/custom/grow-home/hooks/nested/deep.json",
            "/custom/grow-home/hooks",
            "/custom/grow-home/hooks-paths",
        ] {
            assert!(
                path_is_under_user_grow_hook_root(Path::new(path), home),
                "must match under custom grow home: {path}"
            );
        }
        for path in [
            "/custom/grow-home/hooks-disabled/note.json",
            "/custom/grow-home/hooks-evil/note.json",
            "/custom/grow-home/config.toml",
            "/custom/other/hooks/x.json",
            "/custom/grow-home-extra/hooks/x.json",
        ] {
            assert!(
                !path_is_under_user_grow_hook_root(Path::new(path), home),
    ...
    fn sensitive_edit_targets_follow_symlinks() {
        ...
        .unwrap();
        let grow_hook = outside.path().join(".grow/hooks/evil.json");
        std::fs::create_dir_all(grow_hook.parent().unwrap()).unwrap();
        std::fs::write(&grow_hook, b"{}").unwrap();
        symlink(&grow_hook, ws.path().join("grow-hook-link")).unwrap();

        for path in [
            ws.path().join("file-link"),
            ws.path().join("hooks-link/new-hook"),
            ws.path().join("module-hooks-link/new-hook"),
            ws.path().join("grow-hook-link"),
        ] {
            assert!(
    ...
    #[test]
    fn config_files_under_custom_grow_home_are_protected() {
        ...
        for (file, reason) in [
            ...
            let components = [file];
            assert_eq!(
                protected_config_file_with_home(&path, &components, Some(home_path)),
                Some(reason),
        ...
        assert_eq!(
            protected_config_file_with_home(&elsewhere, &["sub", "sandbox.toml"], Some(home_path)),
            None
        );
        assert_eq!(
            protected_config_file_with_home(
    ...
    #[test]
    #[cfg(unix)]
    fn config_under_symlinked_grow_home_is_protected() {
        use std::os::unix::fs::symlink;
    ...
    fn resolved_root_alias_matches_physical_destination() {
        ...
        assert!(resolved_path_is_within_root(
            &resolved_root.join("grow-test"),
            Path::new("/etc")
        ));
        assert!(!resolved_path_is_within_root(
            Path::new("/tmp/grow-test"),
            Path::new("/etc")
        ));
    }
