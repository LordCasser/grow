...
//! `agent` and `tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `tools` for external
...
// workspace crate without a direct tools dependency.
...
///
/// Delegates to [`agent::prompt::skills::list_skills`] with
...
///
/// Delegates to [`agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered =
        agent::plugins::discover_plugins(Some(root_cwd), config, trust_store, project_trusted);
