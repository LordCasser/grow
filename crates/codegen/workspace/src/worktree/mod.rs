...
/// Resolve the grow home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`fast_worktree::resolve_grow_home`), so checkout dirs and
/// the metadata DB always live under the same `.grow` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `config::grow_home()`,
/// so worktree paths also agree with trust/hooks and other grow-home paths.
fn grow_home() -> std::path::PathBuf {
    fast_worktree::resolve_grow_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.grow/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    grow_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.grow/worktrees/<repo_name>`)
/// for a given source path, correctly handling grow-managed worktrees.
///
/// When `source_path` is already under `~/.grow/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the grow worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = grow_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.grow/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the grow-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.grow/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to fast_worktree::WorkingTreeMode
    ...
    // Use fast-worktree for high-performance worktree creation
...
///
/// When the source path is already inside `~/.grow/worktrees/<repo>/`, the
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use fast-worktree -- it handles copying from any worktree path
...
///
/// ACP equivalent of `grow -w -r <session_id>` (optionally with `--ref`).
...
/// Response from `grow/git/worktree/resume_session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response from `grow/session/rehydrate`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
use fast_worktree::{
    ...
    rebuild_worktree_db, resolve_grow_home, resolve_worktree_auto_gc_from_layers,
};
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use config_types::WorktreeKindMaxAge;
...
/// Env + `$GROW_HOME/config.toml` only — **`remote=None` is intentional**.
///
...
/// processes; use `GROW_WORKTREE_AUTO_GC=0` / `GROW_WORKTREE_AUTO_GC_DRY_RUN=1`
/// or local TOML until remote is plumbed into `make_workspace_handle`.
fn resolve_worktree_auto_gc_local() -> fast_worktree::ResolvedWorktreeAutoGc {
    use config_types::WorktreeAutoGcSettings;

    let local = if let Ok(home) = resolve_grow_home() {
...
/// Scan `~/.grow/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/grow/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/grow/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/grow/subagents/standalone-1";
    ...
    /// Point `GROW_HOME` at an isolated tempdir (`resolve_grow_home` re-reads
    ...
    /// and restores `GROW_HOME` on drop (before releasing the lock), so the
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (GrowHomeFixture is not visible across crates).
