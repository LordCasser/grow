...
//! persisted to `~/.grow/tip_cursor.json`.
...
fn cursor_path(grow_home: &Path) -> PathBuf {
    grow_home.join(CURSOR_FILE)
}

/// Load the cursor from `~/.grow/tip_cursor.json`. Returns 0 on any error.
fn load_cursor(grow_home: &Path) -> u64 {
    let text = match std::fs::read_to_string(cursor_path(grow_home)) {
        Ok(t) => t,
        Err(_) => return 0,
    };
...
/// Save the cursor to `~/.grow/tip_cursor.json`. Silently ignores write errors.
fn save_cursor(grow_home: &Path, cursor: u64) {
    if let Ok(text) = serde_json::to_string(&TipState { cursor }) {
        let _ = std::fs::write(cursor_path(grow_home), text);
    }
}
...
/// cursor in `~/.grow/tip_cursor.json`, so every session sees the next tip
...
pub fn pick_and_advance(tips: &[String], grow_home: &Path) -> Option<String> {
    if tips.is_empty() {
        return None;
    }
    let cursor = load_cursor(grow_home);
    let tip = tips[cursor as usize % tips.len()].clone();
    save_cursor(grow_home, cursor + 1);
    Some(tip)
}
