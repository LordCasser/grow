...
pub mod tips;
pub mod uname;
pub use client_support::clipboard;
pub use client_support::stderr::{stderr_lock, with_locked_stderr};
...
/// True for an explicitly configured optional service endpoint. Loopback is
/// also accepted for local development and tests.
pub fn is_cli_chat_proxy_url(url: &str) -> bool {
    if let Ok(configured) = std::env::var("GROW_CLI_CHAT_PROXY_BASE_URL")
        && !configured.trim().is_empty()
        && matches_trusted_base_url(url, &configured)
    {
        return true;
    }
...
/// True only for an explicitly configured optional service endpoint (plus
/// loopback test/development endpoints).
///
/// Scheme-agnostic so credential *refusal* fails closed. To decide where to
/// *attach* a credential, use [`is_service_api_bearer_url`].
pub fn is_service_api_url(url: &str) -> bool {
    is_service_api_url_impl(url, false)
}
/// Like [`is_service_api_url`], but requires `https` on every arm, so a
...
pub fn is_service_api_bearer_url(url: &str) -> bool {
    is_service_api_url_impl(url, true)
}
fn is_service_api_url_impl(url: &str, require_https: bool) -> bool {
    if require_https {
...
/// True if `pid` is a grow process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_grow_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("grow"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
/// Stricter [`is_grow_process`] for the auto-kill zombie path: on macOS/BSD it
...
/// exactly, so this delegates there. Use the permissive [`is_grow_process`] for
/// operator-driven `grow leaders kill`.
pub fn is_grow_process_strict(pid: u32) -> bool {
    #[cfg(all(not(target_os = "linux"), not(windows)))]
    {
        ...
        match cmd.output() {
            Ok(out) if out.status.success() => {
                ...
                comm.lines()
                    ...
                    .is_some_and(|name| name.to_ascii_lowercase().contains("grow"))
            }
            _ => false,
        }
    }
...
mod tests {
    use super::*;
    #[test]
    fn test_is_cli_chat_proxy_url_accepts_loopback() {
        assert!(is_cli_chat_proxy_url("http://127.0.0.1:43123/v1"));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_public_api() {
        assert!(!is_cli_chat_proxy_url("https://api.example.com/v1"));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://service.example.com.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://service.example.com/v11/chat/completions"
        ));
    }
    #[test]
    fn test_is_service_api_url() {
        assert!(is_service_api_url("http://localhost:11434/v1"));
        assert!(!is_service_api_url("https://api.example.com/v1"));
        assert!(!is_service_api_url("https://api.openai.com/v1"));
        assert!(!is_service_api_url("https://api.anthropic.com/v1"));
    ...
    #[test]
    fn test_is_service_api_bearer_url() {
        assert!(!is_service_api_bearer_url("https://api.example.com/v1"));
        assert!(!is_service_api_bearer_url("http://api.example.com/v1"));
        assert!(!is_service_api_bearer_url("http://localhost:11434/v1"));
        {
            assert!(!is_service_api_bearer_url("https://localhost:11434/v1"));
            assert!(!is_service_api_bearer_url("https://127.0.0.2:11434/v1"));
            assert!(!is_service_api_bearer_url("https://[::1]:11434/v1"));
        }
        assert!(!is_service_api_bearer_url("https://api.example.com/v1"));
        assert!(!is_service_api_bearer_url(
            "https://api.example.com@attacker.example/v1"
        ));
        assert!(!is_service_api_bearer_url("https://х.ai/v1"));
    }
    #[test]
    fn test_truncate() {
    ...
    #[test]
    fn is_grow_process_self_true_impossible_pid_false() {
