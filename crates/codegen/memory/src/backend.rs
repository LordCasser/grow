...
impl MemoryBackendImpl {
    ...
    pub fn with_search_config(mut self, config: config_types::MemorySearchConfig) -> Self {
...
impl MemoryBackendImpl {
    ...
    pub fn search_config_for_test(&self) -> &config_types::MemorySearchConfig {
        &self.search_config
    }
}
...
impl MemoryBackend for MemoryBackendImpl {
    ...
    async fn search(
        ...
        if needs_release {
            ...
            if let Some((dirty_count, reindexed_count, sync_start)) = watcher_sync_stats {
                diagnostics::session_ctx::log_event(
                    diagnostics::memory_events::MemoryWatcherSync {
        ...
        if results.is_empty() {
            diagnostics::session_ctx::log_event(diagnostics::memory_events::MemorySearchEmpty {
                session_id: self.session_id.clone(),
            ...
            diagnostics::session_ctx::log_event(diagnostics::memory_events::MemorySearch {
                session_id: self.session_id.clone(),
                query_length: query.len(),
                keyword_count,
...
mod factory_tests {
    ...
    use config_types::{MemoryEmbeddingConfig, MemorySearchConfig};
    ...
    fn test_factory_wires_non_overridable_search_config_fields() {
        ...
        let custom_search = MemorySearchConfig {
            max_results: 7,
            mmr: config_types::MmrConfig {
                enabled: true,
                lambda: 0.42,
            },
            temporal_decay: config_types::TemporalDecayConfig {
                enabled: true,
                half_life_days: 14.0,
            },
            ..Default::default()
