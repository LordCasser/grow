...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "shell=info,pager=trace,tools=info,acp=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let instrumentation_layer = diagnostics::instrumentation::layer();
    let sampling_log_layer = diagnostics::sampling_log::layer();
    let hooks_log_layer = diagnostics::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        .with(fmt_layer.with_filter(env_filter))
        .with(instrumentation_layer)
        .with(sampling_log_layer)
        .with(hooks_log_layer);
    diagnostics::debug_log::install_firehose(registry, "tui");
    TracingHandle { rx }
}
#[cfg(test)]
mod tests {
