...
pub struct SuggestionRow {
    /// Display text (e.g., "/model" or "Grow 4 Fast").
    pub display: String,
...
pub struct SlashInvocation<'a> {
    /// Command token (e.g., "model" for "/model grow-4").
    pub token: &'a str,
...
mod tests {
    ...
    fn parses_invocation_with_args() {
        let inv = parse_invocation("/model grow-code-fast-1").expect("parsed");
        assert_eq!(inv.token, "model");
        assert_eq!(inv.args, "grow-code-fast-1");
    }
    ...
    fn cursor_inside_leading_command_with_args_filters_by_prefix() {
        ...
        // Cursor 3 in "/mod grow-4" clamps the query to "mo".
        ctrl.refresh(&state, "/mod grow-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    fn cursor_inside_recognized_command_with_args_opens_menu() {
        ...
        ctrl.refresh(&state, "/model grow-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    fn mru_beats_tiebreak_on_equal_fuzzy_score() {
        ...
        let mut ctrl = tie_controller(
            &["privacy", "prompts", "plan"],
            &[
                ("privacy", 1_700_000_900),
                ("prompts", 1_700_000_010),
                ("plan", 1_700_000_050),
            ],
        );
    ...
    fn ghost_tracks_selection_when_skill_wins_mru_tie() {
        ...
        // `prompts`" divergence: a builtin (`prompts`) and an ACP skill
    ...
    fn acp_and_skill_commands_tag_from_map() {
        ...
        let skill_meta = serde_json::json!({
            "scope": "local",
            "path": "/home/user/.grow/skills/skill-cmd/SKILL.md",
        })
        .as_object()
        .cloned()
    ...
    fn flat_mru_boosts_recent_command_regardless_of_typed_prefix() {
        ...
        let mut ctrl = tie_controller(
            &["plan", "prompts"],
            &[("plan", 1_700_000_999), ("prompts", 1_700_000_010)],
        );
    ...
    fn mid_text_cursor_in_first_slash_stays_in_command_mode() {
        ...
        let text = "hi /history\n\n  /execute-plan";
        let cursor = text.find("/history").unwrap() + "/history".len();
        ctrl.refresh(&state, text, cursor, &models);
        let snapshot = state.snapshot();
        assert!(
            snapshot.cursor_in_command,
            "cursor at end of /history must stay in command mode for Tab"
        );
        assert_eq!(snapshot.args_range, None);
        assert_eq!(
            snapshot.command_range,
            Some(3..11),
            "Tab must target /history, not the later /execute-plan token"
        );
    }
