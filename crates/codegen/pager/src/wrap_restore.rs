//! Terminal-mode tracking and restore emission for `grow wrap`.
//!
//! `grow wrap` cannot tell a clean child exit from a connection drop: an ssh
...
//! [`crash_handler::terminal`] (`RESTORE_SEQ`) — pinned mechanically by a
...
/// `crash_handler::terminal::RESTORE_SEQ`'s ordering tests.
...
/// `crash_handler::terminal::RESTORE_SEQ` for every element the two
