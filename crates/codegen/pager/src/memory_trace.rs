...
//!   `GROW_MEMTRACE_INTERVAL_SECS` (default 30s) from a detached thread.
...
//!
//! The lib cannot depend on jemalloc; `cli` installs:
...
/// Whether a trace sink is installed ([`start`] ran and `GROW_MEMTRACE` is
...
/// Env: disable with `GROW_MEMTRACE=0|false|off`.
fn enabled_by_env() -> bool {
    !matches!(
        std::env::var("GROW_MEMTRACE").ok().as_deref(),
        Some("0") | Some("false") | Some("off")
    )
}

fn interval_from_env() -> Duration {
    let secs = std::env::var("GROW_MEMTRACE_INTERVAL_SECS")
        .ok()
...
fn first_threshold_from_env() -> u64 {
    std::env::var("GROW_MEMTRACE_THRESHOLD_MB")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1024)
        .saturating_mul(1 << 20)
...
/// `dir` (e.g. `$GROW_HOME/memtrace/`) and spawn the detached sampler
...
/// don't trace. Inert when `GROW_MEMTRACE=0`.
///
...
/// (`grow --version`, `grow trace …`) leave no files behind.
