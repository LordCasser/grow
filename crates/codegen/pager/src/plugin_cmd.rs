...
//!
//! Business logic lives in `shell::plugin` (shared orchestration)
//! and lower crates (`agent`, `plugin-marketplace`). This
...
fn source_identity(source: &plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    source: &plugin_marketplace::MarketplaceSource,
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = plugin_marketplace::git::default_cache_root();
            plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> diagnostics::events::InstallKind {
    if is_git {
...
fn log_plugin_installed(
    install_kind: diagnostics::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    diagnostics::session_ctx::log_event(diagnostics::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = plugin_marketplace::install_resolve::parse_marketplace_ref(source) {
    ...
    match plugin::install_plugin(source, &cwd) {
        ...
        Err(e) => {
            ...
            log_plugin_installed(diagnostics::events::InstallKind::Git, false, Some(cat));
            bail!("{e}");
        }
    }
}
...
fn cmd_install_marketplace(
    source: &str,
    mref: &plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
...
fn cmd_uninstall(name: &str, confirm: bool, keep_data: bool) -> Result<()> {
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            diagnostics::session_ctx::log_event(diagnostics::events::PluginUninstalled {
                confirmed: true,
                success: true,
            });
...
fn marketplace_list(sources: &[plugin_marketplace::MarketplaceSource], json: bool) -> Result<()> {
    if json {
...
fn marketplace_add(
    ...
    if !force && let MarketplaceAddInput::GitUrl(git_url) = &input {
        plugin_marketplace::git::probe_git_remote(git_url).map_err(|e| {
            anyhow::anyhow!(
...
fn find_removal_source<'a>(
    sources: &'a [plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a plugin_marketplace::MarketplaceSource, String> {
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use plugin_marketplace::MarketplaceSource;
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir =
            plugin_marketplace::git::sync_source_cache(&url, Some("main"), cache_root.path())
                .unwrap();
    ...
    fn current_head(repo: &Path) -> String {
        let output = plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
