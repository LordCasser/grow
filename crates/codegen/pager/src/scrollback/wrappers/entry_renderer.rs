...
impl<'a> EntryRenderer<'a> {
    ...
    /// cache size off-screen entries on a bulk load (`grow -r`) without word-wrapping
    ...
    /// estimate for field-joined source (Subagent/BgTask), kept on
...
mod tests {
    ...
    fn gutter_keeps_code_block_background_on_no_background_block() {
        ...
        // carries no code background while this test's local `grownight()`
    ...
    fn affordance_row_adds_one_row_per_diagram() {
        ...
        // invariant a bulk load (`grow -r`) relies on to avoid clipping.
