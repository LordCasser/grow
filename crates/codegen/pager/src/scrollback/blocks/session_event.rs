...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "grow-4.5".into(),
            new_model_id: "grow-build".into(),
            reason: "Model \"grow-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"grow-4.5\" is no longer available. Switched to \"grow-build\"."
        );
    }
