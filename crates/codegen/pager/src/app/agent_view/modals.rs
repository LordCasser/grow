...
impl AgentView {
    ...
    fn log_extensions_modal_action(
        &self,
        action: &str,
        input_method: diagnostics::events::ExtensionsInputMethod,
    ) {
    ...
    fn log_extensions_modal_action_with(
        ...
        if let Some(ref state) = self.extensions_modal {
            diagnostics::session_ctx::log_event(diagnostics::events::ExtensionsModalAction {
    ...
    pub(super) fn handle_extensions_modal_key(
                ...
                crate::views::modal_window::ModalWindowOutcome::CollapseGroup => {
                        ...
                        && self.extensions_modal_set_collapsed(sel, &gk, true)
                    {
                        self.log_extensions_modal_action(
                            "collapse",
                            diagnostics::events::ExtensionsInputMethod::Keyboard,
                        );
                    }
                    return InputOutcome::Changed;
                }
        ...
        match outcome {
            ...
            | crate::views::picker::PickerOutcome::Expand(_) => {
                self.extensions_modal_expand(diagnostics::events::ExtensionsInputMethod::Keyboard)
            }
    ...
    pub(super) fn handle_extensions_modal_mouse(
        ...
        if let Some(ch) = chrome_shortcut_ch {
                ...
                .and_then(|s| crate::views::extensions_modal::resolve_key(s.active_tab, ch))
            {
                self.log_extensions_modal_resolved_action(
                    ch,
                    &action,
                    diagnostics::events::ExtensionsInputMethod::Mouse,
                );
        ...
        match outcome {
            ...
            crate::views::picker::PickerOutcome::Selected(_)
            | crate::views::picker::PickerOutcome::Expand(_) => {
                self.extensions_modal_expand(diagnostics::events::ExtensionsInputMethod::Mouse)
            }
            crate::views::picker::PickerOutcome::NonSelectableClick(idx) => {
                self.extensions_modal_toggle_mcp_section_at(
                    idx,
    ...
    fn execute_modal_button_action(
        ...
        match action {
            ButtonAction::HooksAction(hooks_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    state.modal_message = None;
                    if matches!(hooks_action, extension_types::HooksAction::Reload) {
            ...
            ButtonAction::PluginsAction(plugins_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    ...
                    if matches!(plugins_action, extension_types::PluginsAction::Reload) {
                        // Reload rebuilds the entire plugin registry -- show
                        // tab-level "Loading..." instead of a single-entry badge.
                        state.pending_action = Some("Reloading...".into());
                        state.pending_entry_index = None;
                        ...
                        let label = if matches!(
                            plugins_action,
                            extension_types::PluginsAction::Update { .. }
                        ) {
                            "Updating..."
                        } else {
                            "Processing..."
            ...
            ButtonAction::MarketplaceAction(marketplace_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    ...
                    match &marketplace_action {
                        ...
                        extension_types::MarketplaceAction::Uninstall { .. } => {
                        ...
                        && let Some(hook) = data.hooks.get(idx)
                    {
                        ...
                        return self.prompt_extensions_confirm(
                            ...
                            crate::views::extensions_modal::ConfirmationAction::Hooks(
                                extension_types::HooksAction::Remove { path },
                            ),
                        );
                    }
                }
            ...
            ButtonAction::ToggleSelectedHook => {
                    ...
                    && let Some(hook) = data.hooks.get(idx)
                {
                    ...
                    if is_collapsed {
                        ...
                        let group_hooks: Vec<&extension_types::HookInfo> = data
                            .hooks
                            .iter()
                        ...
                        let action = extension_types::HooksAction::ToggleSource {
                            hook_names,
                            disable: any_enabled,
                        };
                        ...
                        let action = if hook.disabled {
                            extension_types::HooksAction::Enable {
                                hook_name: hook.name.clone(),
                            }
                        } else {
                            extension_types::HooksAction::Disable {
                                hook_name: hook.name.clone(),
                            }
                        };
                        return self.execute_modal_button_action(ButtonAction::HooksAction(action));
            ...
            ButtonAction::ToggleSelectedPlugin => {
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = if plugin.enabled {
                        extension_types::PluginsAction::Disable {
                            plugin_id: plugin.id.clone(),
                        }
                    } else {
                        extension_types::PluginsAction::Enable {
                            plugin_id: plugin.id.clone(),
                        }
                    };
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    ...
                    return self.prompt_extensions_confirm(
                        ...
                        crate::views::extensions_modal::ConfirmationAction::Plugins(
                            extension_types::PluginsAction::Uninstall {
                                plugin_id,
            ...
            ButtonAction::UpdateSelectedPlugin => {
                ...
                if let Some(ref state) = self.extensions_modal
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = extension_types::PluginsAction::Update {
            ...
            ButtonAction::InstallSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    ...
                    |source_url_or_path, plugin_relative_path| {
                        extension_types::MarketplaceAction::Install {
                            source_url_or_path,
            ...
            ButtonAction::UpdateSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    "Updating...",
                    |source_url_or_path, plugin_relative_path| {
                        extension_types::MarketplaceAction::Update {
                            source_url_or_path,
                            plugin_relative_path,
                        }
                    },
                    ...
                    if let TabDataState::Loaded(ref response) = state.marketplace_data {
                        ...
                        if let Some(source) = source {
                            return self.prompt_extensions_confirm(
                                ...
                                crate::views::extensions_modal::ConfirmationAction::Marketplace(
                                    extension_types::MarketplaceAction::RemoveSource {
                                        source_url_or_path: source.source_url_or_path.clone(),
                                    },
                                ),
                            );
    ...
    fn execute_selected_marketplace_plugin_action(
        ...
        make_action: impl FnOnce(String, String) -> extension_types::MarketplaceAction,
    ) -> InputOutcome {
        if let Some(ref mut state) = self.extensions_modal {
            use crate::views::extensions_modal::TabDataState;
            if let TabDataState::Loaded(ref response) = state.marketplace_data
...
mod marketplace_modal_action_tests {
    ...
    fn update_selected_marketplace_plugin_dispatches_update_and_sets_pending_state() {
        ...
        modal.marketplace_data = TabDataState::Loaded(extension_types::MarketplaceListResponse {
            sources: vec![extension_types::MarketplaceScanResult {
...
mod extensions_action_target_tests {
    ...
    fn plugin_info(name: &str, enabled: bool) -> extension_types::PluginInfo {
    ...
    fn plugins_toggle_and_uninstall_resolve_name_and_state() {
        ...
        modal.plugins_data = TabDataState::Loaded(extension_types::PluginsListResponse {
    ...
    fn hook_info(name: &str, source_dir: &str, disabled: bool) -> extension_types::HookInfo {
    ...
    fn hooks_modal(hooks: Vec<extension_types::HookInfo>) -> ExtensionsModalState {
    ...
    fn loading_data_yields_no_target() {
        ...
        for action in [
            ...
            ButtonAction::ToggleSelectedMcpServer,
            ButtonAction::InstallSelectedMarketplacePlugin,
            ButtonAction::PluginsAction(extension_types::PluginsAction::Reload),
        ] {
            let (target, enabled) = AgentView::extensions_action_target(&modal, &action);
...
mod extensions_modal_confirmation_tests {
    ...
    fn plugin_info(name: &str) -> extension_types::PluginInfo {
        extension_types::PluginInfo {
            name: name.into(),
            id: format!("user/abcd1234/{name}"),
            root: "/tmp/p".into(),
    ...
    fn hook_info(name: &str, source_dir: &str) -> extension_types::HookInfo {
        extension_types::HookInfo {
            name: name.into(),
            event: extension_types::HookEvent::PreToolUse,
            handler_type: extension_types::HookHandlerType::Command,
    ...
    fn marketplace_loaded() -> TabDataState<extension_types::MarketplaceListResponse> {
        TabDataState::Loaded(extension_types::MarketplaceListResponse {
            sources: vec![extension_types::MarketplaceScanResult {
                source_name: "test-source".into(),
                featured: false,
                source_kind: "git".into(),
    ...
    fn all_prompt_cases() -> Vec<PromptCase> {
        ...
        plugins.plugins_data = TabDataState::Loaded(extension_types::PluginsListResponse {
        ...
        hooks.hooks_data = TabDataState::Loaded(extension_types::HooksListResponse {
        ...
        vec![
            ...
            PromptCase {
                ...
                expected: ConfirmationAction::Plugins(extension_types::PluginsAction::Uninstall {
            ...
            PromptCase {
                ...
                expected: ConfirmationAction::Hooks(extension_types::HooksAction::Remove {
                    path: source.into(),
                }),
                row: 0,
            },
    ...
    fn plugin_y_sends_confirmed_false_so_server_can_gate_multi() {
        ...
        assert_eq!(
            ...
            Some(extension_types::PluginsAction::Uninstall {
                plugin_id: "user/abcd1234/my-plugin".into(),
                confirmed: false,
            })
        );
