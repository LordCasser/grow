...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        diagnostics::session_ctx::log_event(diagnostics::events::PromptSuggestion {
            action: diagnostics::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    pub(crate) fn notify_plugin_cta_text_changed(&mut self) -> Option<super::actions::Effect> {
        if !self.plugin_cta.featured_source_present || self.plugin_cta.candidates.is_empty() {
            return None;
        }
    ...
    /// Apply an `grow/follow_ups` notification, keyed by `response_id`
    ...
    /// Buffer a stamped `grow/follow_ups` for a turn that is not yet current,
    ...
    /// the running turn's `grow/follow_ups` arrive on the ext channel DURING
...
mod plugin_cta_notify_tests {
    ...
    fn cta_entry(name: &str) -> extension_types::MarketplacePluginEntry {
    ...
    fn notify_skips_debounce_when_no_candidates() {
        ...
        agent.plugin_cta.featured_source_present = true;
    ...
    fn notify_skips_debounce_when_source_absent() {
        let mut agent = make_agent();
        agent.plugin_cta.featured_source_present = false;
    ...
    fn connect_matched_enters_installing_and_emits_effect() {
        ...
        agent.plugin_cta.featured_source_url = Some("https://example.com/plugins.git".to_owned());
