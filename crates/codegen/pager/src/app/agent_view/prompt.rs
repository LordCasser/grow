...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    diagnostics::session_ctx::log_event(diagnostics::events::PromptSuggestion {
                        action: diagnostics::events::PromptSuggestionAction::Accepted,
                        chars,
                        words,
                    });
                    self.prompt.refresh_slash(&self.session.models);
                ...
                && self.prompt.text().is_empty()
            {
                ...
                diagnostics::session_ctx::log_event(diagnostics::events::PromptSuggestion {
                    action: diagnostics::events::PromptSuggestionAction::Dismissed,
                    chars,
                    words,
                });
                return InputOutcome::Changed;
    ...
    /// The pty-only `GROW_ESC_DOUBLE_PRESS_MS` override can exceed this; no
...
mod prompt_suggestion_key_tests {
    ...
    /// right after a turn ends with `grow/suggestPrompt` resolved. Pins the
