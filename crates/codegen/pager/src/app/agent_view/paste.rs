    ...
    pub(crate) fn build_deferred_send_action(&mut self, kind: AgentDeferredSend) -> Option<Action> {
        match kind {
            ...
            AgentDeferredSend::Interject => {
                ...
                Some(Action::SteerPrompt { text, images })
            }
        }
    }
...
pub(super) mod paste_key_tests {
    ...
    -> crate::views::question_view::QuestionViewState {
        let question = tools::implementations::grow_build::ask_user_question::Question {
            question: "Pick one?".to_string(),
            options: vec![
                tools::implementations::grow_build::ask_user_question::QuestionOption {
                    label: "A".to_string(),
