...
pub struct BannerSlotParams<'a> {
    ...
    pub(crate) announcements: &'a [announcements::Announcement],
...
pub struct PluginCtaState {
    /// Featured-source, not-installed candidate plugins for CTA matching.
    pub candidates: Vec<extension_types::MarketplacePluginEntry>,
    /// Whether the featured marketplace source was present in the last catalog scan.
    pub featured_source_present: bool,
    /// Repository or local path backing the featured source.
    pub featured_source_url: Option<String>,
...
/// (`grow/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`grow/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `grow/follow_ups` that arrived for a turn that is NOT yet the
...
fn translate_project_select(
    ...
    use diagnostics::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        diagnostics::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &tools::implementations::grow_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(crate) mod test_fixtures {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `grow/follow_ups`
...
mod prompt_input_mode_tests {
    ...
    fn send_action_maps_to_correct_action_variant() {
        ...
        let t3 = "remember this".to_string();
        assert!(matches!(
            PromptInputMode::Remember.send_action(t3.clone()),
            Action::SendRememberNote(t) if t == t3
        ));
    }
    #[test]
