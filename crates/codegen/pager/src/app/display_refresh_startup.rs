...
/// Inclusive bounds for motion cadence env knobs (`GROW_MIN_DRAW_MS`,
/// `GROW_SCROLL_CADENCE_MS`).
...
pub fn start(user: Option<&TomlValue>, remote: Option<&RemoteSettings>) -> MotionClocks {
    ...
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("GROW_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_diagnostics(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        diagnostics::session_ctx::log_event(t.clone());
        ...
        diagnostics::session_ctx::log_event(diagnostics::events::DisplayRefreshProbe {
            terminal: t,
