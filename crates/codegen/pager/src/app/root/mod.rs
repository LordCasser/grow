...
use super::session::McpInitProgress;
...
/// [`workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `GROW_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `GROW_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("GROW_ESC_DOUBLE_PRESS_MS").ok()))
}
...
pub struct AppView {
    ...
    /// "Don't ask me again" opt-out from [`shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.grow/pager.toml).
    ...
    pub active_announcements: Vec<announcements::Announcement>,
    ...
    /// poll outside leader mode, so we fetch the same `grow/session/list` the
    ...
    /// Reconciled from `grow/queue/changed` broadcasts so
    ...
    /// confirming `grow/queue/changed` broadcast has not yet arrived. Keyed by
    ...
    /// empty and inert outside `--minimal`; the `pager-minimal` crate
    ...
    /// at startup and on settings toggles from `GROW_CONTEXTUAL_HINTS` (master)
    ...
    /// One-shot gate for the SSH `grow wrap` tip: set after the first
    ...
    /// `~/.grow/config.toml`. `None` when the file/section is absent
...
impl AppView {
    ...
    /// [`take_deferred_model_switch`](crate::app::root::dispatch::session::lifecycle::take_deferred_model_switch);
    ...
    /// `grow/queue/changed` broadcast. The broadcast is
    ...
    /// `grow/queue/changed` broadcast replaces it (matched by `id`) rather than
...
impl AppView {
    ...
    pub(crate) fn maybe_trigger_small_screen_tip(&mut self) {
        ...
        self.small_screen_tip_evaluated = true;
        dispatch::show_small_screen_tip(self);
    }
    /// One-shot SSH `grow wrap` tip trigger, run at the top of every `draw`
    ...
    pub(crate) fn maybe_trigger_ssh_wrap_tip_inner(&mut self, env_recommends_wrap: bool) {
        ...
        self.ssh_wrap_tip_evaluated = true;
        dispatch::show_ssh_wrap_tip(self);
    }
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            diagnostics::session_ctx::log_event(diagnostics::events::ContextualTip {
                tip: diagnostics::events::ContextualTipKind::ImageInput,
...
pub(crate) mod tests {
    ...
    use crate::app::session::{AgentSession, AgentState};
    ...
    fn make_test_session(
        app: &AppView,
        id: super::super::session::AgentId,
        sid: &str,
    ) -> AgentSession {
        {
    ...
    pub(crate) fn test_app_with_agent() -> AppView {
        ...
        super::dispatch::switch_to_agent(&mut app, id, super::dispatch::SwitchCause::Load);
        app
    }
    ...
    fn plain_text_block_viewer_survives_animation_tick() {
        ...
        let agent = app
            .agents
            .get_mut(&super::super::session::AgentId(0))
            .unwrap();
    ...
    fn idle_agent_with_content(app: &mut AppView, id: super::super::session::AgentId) {
        app.agents
            .get_mut(&id)
            .unwrap()
            .scrollback
    ...
    fn background_submission_arms_lifecycle_without_visible_frames() {
        let mut app = test_app_with_agent();
        let background = super::super::session::AgentId(1);
        ...
        idle_agent_with_content(&mut app, super::super::session::AgentId(0));
        ...
        app.agents
            ...
            Instant::now()
                - crate::app::root::dispatch::PROMPT_STATUS_WATCHDOG_DELAY
                - Duration::from_millis(1),
        );
        assert!(
            crate::app::root::dispatch::next_prompt_watchdog_deadline(&app).is_some(),
    ...
    fn visible_frame_interval_is_fast_while_goal_active_on_idle_session() {
        ...
        let mut goal = crate::app::session::GoalDisplayState::test_stub();
        goal.status = crate::app::session::GoalDisplayStatus::Active;
        ...
        app.agents
            ...
            .as_mut()
            .unwrap()
            .status = crate::app::session::GoalDisplayStatus::Paused;
        assert_eq!(
            app.visible_frame_interval(TEST_FRAME_INTERVAL),
            None,
    ...
    fn running_bg_task(task_id: &str) -> crate::app::session::BgTaskState {
    ...
    fn idle_child_view(app: &AppView, id_n: usize, sid: &str) -> Box<AgentView> {
        let session = {
            let session = AgentSession::new(
                super::super::session::AgentId(id_n),
                app.acp_tx.clone(),
                Some(sid.to_string().into()),
                ModelState::default(),
                std::path::PathBuf::from("/tmp"),
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![announcements::Announcement {
    ...
    fn todo_badge_flash_uses_one_absolute_ui_deadline() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn prompt_paging_scope_matches_agent_surface() {
        fn focused_app(screen_mode: ScreenMode) -> (AppView, super::super::session::AgentId) {
            let mut app = test_app_with_agent();
    ...
    fn prompt_page_actions_target_visible_fullscreen_child_scrollback() {
        ...
        let _ = super::dispatch::dispatch(action, &mut app);
    ...
    fn esc_owned_before_agent_covers_app_level_owners() {
        ...
        if let Some(d) = app.dashboard.as_mut() {
            d.attached_agent = Some(super::super::session::AgentId(0));
        }
        assert!(app.esc_owned_before_agent(), "dashboard popup owns Esc");
        if let Some(d) = app.dashboard.as_mut() {
            d.attached_agent = Some(super::super::session::AgentId(99));
        }
        assert!(!app.esc_owned_before_agent());
        if let Some(d) = app.dashboard.as_mut() {
    ...
    fn idle_non_empty_double_esc_clears_prompt() {
        ...
        assert!(app.pending_action.is_none());
        let effects = crate::app::root::dispatch::dispatch(Action::ClearPrompt, &mut app);
    ...
    fn mouse_send_retires_armed_clear_so_next_esc_swallows() {
        ...
        assert!(matches!(pending.action, Action::ClearPrompt));
        let _ = crate::app::root::dispatch::dispatch(
            Action::SendPrompt("draft to clear".into()),
            &mut app,
        );
        assert!(
            app.pending_action.is_none(),
    ...
    fn attach_popup(app: &mut AppView) -> super::super::session::AgentId {
        app.active_view = ActiveView::AgentDashboard;
        let id = super::super::session::AgentId(0);
        app.dashboard = Some(crate::views::dashboard::DashboardState::new());
        if let Some(d) = app.dashboard.as_mut() {
    ...
    fn neutral_overlay_app() -> (AppView, super::super::session::AgentId) {
        let mut app = test_app_with_agent();
        let id = super::super::session::AgentId(0);
        app.active_view = ActiveView::Agent(id);
        app.dashboard = Some(crate::views::dashboard::DashboardState::new());
    ...
    fn install_plan_overlay(app: &mut AppView, id: super::super::session::AgentId) {
    ...
    /// plan line viewer.
    fn install_plan_preview_overlay(app: &mut AppView, id: super::super::session::AgentId) {
        let request = crate::views::plan_approval_view::PlanApprovalExtRequest {
            session_id: "s".into(),
            tool_call_id: "c".into(),
            plan_content: "# Plan\n- step one\n- step two".into(),
    ...
    fn overlay_ctrl_x_busy_agent_cancels_turn_without_arming() {
        let (mut app, id) = neutral_overlay_app();
        app.agents.get_mut(&id).unwrap().session.state =
            crate::app::session::AgentState::TurnRunning;
        for _ in 0..2 {
    ...
    fn handle_input_mouse_click_outside_popup_on_row_switches_target() {
        ...
        let _ = attach_popup(&mut app);
        let row_id =
            crate::views::dashboard::DashboardRowId::TopLevel(super::super::session::AgentId(42));
        if let Some(d) = app.dashboard.as_mut() {
