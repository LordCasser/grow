...
pub(super) fn plugin_cta_candidates(
    response: extension_types::MarketplaceListResponse,
) -> (Vec<extension_types::MarketplacePluginEntry>, Option<String>) {
    let mut candidates = Vec::new();
    let mut featured_source_url = None;
    for source in response.sources {
        if !source.featured {
            continue;
        }
        featured_source_url = Some(source.source_url_or_path);
    ...
    (candidates, featured_source_url)
}
...
/// the still-cached candidate entry; falls back to the featured-source layout.
pub(super) fn cta_install_relative_path(
    candidates: &[extension_types::MarketplacePluginEntry],
    name: &str,
) -> String {
    candidates
        .iter()
...
pub(super) fn cta_install_error_category(
    ...
    use extension_types::OutcomeStatus;
    match result {
...
///
/// Gating order: feature flag + featured source present, keyword match,
...
pub(super) fn plugin_cta_phase_for(
    enabled: bool,
    featured_source_present: bool,
    candidates: &[extension_types::MarketplacePluginEntry],
    prompt_text: &str,
    is_dismissed: impl Fn(&str) -> bool,
) -> crate::app::agent_view::CtaPhase {
    use crate::app::agent_view::CtaPhase;
    use plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};

    if !(enabled && featured_source_present) {
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(diagnostics::events::PluginCtaInstalled {
    ...
    let install_result = match result {
        Ok(outcome) if outcome.status == extension_types::OutcomeStatus::Success => {
            Ok(outcome.requires_reload)
        }
        Ok(outcome) => Err(crate::app::root::effects::sanitize_user_error(
            &outcome.message,
        )),
        Err(e) => Err(e),
    };
...
pub(super) fn handle_cta_plugin_reload_done(
    ...
    let reload_result = match result {
        Ok(outcome) if outcome.status == extension_types::OutcomeStatus::Success => Ok(()),
        Ok(outcome) => Err(crate::app::root::effects::sanitize_user_error(
            &outcome.message,
        )),
        Err(e) => Err(e),
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                let (candidates, featured_source_url) = plugin_cta_candidates(response);
                agent.plugin_cta.candidates = candidates;
                agent.plugin_cta.featured_source_present = featured_source_url.is_some();
                agent.plugin_cta.featured_source_url = featured_source_url;
                ...
                if enabled {
                    agent.plugin_cta.dismissed = shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    let new_phase = plugin_cta_phase_for(
                        enabled,
                        agent.plugin_cta.featured_source_present,
                    ...
                    {
                        log_event(diagnostics::events::PluginCtaImpression {
