...
#[test]
fn external_prompt_editor_refuses_pending_paste_with_visible_message() {
...
fn critical_announcement(id: &str) -> announcements::Announcement {
...
fn promo_announcement(id: &str) -> announcements::Announcement {
    announcements::Announcement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    dispatch(Action::NewSession, &mut app);
    let id = AgentId(0);
    app.active_view = crate::app::root::ActiveView::Agent(id);
    let skill_meta = serde_json::json!({
        "scope": "user",
        "path": "/home/user/.grow/skills/pick-best/SKILL.md",
    });
...
fn dispatch_fork_no_flag_always_reopens_modal_after_previous_answer() {
    ...
    let inner = match outcome {
        crate::app::root::InputOutcome::Action(a) => a,
...
fn show_tasks_lists_a_scheduled_task() {
    use crate::app::session::ScheduledTaskInfo;
    let mut app = test_app_with_agent();
    {
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
...
fn build_rows_working_anchor_is_turn_started_at() {
    ...
    agent.session.state = crate::app::session::AgentState::TurnRunning;
    let rows = build_rows(
        &app.agents,
        &std::collections::BTreeSet::new(),
        &[],
...
fn mouse_left_click_attaches_immediately() {
    ...
    match outcome {
        crate::app::root::InputOutcome::Action(crate::app::actions::Action::DashboardAttach(
            actual,
        )) => {
