...
fn plugin_cta_catalog_loaded_sanitizes_components_at_ingestion() {
    ...
    entry.components = Some(extension_types::PluginComponents {
        skills: vec![extension_types::ComponentItem {
...
#[test]
fn plugin_cta_catalog_keeps_featured_not_installed_only() {
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    ...
    let cta = &app.agents[&id].plugin_cta;
    assert!(cta.featured_source_present);
...
fn cta_install_relative_path_prefers_candidate_then_falls_back() {
    ...
    // Unknown name falls back to the featured-source layout.
    assert_eq!(
...
mod cta_e2e {
    ...
    use crate::app::agent_view::CtaPhase;
    use crate::app::root::dispatch::cta::plugin_cta_phase_for;
    use crate::app::root::dispatch::dispatch;
    use crate::app::root::{AppView, InputOutcome};
    ...
    fn figma_candidate() -> extension_types::MarketplacePluginEntry {
        let mut entry = cta_entry("figma", "not_installed");
    ...
    fn app_matched() -> AppView {
        ...
        app.plugin_cta_enabled = true;
        {
            let cta = &mut app.agents.get_mut(&id).unwrap().plugin_cta;
            cta.featured_source_present = true;
            cta.featured_source_url = Some("https://example.com/plugins.git".into());
    ...
    fn dismissed_plugin_suppressed_on_next_recompute() {
        ...
        let dismissed = shell::config::dismissed_plugin_ctas_in_file(&config_path);
        let matched =
        ...
        shell::config::add_dismissed_plugin_cta_to_file("figma", &config_path)
    ...
    fn flag_off_or_source_absent_hides_regardless_of_text() {
        for (enabled, source_present) in [(false, true), (true, false)] {
            ...
            {
                let cta = &mut app.agents.get_mut(&id).unwrap().plugin_cta;
                cta.featured_source_present = source_present;
                cta.candidates = vec![figma_candidate()];
                cta.debounce_generation = 1;
                cta.phase = CtaPhase::Matched {
                    plugin_relative_path: "plugins/figma".into(),
