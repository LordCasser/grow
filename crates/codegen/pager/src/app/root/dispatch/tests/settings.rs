...
fn agent_vim_tab_focuses_scrollback_then_j_navigates() {
    ...
    if let crate::app::root::InputOutcome::Action(a) = app.handle_input(&tab) {
    ...
    assert!(
        matches!(
            outcome,
            crate::app::root::InputOutcome::Action(Action::SelectNext)
        ),
...
fn plugin_cta_catalog_reload_empty_candidates_resets_matched_phase() {
    ...
    let id = AgentId(0);
    {
        ...
        cta.featured_source_present = true;
    ...
    let response = extension_types::MarketplaceListResponse {
        sources: vec![extension_types::MarketplaceScanResult {
            source_name: "Featured".into(),
            featured: true,
    ...
    with_theme_test_env(|| {
        ...
        for meta in reg.all() {
            ...
            for eff in dispatch(reset_action, &mut app) {
                ...
                assert_ne!(
                    toast.as_deref(),
                    Some(crate::app::root::dispatch::ROLLBACK_NO_ARM_TOAST),
...
///
/// Note: we persist `growday` (a non-truecolor theme) here
...
/// `clamp_to_terminal` might fold to GrowNight in non-truecolor
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        let effects = dispatch(Action::SetTheme("growday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("growday"));
                assert_eq!(*rollback_value, SettingValue::Enum("grownight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("growday"));
        assert!(
...
/// for the auto-dark sibling. Uses `growday` to avoid the
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("growday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
...
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("grownight".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*rollback_value, SettingValue::Enum("growday"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="grownight"` (concrete) + system=Dark. User
/// commits `auto_dark_theme="growday"`. The setting is dormant
/// (parent theme is concrete, not auto), so the live display
/// must stay on GrowNight even though we're committing a
/// different theme. Uses `growday` to avoid `clamp_to_terminal`
...
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("grownight".into()), &mut app);
        assert_eq!(
...
///
/// Uses `GrowDay` (non-truecolor-requiring) for the dark-mode
...
/// GrowNight. Using a non-truecolor theme avoids the clamp
...
/// Uses `grownight` for the auto_dark_theme
/// value to avoid `clamp_to_terminal` ambiguity (we want a
/// concrete kind that's clearly different from GrowDay, the
...
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("grownight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::GrowNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("growday".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("Grow Day"),
            "toast must use display name `Grow Day`, not canonical `growday`, got: {toast:?}",
        );
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Grow Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Grow Night"));
    });
}
...
///
/// Uses non-truecolor themes (`growday` ↔ `grownight`) to avoid
...
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("grownight"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("growday"),
                error: "disk full".into(),
            }),
            &mut app,
        );
        assert_eq!(app.current_ui.auto_light_theme.as_deref(), Some("growday"));
    });
}
