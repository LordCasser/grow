...
use super::turn::{
    ...
    dispatch_demote_to_background, dispatch_goal_interrupt_choice, dispatch_kill_bg_task,
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    let effects = match action {
        ...
        Action::OpenNewWorktreeDialog => {
            app.new_worktree_dialog = Some(crate::app::root::NewWorktreeDialogState::new());
            vec![]
        }
        ...
        Action::GoalInterruptChoice(choice) => dispatch_goal_interrupt_choice(app, choice),
            ...
            OutcomeStatus::Success => {
                ...
                if let Some(ref mut modal) = agent.extensions_modal {
                    if !outcome.message.trim().is_empty() && modal.result_notice.is_none() {
                        let entry_index = match modal.last_plugins_action {
                            Some(extension_types::PluginsAction::Uninstall { .. }) => None,
                ...
                if let Some(session_id) = agent.session.session_id.clone() {
                    if outcome.requires_reload {
                        ...
                        crate::app::root::dispatch::transcript::push_marketplace_fetch(
                            modal,
                            &mut effects,
                            agent_id,
                            session_id.clone(),
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let extension_types::PluginsAction::Uninstall {
                            ref mut confirmed, ..
                        } = action
                        {
                            *confirmed = true;
