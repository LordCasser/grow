...
pub(crate) struct TimedInputEvent {
    ...
    pub(crate) arrived_at: std::time::Instant,
}
...
pub(crate) struct TerminalState {
    ...
    pub screen_mode: crate::app::ScreenMode,
...
pub(crate) struct RunResult {
    pub exit_info: Option<crate::app::ExitInfo>,
    ...
    pub relaunch: Option<crate::app::root::ScreenModeRelaunch>,
}
...
struct ReconnectReinit {
    ...
    agent_ids: Vec<crate::app::session::AgentId>,
...
/// [`decide`](workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    let launch_effective_ui = shell::config::load_effective_config()
        .ok()
    ...
    app.show_resolved_model = remote_settings
        ...
        .unwrap_or(true);
    app.plugin_cta_enabled = config::env_bool("GROW_PLUGIN_CTA").unwrap_or(false);
    ...
    let effs = vec![super::super::actions::Effect::FetchChangelog];
    if process_effects(effs, &mut tasks, &mut app) {
        return Ok(make_run_result(&app));
    }
    ...
    let mut xt_filter = crate::app::xt_filter::XtversionFilter::new();
            ...
            Ok(()) = async {
                ...
                match status {
                        ...
                        if generation > last_leader_generation =>
                    {
                        ...
                        let mut agent_ids: Vec<crate::app::session::AgentId> =
...
fn make_run_result(app: &AppView) -> RunResult {
    let exit_info = app.active_agent().and_then(|agent| {
        ...
        let summary = if app.screen_mode.is_fullscreen() {
            ...
            (last_prompt.is_some() || last_response.is_some()).then(|| crate::app::ExitSummary {
...
/// [`CsiFragmentFilter`](crate::app::csi_filter::CsiFragmentFilter) before
...
pub(crate) fn is_bare_esc_press(ev: &Event) -> bool {
    matches!(
        ev,
    ...
    fn tty_suspend_arm_stops_same_batch_before_later_ownership_changes() {
        ...
        app.pending_editor = Some(
            crate::app::external_editor::PendingEditorRequest::PromptDraft {
                agent_id: crate::app::session::AgentId(0),
