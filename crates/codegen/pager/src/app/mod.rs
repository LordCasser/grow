...
///
/// Uses [`ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`diagnostics`'s `KNOWN_SCREEN_MODES`); renaming one
...
fn validate_llm_config(raw: &toml::Value) -> Result<(), String> {
    shell::agent::config::Config::new_from_toml_cfg(raw)?.validate_llm_configuration()
}
...
fn ensure_llm_configured(mut raw: toml::Value) -> anyhow::Result<toml::Value> {
    ...
    let path = tools::util::grow_home::grow_home().join("config.toml");
    ...
    raw = shell::config::load_effective_config()
