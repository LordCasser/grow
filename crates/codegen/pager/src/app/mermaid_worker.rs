...
//! inside [`mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `pager __mermaid-render` (see
...
//!    via [`mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `mermaid`). The parent also rejects obviously-oversized
...
mod tests {
    ...
    fn agent_with_session(name: &str) -> AgentView {
        ...
        let cwd = PathBuf::from("/mermaid-test").join(name);
