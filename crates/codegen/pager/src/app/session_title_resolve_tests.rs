...
fn title_miss_hint_escapes_arg_and_suggests_search() {
    ...
    assert!(
        hint.contains("grow sessions search"),
        "missing hint: {hint}"
    );
}
...
fn worktree_failure_message_hint_follows_threaded_provenance() {
    ...
    assert!(msg.contains("grow sessions search"), "{msg}");
...
async fn pinned_no_match_does_not_retry_title_after_sandbox() {
    ...
    let mut args = crate::app::cli::PagerArgs::try_parse_from(["grow", "-r", "ghost"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
...
async fn duplicate_legacy_id_title_pin_keeps_the_cwd_scoped_profile() {
    ...
    let mut args =
        crate::app::cli::PagerArgs::try_parse_from(["grow", "-r", "locked down"]).unwrap();
    args.pin_local_resume_target_for_cwd(Some(&cwd_str))
        .unwrap();
    assert!(args.resume_target_pinned);
