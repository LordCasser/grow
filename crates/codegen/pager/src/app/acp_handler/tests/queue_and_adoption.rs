#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// `grow/queue/changed` broadcast, and an empty broadcast clears it.
    #[test]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        // Shell and pager share the prompt-queue type, so serializing the payload as the
        ...
        let notif = acp::ExtNotification::new("grow/queue/changed", raw.into());
    ...
    /// broadcasts `grow/queue/changed` with `runningPromptId` = that synthetic
    ...
    fn adopted_running_prompt_lets_subsequent_chunk_through_gate() {
        ...
        let msg = AcpClientMessage::SessionNotification(acp_transport::AcpArgs {
            request,
            response_tx: tx,
        });
        handle(msg, &mut app);
    ...
    fn bash_kind_round_trips_and_adoption_sets_bash_turn() {
        // Shell and pager share the prompt-queue type; pin kind through a serde cycle.
