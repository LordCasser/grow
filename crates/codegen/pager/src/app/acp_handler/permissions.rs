...
fn enqueue_permission(
    ...
    // 1. Parse bash highlights from request meta (imported from shell).
...
/// `tools` for typed access to `command` and `description`.
...
fn cancel_permission(perm: acp_transport::AcpArgs<acp::RequestPermissionRequest>) {
    perm.response_tx
