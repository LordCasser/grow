...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    use shell::sampling::error::{RATE_LIMITED_ERROR_CODE, RATE_LIMITED_USER_MESSAGE};
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use extension_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "grow/marketplace/action" => {
                        ...
                        let req: extension_types::MarketplaceActionRequest = serde_json::from_str(
