...
pub(crate) fn tasks_block_text(agent: &AgentView) -> String {
    ...
    let mut workflows: Vec<_> = agent.session.workflow_runs.iter().collect();
...
mod tests {
    ...
    #[test]
    fn tasks_block_text_labels_workflow_runs() {
        ...
        assert!(
            ...
            "workflow run must render under the Workflow label: {text}"
        );
        assert!(
            text.contains("· Research"),
            "live phase must appear on the workflow row: {text}"
        );
        assert!(
            text.contains("· 1 agent"),
            "running agent count must appear on the workflow row: {text}"
        );
    }
}
