...
pub struct AcpConnection {
    ...
    /// Whether the agent is a shell instance.
    pub is_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<announcements::Announcement>>
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_shell(agent_config, cancel, memory_config).await?;
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = shell::util::grow_home::grow_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a shell agent
    let is_shell = resp
        .meta
        .as_ref()
