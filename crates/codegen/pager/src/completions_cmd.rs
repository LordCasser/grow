//! `grow completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "grow" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("grow");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "grow", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "grow", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `grow worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:grow-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:grow-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("grow");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "grow", &mut buf);
    ...
    // so `grow worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
    fn zsh_completions_drop_prompt_slot_and_dispatch_on_line_1() {
        ...
        assert!(
            fixed.contains(r#"curcontext="${curcontext%:*:*}:grow-command-$line[1]:""#),
            "root dispatch context must use $line[1]"
        );
        // Subcommand dispatch blocks (already on $line[1]) must survive.
        assert!(
            fixed.contains("grow-worktree-command-$line[1]"),
