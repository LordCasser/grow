...
use shell::session::worktree::META_KEY_LABEL;
...
mod tests {
    ...
    fn test_abbreviate_home() {
        if let Ok(home) = std::env::var("HOME") {
            let path = std::path::PathBuf::from(format!("{home}/work/grow"));
            assert_eq!(abbreviate_home(&path), "~/work/grow");
        }
    }
    ...
    fn make_test_record(metadata: Option<serde_json::Value>) -> fast_worktree::WorktreeRecord {
