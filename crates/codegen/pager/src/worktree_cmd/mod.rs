...
pub async fn run(args: WorktreeArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let spawned = crate::acp::spawn::spawn_shell(agent_config.clone(), &cancel, None).await?;
...
async fn dispatch(command: WorktreeCommand, tx: &acp_transport::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &acp_transport::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &acp_transport::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
        WorktreeDbCommand::Stats => {
            let stats: DbStats = ext_call(tx, "grow/git/worktree/db/stats", &()).await?;
        ...
        WorktreeDbCommand::Path => {
            ...
            let resp: PathResp = ext_call(tx, "grow/git/worktree/db/path", &()).await?;
            println!("{}", resp.path);
            Ok(())
        }
        WorktreeDbCommand::Rebuild => {
            let report: RebuildReport = ext_call(tx, "grow/git/worktree/db/rebuild", &()).await?;
...
mod tests {
    ...
    fn ext_request_builds_db_stats_empty_params() {
        let req = ext_request("grow/git/worktree/db/stats", &()).unwrap();
        assert_eq!(req.method.as_ref(), "grow/git/worktree/db/stats");
    }
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.grow/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.grow/worktrees.db");
    }
