...
pub use model::{
    ...
    ProbeStatus, RuntimeFact, TmuxFacts, TmuxOptionFact, TmuxSupportFact,
};
...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `grow wrap` on the local end, so
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Grow also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `grow wrap ssh <host>` on your local computer or use a terminal that \
    ...
    if ctx.is_tmux_backed() && matches!(tmux.control_mode, probes::TmuxProbeResult::Available(true))
    {
        let message = match fullscreen_active {
            ...
            Some(false) => "Grow is using inline mode because tmux control mode limits fullscreen",
...
pub(crate) fn wezterm_kitty_keyboard_warning_from(
    ...
    if shape == WezTermShape::SshXtversion {
        ...
        warning.note = Some(
            "For this session, type `\\` and then press Enter. Grow can't negotiate the Kitty \
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "Grow is using the user profile. Compare `.grow/sandbox.toml` with {}, then rename \
             or remove the conflicting project profile. Project settings can add profile names \
             but can't redefine a user profile.",
            crate::util::display_user_grow_path("sandbox.toml")
        )),
    })
}

/// Pure SSH `grow wrap` recommendation — suggests launching the session
/// through `grow wrap ssh <host>` on the user's local machine, which gives a
...
/// - `!osc52_sink_active` — no wrap is already capturing our output. `grow
...
pub(crate) fn collect_notification_warnings_with_method(
        ...
        && ctx.brand == TerminalName::Unknown
    {
        let mut warning = TerminalWarning::new(
            ...
            "Grow is using the terminal bell because the terminal was not recognized",
            None,
            None,
        );
    ...
    if condition == NotificationCondition::Unfocused && !supports_focus_tracking(ctx.brand) {
        let mut warning = TerminalWarning::new(
            ...
            Some(&crate::util::display_user_grow_path("config.toml")),
        );
        warning.note = Some(
...
pub(crate) fn collect_agent_definition_findings(workspace: &Path) -> Vec<DiagnosticFinding> {
    let invalid = agent::discovery::invalid_primary_agent_definitions(workspace);
    if invalid.is_empty() {
        return Vec::new();
    }
    let details = invalid
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset `NO_COLOR`, then restart Grow.".to_string());
        return Some(warning);
    }
...
mod tests {
    ...
    fn sandbox_profile_conflict_warning_reports_conflicts() {
        ...
        assert!(w.note.as_deref().is_some_and(|note| {
            note.contains("rename or remove")
                && note.contains(".grow/sandbox.toml")
                && note.contains(&crate::util::display_user_grow_path("sandbox.toml"))
    ...
    // -- ssh_wrap_hint: `grow wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("grow wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `grow wrap` — adoption silences the hint by itself.
        ...
        assert!(findings[1].remediation.as_ref().is_some_and(|remediation| {
            remediation.fix.contains("condition = \"always\"")
                && remediation.config_path.as_deref()
                    == Some(crate::util::display_user_grow_path("config.toml").as_str())
        }));
    }
