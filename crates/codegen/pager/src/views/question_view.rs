...
pub use tools::implementations::grow_build::ask_user_question::{
...
/// driven by an ACP `grow/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    /// Shown before creating a session from a non-project directory.
    ProjectSelect {
        ...
        /// Prompt text fallback used by pure translation tests and by callers
        /// that do not have a staged prompt payload.
...
///
/// Created when an `grow/ask_user_question` ext-method request arrives;
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> tools::implementations::grow_build::ask_user_question::AskUserQuestionExtResponse {
    ...
    pub fn send_ext_response(
        &mut self,
        response: tools::implementations::grow_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
