...
pub fn fuzzy_matches_hook(hook: &extension_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn source_has_matching_plugin(
    source: &extension_types::MarketplaceScanResult,
    query: &str,
) -> bool {
...
pub fn filtered_marketplace_count(
    sources: &[extension_types::MarketplaceScanResult],
    query: &str,
) -> usize {
...
pub(crate) fn test_plugin_info(
    ...
    extension_types::PluginInfo {
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: extension_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
...
type GroupedPlugins<'a> =
    std::collections::BTreeMap<(u8, String, String), Vec<(usize, &'a extension_types::PluginInfo)>>;
...
pub fn resolve_marketplace_index<'a>(
    sources: &'a [extension_types::MarketplaceScanResult],
    target: usize,
    query: &str,
...
pub enum MarketplaceHit<'a> {
    ...
    Plugin {
        source_index: usize,
        source: &'a extension_types::MarketplaceScanResult,
        plugin: &'a extension_types::MarketplacePluginEntry,
    },
}
...
fn build_hook_groups<'a>(
    hooks: &'a [extension_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[extension_types::HookInfo],
    current: usize,
...
impl ExtensionsTab {
    ...
    pub fn diagnostics_tab(self) -> diagnostics::events::ExtensionsModalTab {
        use diagnostics::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(extension_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(extension_types::PluginsAction),
    ...
    MarketplaceAction(extension_types::MarketplaceAction),
...
pub enum ConfirmationAction {
    ...
    Hooks(extension_types::HooksAction),
    ...
    Plugins(extension_types::PluginsAction),
    /// Replay a marketplace action (uninstall plugin or remove source).
    Marketplace(extension_types::MarketplaceAction),
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use extension_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use extension_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            extension_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<extension_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<extension_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<extension_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<extension_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[extension_types::PluginInfo]) {
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let grow = config::grow_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user grow home (GROW_HOME-aware)
    // or a project-scoped `{cwd}/.grow/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User grow home (GROW_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(grow.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.grow/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".grow" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    if let Some(name) = plugin_name("plugins").or_else(|| plugin_name("installed-plugins")) {
        return (format!("Plugin: {name}"), false);
    }
    // Global hooks under $GROW_HOME/hooks
    let global_hooks = grow.join("hooks");
    ...
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&grow) {
            let prefix = crate::util::display_grow_home_prefix();
...
fn build_plugin_fields(plugin: &extension_types::PluginInfo) -> Vec<String> {
    use extension_types::McpStatus;
...
fn component_categories(
    components: &extension_types::PluginComponents,
) -> [(&'static str, &[extension_types::ComponentItem]); 6] {
    use extension_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &extension_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &extension_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.grow/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // grow-home branch is GROW_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.grow/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.grow/installed-plugins/vendor-abc123/skills");
    ...
    fn make_skill(name: &str, desc: &str) -> tools::implementations::skills::types::SkillInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> extension_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(extension_types::PluginsListResponse {
            plugins: vec![
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                extension_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(name: &str, source_dir: &str, disabled: bool) -> extension_types::HookInfo {
        extension_types::HookInfo {
            name: name.to_string(),
            event: extension_types::HookEvent::PreToolUse,
            handler_type: extension_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> extension_types::MarketplaceScanResult {
        extension_types::MarketplaceScanResult {
    ...
    fn local_plugins_source() -> extension_types::MarketplaceScanResult {
        extension_types::MarketplaceScanResult {
            source_name: "local-plugins".into(),
            featured: false,
            source_kind: "local".into(),
            source_url_or_path: "/home/user/.grow/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    impl TestPlugin {
        fn build(self) -> extension_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                extension_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                extension_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Marketplace);
        state.marketplace_data = TabDataState::Loaded(extension_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_has_zero_plugins() {
        let error_source = extension_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> extension_types::ComponentItem {
        extension_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> extension_types::PluginComponents {
        extension_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(extension_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = extension_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(plugins: Vec<extension_types::PluginInfo>) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data = TabDataState::Loaded(extension_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use extension_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserGrow);
