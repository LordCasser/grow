...
pub struct PromptWidget {
    ...
    hovered_image_element_id: Option<ratatui_textarea::ElementId>,
    /// Image shown immediately after insertion without moving the edit cursor.
    post_insert_image_preview: Option<(ratatui_textarea::ElementId, usize)>,
        ...
        let applied = if snap.cursor_in_command {
            ...
            if let Some(ref range) = snap.command_range {
                    ...
                    && self.textarea.text().is_char_boundary(range.end)
                {
                    ...
                    // `/mod grow-4` yields `/model grow-4`, not `/model  grow-4`
            ...
            // Replace the args range (e.g., "gr" → "Grow 4 Fast").
...
impl PromptWidget {
    ...
    fn handle_key_inner(&mut self, key: &KeyEvent) -> PromptEvent {
        ...
        if changed {
            ...
            if is_backspace_key && !old_text.is_empty() {
                use diagnostics::events::BackspaceNoEffect;
    ...
    fn image_element_near_cursor(&self) -> Option<&ratatui_textarea::TextElement> {
    ...
    pub fn textarea(&self) -> &ratatui_textarea::TextArea {
        &self.textarea
    }
...
fn map_clean_offset_to_raw(
    text: &str,
    clean_offset: usize,
    elements: &[ratatui_textarea::TextElement],
) -> usize {
    if elements.is_empty() {
