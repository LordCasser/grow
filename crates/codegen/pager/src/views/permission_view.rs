...
pub struct PermissionViewState {
    ...
    pub request: acp_transport::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell Grow what to do differently
...
mod tests {
    ...
    fn empty_view_state(mcp_scope: Option<McpScopeState>) -> PermissionViewState {
        ...
        let perm = acp_transport::AcpArgs {
            request,
            response_tx,
        };
        PermissionViewState {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from grow\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from grow\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/pager
