...
//! render mode.
//!
//! `pager` cannot depend on `pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`cli`) wires it up once at
//! startup by calling `pager_minimal::install()`. When nothing has
