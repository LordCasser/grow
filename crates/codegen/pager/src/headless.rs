...
use crate::acp::spawn::{AgentShutdownGuard, spawn_shell};
...
pub async fn run_single_turn(
    ...
    if options.trust {
        shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    if track_active {
        let _ = shell::active_sessions::register(shell::active_sessions::ActiveSession {
    ...
    if track_active {
        ...
        let _ = shell::active_sessions::try_unregister(&session_id);
    }
