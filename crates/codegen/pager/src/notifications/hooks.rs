...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("GROW_EVENT", event_str)
        .env("GROW_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("GROW_SESSION_ID", sid);
    }
...
fn attach_to_global_scope(child: &std::process::Child) -> Option<Arc<tty_utils::ProcessGroup>> {
    ...
    tty_utils::global_process_scope()
        .register(&group)
        .then_some(group)
}
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'GROW_EVENT=%s\\nGROW_MESSAGE=%s\\nGROW_SESSION_ID=%s\\n' \
             \"$GROW_EVENT\" \"$GROW_MESSAGE\" \"$GROW_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("GROW_EVENT=Turn complete"),
            "missing GROW_EVENT: {content}"
        );
        assert!(
            content.contains("GROW_MESSAGE=hello world"),
            "missing GROW_MESSAGE: {content}"
        );
        assert!(
            content.contains("GROW_SESSION_ID=sess-42"),
            "missing GROW_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("GROW_SESSION_ID"),
            "GROW_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("GROW_EVENT=Turn complete"));
        assert!(content.contains("GROW_MESSAGE=test body payload"));
        assert!(content.contains("GROW_SESSION_ID=test-session-123"));
    }
}
