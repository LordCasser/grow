...
pub(super) fn default_actions(screen_mode: crate::app::ScreenMode) -> Vec<ActionDef> {
    ...
    let mut actions = vec![
        ...
        ActionDef {
            ...
            requires_confirmation: false,
            long_help: Some(
                "Opens the Agent picker to switch the prompt profile for this session.\nPress Ctrl+X, then A; /agents remains the management view.",
            ),
        },
        ActionDef {
    ...
    actions.extend([
        ...
        // rebind any of them through `~/.grow/config.toml`.
        ActionDef {
