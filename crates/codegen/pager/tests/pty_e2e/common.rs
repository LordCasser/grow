...
pub(crate) use pager_pty_harness::{
...
/// an 11-glyph `⣿` run the small art cannot contain (its longest run is 8), so
/// presence on screen proves the big logo tier is rendering.
pub(crate) const WELCOME_BIG_LOGO_MOTIF: &str = "⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣥";

/// Motif from the small logo art (`assets/logo/grow-small.txt`, 50 cols × 22
...
// `wait_for_labels_absent` lives in `pager_pty_harness::flows`
// (re-exported above).
...
// unit tests in hooks::discovery::tests. The PTY E2E test requires
...
pub(crate) use pager_pty_harness::host_clipboard::{
...
pub(crate) use pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
