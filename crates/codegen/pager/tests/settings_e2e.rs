...
use pager::views::settings_modal::{
...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    pager::views::settings_modal::render_settings_modal(&mut buf, area, &mut s, false, None);
...
fn defaults_round_trip_through_registry() {
    use pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    pager::appearance::cache::set_keep_text_selection(pager::appearance::TextSelection::Flash);
    pager::appearance::cache::set_show_thinking_blocks(true);
    pager::appearance::cache::set_prompt_suggestions(true);
    pager::appearance::cache::set_group_tool_verbs(true);
    pager::appearance::cache::set_page_flip_on_send(true);
    pager::appearance::cache::set_combine_queued_prompts(false);
    pager::appearance::cache::set_scroll_mode(pager::appearance::ScrollMode::Auto);
    pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    pager::appearance::cache::set_scroll_lines(3);
    ...
    for meta in reg.all() {
        ...
        let default_value = pager::settings::default_value_for(meta);
...
// `crates/codegen/pager/src/app/dispatch.rs::tests` (next to
...
fn reset_overlay_dims_all_rows_except_target() {
    use pager::views::settings_modal::ResetConfirmOverlay;
    use ratatui::buffer::Buffer;
    ...
    pager::views::settings_modal::render_settings_modal(
        &mut buf,
        area,
        &mut s,
        false,
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        pager::settings::default_value_for(mt_meta),
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use pager::appearance::TextSelection;
    ...
    pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
