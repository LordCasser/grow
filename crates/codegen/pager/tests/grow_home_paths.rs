...
fn grow_home_override_path_helpers() {
    ...
    assert_eq!(pager::util::pager_toml_path(), grow_home.join("pager.toml"));
    assert_eq!(pager::util::display_grow_home_prefix(), "$GROW_HOME");
    assert_eq!(
        pager::util::display_user_grow_path("config.toml"),
        "$GROW_HOME/config.toml"
    );

    let memory_path = grow_home.join("memory/MEMORY.md");
    assert_eq!(
        pager::util::abbreviate_path(&memory_path.display().to_string()),
    ...
    assert_eq!(
        pager::clipboard::display_copy_path(&grow_home.join("last-copy.txt")),
        "$GROW_HOME/last-copy.txt"
    );

    assert!(pager::util::is_under_user_grow_home(&memory_path));
    assert!(!pager::util::is_under_user_grow_home(
