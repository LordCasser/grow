...
Want Grow working on a feature while you (or another Grow session) work on
...
- **From the shell:**
  ...
  ```bash
  grow --worktree=my-feature "refactor the auth module"
  ```
...
- Run two or three Grow sessions on the same repo simultaneously.
...
Running several agents? The **dashboard** (`/agents` or `Ctrl+\`) shows
