...
Reference for key bindings in the Grow TUI. Bindings are built in and cannot currently be remapped.

---

## Input Modes

Grow has two input modes that control how you navigate the scrollback:
...
Simple mode is active by default. To switch to Vim mode, set `vim_mode = true` under `[ui]` in `~/.grow/config.toml`, or toggle it at runtime with `/vim-mode`. See [Configuration](05-configuration.md) for details.
...
> in `~/.grow/config.toml` (or `/vim-mode` to toggle). Arrow keys, `Tab`,
...
**Esc is not a focus key.** It follows the cancel / clear / rewind semantics below. The mid-turn cancel is the only branch gated on `[ui].vim_mode` (scrollback nav); nothing depends on `[ui].simple_mode` (prompt editor). Overlays, modals, slash/file dropdowns, search, and selection still steal Esc first.
...
> **`Alt+V` on Windows** is grow-specific. Windows Terminal's default `Ctrl+V` only pastes plain text and silently drops image clipboards; `Alt+V` bypasses the interceptor. To use `Ctrl+V` for images too, add `{ "command": null, "keys": "ctrl+v" }` to `actions` in your Windows Terminal `settings.json`.
...
- An unmodified middle click in Grow reads **PRIMARY**, the current mouse selection, only when `DISPLAY` is non-empty. Pure X11 can use its native reader fallback; XWayland requires `xclip` or `xsel` on `PATH` so Grow reads the X11 selection rather than Wayland PRIMARY. The press is handled once; the release does not paste again.
...
Over SSH, the remote Grow process usually cannot access the terminal's local X11 selection. Use terminal-native `Shift+Insert` or `Shift+middle click` so the local terminal sends the selected text through the PTY.
...
> **Returning to the welcome screen has no key binding** — use the `/home` slash command from inside a session. See [Slash Commands](04-slash-commands.md).
...
> **Cmd+A is gated to Ghostty.** Grow's in-app `Cmd+A` handler is only
