...
Skills are reusable prompt packages that extend Grow with task-specific instructions. They let you capture a repeatable procedure once, instead of re-explaining it each session.

---

## What Are Skills?

A skill is a directory that contains a `SKILL.md` file. Its markdown body tells Grow how to handle a specific type of task: step-by-step instructions, conventions, and tool-usage patterns.

Use a skill for a repeatable procedure that's too specific for AGENTS.md but too long to retype. Grow activates a skill only when it applies to your current task.

---

## Skill Locations

Grow discovers skills from these directories, in priority order:
...
Add directories, exclude paths, or disable individual skills via `[skills]` in `~/.grow/config.toml`:
...
Each entry in `paths` is a `SKILL.md` file or a directory that Grow walks recursively. `ignore` hides a skill completely; `disabled` keeps it in the list but excludes it from the system prompt and from invocation. `paths` and `ignore` take filesystem paths and support `~` expansion; `disabled` takes skill names.
...
| Field | Description |
|-------|-------------|
| `name` | Skill identifier. Use lowercase letters, digits, and hyphens, up to 64 characters. Grow normalizes spaces and underscores to hyphens. If you omit `name`, Grow uses the skill's directory name. |
| `description` | What the skill does and when to use it. Grow reads this to decide whether to invoke the skill. If you omit it, Grow uses the first paragraph of the body. |

Write a specific `description`. It determines when Grow invokes the skill automatically. Name the trigger phrases and use cases.
...
The `/create-skill` command walks you through building a new skill interactively. Grow asks what you want, drafts the files, and writes them to disk.

### How It Works

When you run `/create-skill`, Grow:

1. **Gathers requirements.** Grow asks for the skill name, the scope to save it under, and a description of the workflow you want to capture. Use a name with lowercase letters, digits, and hyphens (2–64 characters, starting and ending with a letter or digit).

2. **Drafts the description.** Grow writes a `description` that states what the skill does, the phrases that trigger it, and the slash command name. You approve or edit the draft before continuing.

3. **Creates the skill directory.** Grow creates the `<scope>/.grow/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.

4. **Writes SKILL.md.** Grow writes the frontmatter (`name` and `description`) and a markdown body of instructions, along with any supporting files.

5. **Verifies and confirms.** Grow reads the file back, confirms it wrote correctly, and tells you how to run the skill.

### Choosing a Scope

Grow asks where to save the skill:

- **Project** (`<repo_root>/.grow/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Grow recommends this scope inside a git repository.
- **User** (`~/.grow/skills/<name>/`) -- available across all your projects.
...
The new skill appears in the slash menu within a few seconds, because Grow reloads skills when files change on disk.
...
To browse your skills, type `/` to open the slash-command menu. Grow lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `grow inspect` (see [Viewing Skill Details](#viewing-skill-details)).

### Qualified Names

When a skill's name collides with another skill or a built-in command, Grow advertises a qualified name prefixed by the skill's scope -- `local:`, `repo:`, `user:`, or the plugin name. Use the qualified form to choose a specific skill:

```
/local:commit        # The "commit" skill from ./.grow/skills/
/user:commit         # The "commit" skill from ~/.grow/skills/
```

### Automatic Invocation

Grow can invoke a skill on its own when it recognizes a relevant task. Grow matches your prompt against the skill's `description` and `when-to-use` fields, so write both to describe the triggering situation.
...
Run `grow inspect` to see every skill Grow discovers, along with the rest of your configuration:

```bash
grow inspect          # Human-readable summary
grow inspect --json   # Machine-readable report
```

In the human-readable output, the Skills section lists each skill's name and its source -- `project`, `user`, `bundled`, `config` (a `[skills].paths` entry), `server` (skills synced from the skill store in managed workspaces), or `plugin: <name>`. Grow tags any skill disabled via `[skills].disabled` or from a disabled vendor surface with `[disabled]`.
...
Grow distributes platform skills separately from your personal skills. Bundled skills are cached under `~/.grow/bundled/skills/`; Grow never writes them into `~/.grow/skills/`. A same-named local, repo, or user skill overrides the bundled copy. `grow inspect` labels each definition by its actual source. (A plugin skill of the same name does not override a native skill; it stays available under its qualified `plugin:name` form.)

Skills can also come from plugins. When you install a plugin that includes skills, they appear alongside your user and project skills. `grow inspect` labels each plugin-provided skill with its source as `plugin: <name>`.
...
2. **Include concrete steps.** Skills work best when they give Grow a clear, ordered procedure to follow.
