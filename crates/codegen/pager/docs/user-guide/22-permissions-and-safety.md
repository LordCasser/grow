...
When Grow edits a file, runs a command, or calls an external tool, it may pause for approval. Permission modes control how often that happens.
...
```text
hard exact-tool eligibility ∩ projected RWX ∩ permission decision ∩ call-bound permit
```

Hard eligibility comes from registered exact tool identities, the Agent's authored preset/additional tools and allow/deny configuration, session clamps, Behavior/Goal/depth/plugin rules, and MCP inheritance. Runtime-injected native tools cannot enlarge authored Execute/ReadWrite eligibility. `capability_mode` defines immutable initial RWX; it defaults to `read-write`, and a nested child's effective initial mode is its requested/default mode intersected with the immediate parent's immutable delegation ceiling. A hard-eligible call outside initial RWX enters Ask/Auto without changing session authority. On allow, the permit binds call id, exact identity/target, canonical arguments, cwd, projected RWX, actor epoch, and MCP transport generation; dispatch consumes it once and rechecks the bindings. No child approval is persisted or shared with the parent, siblings, descendants, or a new session.
...
### 2. Native Configuration (`~/.grow/config.toml` and `.grow/config.toml`)
...
**Chained commands.** Grow parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full Grow tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. Grow tool names carry no `mcp__` prefix, so a rule written as `mcp__server__tool` never matches an MCP call; write `MCPTool(server__tool)` instead.
...
Interactive grants are stored in Grow's own state directory under your home directory, scoped to the directory you launched Grow from. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.grow/config.toml` instead.
...
```bash
chmod +x ~/.grow/hooks/git-gh-only.sh
```
...
```bash
grow -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
