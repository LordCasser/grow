//! Diagnostic event structs. Every struct needs a `diagnostics_event!` binding.
//! `session_id` and `turn_number` are auto-injected by `log_event` (which
//! lives in shell's integration layer).
//!
//! These structs were extracted from `shell` so they can be
...
/// Binds a stable local diagnostic event name to a serializable payload.
pub trait DiagnosticEvent: Serialize + Send + 'static {
    const NAME: &'static str;
}

macro_rules! diagnostics_event {
    ($struct:path, $name:literal) => {
        impl $crate::events::DiagnosticEvent for $struct {
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `grow wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
/// the product diagnostics event both carry. Formerly re-exported from a shared
...
// ---------------------------------------------------------------------------
// Pager events (called from pager via log_event)
// ---------------------------------------------------------------------------
...
/// Constructed by the pager's `TerminalContext::diagnostics_snapshot()`.
...
pub struct ClipboardCopy {
    ...
    /// An explicit `grow wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
diagnostics_event!(CompactionRetryDegraded, "compaction_retry_degraded");
diagnostics_event!(SubagentLaunched, "subagent_launched");
diagnostics_event!(SubagentCompleted, "subagent_completed");
diagnostics_event!(ModelSwitched, "model_switched");
diagnostics_event!(PluginAdded, "plugin_added");
diagnostics_event!(PluginRemoved, "plugin_removed");
diagnostics_event!(PluginInstalled, "plugin_installed");
diagnostics_event!(PluginUninstalled, "plugin_uninstalled");
diagnostics_event!(PluginReloaded, "plugin_reloaded");
diagnostics_event!(PluginUsed, "plugin_used");
diagnostics_event!(PluginCtaImpression, "plugin_cta_impression");
diagnostics_event!(PluginCtaConnectClicked, "plugin_cta_connect_clicked");
diagnostics_event!(PluginCtaDismissed, "plugin_cta_dismissed");
diagnostics_event!(PluginCtaInstalled, "plugin_cta_installed");
diagnostics_event!(ExtensionsModalOpened, "extensions_modal_opened");
diagnostics_event!(ExtensionsModalAction, "extensions_modal_action");
diagnostics_event!(HookAdded, "hook_added");
diagnostics_event!(HookRemoved, "hook_removed");
diagnostics_event!(HookTrusted, "hook_trusted");
diagnostics_event!(HookExecuted, "hook_executed");
diagnostics_event!(HookBlocked, "hook_blocked");
diagnostics_event!(ClientHookGate, "client_hook_gate");
diagnostics_event!(SkillAdded, "skill_added");
diagnostics_event!(SkillRemoved, "skill_removed");
diagnostics_event!(SkillDispatched, "skill_dispatched");
diagnostics_event!(McpServerConnected, "mcp_server_connected");
diagnostics_event!(McpServerFailed, "mcp_server_failed");
diagnostics_event!(McpInitCompleted, "mcp_init_completed");
diagnostics_event!(McpToolCalled, "mcp_tool_called");
diagnostics_event!(SessionHarness, "session_harness");
diagnostics_event!(SessionLoad, "session_load");
diagnostics_event!(SessionNew, "session_new");
diagnostics_event!(PromptSubmitted, "prompt_submitted");
diagnostics_event!(UserFeedback, "user_feedback");
diagnostics_event!(PrCreated, "pr_created");
diagnostics_event!(PrMerged, "pr_merged");
diagnostics_event!(MultiAgentFollowup, "multi_agent_followup");
diagnostics_event!(MultiAgentApply, "multi_agent_apply");
diagnostics_event!(MultiAgentDiscard, "multi_agent_discard");
diagnostics_event!(RepoChanges, "repo_changes");
diagnostics_event!(NonGitDecisionEvent, "non_git_decision");
diagnostics_event!(PromptLatency, "prompt_latency");
diagnostics_event!(TurnCompleted, "turn_completed");
diagnostics_event!(ShellTrueNoop, "shell_true_noop");
diagnostics_event!(ActionStationarityStop, "action_stationarity_stop");
...
diagnostics_event!(ClipboardImagePaste, "clipboard_image_paste");
diagnostics_event!(PasteKeyEmptyHostClipboard, "paste_key_empty_host_clipboard");
diagnostics_event!(ClipboardCopy, "clipboard_copy");
diagnostics_event!(NotificationEmitted, "notification_emitted");
diagnostics_event!(DashboardOpened, "dashboard_opened");
diagnostics_event!(DashboardClosed, "dashboard_closed");
...
diagnostics_event!(crate::session_metrics::SessionStarted, "session_started");
diagnostics_event!(crate::session_metrics::Turn, "turn");
diagnostics_event!(
...
// Memory subsystem
diagnostics_event!(
    crate::memory_events::MemorySessionInit,
    "memory_session_init"
);
diagnostics_event!(crate::memory_events::MemorySearch, "memory_search");
diagnostics_event!(
    crate::memory_events::MemorySearchEmpty,
    "memory_search_empty"
);
diagnostics_event!(crate::memory_events::MemoryFlushStart, "memory_flush_start");
diagnostics_event!(
    crate::memory_events::MemoryFlushComplete,
    "memory_flush_complete"
);
diagnostics_event!(crate::memory_events::MemoryInjection, "memory_injection");
diagnostics_event!(crate::memory_events::MemoryReindex, "memory_reindex");
diagnostics_event!(
    crate::memory_events::MemoryWatcherSync,
    "memory_watcher_sync"
);
diagnostics_event!(
    crate::memory_events::MemorySessionSummary,
...
mod tests {
    ...
    fn terminal_diagnostics_fixture() -> TerminalDiagnostic {
        TerminalDiagnostic {
            brand: "Unknown".into(),
    ...
    fn clipboard_copy_serialization_preserves_boolean_and_adds_delivery_evidence() {
        for delivery in ["confirmed", "unverified", "failed"] {
            let value = serde_json::to_value(ClipboardCopy {
                terminal: terminal_diagnostics_fixture(),
                source: "copy_text",
                text_len: 12,
                route_native: true,
                route_tmux: false,
