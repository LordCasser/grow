...
pub use grow_workspace_types::MCP_TOOL_NAME_DELIMITER;
...
pub struct McpState {
    ...
    pub mcp_server_scopes: std::collections::HashMap<String, xai_tool_protocol::ToolScope>,
...
/// External-config overrides for an MCP server, surfaced to grow-mcp
...
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = grow_config::grow_home().join("logs").join("mcp");
