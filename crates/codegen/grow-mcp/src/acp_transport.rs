...
//!
//! In-process SDK MCP servers (the official `grow-agent-sdk`'s `@tool` /
...
//! reverse `grow/mcp/sdk_call` request and feeding the response back. This module
...
/// channel (`grow/mcp/sdk_call`) and returns its JSON-RPC response. The `Err` string is
...
/// `grow/mcp/sdk_call` and writes the server→client response back. The pump exits when
...
async fn read_requests(
    ...
    loop {
        ...
        // `grow/mcp/sdk_call`s without a JSON-RPC id, so id-less messages (e.g. rmcp's
