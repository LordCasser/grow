...
Goal is objective-governed continuation. GoalTracker owns only the durable objective, lifecycle status, budget, and usage; Behavior owns the collaboration protocol. Every idle continuation begins by auditing the entire objective against concrete evidence. Remaining work is planned as ordinary short-lived task/todo slices, never as a second persistent plan. `update_goal(status=complete)` returns to Normal only when the whole objective is achieved; blocked, usage-limited, paused, and budget-limited Goals stay selected without automatic continuation.
