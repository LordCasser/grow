...
Audience guidance schedules work around the facts, interfaces, and artifacts that each consumer needs. Exploration, implementation, and acceptance can have different prerequisites. The primary Agent owns shared decisions, write handoffs, and integration; the child works autonomously within its assignment, stops work affected by invalid premises, and returns evidence with any remaining blocker. Candidate results become accepted through proportionate verification, not through runtime completion alone. Existing task context holds this reasoning; there is no additional scheduler, persistent task graph, or Jev decision layer.

Role bodies specialize the work: general-purpose handles bounded implementation and verification, while explore investigates a specific question without writing and stops when it has sufficient evidence. Tool descriptions supply the concrete delegation and waiting guidance with finalized tool and parameter names. In particular, a positive multi-task wait still waits for all selected tasks, an inquiry does not trigger foreground action or grant authority, and message receipt does not establish a safe write handoff. See the [local coordination contract](../../../openspec/specs/local-coordination/spec.md) and the [prompt change delta](../../../openspec/changes/improve-dependency-driven-agent-prompts/specs/local-coordination/spec.md) for the behavioral requirements and pending evaluation scope.
...
`toolPreset` is resolved first, followed by `additionalTools`, fixed runtime injection, Agent denies and subagent policy, session clamps, depth/ownership/plugin/MCP eligibility, and ToolBridge finalization. Native hard eligibility normally requires both an authored exact identity from `toolPreset`/`additionalTools` and a surviving implementation in the finalized bridge. A concrete implementation may instead declare descriptor-owned `subagentReview: required`; if that finalized identity survives every later filter, it is admitted only as approval-required and can never enter the initial-RWX fast path. Thus runtime injection cannot silently grant ordinary authority, while a deliberately reviewable tool such as whole-file `write` remains truthfully discoverable. For a subagent, `capabilityMode` establishes immutable initial RWX rather than deleting latent tools or creating mutable session authority. A projected ordinary call inside initial RWX follows the ordinary permission path; an ordinary hard-eligible call outside it and every approval-required call enter the configured permission Gate for one exact frozen call. `ToolKind` remains presentation/discovery metadata; the finalized descriptor, projected RWX, exact identity, permission decision, and call-bound permit own authorization.
...
registered by Tool Preset / Registry
  ...
  ∩ projected call RWX
  → ordinary identity inside initial RWX: ordinary permission path
  → ordinary identity outside initial RWX but hard-eligible: configured permission Gate
  → descriptor review-required identity: configured permission Gate regardless of initial RWX
  → call-bound permit + dispatch-time identity/argument/transport revalidation
```

The first three lines plus a trusted review-required descriptor form the subagent's immutable hard eligibility ceiling. The capability catalog truthfully distinguishes available, call-projected, approval-required, and forbidden identities; it is not an authorization API. A projected locked call goes directly through the ordinary permission manager. Shell freezes the actual wire identity and canonical argument hash before that request; whole-file `write` also contributes a bounded untrusted summary containing its operation, path, byte length, digest, preview budget, and truncation flag. Classifier input and permission audit use this exact identity instead of guessing a tool name from coarse `Edit(path)` policy data. Allowing one does not mutate child authority, expose a server for the rest of the session, or change the next sample. The permit binds call id, exact tool/dispatch target, canonical arguments, cwd, projected RWX, actor epoch, and MCP transport generation; dispatch consumes it once and repeats hard-eligibility and transport checks.

The capability catalog is an audience/runtime layer, not role prose. The subagent audience explains exact-call authority; a native system reminder lists fully covered, call-projected, approval-required, and forbidden identities; MCP reminders list inherited eligible servers. `search_tool` searches only the finalized child MCP index and labels results `call_bound`. The parent's model-visible MCP allowset is a live, depth-preserving authority: search and dispatch recheck it, while catalog changes reconcile registrations in the existing ToolBridge and continue through system reminders.
