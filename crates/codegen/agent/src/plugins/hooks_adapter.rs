...
//! `hooks`' parser and inject plugin env vars. Not a second engine.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        spec.layer = hooks::config::HookProvenance::Plugin;
        spec.name = format!(
            "{}{}/{}",
            hooks::config::PLUGIN_HOOK_PREFIX,
            plugin_name,
            spec.name
        );
        ...
        if let Some(cmd) = &spec.command {
            ...
            let expanded = config::expand_env_vars_in_string(&substituted);
            if expanded != cmd_str {
