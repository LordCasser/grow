...
impl LoadedPlugin {
    /// Data directory for this plugin: `~/.grow/plugin-data/<plugin_id>/`.
    pub fn data_dir(&self) -> PathBuf {
        config::grow_home().join("plugin-data").join(&self.id.0)
    }
...
pub fn skill_md_paths(skill_dirs: &[PathBuf]) -> Vec<PathBuf> {
    use tools::implementations::skills::discovery::{find_skill_md_paths, normalize_skill_name};
...
mod tests {
    ...
    fn data_dir_uses_plugin_id() {
        ...
        // Should be under ~/.grow/plugin-data/<plugin_id>/
