...
//! Cache root: `~/.grow/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = grow_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(grow_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    grow_config::grow_home().join("marketplace-cache")
}
...
mod tests {
    ...
    #[test]
    fn default_cache_root_under_grow() {
