...
//!    the lookup order — `.grow-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = grow_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(grow_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let grow_dir = dir.path().join(".grow-plugin");
        std::fs::create_dir_all(&grow_dir).unwrap();
        std::fs::write(
            grow_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn grow_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        make_plugin(dir.path(), "grow-plugin", "1.0.0");
        ...
        std::fs::write(
            grow_dir.join("marketplace.json"),
            r#"{
                "name": "grow-marketplace",
                "plugins": [{
                    "name": "grow-plugin",
                    "description": "From the .grow-plugin index",
                    "category": "design",
                    "source": { "type": "local", "path": "./plugins/grow-plugin" },
                    "tags": ["grow"]
                }]
            }"#,
        )
        ...
        let plugins = scan_marketplace(dir.path()).entries;
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].name, "grow-plugin");
        assert_eq!(plugins[0].category.as_deref(), Some("design"));
        assert_eq!(plugins[0].tags, vec!["grow"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_grow_file(dir: &Path, file: &str, content: &str) {
        let grow_dir = dir.join(".grow-plugin");
        std::fs::create_dir_all(&grow_dir).unwrap();
        std::fs::write(grow_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_grow_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_grow_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_grow_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_grow_file(dir.path(), "plugin-index.json", "not json");
