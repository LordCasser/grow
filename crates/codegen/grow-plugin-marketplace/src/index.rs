...
//!
//! Catalog file lookup, in order: `.grow-plugin/marketplace.json` (preferred),
//! `.grow-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
pub fn load_index(marketplace_root: &Path) -> Result<Option<MarketplaceIndex>, String> {
    let grow_dir = marketplace_root.join(".grow-plugin");
...
mod tests {
    ...
    fn load_index_valid_grow_dir() {
        ...
        assert_eq!(result.unwrap().name, "grow");
    }

    #[test]
    fn load_index_grow_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".grow-plugin", "grow"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "grow");
    }
