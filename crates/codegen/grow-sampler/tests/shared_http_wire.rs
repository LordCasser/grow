...
use grow_sampler::SamplingClient;
use grow_test_support::spawn_counting_server;
use support::{send_one, test_config};

/// Pin the env these assertions depend on before any client is built, so
/// ambient shell exports (`GROW_SAMPLER_SHARED_CLIENT=0`,
/// `GROW_POOL_MAX_IDLE=0`) cannot flip the expected pooling behavior.
fn pin_env() {
    ...
    PIN.call_once(|| {
        ...
        unsafe {
            std::env::remove_var("GROW_SAMPLER_SHARED_CLIENT");
            std::env::set_var("GROW_POOL_MAX_IDLE", "2");
            std::env::set_var("GROW_POOL_IDLE_TIMEOUT_SECS", "90");
        }
    });
}
