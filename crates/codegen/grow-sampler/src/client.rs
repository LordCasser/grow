...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = grow_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<grow_sampling_types::DoomLoopRecoveryPolicy>,
}
...
mod tests {
    use super::*;
    use grow_sampling_types::types::ChatRequestMessage;
    use indexmap::IndexMap;
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the grow-shell agent product.
        assert!(ua.starts_with("my-client grow-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = grow_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
