...
/// grow config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_grow_home().as_deref(), USER_CONFIG_FILENAME)
}

/// User config filename (`$GROW_HOME/config.toml`), shared by the loaders here.
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_grow_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.grow/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.grow` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/grow`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_grow_home().as_deref())
}
...
/// A hook's origin (held by `grow_hooks::HookSpec::layer`). Defined here, not
/// in `grow-hooks`, since the dep direction is `grow-hooks -> grow-config`;
/// this crate sets the config tiers, `File`/`Plugin` are set downstream.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum HookProvenance {
    /// `/etc/grow/managed_config.toml`.
    SystemManaged,
    /// `$GROW_HOME/managed_config.toml` (server-synced).
    Managed,
...
pub fn hook_config_layers() -> Vec<HookConfigLayer> {
    hook_config_layers_at(system_config_dir().as_deref(), user_grow_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `GROW_CAMPAIGNS_OVERRIDE` env layer.
...
/// `GROW_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
...
/// (`grow_shell::util::config::load_effective_config`) when remote campaigns
/// or `GROW_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// Path to `$GROW_HOME/campaigns_state.json` under `home`.
pub fn campaigns_state_path(home: &std::path::Path) -> std::path::PathBuf {
    home.join(CAMPAIGNS_STATE_FILE)
}

/// Fail-open dismissed ids from `$GROW_HOME/campaigns_state.json`.
...
/// can't be parsed (broken `GROW_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match grow_version::installed_semver() {
...
mod tests {
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("grow-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    /// `GROW_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `GROW_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("GROW_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads GROW_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("GROW_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("GROW_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("GROW_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("GROW_CAMPAIGNS") },
        }
    }
}
