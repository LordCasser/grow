...
**症状**：`cargo test -p grow-shell --lib` 下某些 `#[tokio::test(flavor = "current_thread")]`
...
2. **确认测试线程的实际栈范围**（lldb，崩溃时）：
   ...
   ```bash
   lldb -b --no-lldbinit -s script.lldb -- ./target/debug/deps/grow_shell-<hash> '<test>' --exact
...
- 共享 `CARGO_TARGET_DIR` 时，**不同 worktree（不同源码版本）编译同一包会互相
  ...
  `cannot find function max_wait_block_ms in crate xai_tool_types`）。此时
