...
//!
//! ```text
//! cargo bench -p xai-grow-shell --bench skills_watcher_startup
//! # optional scale:
//! GROW_SKILLS_WATCHER_BENCH_DIRS=12000 cargo bench -p xai-grow-shell --bench skills_watcher_startup
//! ```
//!
...
use grow_shell::config::watcher::SkillsFileWatcher;
...
fn worktree_dir_count() -> usize {
    std::env::var("GROW_SKILLS_WATCHER_BENCH_DIRS")
        .ok()
...
fn build_fixture(worktree_dirs: usize) -> Fixture {
    ...
    let grow_home = root.path().join("grow-home");
