...
/// All tests share this as GROW_HOME (the `OnceLock` in grow-config
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("GROW_HOME", &path) };
        path
    })
}
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = grow_shell::config::load_effective_config().unwrap();
    let effective_cfg = grow_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    grow_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = grow_shell::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("grow-3"));
}
