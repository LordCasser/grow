...
//!
//! Runs the built grow binary headless against the mock inference server with
//! a config-defined BYOK model whose bearer comes from a mock auth binary (a
//! script the test writes to disk). The harness's `GROW_API_KEY` stands in for
//! the session-tier credential.
//!
//! `#[ignore]` (needs a built binary). The CI lifecycle lanes run it against
//! the release artifact via `GROW_BINARY`; run locally (auto-builds the pager):
//! ```bash
//! cargo test -p grow-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    sandbox.remove_env("GROW_LEADER_SOCKET");

    let grow_home = sandbox.grow_home().to_path_buf();
    std::fs::create_dir_all(&grow_home).expect("create .grow home");

    let counter = grow_home.join("mint-count");
    let helper = grow_home.join("mock-auth.sh");
    std::fs::write(
        &helper,
        format!(
...
"#,
    ...
    // `test-key-for-ci` is the harness's GROW_API_KEY; it stands in for the
...
async fn provider_with_args_and_json_output_sends_minted_token() {
    ...
    let seen_args = grow_home.join("seen-args");
    let helper = grow_home.join("mock-auth-json.sh");
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\nprintf '%s' \"$*\" > {}\n\
