...
use grow_shell::cpu_profile::ControlErrorCode;
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, grow_shell::leader::ServerHandle) {
...
async fn test_session_new_valid_default_model_injected() {
    ...
    let meta = &json["params"]["_meta"];
    assert_eq!(meta["modelId"], "grow-3-fast");
...
/// `grow/session_notification` (carrying the `ModelChanged` update) out
...
async fn test_set_model_broadcasts_to_session_subscribers() {
    ...
    let broadcast = format!(
        r#"{{"jsonrpc":"2.0","method":"grow/session_notification","params":{{"sessionId":"{}","update":{{"sessionUpdate":"model_changed","model_id":"grow-4","reasoning_effort":"high"}}}}}}"#,
        shared_sid
    );
    ...
    assert_eq!(f["method"], "grow/session_notification");
...
async fn test_extension_method_roundtrip() {
    ...
    let ext_call = r#"{"jsonrpc":"2.0","id":50,"method":"_grow/search/fuzzy/open","params":{"sessionId":"sess-123","hidden":false}}"#;
    ...
    assert_eq!(json["method"], "_grow/search/fuzzy/open");
...
async fn test_detached_session_notifications_do_not_fallback() {
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(grow_shell::leader::ShutdownReason::Manual);
    ...
    assert_eq!(eviction_json["method"], "grow/internal/evict_sessions");
    ...
    let probe = r#"{"jsonrpc":"2.0","method":"grow/probe","params":{"ping":true}}"#;
...
/// Verify that an `grow/code/status` extension request is forwarded to the
...
async fn test_code_status_ext_request_forwarded_to_agent() {
    ...
    // Send grow/code/status with a sessionId — the leader must forward it to the agent.
    let status_req = r#"{"jsonrpc":"2.0","id":42,"method":"extensions/ext","params":{"method":"grow/code/status","params":{"sessionId":"sess-web-1","cwd":"/repo"}}}"#;
    ...
    assert_eq!(json["params"]["method"], "grow/code/status");
...
/// Integration test: a connected client receives `grow/leader/version_mismatch`
...
async fn test_version_mismatch_notification_sent_to_client() {
    use grow_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
    use std::sync::Arc;
    use std::sync::atomic::{AtomicBool, AtomicUsize};
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use grow_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != grow_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        grow_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
async fn test_leader_code_nav_isolation_end_to_end() {
    ...
    // Web client sends grow/code/status (the primary non-starting code-nav call).
    let status_with_session = r#"{"jsonrpc":"2.0","id":10,"method":"extensions/ext","params":{"method":"grow/code/status","params":{"sessionId":"web-session","cwd":"/repo"}}}"#;
    ...
    assert_eq!(status_json["params"]["method"], "grow/code/status");
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        grow_shell::leader::DisconnectReason::Connected,
