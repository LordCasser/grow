...
//! ```bash
//! cargo test -p grow-shell --test test_debug_logging -- --ignored
//! ```
...
/// Build a headless `grow -p` command with a pinned `$GROW_HOME` so the firehose
/// lands under `<home>/.grow/debug`. Firehose env knobs are cleared so the test
...
async fn read_session_firehose_when_ready(path: &Path, client: &GrowStdioClient) -> String {
...
/// headless `grow -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "grow --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "grow headless (no --debug)", Some(&server));
    assert!(
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let client =
            GrowStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
...
/// The `--debug` FLAG (not `GROW_DEBUG_LOG` directly) drives the firehose end to
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // GROW_DEBUG_LOG env. The sandbox baseline excludes inherited firehose
        // toggles, so the `--debug` flag is the only thing enabling logging here.
        let client = GrowStdioClient::spawn_with_sandbox_env_and_args(
            &server,
            workdir.workspace(),
            sandbox,
            &[],
        ...
        // `~/.grow/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // GROW_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = grow_home.join("logs").join("sampling.jsonl");
...
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "grow --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
async fn grow_log_file_explicit_path_is_written() {
    ...
    sandbox.set_env("GROW_LOG_FILE", &custom);
    let result = run_headless_in_sandbox(cmd, sandbox).await;

    assert_headless_success(&result, "grow GROW_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
