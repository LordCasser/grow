...
//! 4. Smoke-validate session JSON under `$GROW_HOME/sessions/` after exit.
//!
//! Requires a built `grow` binary (`GROW_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p grow-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p grow-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use grow_agent::plugins::SharedPluginRegistryHandle;
use grow_agent::plugins::discovery::DiscoveryConfig;
use grow_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _grow_guard = EnvVarGuard::set("GROW_HOME", &grow_home);
    ...
    let mut registry = InstallRegistry::empty(grow_home.join("installed-plugins"));
    ...
    // grow_home() caches the first GROW_HOME per process; a separate test
    ...
    assert_eq!(plugin.scope, grow_agent::plugins::PluginScope::CliOverride);
...
#[tokio::test]
#[ignore = "requires pre-built grow binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/GROW_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via grow_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _grow_guard = EnvVarGuard::set("GROW_HOME", &grow_home);
    ...
    // Smoke: session storage under GROW_HOME/sessions has JSON artifacts.
