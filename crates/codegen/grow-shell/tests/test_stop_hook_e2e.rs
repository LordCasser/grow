...
//! default since they need the grow binary (`GROW_BINARY` or a local debug build):
//! ```bash
//! cargo test -p grow-shell --test test_stop_hook_e2e -- --ignored
//! ```
...
async fn stop_block_loop_ends_at_continuation_cap() {
    ...
    assert_eq!(
        run.invocations(),
        grow_shell::session::MAX_STOP_HOOK_CONTINUATIONS_PER_TURN,
