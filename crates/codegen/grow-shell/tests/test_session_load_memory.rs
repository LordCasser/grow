...
//!
//! Run:
//!   cargo test -p grow-shell --features dhat-heap --test test_session_load_memory \
...
use grow_shell::session::storage::{JsonlStorageAdapter, StorageAdapter, prepare_replay_lines};
use grow_shell::session::testkit::synth::{self, SessionSpec};
use grow_test_support::env::env_parse;
    ...
    async fn count_replayed_notifications(session_id: acp::SessionId, cwd: PathBuf) -> u64 {
        ...
        local
            .run_until(async move {
                ...
                let loaded = grow_shell::session::testkit::e2e::load_session_via_agent(
