...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p grow-shell --test test_leader_soak -- --ignored --nocapture
//! ```
...
use grow_test_support::resources::ResourceSnapshot;
...
/// Per-registry entry counts. `grow/debug/agent` answers under the extension
...
async fn registry_counts(client: &mut LeaderClient, id: u64) -> serde_json::Value {
    ...
    assert!(
        counts.is_object(),
        "grow/debug/agent returned no registries: {resp}"
    );
    counts
}
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    local
        .run_until(async {
            ...
            grow_shell::leader::in_process::spawn_agent(acp_rx, response_tx.clone());
            ...
            rpc(
                &mut bootstrap,
                r#"{"jsonrpc":"2.0","id":2,"method":"authenticate","params":{"methodId":"provider.api_key","_meta":{"headless":true}}}"#.to_string(),
                2,
                "authenticate",
            )
            .await;
