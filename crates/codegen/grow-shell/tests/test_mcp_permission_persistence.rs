...
/// Shared `GROW_HOME` for the entire test binary. The `OnceLock` in
/// `grow-config` only allows one value per process, so all tests share
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::GrowPager, |handle, gw, cwd| async move {
...
async fn dont_ask_policy_denies_without_prompting() {
    use grow_workspace::permission::types::{PermissionConfig, PromptPolicy};
