...
use grow_shell::auth::{AuthManager, AuthMode, ProviderAuth, ServiceAuthConfig};
use grow_shell::heap_profile::{
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = ServiceAuthConfig::default().auth_scope();
...
fn assert_storage_auth(uploads: &[grow_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), ServiceAuthConfig::default()));
