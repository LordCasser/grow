...
Usage:
    ...
    python3 crates/codegen/grow-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/grow-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/grow-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    GROW_BINARY=/path/to/grow-pager python3 run_tests.py --fast
"""
...
def build_binary():
    """Build grow-pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
    ...
    result = subprocess.run(
        ["cargo", "build", "-p", "grow-pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
