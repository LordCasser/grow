...
//! a throwaway `GROW_HOME`, no network, and a provider command that needs no
...
use chrono::Utc;
use grow_shell::auth::{AuthMode, ProviderAuth, ServiceAuthConfig, try_ensure_fresh_auth};

const SEED_TOKEN: &str = "stale-token-that-must-be-replaced";

/// Point the process at a throwaway grow home. `grow_home()` memoizes into a
...
fn seed_expired_credential(home: &Path, scope: &str) {
    ...
    let store: BTreeMap<String, ProviderAuth> = [(scope.to_owned(), expired)].into_iter().collect();
    std::fs::write(
        home.join("auth.json"),
...
async fn auth_provider_command_mints_the_session_credential() {
    ...
    use_temp_grow_home(home.path());

    // `echo <token>` is valid in both `sh -c` and `cmd /C`, so this phase needs
    // no external binary and runs identically on every platform.
    let token = mint_with_provider(home.path(), "echo grow-ext-token").await;
    assert_eq!(token, "grow-ext-token");
