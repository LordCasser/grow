...
//!
//! ```bash
//! cargo test -p grow-shell --test test_nonblocking_startup -- --ignored
//! ```
...
use grow_test_support::leader::LeaderFixture;
...
/// The background catalog refresh must re-fetch and push `grow/models/update`
        ...
        .run_until(async {
            ...
            common::leader::run_with_cleanup(&fixture, &mut clients, |fixture, clients| {
                Box::pin(async move {
                    ...
                    // (5s-base backoff) re-fetches and pushes `grow/models/update`.
                    ...
                    assert!(
                        healed,
                        "no grow/models/update after the endpoint recovered\nstderr:\n{}",
                        clients[0].stderr_text(),
                    );
                })
            })
