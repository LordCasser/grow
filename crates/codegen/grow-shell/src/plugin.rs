...
use grow_agent::plugins::discovery::PluginScope;
use grow_agent::plugins::git_install::{self, UpdateStatus};
use grow_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use grow_plugin_marketplace::git::{self, SourceCacheLease};
use grow_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.grow/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &grow_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `grow plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `grow plugin install ./{name}`, or add a \
                     source with `grow plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `grow plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `grow plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub fn marketplace_require_sha() -> bool {
    grow_config::load_effective_config_disk_only()
        .map(|c| grow_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| grow_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &grow_workspace::permission::resolution::MarketplaceAllowlist,
...
fn plan_install(
    ...
    match qualifier {
        ...
        None => {
            ...
            let chosen_is_featured = sources[chosen_source_index].featured;
            ...
            drop(scanned);
            if !chosen_is_featured && !skipped_sources.is_empty() {
                return Err(MarketplaceInstallError::PartialScan {
                    name: name.to_string(),
                    skipped_sources,
                });
...
pub fn remove_toml_marketplace_block(content: &str, source_identity: &str) -> Option<String> {
    ...
    // Keep unrelated `[marketplace]` keys when `sources` empties; drop the
    // table only when fully empty.
...
mod tests {
    ...
    fn remove_toml_preserves_sibling_keys_when_sources_empty() {
        ...
        let new = remove_toml_marketplace_block(content, "https://a.com").unwrap();
        assert!(
            new.contains("custom = true"),
            "sibling key must survive removing the last source: {new}"
        );
        assert!(
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            ...
            "Featured Marketplace (example-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "Featured Marketplace (pin: sentry@example-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "Featured Marketplace (example-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - Featured Marketplace (example-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("grow plugin install ./sentry"), "{msg}");
        assert!(msg.contains("grow plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec![
                "Featured Marketplace (pin: sentry@example-org/plugin-marketplace)".into(),
            ],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - Featured Marketplace (pin: sentry@example-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("grow plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "Featured Marketplace (example-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("Featured Marketplace", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("example-org/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "Featured Marketplace");
            }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> grow_workspace::permission::resolution::MarketplaceAllowlist {
        grow_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = grow_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        assert_eq!(name, "Featured Marketplace");
    }
