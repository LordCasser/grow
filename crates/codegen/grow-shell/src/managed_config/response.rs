...
pub enum ManagedConfigError {
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.grow. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<grow_config::signed_policy::SignatureEnvelope>>,
    ...
    pub(super) managed_identity_signatures:
        Option<Vec<grow_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        ...
        pick_trusted_envelope(
            ...
            grow_config::signed_policy::embedded_key_id_trusted,
        )
    }
...
fn pick_trusted_envelope(
    envelopes: Option<&[grow_config::signed_policy::SignatureEnvelope]>,
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: grow_config::signed_policy::SignatureEnvelope,
    pub(super) payload: grow_config::signed_policy::SignedPayload,
}
...
mod tests {
    ...
    fn pick_trusted_envelope_prefers_trusted_then_falls_back() {
        use grow_config::signed_policy::SignatureEnvelope;
