...
fn marker_is_not_a_managed_artifact() {
    assert!(
        !MANAGED_ARTIFACT_FILES.contains(&grow_config::MANAGED_CONFIG_CACHE_FILE),
...
fn purge_keeps_marker_when_an_artifact_removal_fails() {
    ...
    std::fs::write(home.join(grow_config::MANAGED_CONFIG_CACHE_FILE), "{}").unwrap();
    ...
    remove_managed_config_files(home);
    assert!(
        home.join(grow_config::MANAGED_CONFIG_CACHE_FILE).exists(),
...
fn claim_persists_only_when_bound_to_served_principal() {
    let claim = |principal: &str| grow_config::signed_policy::ManagedIdentityClaim {
        typ: grow_config::signed_policy::MANAGED_IDENTITY_TYP.into(),
