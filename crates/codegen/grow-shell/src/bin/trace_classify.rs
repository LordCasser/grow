...
use clap::Parser;
use grow_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// API key. Overrides `$LLM_API_KEY` when set.
    #[arg(long)]
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert_eq!(cli.api_base_url, "https://provider.example/v1");
    ...
    /// Provider identity and endpoint have no compiled defaults.
    #[test]
    fn cli_defaults_match_documented_values() {
        ...
        assert!(by_id("model").is_empty());
        assert!(by_id("api_base_url").is_empty());
    ...
    fn cli_min_confidence_rejects_bad_values() {
        for bad in ["1.5", "-0.1", "nan", "inf", "not-a-float"] {
            ...
            let err = Cli::try_parse_from(required_args().into_iter().chain([arg.as_str()]))
                .expect_err(bad);
