...
//!
//! ```ignore
//! use grow_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
const CLIENT_LEADER_VERSION: &str = grow_version::VERSION;
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!({
                            ...
                            "reason": reason,
                            "killer_ver": grow_version::VERSION,
                        })),
                    );
                }
                Err(e) => {
...
/// `grow_update::auto_update::resolve_restart_exe`.
///
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let leader_log = std::env::var("GROW_LEADER_LOG")
        ...
        .unwrap_or_else(|_| "grow_shell=info,xai_acp_lib=warn,grow_mcp=warn".into());
