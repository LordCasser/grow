...
use grow_workspace::WorkspaceHandle;
...
const DEFAULT_COMPUTER_HUB_URL: &str = "";
...
async fn handle_workspace_start(
    ...
    let url_str = hub_url
        ...
        .unwrap_or_else(|| DEFAULT_COMPUTER_HUB_URL.to_string());
...
mod tests {
    ...
    async fn workspace_start_errors_when_cancelled_before_auth() {
        let state = default_test_control_state(Path::new("/tmp/grow-ws-auth-test.sock"));
        let cancel = CancellationToken::new();
        cancel.cancel();
