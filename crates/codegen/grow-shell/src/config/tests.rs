...
fn without_grow_memory<T>(f: impl FnOnce() -> T) -> T {
...
fn with_grow_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "GROW_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("GROW_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
...
#[test]
fn validate_hooks_path_accepts_grow_hooks_subdir() {
    let grow_home = crate::util::grow_home::grow_home();
...
fn simulate_release_build() -> grow_test_support::EnvGuard {
    grow_test_support::EnvGuard::set(grow_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
/// in `grow-agent`. An ABSOLUTE plugin path is used so the merged
...
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use grow_agent::plugins::{TrustStore, discover_plugins};
    use grow_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROW_HOME", home.path());
    let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
...
/// Writeback requires service.example.com auth: remote may advertise it, but a non-xai
