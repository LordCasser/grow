...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> grow_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> grow_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    fn parse_skills_config_empty() {
        ...
        assert_eq!(skills, grow_agent::prompt::skills::SkillsConfig::default());
    }
