...
fn discovery_change_for_path(path: &Path) -> Option<DiscoveryChange> {
    if path.file_name().is_some_and(|name| name == ".grow") {
...
/// recursively: `grow_home` (`~/.grow`, or `$GROW_HOME`) or a known vendor dir
...
fn is_global_config_dir(dir: &Path, grow_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, grow_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".grow", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, grow_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(grow_home) {
        return true;
    }
...
impl ProjectDiscoveryWatcher {
    pub fn start(cwd: &Path) -> Option<(Self, mpsc::UnboundedReceiver<DiscoveryChange>)> {
        ...
        let workflows = project_grow.join("workflows");
        let (tx, rx) = mpsc::unbounded_channel();
        ...
        let refresh_dirs = vec![
            (project_grow, RecursiveMode::NonRecursive),
            (
                project_root.join(".grow").join("skills"),
                RecursiveMode::Recursive,
            ),
            (
                project_root.join(".grow").join("commands"),
...
impl SkillsFileWatcher {
    ///
    /// Uses [`collect_skill_config_dirs`](grow_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let user_skill_roots = grow_agent::prompt::skills::user_skill_roots();
        ...
        let dirs_to_watch = grow_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &user_skill_roots,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &grow_home) {
        ...
        let mut refresh_dirs = vec![(grow_home.join("workflows"), RecursiveMode::NonRecursive)];
        if let Some(cwd) = cwd {
            let project_root = crate::session::workflow::registry::project_root(cwd);
            let project_grow = project_root.join(".grow");
            let parent_watch = if project_grow.is_dir() {
                project_grow.clone()
            } else {
                project_root
            };
            ...
            refresh_dirs.push((project_grow.clone(), RecursiveMode::NonRecursive));
            refresh_dirs.push((project_grow.join("workflows"), RecursiveMode::NonRecursive));
        }
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only grow_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_grow_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let grow_home = home.join(".grow");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &grow_home, Some(home));

        // grow_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&grow_home));
        ...
        assert!(!g(&home.join("repo").join(".grow")));
    }

    /// Regression for the ~/.grow inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.grow/worktrees/`) must not
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        ...
        assert_eq!(
            ...
            "first .grow creation must take the broader skills reload path"
        );
        assert_eq!(
