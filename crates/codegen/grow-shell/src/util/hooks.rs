...
use grow_config::resolve_global_hook_sources;
use grow_hooks::discovery::HookSource;
use grow_hooks::error::HookError;
...
fn include_claude_hooks(compat: &grow_tools::types::compat::CompatConfig) -> bool {
    compat.claude.hooks
        && !crate::claude_import::is_claude_import_marked_with_log("discover_hook_source_paths")
}

fn include_cursor_hooks(compat: &grow_tools::types::compat::CompatConfig) -> bool {
    compat.cursor.hooks
}
...
pub fn discover_hooks(
    ...
    compat: &grow_tools::types::compat::CompatConfig,
    trusted: bool,
) -> (grow_hooks::discovery::HookRegistry, Vec<HookError>) {
...
/// dedup in [`grow_hooks::discovery::registry_from_specs_deduped`], a config
...
pub fn assemble_hooks(
    config_layers: &[grow_config::HookConfigLayer],
    git_root: Option<&Path>,
    compat: &grow_tools::types::compat::CompatConfig,
    trusted: bool,
) -> (grow_hooks::discovery::HookRegistry, Vec<HookError>) {
    let (mut specs, mut errors) = grow_hooks::config::parse_hooks_from_config_layers(config_layers);
    ...
    let (file_specs, file_errors) =
        grow_hooks::discovery::collect_specs_from_sources(&global_sources, &project_sources);
    specs.extend(file_specs);
    errors.extend(file_errors);

    (
        grow_hooks::discovery::registry_from_specs_deduped(specs),
        errors,
    )
}
