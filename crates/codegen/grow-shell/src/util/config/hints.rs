...
/// env master `GROW_CONTEXTUAL_HINTS` (all-on/off) > user config
...
fn merge_hints_config_layers(
    ...
    if let Some(m) = managed {
        grow_config::deep_merge_toml(&mut merged, m);
    }
    if let Some(u) = user {
        grow_config::deep_merge_toml(&mut merged, u);
    }
    if let Some(r) = requirements {
        grow_config::deep_merge_toml(&mut merged, r);
    }
    merged
}
