...
/// Process-wide write lock for `~/.grow/config.toml`.
///
...
"#;
        ...
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("grow-4".to_string());
        let mut table = root.as_table().unwrap().clone();
    ...
    fn models_config_serializes_only_some_fields() {
        ...
        if let TomlValue::Table(t) = v {
            ...
            assert_eq!(t.get("default").and_then(|x| x.as_str()), Some("grow-3"));
        } else {
    ...
    fn merge_section_models_only_updates_set_fields_preserves_others() {
        ...
        models.insert(
            "custom_model_setting".into(),
            TomlValue::String("keep-me".into()),
        );
        ...
        let cfg = crate::agent::config::ModelsConfig {
            default: Some("grow-new".to_string()),
            ..Default::default()
        };
        merge_section(&mut table, "models", &cfg);
        let m = table.get("models").unwrap().as_table().unwrap();
        assert_eq!(m.get("default").and_then(|v| v.as_str()), Some("grow-new"));
        assert_eq!(
            m.get("custom_model_setting").and_then(|v| v.as_str()),
            Some("keep-me")
        );
        assert_eq!(
    ...
    fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
        ...
        let root: TomlValue = toml::from_str(original).unwrap();
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("grow-persisted".to_string());
        ...
        assert_eq!(reloaded.models.default.as_deref(), Some("grow-persisted"));
        let cfg2 = crate::agent::config::Config::new_from_toml_cfg(&reloaded_root)
            .expect("new_from_toml_cfg");
        assert_eq!(cfg2.models.default.as_deref(), Some("grow-persisted"));
    }
    #[test]
    ...
    mod resolve_auto_compact {
        ...
        use std::sync::Mutex;
        const TEST_MODEL: &str = "grow-4.5";
        const OTHER_MODEL: &str = "grow-4.3";
        /// Serialize tests that mutate `GROW_AUTO_COMPACT_THRESHOLD_PERCENT`.
    ...
    fn settings_helpers_target_correct_ui_fields() {
        ...
        let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("growday".to_string()));
        assert_eq!(cfg.ui.auto_light_theme, Some("growday".to_string()));
