...
/// fetches from an explicitly configured service backend are allowed, including the deployment-config sync
...
pub fn resolve_remote_fetch_enabled() -> bool {
    match crate::config::ConfigLayers::load() {
        ...
        // Fail closed when configuration cannot be loaded: Grow has no built-in
        // remote service that is safe to contact implicitly.
