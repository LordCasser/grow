...
/// [`grow_config::env_bool`] parser (`GROW_TOOLS_FIND_BFS` /
/// `GROW_TOOLS_GREP_UGREP`, plus the `GROW_FIND_BFS` / `GROW_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.grow/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`grow_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = grow_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = grow_config::env_bool(primary).or_else(|| grow_config::env_bool(alias));
...
) -> Option<grow_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<grow_tools::util::ShellEnvironmentPolicy>() {
...
/// the shared [`grow_config::env_bool`] via `BoolFlag`). The secs env var
...
///
/// Precedence: requirements > env (`GROW_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            grow_tools::implementations::grow_build::ask_user_question::RESPONSE_TIMEOUT.as_secs(),
        )
}
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        grow_tools::implementations::grow_build::ask_user_question::response_timeout_env_secs(),
...
) -> grow_tools::implementations::grow_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use grow_tools::implementations::grow_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
...
mod tests {
    ...
    // Assumes GROW_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
...
mod shell_env_policy_tests {
    use super::*;
    use grow_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
