...
///
/// This is a lightweight duplicate of `grow_update::channel_name()` for
/// use in `grow-shell` which cannot depend on `grow-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        let current = semver::Version::parse(grow_version::VERSION).ok()?;
