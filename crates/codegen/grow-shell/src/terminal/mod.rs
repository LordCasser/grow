...
/// Resolved absolute path to bash. On Unix uses the `grow_config` shell
/// resolution cascade (`$GROW_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        grow_config::shell::unix_shell_path(grow_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `grow-tools`.
pub use grow_tools::util::pager_env;
...
/// tool output. Used when the client sets `grow/bashOutputNoColor: true` (e.g.
/// grow-pager which renders its own UI and doesn't need raw ANSI codes).
///
