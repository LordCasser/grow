...
pub fn extract_builtin_files(grow_home: &std::path::Path) {
    ...
    let marker = grow_home.join(".metadata_version");
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(grow_home.join(stale));
    }

    for &(filename, content) in BUILTIN_FILES {
        if let Err(e) = std::fs::write(grow_home.join(filename), content) {
...
mod tests {
    ...
    fn same_version_does_not_restore_missing_or_delete_legacy_skills() {
        ...
        std::fs::write(home.join(".metadata_version"), grow_version::VERSION).unwrap();
