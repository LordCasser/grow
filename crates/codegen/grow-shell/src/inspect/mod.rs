...
use grow_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[auth]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `GROW_DISABLE_API_KEY_AUTH`).
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = grow_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = grow_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = grow_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        grow_version: grow_version::VERSION.to_string(),
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = grow_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        grow_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `grow_agent::prompt::agents_md::read_agents_config_with_paths`
...
/// (`[auth]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let auth = config.map(|c| c.auth.clone()).unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: auth.api_key_auth_disabled(),
        disable_api_key_auth: auth.disable_api_key_auth,
        force_login_team_uuid: auth.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[grow_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = grow_tools::types::compat::CompatConfig::default();
    ...
    let config_layers = grow_config::hook_config_layers();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            let source = match grow_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(grow_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(grow_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(grow_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &grow_agent::plugins::PluginRegistry,
    skills_config: &grow_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = grow_agent::prompt::skills::list_skills_with_plugins(
...
fn skill_entry_source(s: &grow_agent::prompt::skills::SkillInfo) -> ConfigSource {
...
fn list_agents(
    ...
    let agents = grow_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[grow_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                grow_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                grow_agent::plugins::PluginScope::Project => Scope::Project,
                grow_agent::plugins::PluginScope::User => Scope::User,
                grow_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: grow_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    grow_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = grow_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.grow_version, r.channel);
...
mod tests {
    use super::*;
    use grow_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use grow_tools::implementations::skills::types::SkillScope;
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = grow_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
