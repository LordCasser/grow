...
/// Env lane (desktop `GROW_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`GROW_CHAT_MODE`); hard-off in release builds.
...
/// Parse `grow/session/list` params and, under process-wide chat mode, force
...
pub struct ListReq {
    ...
    /// not a git repo. Relaxed responses set `_meta["grow/listScope"]`.
...
/// `as_str` value (`grow/listScope`), so no serde derive is needed.
...
pub fn force_kind_chat(req: &mut ListReq) {
    ...
    let mut filters = match meta.remove("grow/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
...
mod tests {
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["grow/session"]["kind"], "build");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!({
            "grow/facetFilters": { "kind": ["build"], "starred": true },
            "grow/query": "antelope",
            "grow/limit": 5,
        });
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "grow/facetFilters": { "kind": ["build"], "starred": [true], "workspace": ["w1"] },
                "grow/query": "antelope",
                "grow/limit": 5,
            })),
            ..ListReq::default()
    ...
    fn xai_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        ...
        am.hot_swap(crate::auth::ProviderAuth {
            auth_mode: crate::auth::AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::TEST_OAUTH2_ISSUER.to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        let addr = spawn_conversations_stub(
            ...
            .await;
        let _env = grow_test_support::EnvGuard::set(
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = grow_test_support::EnvGuard::unset("GROW_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = grow_test_support::EnvGuard::set("GROW_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = grow_test_support::EnvGuard::set("GROW_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::GROW_CHAT_MODE_ENV;
        let _chat_off = grow_test_support::EnvGuard::unset(GROW_CHAT_MODE_ENV);
        let _desktop_off = grow_test_support::EnvGuard::unset("GROW_SESSION_LIST_CONVERSATIONS");
        assert!(
            !conversations_lane_active(),
            "no env ⇒ lane off (Build-mode default)"
        );
        {
            let _desktop = grow_test_support::EnvGuard::set("GROW_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = grow_test_support::EnvGuard::set(GROW_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!({
            "_meta": { "grow/facetFilters": { "kind": ["build"], "starred": [true] } },
        })
        .to_string();
        {
            let _off = grow_test_support::EnvGuard::unset(GROW_CHAT_MODE_ENV);
        ...
        {
            let _on = grow_test_support::EnvGuard::set(GROW_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
    ...
    /// Wire pin for the cross-crate `grow/partial` envelope the pager parses:
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = grow_workspace::session::git::normalize_repo_url(this_repo).unwrap();
    ...
    /// Send-side wire pin: `grow/listScope` present iff the scope relaxed.
    #[test]
    fn ext_list_response_serializes_scope() {
        ...
        assert_eq!(with["_meta"]["grow/listScope"], serde_json::json!("repo"));
        let without =
            serde_json::to_value(ext_list_response(result(ListScope::Cwd))).expect("serialize");
        assert!(without["_meta"].get("grow/listScope").is_none());
    }
}
