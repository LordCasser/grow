...
use bytes::Bytes;
use grow_tools::util::image_compress::{FilterType, ReEncodeParams, re_encode_under_limit};
use std::borrow::Cow;
...
/// `grow_tools::reminders` (free-fn shape because this module has no
...
pub(crate) fn persisted_image_reject_reason(bytes: &[u8]) -> Option<String> {
    use grow_tools::util::image_validate as iv;
...
pub(crate) fn inline_attach_verdict(data_b64: &str) -> InlineAttachVerdict {
    ...
    let Ok((w, h, _)) = grow_tools::util::image_validate::validate_image_bytes_with(&raw, false)
    else {
        return InlineAttachVerdict::Unreadable;
    };
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        grow_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !grow_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = grow_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
...
mod tests {
    ...
    fn re_encode_fallback_notice_picks_tag_per_harness() {
        ...
        let grow = render_re_encode_fallback_notice(&notes, false);
    ...
    fn image_dropped_notice_picks_tag_per_harness() {
        ...
        let grow = render_image_dropped_notice(&notes, false);
        assert!(grow.contains("<system-reminder>"));
        assert!(grow.contains("</system-reminder>"));
        assert!(!grow.contains("<system_reminder>"));
