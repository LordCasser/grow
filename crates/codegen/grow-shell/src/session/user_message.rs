...
// Re-export from grow-chat-state — canonical definition lives there.
pub use grow_chat_state::compaction_utils::extract_user_query;
    ...
    match result {
        Ok(Ok(status)) => {
            ...
            let status = if vcs_kind.is_jj() {
                ...
                grow_agent::prompt::user_message::normalize_git_status(&status)
            };
...
// Tests for extract_user_query now live in grow_chat_state::compaction_utils.

#[cfg(test)]
mod tests {
    use super::*;
    use grow_workspace::file_system::FsError;
    ...
    async fn construct_user_message_returns_without_git_status_on_bad_dir() {
        let dir = std::path::Path::new("/tmp/nonexistent-grow-test-dir");
