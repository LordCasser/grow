...
pub(crate) struct RoleToolNames {
    ...
    /// (the default grow-build host's `search_replace` mutator) when `Write`
...
impl RoleToolNames {
    ...
    /// `search_replace` on the default grow-build host) instead of the literal
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default grow-build host's pre-spawn describe probe exposes only
...
pub(crate) mod tests {
    ...
    #[test]
    fn from_summary_uses_grow_build_names() {
        // A grow-build toolset: client names match the literal defaults.
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_grow_build_host() {
        // Default grow-build host: the pre-spawn describe probe exposes only
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default grow-build parent bridge: no `Write`, only `Edit`
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_grow_build_mutator() {
        ...
        // default grow-build host (Edit-only toolset), so a fail-open retry
