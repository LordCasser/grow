...
//! both matched by the agent ([`grow_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| grow_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value))
...
impl SessionActor {
    ...
    fn client_gate_responses<'a>(
        ...
        let default_timeout =
            if envelope.hook_event_name.traits().gate == grow_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                grow_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> grow_hooks::dispatcher::StopDispatchResult {
        use grow_hooks::result::HookRunResult;

        let mut out = grow_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed)) in ordered {
            ...
            let detail = grow_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                grow_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    grow_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
