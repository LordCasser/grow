...
//!
//! Core worktree lifecycle logic lives in [`grow_workspace::worktree`].
...
/// manual `create_from_worktree_sync` path used by `grow -w --ref`).
...
async fn cleanup_worktree_on_failure(source_cwd: &str, worktree_path: &str) {
    ...
    if is_jj {
        ...
        if let Ok(root) = find_git_root_from_path(std::path::Path::new(source_cwd)) {
            let _ = grow_workspace::session::git::git_cli(&root, &["worktree", "prune"]).await;
        }
    }
}
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> grow_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return grow_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    grow_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<grow_workspace::session::git::RestoreDegree>,
}
/// Thin wire-format adapter over the shared
/// [`grow_workspace::session::git::build_restore_decision`] helper.
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &grow_workspace::session::git::CheckoutSessionOutcome,
    kind: grow_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = grow_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(grow_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(grow_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(grow_workspace::session::git::RestoreDegree::HeadOnly)
        );
    ...
    async fn checkout_persisted_head_stashes_dirty_worktree_state() {
        ...
        assert!(
            list_out.contains("grow: pre-restore-code sess-dirty-wt"),
