...
use grow_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
use tokio::sync::mpsc;
...
/// Dedup key for `grow/git_head_changed`, shared by the watcher's `GitHead`
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> grow_workspace::file_system::FileIndexDelta {
    use grow_workspace::file_system::FileIndexDelta;
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `grow/gitHeadChanged`; opt-in (absent => off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `grow/fs_notify` wire protocol;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = grow_workspace::session::git::get_branch(&self.cwd).await;
        let commit = grow_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = grow_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = grow_workspace::session::git::GitHeadChanged {
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root = grow_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
