use grow_diagnostics::events::SessionHarness;
use grow_workspace::permission::Decision;
...
pub(crate) fn skill_source_label(skill_path: &str, cwd: &str) -> &'static str {
    ...
    let user = crate::util::grow_home::grow_home()
        .parent()
...
pub(crate) fn format_hook_name(spec: &grow_hooks::config::HookSpec) -> String {
...
/// Provenance for diagnostics, mapped from the shared [`hook_origin`] classifier so
/// this and `/hooks` inspect can't diverge.
fn format_hook_source(spec: &grow_hooks::config::HookSpec) -> &'static str {
    use grow_hooks::config::HookOrigin as O;
    match grow_hooks::config::hook_origin(spec) {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &grow_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: grow_diagnostics::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: grow_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: grow_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names =
            grow_agent::prompt::agents_md::read_agents_config_with_paths(&self.cwd, self.compat)
                .await
                .iter()
                .filter_map(|f| {
                ...
                .collect();
        let skill_names = grow_agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: grow_diagnostics::context::collect_git_context(&self.cwd).is_git_repo,
