...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(grow_version::VERSION.to_string());
    s
}
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if let Some(user_meta) = crate::agent::mvp_agent::parse_json_object_env("GROW_USER_METADATA") {
    ...
    if telemetry_enabled {
        ...
        grow_telemetry::session_ctx::log_event(grow_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `GROW_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `GROW_TELEMETRY_ENABLED`. These are analytics data that
...
pub struct FeedbackManager {
    ...
    upload_queue_stats: std::sync::OnceLock<Arc<crate::save::UploadQueueStats>>,
}
...
impl FeedbackManager {
    ...
    pub fn set_upload_queue_stats(&self, stats: Arc<crate::save::UploadQueueStats>) {
    ...
    /// `grow/debug/trigger_feedback` ACP extension method to exercise
    ...
    async fn sync_signals_inner(&self, force: bool) -> anyhow::Result<()> {
        ...
        if let Some(stats) = self.upload_queue_stats.get() {
            self.signals_handle.snapshot_gcs_queue(stats.as_ref());
        }
    ...
    pub async fn shutdown(&self, queue: Option<&crate::save::UploadQueue>) {
...
mod tests {
    ...
    async fn test_shutdown_with_upload_queue_drains() {
        ...
        use crate::save::{UploadQueue, TraceExportSource, UploadRetryPolicy};
        use std::sync::Arc;
    ...
    async fn test_is_auth_permanently_failed_reads_auth_manager() {
        ...
        use crate::auth::{AuthManager, ProviderAuth, ServiceAuthConfig};
        use std::sync::Arc;
...
mod author_identity_tests {
    ...
    fn text_submission() -> FeedbackSubmission {
        ...
        s.model_id = Some("grow-4".to_string());
        s
    }
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            grow_test_support::env::EnvGuard::set("GROW_TEST_WORK_EMAIL", "ada@corp.example");
        let _name = grow_test_support::env::EnvGuard::set("GROW_TEST_WORK_NAME", "Ada Lovelace");

        // The loader expands `$VAR` at load, exactly as a trusted config tier ships it.
        let mut value = toml::from_str::<toml::Value>(
            r#"
[feedback.user]
name = ["$GROW_TEST_WORK_NAME"]
email = ["$GROW_TEST_WORK_EMAIL"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(persisted.model_id.as_deref(), Some("grow-4"));
    }

    /// `GROW_USER_METADATA` is merged into the submission and travels with it:
    ...
    async fn workflow_merges_user_metadata_into_submission() {
        let _guard = grow_test_support::env::EnvGuard::set(
            "GROW_USER_METADATA",
