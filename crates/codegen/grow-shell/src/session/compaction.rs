...
impl SessionActor {
    ...
    async fn run_compact_inner(
        ...
        self.dispatch_hook(
            grow_hooks::event::HookEventName::PreCompact,
            grow_hooks::event::HookPayload::PreCompact {
        ...
        let fr_config = grow_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match grow_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(grow_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(grow_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(grow_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                grow_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                grow_tools::computer::types::TaskKind::Bash => {
        ...
        let memory_ref: Option<&dyn grow_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn grow_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        grow_sampling_types::ContentPart::Text { text } => {
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &grow_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<grow_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    use grow_workspace::file_system::MockFs;
    use grow_workspace::permission::PermissionHandle;
    ...
    fn api_error_with_context_window(context_window: u64) -> grow_sampler::SamplingErrorInfo {
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = grow_sampler::SamplingErrorInfo {
                    kind: grow_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
