...
//! `grow_sampler::SamplerActor` against a `MockInferenceServer`, scripting
...
/// `grow-sampler/src/stream/messages.rs` mapping).
...
/// against grow-sampler's `ChatCompletionChunk` deserialization and
/// `stream/chat_completions.rs` field mapping (see also
/// `grow-test-support/src/sse.rs` `chat_completion_script_from_deltas`):
...
async fn actor_with_sampler(
    ...
    api_backend: grow_sampling_types::ApiBackend,
) -> (
    ...
    let sampler_config = grow_sampler::SamplerConfig {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        mpsc::unbounded_channel::<grow_sampler::SamplingEvent>();
    actor.sampler_handle = grow_sampler::SamplerActor::spawn(
        sampler_config,
...
fn truncation_tool_use_incomplete() {
    run_with_session_stack(|| {
        ...
        rt.block_on(local.run_until(async {
            ...
            let assistant_items: Vec<&grow_sampling_types::AssistantItem> = conversation
                .iter()
                .filter_map(|item| match item {
...
fn context_window_exceeded_triggers_compaction() {
    run_with_session_stack(|| {
        ...
        rt.block_on(local.run_until(async {
            ...
            // grow-compaction MIN_SUMMARY_SEED_CHARS) or the retry loop
