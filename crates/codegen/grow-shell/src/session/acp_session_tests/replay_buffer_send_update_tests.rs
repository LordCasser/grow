...
async fn channel_tokens_accumulate_into_streaming_capture() {
    use grow_sampler::{RequestId, SamplingChannel, SamplingEvent};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
async fn completed_event_clears_slot_keeps_prior_uncommitted_segments() {
    use grow_sampler::{InferenceLatencyStats, RequestId, SamplingChannel, SamplingEvent};
    use grow_sampling_types::{ConversationItem, ConversationResponse};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let fixture = make_replay_send_update_fixture().await;
...
async fn observe_only_confident_completion_stays_warn_only() {
    ...
    local
        .run_until(async {
            ...
            fixture.actor.doom_loop_recovery = Some(grow_sampling_types::DoomLoopRecoveryPolicy {
                max_threshold: 8,
                max_retries: 0,
            });
            let actor = Arc::new(fixture.actor);
            *actor
            ...
            actor
                ...
                .await;
            let response = grow_sampling_types::ConversationResponse {
                items: vec![grow_sampling_types::ConversationItem::assistant(
                    "answer kept as-is",
                )],
                stop_reason: None,
                usage: None,
                cost_usd_ticks: None,
                message_chunks_emitted: 1,
                doom_loop_signals: vec![grow_sampling_types::doom_loop::DoomLoopSignal::parse(
...
async fn doom_loop_recovery_stamps_capture_segments_and_counters() {
    use grow_sampler::{RequestId, SamplingChannel, SamplingErrorKind, SamplingEvent};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let mut fixture = make_replay_send_update_fixture().await;
            fixture.actor.doom_loop_recovery =
                Some(grow_sampling_types::DoomLoopRecoveryPolicy::default());
            let actor = Arc::new(fixture.actor);
            *actor
                .current_prompt_id
                .lock()
...
async fn tool_call_delta_on_idle_slot_leaves_phase_pending() {
    use grow_sampler::{RequestId, SamplingEvent};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let fixture = make_replay_send_update_fixture().await;
...
async fn reasoning_only_doomloop_turn_captures_every_generation_as_segments() {
    ...
    use grow_sampling_types::{EmptyReason, EmptyResponseContext};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let ReplaySendUpdateFixture {
