...
fn persist_ack_waits_for_disk_flush_before_success() {
    std::thread::Builder::new()
        .name("persist_ack_waits_for_disk_flush_before_success".into())
        ...
        .spawn(|| {
            ...
            local
                .run_until(async {
                    ...
                    let sampling_client = crate::sampling::Client::new(grow_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            agent
                .tool_bridge()
                .update_resource(
                    grow_tools::implementations::grow_build::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
...
/// user's next queued prompt) and rebroadcasts `grow/queue/changed`. The
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![grow_sampling_types::ToolCall {
                    id: "call-1".into(),
    ...
    local
        .run_until(async {
            ...
            let cfg = grow_sampler::SamplerConfig {
                ...
                temperature: None,
                top_p: None,
                api_backend: grow_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                query_params: Default::default(),
                env_http_headers: Default::default(),
            ...
            let sampler_handle = grow_sampler::SamplerActor::spawn(
                cfg,
                grow_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<xai_acp_lib::AcpClientMessage>();
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(grow_sampling_types::UserItem {
                    content: vec![grow_sampling_types::ContentPart::Text { text: "hi".into() }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use grow_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    grow_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
