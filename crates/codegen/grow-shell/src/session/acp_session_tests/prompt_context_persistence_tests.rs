...
fn test_json_round_trip() {
    let ctx = grow_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: grow_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `grow_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = grow_agent::PromptContext::default();
    ...
    let loaded: grow_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a Grow subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: grow_agent::PromptContext = serde_json::from_str(
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<grow_agent::PromptContext, _> = serde_json::from_str(&json);
...
fn fake_prompt_path() -> std::path::PathBuf {
    std::path::PathBuf::from("/tmp/grow-test-home/sessions/cwd/sid/prompts/prompt_0.txt")
}
...
//
// `grow_home()` is a process-wide `OnceLock`, so the real async method is
