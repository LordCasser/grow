...
//!
//! These tests verify the parallel dispatch path (GROW_PARALLEL_TOOL_DISPATCH):
...
#[test]
fn lock_path_for_args_matches_grow_build_target_file() {
    // grow_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use grow_sampling_types::{ConversationItem, SyntheticReason};
