...
use super::support::*;
use super::*;
/// Build an actor whose toolset parses grow `search_replace` plus the plan
...
async fn build_gate_actor() -> SessionActor {
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
        ...
        ToolConfig::from_id("Grow:read_file"),
...
async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conv.iter()
        ...
        .find_map(|item| match item {
            grow_sampling_types::ConversationItem::ToolResult(tr) if tr.tool_call_id == call_id => {
