...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> grow_agent::Agent {
    use grow_tools::implementations::grow_build::update_goal::UpdateGoalTool;
...
#[cfg(test)]
pub(crate) async fn test_grow_build_agent_with_todo() -> grow_agent::Agent {
    use grow_tools::implementations::grow_build::todo::TodoWriteTool;
    use grow_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<grow_tools::registry::types::ToolConfig>,
) -> grow_agent::Agent {
    test_agent_from_config(
        grow_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        grow_agent::AgentDefinition::default_grow_build(),
        std::sync::Arc::new(grow_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: grow_agent::prompt::user_message::UserMessageTemplate,
) -> grow_agent::Agent {
...
async fn test_agent_from_config(
    ...
    definition: grow_agent::AgentDefinition,
    backend: std::sync::Arc<dyn grow_tools::computer::types::TerminalBackend>,
) -> grow_agent::Agent {
    use grow_tools::computer::local::LocalFs;
    use grow_tools::computer::types::AsyncFileSystem;
...
pub(crate) async fn create_test_actor_ex(
    ...
    let cwd = grow_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(grow_workspace::file_system::MockFs::new(cwd.to_path_buf()));
    ...
    tool_context.task_completion_reservations =
        Some(grow_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(grow_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: grow_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
        ...
        vcs_kind: grow_workspace::session::git::VcsKind::Git,
