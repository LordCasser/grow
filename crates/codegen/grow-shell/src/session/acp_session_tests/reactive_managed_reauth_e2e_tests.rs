...
//! `grow/mcp/server_status` pushes that clients consuming only `server_status`
...
//! handshake against rmcp 2.1 in `grow-mcp/tests/repro_sse_flood.rs`): a
...
use chrono::Utc;
use grow_mcp::servers::{ClientStateKind, HttpConfig, McpClient};
use std::collections::HashMap;
use std::sync::Arc;
use std::sync::atomic::{AtomicBool, AtomicUsize, Ordering};

const MANAGED: &str = "grow_managed_testconnector";
...
/// Drain all `grow/mcp/server_status` pushes currently queued on `gw_rx`.
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use grow_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `grow_managed_*` tools enter entry-B.
