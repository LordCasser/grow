...
/// shape, so it also covers the laxer grow ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use grow_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use grow_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // grow backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = grow_sampling_types::build_messages_request(&req);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = grow_sampling_types::ApiBackend::Responses;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = grow_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use grow_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use grow_test_support::MockInferenceServer;
