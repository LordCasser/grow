use super::support::*;
use super::*;
use grow_tools::reminders::task_completion::consumed_completion_ids;
use grow_tools::types::output::{BashOutput, TextOutput, ToolOutput};
use xai_tool_types::{
...
async fn cancel_barrier_rejects_task_completion_wake_without_reporting_it() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                res.get::<grow_tools::types::resources::State<
                    grow_tools::reminders::task_completion::ReportedTaskCompletions,
                >>()
                .is_none(),
                "declined admission must not report before user re-engagement"
            );
            drop(res);
            let reminder = grow_tools::reminders::TaskCompletionReminder;
            let reminders = grow_tools::types::tool::Reminder::collect_reminders(
                &reminder,
                resources,
        ...
        .run_until(async {
            ...
            assert!(
                resources
                    .lock()
                    .await
                    .get::<grow_tools::types::resources::State<
                        grow_tools::reminders::task_completion::ReportedTaskCompletions,
                    >>()
                    .is_none(),
...
async fn genuine_user_start_consumes_deferred_completions_without_notification_turn() {
    ...
    local
        .run_until(async {
            ...
            let body = grow_tools::reminders::task_completion::format_monitor_completion(
                &grow_tools::types::TaskSnapshot {
                    ...
                    signal: None,
                    completed: true,
                    kind: grow_tools::computer::types::TaskKind::Monitor,
                    block_waited: false,
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use grow_tools::reminders::task_completion::ReportedTaskCompletions;
    use grow_tools::types::resources::State;
...
struct OneTaskTerminal {
    tasks: Vec<grow_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl grow_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: grow_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        grow_tools::computer::types::TerminalRunResult,
        grow_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<grow_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> grow_tools::computer::types::KillOutcome {
        grow_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<grow_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<grow_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> grow_tools::computer::types::TaskSnapshot {
    grow_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
