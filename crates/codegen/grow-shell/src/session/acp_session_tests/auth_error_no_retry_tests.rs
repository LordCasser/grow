...
/// attribution.
fn auth_error() -> grow_sampler::SamplingErrorInfo {
    grow_sampler::SamplingErrorInfo {
        kind: grow_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: grow_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        grow_chat_state::AuthType::SessionToken => "cached_token",
        grow_chat_state::AuthType::ApiKey => "provider.api_key",
    };
...
/// `ApiKey` (session-token cache miss + `GROW_API_KEY`).
...
/// `GROW_API_KEY`) must NOT attempt an OIDC session-token refresh. The
...
fn model_not_found_error() -> grow_sampler::SamplingErrorInfo {
    grow_sampler::SamplingErrorInfo {
            kind: grow_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model grow-build does not exist or your team does not have access".into(),
...
fn unauthorized_401_error() -> grow_sampler::SamplingErrorInfo {
    grow_sampler::SamplingErrorInfo {
            kind: grow_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://provider.oauth/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_grow_token_auth=grow-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
/// Negative: a genuine `provider.api_key` method keeps its configured key on the
...
/// End-to-end for the frozen-gate bug: a session born on `provider.api_key` (gate
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = grow_sampler::SamplerConfig {
...
/// never ride a later request to `api.example.com`. Mirrors the forward direction in
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = grow_sampler::SamplingErrorKind::Api;
