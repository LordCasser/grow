...
fn install_client_hook(
    actor: &SessionActor,
    event: grow_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(&actor, grow_hooks::event::HookEventName::Stop, &["cb_0"]);

            actor.fire_hook(
                grow_hooks::event::HookEventName::Stop,
                None,
                grow_hooks::event::HookPayload::Stop {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_with_tools(vec![grow_tools::registry::types::ToolConfig::for_tool::<
                    grow_tools::implementations::use_tool::UseTool,
                >()])
                .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        grow_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
fn file_registry_with_stop_spec(
    event: grow_hooks::event::HookEventName,
    script: &str,
) -> grow_hooks::discovery::HookRegistry {
    let (mut registry, _) = grow_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![grow_hooks::config::HookSpec {
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                grow_hooks::event::HookEventName::SubagentEnd,
                None,
                grow_hooks::event::HookPayload::SubagentStop {
                    phase: grow_hooks::event::SubagentStopPhase::Observe,
