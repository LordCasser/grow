...
//! `grow_shell_session_support::managed_mcp` and is re-exported here so
...
pub fn merge_managed_mcp_servers_with_policy(
    ...
    let allowlist = &grow_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
impl McpDisabledReason {
    ...
    pub fn for_blocked_server(
        policy: &grow_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &grow_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: grow_tools::util::grow_home::grow_home().join("config.toml"),
    };
...
/// Auto-create `grow_managed_*` entries for managed configs not already in `merged`.
...
mod tests {
    ...
    fn auto_inject_creates_server_for_unmatched_managed_config() {
        ...
        let slack = merged
            .iter()
            .find(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "grow_managed_slack"));
        let slack = slack.expect("should have auto-injected grow_managed_slack");
        match slack {
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use grow_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
    ...
    fn merge_drops_server_denied_by_name_including_managed_prefix() {
        ...
        let tagged = apply_mcp_server_policy(
            vec![
                acp::McpServer::Http(
                    acp::McpServerHttp::new("grow_managed_slack", "https://mcp.slack.com/sse")
                        .headers(vec![]),
                ),
        ...
        let slack = tagged
            .iter()
            .find(|s| mcp_server_name(&s.server) == "grow_managed_slack")
    ...
    fn auto_inject_dedup_by_display_name_first_scope_wins() {
        ...
        let linear_count = merged
            .iter()
            .filter(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "grow_managed_linear"))
            .count();
    ...
    fn auto_inject_skips_existing_server() {
        ...
        let slack_count = merged
            .iter()
            .filter(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "grow_managed_slack"))
            .count();
    ...
    fn auto_inject_skips_disabled() {
        ...
        let disabled: std::collections::HashSet<String> =
            ["grow_managed_slack".to_string()].into_iter().collect();
        let mut merged = vec![];
        auto_inject_managed_servers_with_disabled(&mut merged, &managed, &disabled);

        let has_slack = merged
            .iter()
            .any(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "grow_managed_slack"));
        let has_linear = merged
            .iter()
            .any(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "grow_managed_linear"));
...
"#,
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        ...
        use grow_agent::plugins::manifest::{PathOrInline, PluginManifest};
