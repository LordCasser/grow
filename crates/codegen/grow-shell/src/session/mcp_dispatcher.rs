...
//!
//! Receives [`grow_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`grow_mcp::liveness`]),
//! - the [`grow_mcp::servers::GrowClientHandler`] (server-pushed
...
pub enum McpServerStatus {
    /// Client is in [`grow_mcp::servers::ClientStateKind::Ready`]
...
/// [`grow_mcp::servers::McpClientEventKind`]). Used as the
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        use grow_mcp::servers::{McpClient, McpState};
