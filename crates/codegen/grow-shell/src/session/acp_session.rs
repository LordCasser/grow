...
use grow_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use grow_agent::prompt::skills::SkillsConfig;
use grow_sampler::SamplerConfig as SamplingConfig;
use grow_sampling_types::truncate_bytes;
...
use grow_workspace::file_system::CodebaseIndexManager;
use grow_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use grow_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
pub(crate) struct SessionActor {
    ...
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<grow_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) agent: std::cell::RefCell<grow_agent::Agent>,
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<grow_tools::implementations::grow_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, grow_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    /// Safe: session actor is single-threaded (LocalSet), no concurrent access.
    pub(crate) hook_registry: std::cell::RefCell<Option<Arc<grow_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) vcs_kind: grow_workspace::session::git::VcsKind,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<grow_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<grow_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `grow-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: grow_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`grow_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`grow_tools::bridge::ToolBridge`], tool
...
impl SessionActor {
    ...
    fn build_command_availability(
        ...
        use grow_tools::implementations::memory::{MEMORY_GET_TOOL_NAME, MEMORY_SEARCH_TOOL_NAME};
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names
                .iter()
                .any(|n| n == grow_tools::implementations::grow_build::SCHEDULER_CREATE_TOOL_NAME),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == grow_tools::implementations::grow_build::workflow::WORKFLOW_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<grow_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &grow_agent::PromptContext) {
...
#[expect(dead_code, reason = "API for future viewers/debug tools")]
pub(crate) fn load_prompt_context(session_info: &SessionInfo) -> Option<grow_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
    load_prompt_context_from_dir(&dir)
}
fn load_prompt_context_from_dir(
    session_dir: &std::path::Path,
) -> Option<grow_agent::PromptContext> {
...
mod tool_meta_stamp_tests {
    ...
    use grow_tools::tool_taxonomy::TOOL_META_KEY;
    use grow_workspace::permission::PermissionCommand;
    use tokio::sync::mpsc;
...
mod managed_gateway_tool_tests {
    ...
    use grow_tools::types::output::{MCPOutput, ToolOutput};
    use grow_tools::types::tool::{ToolKind, ToolNamespace};
    use grow_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<grow_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
