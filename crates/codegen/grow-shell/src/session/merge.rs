...
pub(crate) fn merge(local: Vec<Summary>, query: Option<&str>, limit: usize) -> Vec<MergedSession> {
    let query = query.map(str::to_lowercase);
    let mut sessions: Vec<MergedSession> = local
        .into_iter()
        .filter(|summary| {
            query.as_ref().is_none_or(|query| {
                summary.session_summary.to_lowercase().contains(query)
...
fn summary_to_session(summary: Summary) -> MergedSession {
    ...
    MergedSession {
        ...
        summary: summary.display_title().to_owned(),
        first_prompt: None,
...
fn effective_sort_time(session: &MergedSession) -> Option<chrono::DateTime<chrono::FixedOffset>> {
    session
        .last_active_at
        .as_deref()
...
mod tests {
    ...
    #[test]
    fn merge_filters_and_keeps_only_newest_empty_session_per_cwd() {
        let sessions = merge(
            vec![
