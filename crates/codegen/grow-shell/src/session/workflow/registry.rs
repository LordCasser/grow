...
pub(crate) fn project_root(session_cwd: &Path) -> PathBuf {
    grow_workspace::session::git::find_git_root_from_path(session_cwd)
...
impl WorkflowRegistry {
    pub(crate) fn scan(session_cwd: Option<&Path>) -> Self {
        ...
        {
            dirs.push((project_root(cwd).join(".grow").join("workflows"), "project"));
        }
...
pub(crate) fn save_project_workflow(
    ...
    let dir = canonical_root.join(".grow").join("workflows");
