...
impl RelocationJournal {
    ...
    pub(super) fn validate(&self, grow_home: &Path) -> Result<()> {
        ...
        if session_dir_at(grow_home, &self.source_cwd, &self.session_id)
            == session_dir_at(grow_home, &self.target_cwd, &self.session_id)
        {
            return Err(RelocationError::Inconsistent(
...
pub(super) fn acquire(grow_home: &Path, session_id: &str) -> Result<RelocationLease> {
    validate_component("session id", session_id)?;
    let dir = relocation_dir(grow_home);
...
pub(super) fn read(grow_home: &Path, session_id: &str) -> Result<RelocationJournal> {
    validate_component("session id", session_id)?;
    let path = journal_path(grow_home, session_id);
...
pub(super) fn write(
    ...
    let path = journal_path(grow_home, &journal.session_id);
...
pub(super) fn relocation_dir(grow_home: &Path) -> PathBuf {
    grow_home.join(RELOCATION_DIR)
}

pub(super) fn journal_path(grow_home: &Path, session_id: &str) -> PathBuf {
    relocation_dir(grow_home).join(format!("{session_id}.json"))
}

pub(super) fn session_dir_at(grow_home: &Path, cwd: &str, session_id: &str) -> PathBuf {
    grow_home
        .join("sessions")
...
pub(super) fn sync_namespace(grow_home: &Path) -> Result<()> {
    let dir = relocation_dir(grow_home);
