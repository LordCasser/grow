...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use grow_paths::RelPathBuf;
    use grow_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use grow_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
fn fork_filter_removes_synthetic_user_messages() {
    use grow_sampling_types::conversation::*;
    let mut items = vec![
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    grow_sandbox::set_configured_profile("workspace");
    let expected = grow_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_turn_with_reasoning_between_user_and_assistant() {
    ...
    let mut items = vec![
            ConversationItem::system("sys"),
            ConversationItem::user("q"),
            ConversationItem::Reasoning(grow_sampling_types::synthesized_reasoning_item(
                "thinking",
            )),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
            ...
            ConversationItem::Reasoning(grow_sampling_types::synthesized_reasoning_item("mid")),
...
/// path end-to-end (loader + serde + grow_sampling_types::
...
/// the legacy grow-build / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"grow-build"}"#,
        ],
    );
    assert_eq!(
            items.len(),
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let grow_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"grow-build"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"grow-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    let grow_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"grow-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"grow-build"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
