...
pub(super) fn is_file_backed_exit_plan_kind(
    kind: Option<grow_tools::types::tool::ToolKind>,
) -> bool {
...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// Grow-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as grow edits). Deleting
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        grow_tools::normalization::merge_tool_meta(
            &toolset,
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(grow_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self.hook_event_active(grow_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            grow_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            grow_hooks::event::HookEventName::PostToolUseFailure,
                            grow_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    grow_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    grow_hooks::event::HookEventName::PostToolUse,
                    grow_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(grow_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                grow_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                grow_hooks::event::HookEventName::PreToolUse,
                None,
                grow_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    grow_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx).await;
                ...
                .await;
                if let grow_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let edit_path_context = matches!(&access_kind, AccessKind::Edit(_)).then(|| {
                grow_workspace::permission::types::EditPathContext {
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        grow_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        grow_hooks::event::HookEventName::PermissionDenied,
                        grow_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
                ...
                (
                    ...
                    vec![
                        acp::ToolCallLocation::new(read_file.path)
                            ...
                            .line(
                                grow_tools::normalization::norm_offset_i64(read_file.offset)
                                    .map(|l| l as u32),
                            ),
                    ],
                    Vec::new(),
    ...
    /// `grow_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            grow_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                grow_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let grow_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            grow_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            grow_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`grow_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: grow_sampler::SamplingEvent,
    ) {
...
fn execute_tool_call_parts(
    ...
    let display = grow_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod exit_plan_tail_predicate_tests {
    ...
    fn exit_plan_kind_is_file_backed_exit() {
        ...
        assert!(is_file_backed_exit_plan_input(&ToolInput::ExitPlanMode(
            grow_tools::implementations::grow_build::exit_plan_mode::ExitPlanModeInput {}
        )));
    }
...
mod plan_mode_edit_gate_tests {
    ...
    /// Grow edit tools are plan-file-only while plan mode is active — the
    ...
    fn grow_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "grow tools get no markdown exception — plan file only"
        );
    }
...
mod wait_interrupt_tests {
    ...
    use grow_tools::types::output::ToolOutput;
