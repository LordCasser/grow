...
fn partition_rules_by_scope(
    files: Vec<grow_agent::prompt::agents_md::AgentConfigFile>,
    grow_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<grow_agent::prompt::user_message::RuleEntry>,
    Vec<grow_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = grow_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use grow_agent::prompt::agents_md::AgentConfigFile;
    use std::path::Path;
    ...
    fn paths(entries: &[grow_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(grow_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    pub(super) async fn build_user_message_prefix(&self) -> String {
        ...
        use grow_agent::prompt::user_message::UserMessageTemplate;
        let template = self
            .agent
            .borrow()
            .definition()
    ...
    async fn build_templated_user_message(
        ...
        use grow_agent::prompt::agents_md::read_agents_config_with_paths;
        use grow_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use grow_workspace::file_system::{git_status_short, jj_status};
        use grow_workspace::session::git::VcsKind;
    ...
    ) -> Vec<grow_agent::prompt::user_message::McpServerEntry> {
        use grow_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &grow_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
