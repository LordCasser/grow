...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<grow_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<grow_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
                ...
                event = chat_state_event_rx.recv() => {
                    match event {
                        Some(grow_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
                        ...
                        Some(grow_chat_state::ChatStateEvent::ImageBudget {
                            body_bytes,
                            trigger_bytes,
                        ...
                        Some(grow_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
                        Some(grow_chat_state::ChatStateEvent::TokensUpdated { .. }) => {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    let Some(cmd) = maybe_cmd else {
                        ...
                        let envelope = session.fire_hook(
                            grow_hooks::event::HookEventName::SessionEnd,
                            None,
                            grow_hooks::event::HookPayload::SessionEnd {
                        ...
                        if let Some(registry) = session.hook_registry.borrow().clone() {
                            ...
                            let results = grow_hooks::dispatcher::dispatch_non_blocking(
                                &registry,
                                grow_hooks::event::HookEventName::SessionEnd,
                                &envelope,
                                &ctx,
                    ...
                    match cmd {
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        grow_tools::implementations::grow_build::monitor::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = grow_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            {
                                let _ = tx.send(
                                    grow_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                grow_hooks::event::HookEventName::SessionStart,
                                None,
                                grow_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
