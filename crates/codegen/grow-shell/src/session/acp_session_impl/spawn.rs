...
fn drop_cli_catchall_allows(
    rules: Vec<grow_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<grow_workspace::permission::types::PermissionRule>,
    Vec<grow_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if grow_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use grow_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use grow_workspace::permission::rules::parse_permission_rule;
    use grow_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    hook_registry_override: Option<std::sync::Arc<grow_hooks::discovery::HookRegistry>>,
    workspace_ops: grow_workspace::WorkspaceOps,
    cli_permission_rules: Vec<grow_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn grow_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        grow_tools::implementations::grow_build::scheduler::types::SchedulerHandle,
    >,
    ...
    grow_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(grow_agent::AgentBuildError::InvalidConfig(
    ...
    let chat_state_sampling_config = grow_sampling_types::SamplingConfig {
    ...
    let resolve_search_shadows = || {
        ...
        grow_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn grow_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn grow_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn grow_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(grow_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(grow_tools::computer::local::LocalFs)
        };
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn grow_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        ...
        let backend: std::sync::Arc<dyn grow_tools::types::memory_backend::MemoryBackend> =
    ...
    let resolved_task_output =
        grow_tools::reminders::task_completion::resolve_task_output_tool_name(agent.tool_bridge())
            .await;
    let resolved_read =
        grow_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge()).await;
    ...
    let scheduler_handle_for_handle = {
        ...
        res.get::<grow_tools::implementations::grow_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let sampler_retry_policy = grow_sampler::RetryPolicy {
    ...
    let sampler_handle = grow_sampler::SamplerActor::spawn(
    ...
    let built_hook_registry: Option<Arc<grow_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = grow_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        grow_tools::implementations::grow_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use grow_tools::implementations::grow_build::workflow::WorkflowLaunchAck;
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: grow_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match grow_workspace::session::git::discover_git_root(root) {
                grow_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    grow_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => grow_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(grow_tools::types::tool_index::ToolIndex(
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            grow_tools::implementations::grow_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                grow_tools::implementations::grow_build::update_goal::GoalUpdateHandle(
    ...
    }
    {
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use grow_tools::implementations::grow_build::ask_user_question::AskUserQuestionMode;
...
> {
    let (init_tx, init_rx) =
        tokio::sync::oneshot::channel::<Result<SessionInitResult, grow_agent::AgentBuildError>>();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx.send(Err(grow_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    grow_sampling_types::conversation::ContentPart::Text { text }
