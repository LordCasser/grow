...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &grow_tools::implementations::grow_build::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
fn role_tool_names_from(
    ...
    use grow_tools::implementations::grow_build::task::types::SubagentDescribeOutcome;
...
impl SessionActor {
    async fn evaluate_goal_round(
        ...
        let plan = match plan_file {
            Some(path) => tokio::fs::read_to_string(path)
                .await
                .ok()
                .map(|text| grow_tools::util::truncate_str(&text, 16 * 1024).to_owned()),
            None => None,
        };
            ...
            let (client, model) = if requested_model == active_model {
                ...
                match self.resolve_aux_sampler_config(requested_model).await {
                    Some(mut config) => {
                        ...
                        match grow_sampler::SamplingClient::new(config) {
    ...
    fn record_goal_round_progress(&self, detail: &str, failed: bool) {
        let detail = grow_tools::util::truncate_str(detail.trim(), 500).to_owned();
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        if grow_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use grow_sampling_types::conversation::SyntheticReason;
...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<grow_tools::implementations::grow_build::update_goal::UpdateGoalEnvelope>,
    ) {
        use grow_tools::implementations::grow_build::update_goal::{RejectReason, UpdateGoalAck};
        if !self.goal_harness_enabled() {
    ...
    async fn apply_classifier_outcome_legacy(
        ...
        outcome: crate::session::goal_classifier::GoalClassifierOutcome,
        notify: &crate::session::goal_orchestrator::GoalNotifySender,
    ) -> grow_tools::implementations::grow_build::update_goal::UpdateGoalAck {
        use crate::session::goal_classifier::GoalClassifierOutcome;
        use crate::session::goal_tracker::GoalClassifierVerdict;
        use grow_tools::implementations::grow_build::update_goal::UpdateGoalAck;
