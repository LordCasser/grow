...
/// `list_dir`=`target_directory`. Grammar lives in `grow_chat_state`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<grow_chat_state::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    grow_chat_state::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command =
            grow_tools::util::strip_redundant_session_cd(&command, self.tool_context.cwd.as_path());
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(grow_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from grow-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, grow-shell would sanitize
