...
/// `description` instead.
fn stop_entry_from_task(task: &grow_tools::types::TaskSnapshot) -> StopBackgroundTask {
    let command_text =
...
fn stop_entry_from_subagent(
    summary: &grow_tools::implementations::grow_build::task::types::ActiveSubagentSummary,
) -> StopBackgroundTask {
    StopBackgroundTask {
...
fn stop_cron_from_scheduled(
    task: &grow_tools::implementations::grow_build::scheduler::types::ScheduledTask,
) -> StopSessionCron {
    StopSessionCron {
        id: task.id.clone(),
        schedule: grow_tools::implementations::grow_build::scheduler::interval::interval_to_human(
            task.interval_secs,
        ),
...
pub(super) fn demote_ignored_blocks(
    ...
    use grow_hooks::result::HookRunResult;
    results
        .into_iter()
...
impl SessionActor {
    ...
    pub(crate) async fn list_active_subagents(
        &self,
    ) -> Vec<grow_tools::implementations::grow_build::task::types::ActiveSubagentSummary> {
...
mod stop_gate_snapshot_tests {
    ...
    fn task_snapshot(
        kind: grow_tools::computer::types::TaskKind,
    ...
    fn task_snapshot_maps_to_stop_entry() {
        let shell =
            stop_entry_from_task(&task_snapshot(grow_tools::computer::types::TaskKind::Bash));
    ...
    fn subagent_summary_maps_to_stop_entry() {
        let summary = grow_tools::implementations::grow_build::task::types::ActiveSubagentSummary {
    ...
    fn scheduled_task_maps_to_stop_cron() {
        let task = grow_tools::implementations::grow_build::scheduler::types::ScheduledTask::new(
            300,
