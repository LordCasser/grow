...
impl SessionActor {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<grow_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, grow_workspace::permission::ClassifierFailure>,
            >,
        )>();
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                        ...
                        None => {
                            let client =
                                session.prepare_chat_completion(false).await.map_err(|e| {
                                    grow_workspace::permission::ClassifierFailure::TransportError(
                                        e.to_string(),
                                    )
                                })?;
                            let model = session
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            grow_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            grow_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| grow_workspace::permission::ClassifierFailure::Timeout)?
                        .map_err(|e| {
                            grow_workspace::permission::ClassifierFailure::TransportError(
                                e.to_string(),
                            )
        ...
        let clf =
            grow_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(grow_sampler::SamplingClient, String)> {
        ...
        let client = grow_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
    ...
    pub(super) async fn prepare_chat_completion(
        ...
        let sampling_client =
            grow_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: grow_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use grow_sampler::SamplingErrorKind;
        if self.tool_context.task_output_token_budget.is_some() {
    ...
    pub(crate) async fn run_turn_via_sampler(
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = grow_sampler::SamplingErrorInfo::from(&rich_err);
