...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            grow_workspace::session::git::get_worktree_info(cwd),
            grow_workspace::session::git::get_branch(cwd),
        );
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<grow_tools::implementations::grow_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn stop_failure_error_type(err: &acp::Error) -> grow_hooks::event::StopFailureKind {
        use grow_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &grow_agent::plugins::install_registry::InstallError,
    ) -> String {
