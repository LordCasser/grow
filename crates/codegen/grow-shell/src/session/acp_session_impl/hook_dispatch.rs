...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> grow_hooks::runner::RunContext<'_> {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[grow_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, grow_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: grow_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = GrowSessionUpdate::TaskCompleted {
            task_snapshot: grow_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
