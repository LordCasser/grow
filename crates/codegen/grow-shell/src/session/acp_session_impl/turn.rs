...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &grow_tools::implementations::grow_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = grow_tools::util::base64_images::extract_base64_images(query);
        ...
        self.tool_bridge_handle()
            .update_resource(grow_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            grow_hooks::event::HookEventName::UserPromptSubmit,
            grow_hooks::event::HookPayload::UserPromptSubmit {
            ...
            Ok(TurnOutcome::Completed { refusal, .. }) => {
                ...
                if let Some(explanation) = refusal {
                    ...
                    self.dispatch_hook(
                        ...
                        grow_hooks::event::HookPayload::StopFailure {
                            error: grow_hooks::event::StopFailureKind::InvalidRequest,
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = grow_tools::implementations::grow_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = grow_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = grow_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use grow_tools::types::memory_backend::MemoryBackend as _;
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<grow_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn is_run_true_step(
        &self,
        tool_calls: &[grow_sampling_types::conversation::ToolCall],
    ) -> bool {
