...
//! [`grow_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`grow_agent::AgentBuilder::new`]. Both initial session spawn
...
//!
//! ## WHEN ADDING A NEW [`grow_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use grow_agent::error::AgentBuildError;
use grow_agent::prompt::context::PromptAudience;
use grow_agent::prompt::skills::SkillsConfig;
use grow_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub managed_gateway_tool_client: Option<grow_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use grow_tools::implementations::grow_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(grow_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(grow_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(grow_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use grow_tools::implementations::grow_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            grow_tools::computer::local::LocalTerminalBackend::new_local(
                grow_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: grow_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(grow_tools::types::tool::ToolKind::Task)
