...
use grow_workspace::file_system::{FileReference, render_embedded_resource, render_file_reference};
...
{}
    ...
    if !resource_links.is_empty() {
        ...
        context.push_str(&render_resource_links_grow(resource_links));
    }
    (context, query)
}
...
/// Grow-format resource links: `<focused_files>` / `<open_files>` with
/// metadata inside a `<system-reminder>` wrapper.
fn render_resource_links_grow(resource_links: &[acp::ResourceLink]) -> String {
...
mod tests {
    ...
    #[test]
    fn test_grow_render_plain_message() {
        let result = render_grow("hello", vec![], vec![], &[], false);
        assert_eq!(result, "<user_query>\nhello\n</user_query>");
    }
    #[test]
    fn test_grow_render_with_attachments_uses_system_reminder_wrapper() {
        let result = render_grow(
            "check this",
        ...
        assert!(
            ...
            "Grow should start with <user_query>, got: {result}"
        );
    }
    #[test]
    fn test_grow_render_user_query_first() {
        let link = acp::ResourceLink::new("doc.md", "file:///doc.md")
            .title(Some("My Doc".into()))
            .size(Some(1024));
        let result = render_grow("hello", vec![], vec![], &[link], false);
        let uq_pos = result.find("<user_query>").unwrap();
        let rr_pos = result.find("Referenced resources:").unwrap();
        assert!(
            uq_pos < rr_pos,
            "Grow: <user_query> ({uq_pos}) should come before resource links ({rr_pos})\ngot: {result}"
        );
        assert!(result.contains("<system-reminder>"));
    }
    #[test]
    fn test_grow_render_resource_links_use_focused_files_format() {
        let links = vec![
        ...
        let result = render_grow("hello", vec![], vec![], &links, false);
