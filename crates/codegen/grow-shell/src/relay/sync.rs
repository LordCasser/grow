...
/// The web base URL must be configured explicitly with `GROW_CODE_WEB_URL`.
pub fn build_share_url(session_id: &str) -> String {
    let base_url = std::env::var("GROW_CODE_WEB_URL").unwrap_or_default();
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://service.example.com/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://service.example.com/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
