...
//! grow-tools. This struct is NOT used for tool execution — it's session infrastructure.
//!
...
use grow_workspace::file_system::{AsyncFileSystem, AsyncFsWrapper};
use grow_workspace::session::file_state::FileStateHandle;
...
pub(crate) fn subagent_foreground_wait(
    state: Arc<BlockingWaitState>,
) -> grow_tools::implementations::grow_build::task::types::SubagentForegroundWait {
    grow_tools::implementations::grow_build::task::types::SubagentForegroundWait::new(move || {
...
pub struct ToolContext {
    ...
    pub lsp: Option<Arc<dyn grow_tools::implementations::lsp::LspBackend>>,
    ...
    pub monitor_event_buffer:
        Option<grow_tools::implementations::grow_build::monitor::types::MonitorEventBuffer>,
...
impl ToolContext {
    ...
    pub fn new(
        ...
        let session_env = grow_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
