//! Notification bridge: translates `grow-tools` `ToolNotification` events
//! into `grow-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use grow_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use grow_tools::types::output::{BashOutput, ToolOutput};
use grow_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    /// next-turn exit reminder. Grow-build leaves this `false` — its
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: grow_tools::notification::ScheduledTaskRemoved,
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor = task_snapshot.kind == grow_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    grow_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    grow_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, grow_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, grow_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use grow_tools::computer::types::TaskKind;
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(grow_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            grow_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification =
            ToolNotification::BashOutputChunk(grow_tools::notification::types::BashOutputChunk {
                base: grow_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_removed_is_persisted() {
        let (config, _gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
        let removed = grow_tools::notification::ScheduledTaskRemoved {
            task_id: "loop-1".into(),
            generation: "generation-a".into(),
            revision: 2,
        };
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            grow_tools::notification::types::BashExecutionBackgrounded {
                base: grow_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            grow_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        let notification =
            ToolNotification::PlanModeExited(grow_tools::notification::types::PlanModeExited {
    ...
    /// Default (grow) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
        ...
        let notification =
            ToolNotification::PlanModeExited(grow_tools::notification::types::PlanModeExited {
                tool_call_id: "tc-exit-grow".into(),
                plan_content: Some("- step 1".into()),
                plan_file_path: "/tmp/test-session/plan.md".into(),
            });
        let mut offsets = HashMap::new();
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        let (config, mut gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
        let notification =
            ToolNotification::PlanModeEntered(grow_tools::notification::types::PlanModeEntered {
