...
//! | Method | Description |
//! |--------|-------------|
//! | `grow/code/goto-definition` | Definition location(s) for symbol at position |
//! | `grow/code/goto-references` | Reference location(s) for symbol at position |
//! | `grow/code/find-definitions` | All definitions of a symbol by name |
//! | `grow/code/find-references` | All references to a symbol by name |
//! | `grow/code/status` | Indexing status |
...
/// Reason string for the `grow/code/status` response.
///
...
pub async fn handle(
    ...
    use grow_workspace::workspace_ops::*;

    match args.method.as_ref() {
        "grow/code/goto-definition" => {
...
fn to_code_nav_ext_response(resp: grow_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for grow-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise grow/codeNavigation.enabled to use code navigation"
        }
