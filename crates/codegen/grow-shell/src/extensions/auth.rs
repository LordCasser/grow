//! `grow/auth/*` and legacy `grow/{get,set}ApiKey` extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "grow/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "grow/getApiKey" => handle_get_api_key(),
        "grow/setApiKey" => handle_set_api_key(args),
        "grow/auth/submit_code" => handle_submit_code(agent, args),
        "grow/auth/get_url" => handle_get_url(agent).await,
        "grow/auth/cancel" => handle_cancel(agent, args),
        "grow/auth/logout" => handle_logout(agent, args).await,
        "grow/auth/info" => handle_info(agent),
        "grow/auth/check_subscription" => handle_check_subscription(agent).await,
...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_provider_api_key_env().ok();
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&grow_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("GROW_API_KEY") };
        } else {
            crate::auth::store_api_key(&grow_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::set_var("GROW_API_KEY", k) };
        }
    } else {
        crate::auth::clear_api_key(&grow_home)
            .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `grow-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a grow-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("grow-asset:///{key}")),
        None => None,
    };
