...
// upload::trace and upload::gcs removed — xai_file_utils::{gcs, queue} are gone
...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Upload share metadata to cloud storage removed — xai_file_utils is gone

    let response = ShareSessionResponse { share_url };
    to_raw_response(&response)
}

// upload_share_data_to_gcs removed — xai_file_utils::gcs is gone

fn require_xai_auth_for_share(
    auth_manager: &crate::auth::AuthManager,
) -> Result<crate::auth::ProviderAuth, acp::Error> {
    super::auth_gate::require_xai_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `grow login` to authenticate.",
    )
}
...
mod tests {
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        // Only OIDC tokens against https://login.example.com (or the local-dev equivalent)
        // return true from is_service_auth(). This is required for the share tests to
    ...
    #[test]
    fn share_rejects_third_party_auth_with_actionable_grow_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            ServiceAuthConfig::default(),
        ...
        let third_party = ProviderAuth {
            auth_mode: AuthMode::ApiKey,
            key: "xai-test-key".into(),
            create_time: Utc::now(),
            ..Default::default()
        };
        mgr.hot_swap(third_party);
