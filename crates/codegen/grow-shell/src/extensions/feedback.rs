//! `grow/feedback`, `grow/feedback/dismiss`, `grow/btw`, and `grow/review/*`
//! extension handlers.
//!
...
// WithAuth and upload_bytes removed — xai_file_utils::gcs is gone
...
/// Handle `grow/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set GROW_FEEDBACK_ENABLED=true or \
...
///
/// Methods:
/// - `grow/review/comment`: record a new inline code comment to cloud storage
/// - `grow/review/comment/delete`: record a tombstone event for a deleted comment
    ...
    match args.method.as_ref() {
        "grow/review/comment" => {
                ...
                .await
            {
                ...
                tracing::debug!(comment_id = %comment_id, "Comment GCS upload skipped (gcs module removed)");
            }
        ...
        "grow/review/comment/delete" => {
            ...
            if let Some(_gcs_config) = agent
                .build_gcs_config(format!("{}/comments", request.session_id))
                .await
            {
                // GCS upload removed — xai_file_utils::gcs is gone
                tracing::debug!("Comment delete GCS upload skipped (gcs module removed)");
            }
