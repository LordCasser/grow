...
// rmcp is quarantined in grow-mcp; see that crate's docs.
use grow_mcp::rmcp;
...
/// `grow_mcp::wire`), these methods are private to the agent↔client channel and
...
//
// Implements the `McpResourceProvider` trait from grow-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `grow-mcp` directly.
...
impl grow_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<grow_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(grow_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<grow_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(grow_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(grow_tools::types::resources::McpResourceContent::Text(text)),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                uri: content_uri,
                mime_type,
                ...
                description: None,
                mime_type,
                content: Some(grow_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        append_disabled_like_handle_list(
            &mut servers,
            &["grow_managed_slack", "grow_mcp_linear"],
            true,
        );
        assert!(
            servers.is_empty(),
