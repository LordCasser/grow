...
use grow_tools::types::{KillOutcome, TaskSnapshot};
...
/// Wire DTO for the `grow/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (grow-pager) build
...
/// Wire DTO for the `grow/task/kill` ext response payload (nested under
...
/// Wire DTO for the `grow/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (grow-pager) build
...
/// Handle `grow/scheduler/*` extension methods.
...
/// Handle `grow/subagent/*` extension methods.
...
mod tests {
    ...
    // ── grow/subagent/cancel outcome wire DTO ──────────────────────────
