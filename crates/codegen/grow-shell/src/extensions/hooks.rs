...
use grow_hooks::event::{HookEventEnvelope, HookEventName};
use grow_hooks::matcher::HookMatcher;
...
pub fn hook_spec_to_info(spec: &grow_hooks::config::HookSpec) -> HookInfo {
    use grow_hooks::event::HookEventName;
    ...
    HookInfo {
        ...
        disabled: grow_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
/// for both the `grow/hooks/run` request (gate) and the `grow/hooks/event` notification
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[grow_hooks::event::HookEventName] = &[
    grow_hooks::event::HookEventName::PreToolUse,
    grow_hooks::event::HookEventName::Stop,
    grow_hooks::event::HookEventName::SubagentStop,
];
...
pub(crate) fn parse_client_hooks(meta: Option<&acp::Meta>) -> ClientHooks {
    ...
    for (event_name, value) in map {
        ...
        let Ok(event) = HookEventName::deserialize(de) else {
            tracing::warn!(event = %event_name, "ignoring unknown grow/hooks event");
            continue;
        };
        let Some(array) = value.as_array() else {
            tracing::warn!(event = %event_name, "grow/hooks event value is not an array; skipping");
            continue;
        };
...
/// clear) when the request meta carries `grow/hooks`, else `None` so a reconnect that
...
pub(crate) fn reconnect_client_hooks(meta: Option<&acp::Meta>) -> Option<ClientHooks> {
    meta.and_then(|m| m.get("grow/hooks"))
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let group = WireGroup::deserialize(value)
        .inspect_err(|err| tracing::warn!(%event, %err, "ignoring malformed grow/hooks group"))
        .ok()?;
    if group.hook_callback_ids.is_empty() {
        tracing::warn!(%event, "ignoring grow/hooks group with no hookCallbackIds");
        return None;
    }
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern) if event.traits().matcher == grow_hooks::event::MatcherPolicy::Ignored => {
        ...
        Some(pattern) => match HookMatcher::new(pattern) {
            ...
            Err(err) => {
                tracing::warn!(%event, pattern, %err, "ignoring grow/hooks group with invalid matcher");
                return None;
            }
        },
    };
...
mod tests {
    ...
    fn reconnect_client_hooks_only_when_key_present() {
        ...
        let cleared = reconnect_client_hooks(serde_json::json!({ "grow/hooks": {} }).as_object());
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use grow_hooks::event::{HookEventEnvelope, HookPayload};
