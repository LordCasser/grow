//! `grow/marketplace/*` extension handlers.
//!
//! Provides marketplace browsing and install endpoints for the pager modal.
//! Delegates to `grow-plugin-marketplace` crate for scanning and install logic.
...
fn load_filtered_marketplace_sources() -> Vec<grow_plugin_marketplace::MarketplaceSource> {
    crate::plugin::load_filtered_marketplace_sources()
}

pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "grow/marketplace/list" => handle_list().await,
        "grow/marketplace/action" => handle_action(agent, args).await,
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                grow_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                grow_plugin_marketplace::SourceKind::Local { path } => path.to_str().unwrap_or("?"),
            };
            format!("{}={}", s.name, url)
        })
        .collect();
...
fn refresh_sources(
    sources: &[grow_plugin_marketplace::MarketplaceSource],
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                grow_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
                grow_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let grow_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = grow_plugin_marketplace::git::default_cache_root();
            if let Err(e) = grow_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
...
async fn handle_update(
    ...
    use grow_plugin_marketplace::installer;
    use xai_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &grow_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            grow_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            grow_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    ...
    let plugin_path =
        match grow_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        grow_plugin_marketplace::SourceKind::Local { path } => {
            marketplace_lease = None;
            path.clone()
        }
        grow_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = grow_plugin_marketplace::git::default_cache_root();
            match grow_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                grow_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = grow_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = grow_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = grow_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        ...
        Err(grow_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            grow_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match grow_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    grow_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &grow_plugin_marketplace::MarketplaceSource,
    ...
    let scan = grow_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = grow_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match grow_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: grow_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.grow/config.toml`.
async fn handle_add_source(url: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &grow_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            grow_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: grow plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
    ...
    let grow_home = grow_config::grow_home();
    let _save_guard = crate::util::config::lock_config_writes().await;
    let write = {
        let name = name.clone();
        tokio::task::spawn_blocking(move || {
            let _flock = acquire_init_lock(&grow_home).ok();
            add_marketplace_source(&config_path, &name, &input)
        })
        .await
    };
    match write {
...
/// Append a `[[marketplace.sources]]` entry. Idempotent on normalized git URL
/// or local path and preserves comments.
...
/// Remove a marketplace source from `~/.grow/config.toml` and uninstall all
...
/// flock for the whole read-modify-write.
fn remove_source_locked(source_url_or_path: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    if let Some(removed) = plugin::remove_toml_marketplace_block(&content, source_url_or_path) {
        if let Err(e) = crate::util::config::atomic_write_string(&config_path, &removed) {
            return ActionOutcome {
                status: OutcomeStatus::InternalError,
                message: format!("Failed to write config: {e}"),
                requires_reload: false,
...
/// Acquire an advisory exclusive `flock` on `<grow_home>/.config-init.lock`,
...
fn acquire_init_lock(grow_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(grow_home);
    let lock_path = grow_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a grow_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub fn purge_default_skills_installs(grow_home: &std::path::Path) {
    purge_default_skills_installs_impl(grow_home, || {
        grow_agent::plugins::install_registry::InstallRegistry::try_load_from(
            grow_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}
...
fn purge_default_skills_installs_impl(
    ...
    load_registry: impl FnOnce() -> Result<
        grow_agent::plugins::install_registry::InstallRegistry,
        grow_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = grow_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(grow_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %grow_home.join(".config-init.lock").display(),
...
mod default_skills_purge_tests {
    use super::*;
    use grow_agent::plugins::install_registry::{
    ...
    fn purged_flag_toml_preserves_other_content() {
        ...
        assert!(out.contains("custom = true"), "{out}");
        assert!(
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = grow_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
