//! Jujutsu extension handlers — delegates to [`grow_workspace::session::jj`].

use agent_client_protocol as acp;

use super::{Empty, ExtResult, to_ext_response, to_ext_response_partial};
use grow_workspace::session::git::{CommitData, StageData};
use grow_workspace::session::jj;

/// Handle a `grow/git/*` method for a jj-colocated repo.
///
...
pub async fn try_handle(
    ...
    match method {
        "grow/git/status" => Some(to_ext_response(jj::status(git_root).await)),
        "grow/git/info" => Some(to_ext_response(jj::info(git_root).await)),
        // git HEAD points at `@-` in a colocated repo; route to jj so we report
        // the working-copy commit (`@`), consistent with `status`/`info`.
        "grow/git/current_commit" => Some(to_ext_response(jj::current_commit(git_root).await)),
        "grow/git/branches" => Some(to_ext_response(jj::list_bookmarks(git_root).await)),

        // jj has no staging area — stage/unstage are no-ops
        "grow/git/stage" => Some(to_ext_response(Ok(StageData { paths: Vec::new() }))),
        "grow/git/stage/content" | "grow/git/unstage" => Some(to_ext_response(Ok(Empty {}))),
        ...
        "grow/git/checkout" => Some(Err(acp::Error::invalid_params()
            .data("checkout is not supported in jj repos; use `jj new` or `jj edit`"))),
        "grow/git/stash" => Some(Err(acp::Error::invalid_params()
