//! Data APIs for `grow models`. Clients own display.
...
/// Status for the `grow models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `service_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.auth.service_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_provider_api_key(
            agent_config.auth.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `GROW_AUTH_PATH` (not `GROW_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 6]) {
        ...
        let guards = [
            EnvGuard::unset(GROW_API_KEY_ENV_VAR),
            EnvGuard::unset("GROW_AUTH"),
            EnvGuard::set("GROW_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("GROW_DEPLOYMENT_KEY"),
            EnvGuard::unset("GROW_WS_ORIGIN"),
            EnvGuard::unset("GROW_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(GROW_API_KEY_ENV_VAR, "provider-test-key");
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("GROW_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("service.example.com".to_owned())
        );
    }
