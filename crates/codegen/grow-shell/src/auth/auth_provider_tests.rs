...
async fn provider_refresh_sets_expired_env() {
    let provider = AuthProviderRef::new(
        ...
        AuthProviderConfig {
            command: "printf 'tok-%s' \"${GROW_AUTH_EXPIRED:-0}\"".to_owned(),
            args: None,
            token_ttl_secs: Some(3600),
            timeout_secs: None,
            cwd: None,
    ...
    assert_eq!(
        ...
        Some("tok-0"),
        "first mint runs without GROW_AUTH_EXPIRED"
    );
    test_expire_provider_token("test-expired-env");
    assert_eq!(
        provider.ensure_fresh_token(None).await.rotated().as_deref(),
        Some("tok-1"),
        "re-mints run with GROW_AUTH_EXPIRED=1"
    );
}
...
async fn provider_expiry_source_precedence() {
    ...
    fn jwt_with_exp(exp: i64) -> String {
        ...
        let claims = enc.encode(serde_json::json!({ "exp": exp }).to_string());
...
/// via `GROW_AUTH_PROVIDER_*`, so a refresh-grant command can refresh instead of
...
async fn re_mint_hands_the_prior_token_back_to_the_command() {
    let provider = AuthProviderRef::new(
        ...
        AuthProviderConfig {
            command: "printf 'seen-%s' \"${GROW_AUTH_PROVIDER_ACCESS_TOKEN:-none}\"".to_owned(),
            args: None,
            token_ttl_secs: Some(3600),
            timeout_secs: None,
            cwd: None,
