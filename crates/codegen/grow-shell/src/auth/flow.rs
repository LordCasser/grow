...
fn is_cached_credential_compatible(credential: &ProviderAuth, config: &ServiceAuthConfig) -> bool {
    ...
    let expected_issuer = config
        ...
        .or_else(|| config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    let issuer_compatible = match (credential.oidc_issuer.as_deref(), expected_issuer) {
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(config) {
        let actual = crate::auth::oidc::peek_access_token_principal_id(&credential.key);
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("GROW_LOGIN_DEVICE_FLOW")
...
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("GROW_LOGIN_DEVICE_FLOW");
...
/// How login presents itself; surfaced to the TUI via `grow/auth/get_url`.
...
impl AuthUrlMode {
    /// Wire string for the `grow/auth/get_url` ACP response.
...
async fn run_external_auth_provider(
...
) -> anyhow::Result<(ProviderAuth, bool)> {
    ...
    cmd.envs(grow_tools::util::pager_env());
    ...
    let principal_policy = crate::auth::oidc::login_principal_policy(auth_manager.service_config());
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = auth.oidc.is_some(),
        has_oauth2 = auth.oauth2.is_some(),
        has_external_auth = auth.auth_provider_command.is_some(),
        reauth,
    ...
    if let Some(ref cmd) = auth.auth_provider_command {
    ...
    // `grow login --devbox` uses run_devbox_login and is not gated here.
    if !auth.blocks_automatic_oidc() && crate::auth::devbox_login::is_devbox_environment() {
    ...
    if crate::auth::oidc::is_configured(auth) {
        return crate::auth::oidc::run_login_flow(auth, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = auth.oauth2 {
...
pub async fn try_ensure_fresh_auth(auth: &ServiceAuthConfig) -> Option<ProviderAuth> {
    try_ensure_fresh_auth_with(&build_startup_auth_manager(auth)).await
}
...
async fn try_ensure_fresh_auth_with(auth_manager: &Arc<AuthManager>) -> Option<ProviderAuth> {
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_noninteractive_auth_no_mint(
    ...
    try_noninteractive_auth_no_mint_with(&build_startup_auth_manager(auth)).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<ProviderAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_service_auth() && a.refresh_token.is_some())
}
...
pub(crate) async fn mint_session_noninteractive(
    ...
    let auth = auth_manager.service_config();
    // preferred_method=api_key: never auto-mint OIDC (fail-closed).
    if auth.blocks_automatic_oidc() {
        tracing::debug!(
    ...
    if let Some(cmd) = auth.auth_provider_command.as_deref() {
...
fn report_signed_in(auth: &ProviderAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated(
    ...
    ensure_authenticated_with_override(auth, reauth, message_prefix, LoginTransportOverride::None)
        .await
}
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(auth).await)
    } else {
        ensure_authenticated(auth, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `grow login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `GROW_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.auth, login_override).await {
        if config.auth.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!(
                "Sign-in is not available for this deployment. Set GROW_API_KEY instead."
            );
        }
        let grow_home = grow_home::grow_home();
        let auth_manager = Arc::new(AuthManager::new(&grow_home, config.auth.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.auth) {
            eprintln!(
    ...
    // config was actually applied; `grow setup` reports the no-config case.
...
fn configured_oauth_provider(
...
) -> anyhow::Result<(String, super::AuthProviderConfig)> {
    let resolve = |name: &str| {
        config
            .auth_providers
            .get(name)
            .or_else(|| config.auth_providers.get(&format!("model_provider:{name}")))
            .filter(|provider| provider.is_oauth())
            .cloned()
    };
    if let Some(name) = requested {
        return resolve(name)
            .map(|provider| (name.to_owned(), provider))
            .ok_or_else(|| anyhow::anyhow!("OAuth provider '{name}' is not configured"));
    }
    ...
    let mut providers = config
        ...
        .map(|(name, provider)| {
            (
                name.strip_prefix("model_provider:")
                    .unwrap_or(name)
                    .to_owned(),
                provider.clone(),
            )
    ...
    let Some(first) = providers.next() else {
        anyhow::bail!(
            "No OAuth LLM provider is configured. Add `type = \"oauth\"` under an auth provider, then run `grow login <provider>`."
        );
    };
    if providers.next().is_some() {
        anyhow::bail!("More than one OAuth provider is configured; run `grow login <provider>`.");
    }
    Ok(first)
}

/// Authenticate one configured LLM provider. OAuth is an optional credential
/// source beside BYOK; it never changes the selected model or provider.
pub async fn run_provider_login(
    config: &crate::agent::config::Config,
    provider: Option<&str>,
) -> anyhow::Result<()> {
    let (name, provider) = configured_oauth_provider(config, provider)?;
    let service = provider
        .oauth_service_config()
        .ok_or_else(|| anyhow::anyhow!("OAuth provider '{name}' is incomplete"))?;
    let manager = Arc::new(AuthManager::new_scoped(
        &grow_home::grow_home(),
        service.clone(),
    ...
    eprintln!("OAuth credentials saved for provider '{name}'.");
    Ok(())
}
...
pub fn run_provider_logout(
    ...
    let manager = AuthManager::new_scoped(&grow_home::grow_home(), service, scope.clone());
    let result = perform_logout(&manager, Some(&scope))
        .map_err(|error| anyhow::anyhow!("Failed to clear provider auth: {error}"))?;
    if result.was_logged_in {
        eprintln!("Logged out provider '{name}'.");
    } else {
        eprintln!("No cached OAuth credentials for provider '{name}'.");
    }
    Ok(())
}
...
pub struct LogoutResult {
    ...
    /// `true` if the `GROW_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_provider_api_key_env(),
    })
}

/// `grow logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let grow_home = grow_home::grow_home();
    let auth_manager = AuthManager::new(&grow_home, config.auth.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via GROW_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("GROW_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::TEST_OAUTH2_ISSUER;
    use crate::env::EnvVarGuard;
    use chrono::Utc;

    /// Run `f` with `GROW_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("GROW_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("GROW_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("GROW_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only grow_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), ServiceAuthConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("GROW_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("GROW_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `grow login` must use loopback, not device —
    ...
    fn oidc_auth(issuer: &str) -> ProviderAuth {
        ProviderAuth {
    ...
    fn pinned_cfg(team: &str) -> ServiceAuthConfig {
        ServiceAuthConfig {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = ProviderAuth {
            key: "fresh-token-from-disk".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() + chrono::Duration::hours(1)),
            refresh_token: Some("new-rt".into()),
            oidc_issuer: Some(TEST_OAUTH2_ISSUER.into()),
    ...
    async fn run_auth_flow_defers_to_consumer_refresh_on_transient_failure() {
        ...
        let expired_with_rt = ProviderAuth {
            key: "expired-access-token".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // GROW_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Grow: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `grow login` passes `on_stderr=None`; stderr must be inherited so
