...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<ProviderAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<ProviderAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.service_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<ProviderAuth> {
        let _lock = self
            .auth_manager
    ...
    async fn try_refresh_from_authority(&self) -> Result<ProviderAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &ProviderAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, ProviderAuth};
    ...
    fn rejected_cred() -> Option<ProviderAuth> {
        Some(ProviderAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), ServiceAuthConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(
        m: &Arc<AuthManager>,
    ) -> (Result<ProviderAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.service_config().auth_scope();
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.service_config().auth_scope();
        let expired_different = ProviderAuth {
