...
use super::super::config::{OidcAuthConfig, ServiceAuthConfig};
use super::super::{AuthManager, ProviderAuth};
use super::protocol::{
    OidcError, build_authorize_url, build_provider_auth, discover, enforce_login_principal,
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => ("Signed in", "You can close this window and return to Grow."),
...
pub async fn run_login_flow_with_config(
    ...
    let oauth2 = auth_manager.service_config().oauth2.as_ref();
    ...
    if has_client_ui {
        ...
        eprintln!();
        eprintln!("Signing in with {}...", oidc.issuer);
        eprintln!();
        if let Err(e) = webbrowser::open(&auth_url) {
            tracing::debug!(error = %e, "OIDC: failed to open browser");
        }
    ...
    let principal_policy = login_principal_policy(auth_manager.service_config());
    ...
    let mut auth = build_provider_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), ServiceAuthConfig::default())
        ...
        .await
        .unwrap();
        let auth = build_provider_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
