...
fn api_key_id_for(auth: Option<&crate::auth::ProviderAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| grow_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: ProviderAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = ProviderAuthCredentials::new(None);
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = ProviderAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        grow_telemetry::external::set_identity(
            grow_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `grow-pager` and `grow-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> grow_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let auth = crate::auth::ServiceAuthConfig::default();
    let traces_url = endpoints.resolve_otlp_traces_endpoint();
    let exporter = grow_telemetry::otel_layer::OtelExporterConfig {
        enabled: traces_url.is_some()
            && endpoints.resolve_traces_export_enabled()
...
mod tests {
    ...
    use grow_auth::AuthCredentialProvider;
    use std::sync::Mutex;
    /// Serializes tests that pin `GROW_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `GROW_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("GROW_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("GROW_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("GROW_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("GROW_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> ProviderAuth {
        ProviderAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<ProviderAuth>) -> Arc<AuthManager> {
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use grow_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
    ...
    async fn otel_live_refresh_after_unauthorized_drives_recovery() {
        ...
        live_mgr.hot_swap(ProviderAuth {
            key: "stale".into(),
            auth_mode: crate::auth::AuthMode::Oidc,
            create_time: chrono::Utc::now() - ChronoDuration::hours(2),
            user_id: "u".into(),
    ...
    /// user token, matching the precedence in `ProviderAuthCredentials::apply`.
