...
//!
//! [`grow_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`grow_sampler::Auth401AttributionCallback`] at
...
impl ShellAttribution {
    ...
    /// [`grow_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `grow_sampler::SamplerConfig` field expects exactly that;
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `grow_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// and the mint/expiry fields from the resulting `ProviderAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `ProviderAuth` (or
    // `None`) once and derive every other field from it. `current_or_expired`
    // keeps a hard-expired held token visible -- most 401s arrive exactly in
    // that window (`current()` would blank it).
    ...
    // `auth.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.grow/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> ProviderAuth {
        ProviderAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    fn hard_expired_auth(key: &str) -> ProviderAuth {
        ProviderAuth {
            key: key.to_string(),
