...
use super::model::{API_KEY_SCOPE, AuthMode, AuthStore, ProviderAuth, lookup_auth};
...
/// Falls back to the legacy `https://login.example.com/sign-in` scope key
/// when the requested scope is not found (devbox auth.json migration).
pub fn read_token_by_scope(grow_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = grow_home.join("auth.json");
    let store =
        read_auth_json(&path).map_err(|_| anyhow::anyhow!("Not logged in. Run `grow login`."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your auth token is invalid. Run `grow login` to re-authenticate.")
    })
}
...
pub fn store_api_key(grow_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = grow_home.join("auth.json");
