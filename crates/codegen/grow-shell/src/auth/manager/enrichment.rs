...
use crate::auth::model::{ProviderAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: ProviderAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.service_config.token_header;
    ...
    let response = http_client
        ...
        .header("X-Grow-Token-Auth", token_header.as_str())
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut ProviderAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: ProviderAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut ProviderAuth, user_info: UserInfo) {
