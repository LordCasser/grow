...
pub(crate) trait AuthSnapshot: Send + Sync {
    ...
    fn current(&self) -> Option<ProviderAuth>;
    /// Read the expired in-memory bearer (for its `refresh_token`).
    fn expired_auth(&self) -> Option<ProviderAuth>;
    ...
    /// diagnostics (not credential mutation).
    fn read_disk_auth(&self) -> Option<ProviderAuth>;
...
pub(crate) trait ExternalCommandRunner: Send + Sync {
    ...
    async fn run_external_command(&self, command: &str) -> Option<ProviderAuth>;
}
...
impl RefreshOutcome {
    ...
    pub(crate) fn success(auth: ProviderAuth) -> Self {
    ...
    /// flow). Any refresher holding the [`ProviderAuth`] it sent must use
    ...
    ///
    /// Prefer this wherever the attempted [`ProviderAuth`] is in hand: it captures
...
mod tests {
    use super::*;
    use crate::auth::{AuthMode, ProviderAuth, ServiceAuthConfig};
