...
//! attempt (or an explicit `grow/auth/cancel`) cancels the previous one, so
...
//! scopes explicit cancels so a delayed `grow/auth/cancel` cannot tear down a
//! successor login.
...
pub(crate) struct AttemptChannels {
    /// Forwards pasted codes from `grow/auth/submit_code` to the flow.
    code_tx: tokio::sync::mpsc::Sender<String>,
    /// Yields the auth URL to `grow/auth/get_url`. `Option` so
