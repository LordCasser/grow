...
use super::model::{
    AuthMode, ProviderAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<ProviderAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    /// Model `api_key` / resolved `env_key` for tools without a session.
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<ProviderAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(grow_home: &Path, service_config: ServiceAuthConfig) -> Self {
        let scope = service_config.auth_scope();
        Self::new_scoped(grow_home, service_config, scope)
    }

    /// Build an auth manager whose durable credential key is owned by a named
    /// LLM provider rather than by the issuer/client pair alone.
    pub(crate) fn new_scoped(
        ...
        grow_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "grow_home": grow_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "GROW_HOME": std::env::var("GROW_HOME").unwrap_or_else(|_| "(unset)".into()),
                "GROW_AUTH_PATH": std::env::var("GROW_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "GROW_AUTH": std::env::var("GROW_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // GROW_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("GROW_AUTH") {
            if let Ok(auth) = serde_json::from_str::<ProviderAuth>(&inline_json) {
            ...
            tracing::warn!("GROW_AUTH set but failed to parse as JSON, falling back to file");
        }

        // GROW_AUTH_PATH: custom file path (overrides default $GROW_HOME/auth.json).
        let path = std::env::var("GROW_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| grow_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &ProviderAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.service_config)?;
    ...
    fn vet_cached(&self, auth: ProviderAuth) -> Option<ProviderAuth> {
    ...
    pub(crate) fn current(&self) -> Option<ProviderAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<ProviderAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&ProviderAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<ProviderAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<ProviderAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_hard_expired(&self, auth: &ProviderAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &ProviderAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.service_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `ProviderAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(
        self: &Arc<Self>,
        auth: ProviderAuth,
    ) -> std::io::Result<ProviderAuth> {
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: ProviderAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut ProviderAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn service_config(&self) -> &ServiceAuthConfig {
        &self.service_config
    }
    ...
    pub(crate) async fn run_external_refresh_command(&self, command: &str) -> Option<ProviderAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: ProviderAuth) {
    ...
    fn inner_auth_or_external_default(&self) -> ProviderAuth {
        self.inner.read().clone().unwrap_or_else(|| ProviderAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: ProviderAuth) -> Option<ProviderAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<ProviderAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<ProviderAuth>, DiskAuthState) {
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<ProviderAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<ProviderAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<ProviderAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.service_config.blocks_automatic_oidc()
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.service_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<ProviderAuth, AuthError> {
        if self.service_config.blocks_automatic_oidc() {
            tracing::debug!(
    ...
    pub(crate) fn is_different_token(&self, candidate: &ProviderAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[provider_oauth] auth_token_ttl` to drive a real schedule.
        None => BACKOFF_INTERVAL,
    }
}

/// Tool bearer. Static: env → process model key → disk.
/// Kill-switch / `preferred_method = oidc` block static keys.
pub(crate) struct SharedAuthKeyProvider(pub Arc<AuthManager>);

impl grow_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.service_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.service_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.service_config.preferred_method,
    ...
    non_empty_key(crate::agent::auth_method::read_provider_api_key_env().ok())
...
) -> grow_tools::types::SharedApiKeyProvider {
