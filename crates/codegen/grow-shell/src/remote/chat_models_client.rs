//! service.example.com chat-product model catalog (`POST /rest/modes`) — the models
//! grow-web's chat picker shows, distinct from the CLI `/v1/models` build
...
pub enum ChatModelsError {
    #[error("the optional chat-model service is not configured or authenticated")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("GROW_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("GROW_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("GROW_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| GROW_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid service.example.com bearer — deliberately NOT `is_service_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-Grow-Token-Auth",
                self.auth.service_config().token_header.clone(),
            )
