...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.inference_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_provider_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set GROW_API_KEY.".into(),
                    )
                })?;
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(grow_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
    ...
    fn parse_openai_format_uses_id_field() {
        ...
        let result = parse_remote_model_value(&value, "https://api.example.com/v1").unwrap();
        assert_eq!(result.model, "grow-3");
        assert_eq!(result.base_url, "https://api.example.com/v1");
        assert_eq!(result.name.as_deref(), Some("grow-3"));
    }
    #[test]
    ...
    fn inference_url_defaults_to_proxy() {
        let ep = endpoints("https://proxy.example/v1", None, None);
        assert_eq!(ep.resolve_inference_base_url(), "https://proxy.example/v1");
    }
    #[test]
    ...
    /// ApiKey → `inference_base_url` (ApiKey, public default when unset); a custom
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(session.url, "https://service.example.com/v1/models");
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        assert_eq!(url, "https://service.example.com/v1/deployment/config");
        assert!(
