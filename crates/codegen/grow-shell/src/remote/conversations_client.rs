...
/// Body for `PUT /rest/app-chat/conversations/{id}` (grow-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("GROW_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_xai_auth(&self) -> Result<ProviderAuth, ConvError> {
