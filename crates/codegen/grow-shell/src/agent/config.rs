...
pub use grow_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(grow_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: grow_tools::types::compat::CompatCell,
...
impl PluginsConfig {
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                grow_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> grow_agent::plugins::discovery::DiscoveryConfig {
        grow_agent::plugins::discovery::DiscoveryConfig {
...
pub struct Config {
    ...
    /// `[marketplace]` — also read by `grow_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<grow_agent::config::AgentDefinition>,
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_mode: Option<grow_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut grow_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut grow_agent::config::AgentDefinition) {
...
pub use grow_agent::config::AgentDefinition;
pub use grow_agent::config::Effort;
pub use grow_agent::config::PermissionMode;
pub use grow_shared::ui_config::{ContextualHints, UiConfig};
...
pub struct AgentSelectionConfig {
    ...
    /// Looked up via `grow_agent::discovery::by_name_in_cwd()`.
...
impl Config {
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<grow_sampling_types::DoomLoopRecoveryPolicy> {
        use grow_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = grow_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    grow_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<grow_workspace::permission::ClassifierPromptType>,
...
mod tests {
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use grow_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn shell_environment_policy_known_keys_track_the_policy_struct() {
        let grow_tools::util::ShellEnvironmentPolicy {
            inherit: _,
            ignore_default_excludes: _,
            exclude: _,
            set: _,
            include_only: _,
        } = grow_tools::util::ShellEnvironmentPolicy::default();
...
"#,
        ...
        grow_config::apply_version_overrides(&mut value, &v).unwrap();
