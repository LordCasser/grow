...
/// Blocking `/v1/settings` prefetch. Model catalogs are local configuration.
    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.inference_base_url.clone()),
                _ => None,
            };
...
/// Effective startup endpoints, resolved config-aware (not env-only) so the prefetch can't leak the bearer to api.example.com.
...
pub fn start_early_prefetch_with_auth(auth: Option<ProviderAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(auth: Option<ServiceAuthConfig>) -> Option<EarlyPrefetchHandle> {
    start_early_prefetch_impl(auth, true)
}
...
pub fn start_early_prefetch_settings_only(
    ...
    start_early_prefetch_impl(auth, false)
}
...
fn start_early_prefetch_impl(
    ...
    let auth = AuthManager::new(&grow_home, auth.unwrap_or_default()).current();
