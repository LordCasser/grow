...
pub(crate) struct ModelsCache {
    ...
    pub(crate) grow_version: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ModelsCacheManager {
    pub(crate) fn new() -> Self {
        Self {
            path: crate::util::grow_home::grow_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    pub(crate) fn load_fresh(
        ...
        if cache.grow_version.as_deref() != Some(grow_version::VERSION) {
    ...
    /// Per-writer temp path: `~/.grow` is shared across concurrent CLI
