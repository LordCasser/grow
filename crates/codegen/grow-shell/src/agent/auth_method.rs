...
/// Env var that, when set, advertises `provider.api_key` as a viable auth method.
///
...
/// Returns `true` if the generic API-key override is set.
pub fn has_provider_api_key_env() -> bool {
    read_provider_api_key_env().is_ok()
}

/// Whether `provider.api_key` should be advertised (and pushed FIRST) when building
/// the `auth_methods` list at `initialize()` time.
///
/// Regression: `provider.api_key` must stay first when only per-model credentials
/// exist (no global `GROW_API_KEY`). Deferring it made BYOK users hit the login
...
///
/// A configured provider is sufficient: keyless local endpoints must not be
/// sent through product login. Static credentials remain a valid signal for
/// callers that construct a catalog entry directly.
///
/// `disable_api_key_auth` (`[auth] disable_api_key_auth` /
/// `GROW_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
/// method is never advertised, regardless of available credentials, so
/// `GROW_API_KEY` can't bypass a deployment's forced IdP login.
    ...
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    let mut has_static_credential = false;
    for model in models {
        has_configured_provider = true;
        has_static_credential |= model.has_own_credentials();
    }
    has_configured_provider
        || (!disable_api_key_auth && (has_provider_api_key_env() || has_static_credential))
}
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// Optional display label for the login method (`provider.oauth` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `auth.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `provider.oauth` method).
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.auth.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.auth.oidc.as_ref().unwrap()`
        ...
        methods.push(provider_oauth_auth_method(
            login_label,
            has_auth_provider_command,
        ));
    }
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            ...
            PROVIDER_OAUTH_METHOD_ID => Self::ProviderOAuth,
    ...
    /// `true` for session-based methods (cached_token, provider.oauth, oidc).
    pub fn is_session_based(self) -> bool {
        matches!(self, Self::CachedToken | Self::ProviderOAuth | Self::Oidc)
    }

    /// Requires user interaction (browser, OIDC redirect, or external auth command).
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::ProviderOAuth | Self::Oidc)
    }
...
/// `true` for session-based ACP methods (cached_token, provider.oauth, oidc).
pub fn is_session_based_method(method_id: &acp::AuthMethodId) -> bool {
    AuthMethodKind::from_id(method_id).is_session_based()
}

/// Per-model BYOK status: whether the selected model belongs to the configured
/// provider catalog. Credentials are optional for keyless local providers.
...
/// `GROW_API_KEY` is set, which demoted live OIDC sessions to non-refreshable
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `grow login` to re-authenticate.";
...
pub fn method_id_after_cached_token_unavailable(
    ...
    match preferred_method {
        ...
        None => Some(if has_external_api_key {
            ...
            PROVIDER_OAUTH_METHOD_ID
        }),
    }
}

/// Error when `preferred_method=api_key` but no key/BYOK credentials exist.
pub const PREFERRED_API_KEY_UNAVAILABLE: &str = "preferred_method=api_key but no API key is configured (set GROW_API_KEY or model api_key/env_key in config.toml).";

/// Error when `preferred_method=oidc` but the session path cannot proceed.
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `grow login` to authenticate.";
...
pub fn provider_api_key_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some(format!(
            "{GROW_API_KEY_ENV_VAR} or api_key/env_key in config.toml"
        ))),
    )
}
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.grow/auth.json".to_string())),
    )
}
...
pub fn provider_oauth_auth_method(
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(
            acp::AuthMethodId::new(PROVIDER_OAUTH_METHOD_ID),
            name.to_string(),
        )
...
mod tests {
    ...
    /// No advertiseable API-key credentials → interactive `provider.oauth`.
    #[test]
    fn after_cached_token_unavailable_falls_to_grow_managed_without_api_key() {
        assert_eq!(
            method_id_after_cached_token_unavailable(false, None),
            Some(PROVIDER_OAUTH_METHOD_ID),
        );
    }
    ...
    /// `provider.oauth` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): only `provider.oauth` is
    ...
    #[test]
    fn fresh_user_only_advertises_grow_managed_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::ProviderOAuth)
        );
        assert!(built.default_auth_method_id.is_none());
        assert_eq!(built.methods.len(), 1);
    }

    /// Enterprise OIDC replaces `provider.oauth` (mutually exclusive). provider.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_grow_managed_but_provider_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::ProviderOAuth),
            "provider.oauth and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `provider.oauth` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let grow = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::ProviderOAuth)
            .expect("provider.oauth must be advertised");
        assert_eq!(grow.name(), "Acme Corp");
        let meta = grow.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.grow/config.toml` skeleton from the bug report, walks it through
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        let _global = EnvGuard::unset(GROW_API_KEY_ENV_VAR);

        let toml: toml::Value = toml::from_str(
            r#"
            [models]
            default = "enterprise/model"
            ...
            env_key = "TEST_ENTERPRISE_REGRESSION_AUTH_TOKEN"

            [provider.enterprise.models.model]
        ...
        // A keyless or temporarily-unset provider remains BYOK. Product login
        // is unrelated to whether this provider currently has a secret.
        {
            ...
            assert_eq!(
                first_kind(&built.methods),
                Some(AuthMethodKind::ProviderApiKey),
                "configured providers must bypass product login even without a key",
            );
        }
        ...
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // provider.oauth login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
    ...
    /// `GROW_API_KEY` alone (no per-model creds) also triggers
    /// advertising `provider.api_key` as the first method. Historical "external
    ...
    /// AND per-model env_key), so the builder never advertises `provider.api_key`
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g2 = EnvGuard::unset("GROW_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::ProviderOAuth),
            "no cached token AND no api key: pager must show login (provider.oauth first)",
        );
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, PROVIDER_OAUTH_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![PROVIDER_OAUTH_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
