...
/// What a [`ConfigWarning`] is about. Serialize-only: `grow inspect --json`
...
pub(crate) fn log_config_warnings(warnings: &[ConfigWarning]) {
    ...
    if !warnings.is_empty() {
        tracing::warn!(
            ...
            "config: parsed with warnings; run `grow inspect` for details"
        );
    }
}
...
mod tests {
    ...
    fn duplicate_compactions_keys_keeps_model() {
        ...
        let model = cfg
            .config_models
            .get("grow-4.5")
            .expect("grow-4.5 must remain in catalog");
        assert_eq!(
        ...
        assert!(resolved.contains_key("grow-4.5"));
    }
    ...
    fn legacy_alias_alone_parses_without_warning() {
        ...
        let model = cfg.config_models.get("grow-4.5").unwrap();
        assert_eq!(
            model.compactions_remaining,
    ...
    fn invalid_reasoning_effort_skips_field_keeps_model() {
        ...
        let model = cfg
            ...
            .get("grow-4.5")
            .expect("grow-4.5 must remain in catalog");
        assert_eq!(model.model.as_deref(), Some("grow-4.5"));
    ...
    fn unknown_field_warns_but_keeps_known_fields() {
        ...
        let entry = models.get("grow-4.5").unwrap();
        assert_eq!(entry.model.as_deref(), Some("grow-4.5"));
        assert_eq!(
