...
fn resolve_agent_definition_acp_profile_wins_for_new_session() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = grow_agent::AgentDefinition::from_json(&serde_json::json!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use grow_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = grow_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: grow_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: grow_tools::implementations::grow_build::image_gen::ImageGenConfig::default(),
        video_gen_config: grow_tools::implementations::grow_build::video_gen::VideoGenConfig::default(),
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use grow_tools::implementations::grow_build::video_gen::{
...
/// Build traffic via the `x-grow-client-identifier` header. If this header is
...
/// `grow/folder_trust/request` round-trip.
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                grow_subagent_resolution::config::SubagentRole {
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _sim = EnvGuard::set(grow_version::TEST_VERSION_ENV, "0.0-sim");
...
/// Pull the next `grow/folder_trust/request` reverse-request off the gateway and
...
async fn answer_folder_trust_request(
    ...
    assert_eq!(args.request.method.as_ref(), "grow/folder_trust/request");
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use grow_test_support::EnvGuard;
    use grow_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROW_HOME", home.path());
    let _sim = EnvGuard::set(grow_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
...
mod direct_hub_cloud_removed {
    ...
    fn cloud_server_id_meta_is_hard_error() {
        let meta = serde_json::json!({ "grow/cloud_server_id": "srv-123" });
    ...
    fn cloud_server_id_null_still_present_is_hard_error() {
        let meta = serde_json::json!({ "grow/cloud_server_id": null });
        let err = reject_direct_hub_cloud_meta(meta.as_object()).expect_err("must reject");
        assert_direct_hub_error(err);
    }
    #[test]
    fn cloud_server_id_with_gateway_meta_still_hard_error() {
        let meta = serde_json::json!({
            "grow/cloud_server_id": "srv-legacy",
            "envId": "env-1",
