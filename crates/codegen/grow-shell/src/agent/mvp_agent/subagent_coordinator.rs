...
impl grow_tools::implementations::grow_build::task::coordinator::ChildRunner for ShellChildRunner {
    ...
    type RunFuture = grow_tools::implementations::grow_build::task::coordinator::LocalBoxFuture<
        grow_tools::implementations::grow_build::task::coordinator::ChildRunOutput<
            Self::CompletionData,
        >,
    >;
    type ValidateFuture =
        grow_tools::implementations::grow_build::task::coordinator::LocalBoxFuture<
            grow_tools::implementations::grow_build::task::types::SubagentValidateTypeOutcome,
        >;
    type DescribeFuture =
        grow_tools::implementations::grow_build::task::coordinator::LocalBoxFuture<
            grow_tools::implementations::grow_build::task::types::SubagentDescribeOutcome,
        >;
    fn run(
        &self,
        run: grow_tools::implementations::grow_build::task::coordinator::ChildRunRequest<
            Self::Control,
        >,
    ) -> Self::RunFuture {
        let agent_ref = self.agent_ref.clone();
        Box::pin(async move {
            let grow_tools::implementations::grow_build::task::coordinator::ChildRunRequest {
                request,
                cancellation,
                reporter,
            } = run;
            ...
            let Some(mut ctx) = this.try_build_subagent_spawn_context(&parent_sid) else {
                ...
                return grow_tools::implementations::grow_build::task::coordinator::ChildRunOutput {
                    result: grow_tools::implementations::grow_build::task::types::SubagentResult {
                        success: false,
                        error: Some(
        ...
        Box::pin(async move {
            ...
            match this.try_build_subagent_spawn_context(&parent_session_id) {
                ...
                None => {
                    ...
                    grow_tools::implementations::grow_build::task::types::SubagentDescribeOutcome::Unavailable
                }
            }
        })
    }
    fn on_completed(
        &self,
        completion: grow_tools::implementations::grow_build::task::coordinator::ChildCompletion<
            Self::CompletionData,
        >,
    ) {
...
impl MvpAgent {
    ...
    /// [`SubagentCoordinator`](grow_tools::implementations::grow_build::task::coordinator::SubagentCoordinator)
    ...
    pub(super) fn start_subagent_coordinator(&self) {
        ...
        let config =
            grow_tools::implementations::grow_build::task::coordinator::CoordinatorConfig {
                foreground_budget:
                    grow_tools::implementations::grow_build::task::backend::env_duration_or(
                ...
                ps.map(|h| h.tool_context.fs.inner().clone())
                    .unwrap_or_else(|| {
                        let cwd = ps
                            ...
                            .unwrap_or_default();
                        std::sync::Arc::new(grow_workspace::file_system::LocalFs::new(cwd))
                    }),
