...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = grow_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                grow_workspace::permission::spawn_permission_manager(
                    ...
                    gateway,
                    cwd,
                    grow_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                grow_workspace::permission::AccessKind::Read(Some(".env".into())),
                grow_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        grow_workspace::permission::Decision::PolicyDeny(_)
                    ),
