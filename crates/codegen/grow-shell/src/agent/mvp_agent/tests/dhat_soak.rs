...
//!   cargo test -p grow-shell --features dhat-heap \
...
fn populate_and_evict(agent: &MvpAgent, i: usize) {
    ...
    // toolset for the life of the process.
    {
        ...
        let toolset = std::sync::Arc::new(
            grow_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
...
fn leader_session_lifecycle_heap_steady_state() {
    run_local_for_bridge_test(|| async {
        ...
        *agent.workspace_ops.borrow_mut() = Some(grow_workspace::WorkspaceOps::for_test());
