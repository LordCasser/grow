...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`grow/session/close`). It must
    ...
    pub(crate) fn remove_session(&self, id: &acp::SessionId) {
        ...
        let _ = self
            .subagent_event_tx
            .send(grow_tools::implementations::grow_build::task::types::SubagentEvent::DiscardSessionCompletions {
    ...
    /// (`grow/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `grow/sessions/changed` notification with the session in `removed` so
    ...
    /// `grow/sessions/changed` notification with the current entry in
    ...
    /// Emit an `grow/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `grow/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `grow/sessions/changed` special-case in `leader/server.rs`).
...
/// Field names are the wire contract of `grow/debug/agent`'s `registries`
