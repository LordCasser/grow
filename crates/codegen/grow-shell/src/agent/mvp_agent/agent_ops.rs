...
impl MvpAgent {
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_service_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            let (user_id, team_id) = if auth.is_service_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            grow_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
    ...
    /// effects, and push `grow/settings/update` to clients. Called from both
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::ProviderAuth) {
        ...
        let is_xai = auth.is_service_auth();
        ...
        let (
            ...
            let grow_user_id = is_xai.then(|| user_id.clone());
            let grow_team_id = is_xai.then(|| team_id.clone()).flatten();
    ...
    /// reaches clients via `grow/settings/update`. Its own guard keeps an
    ...
    pub fn with_models(
        ...
        let instance = Self {
            ...
            plugin_registry_handle: grow_agent::plugins::SharedPluginRegistryHandle::new(
                None,
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<grow_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub async fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> grow_tools::implementations::grow_build::task::types::SubagentCancelOutcome {
    ...
    pub(crate) async fn list_running_subagents(
        ...
        grow_tools::implementations::grow_build::task::types::SubagentInspection,
    > {
        grow_tools::implementations::grow_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub(crate) async fn query_subagent(
        ...
        grow_tools::implementations::grow_build::task::types::SubagentSnapshot,
    > {
        grow_tools::implementations::grow_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.clone(),
            )
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<grow_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<grow_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn trace_upload_config_snapshot(
        ...
        let auth_token = if cfg.endpoints.deployment_key.is_none() {
            self.auth_manager
                .current_or_expired()
                .filter(|auth| auth.is_service_auth())
                .map(|auth| auth.key)
        } else {
            None
        };
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                grow_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<grow_agent::AgentDefinition>,
        persisted_agent_name: Option<&str>,
    ) -> grow_agent::AgentDefinition {
        use grow_agent::AgentDefinition;
        if let Some(name) = persisted_agent_name {
            if let Some(definition) = grow_agent::discovery::by_name_in_cwd(name, cwd) {
                return definition;
            }
            tracing::warn!(
                agent_name = %name,
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = grow_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn grow_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let has_service_auth = self.auth_manager.current().is_some_and(|a| a.is_service_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_service_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
        ...
        let mut session_env = grow_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                grow_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = grow_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = grow_tools::util::grow_home::grow_home()
                    .join("lsp.json");
                ...
                use grow_tools::implementations::lsp::{
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = grow_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                        &format!(
                        "{}{}",
                        grow_hooks::config::AGENT_HOOK_PREFIX,
