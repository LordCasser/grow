...
//!
//! The pager drives the polling via `grow/auth/check_subscription`: the 5s
...
/// Subscription tiers that qualify for Grow access.
...
async fn fetch_user_info(
    ...
    let request = http_client
        ...
        .header(
            "X-Grow-Token-Auth",
            auth_manager.service_config().token_header.as_str(),
        )
...
/// the paywall is shown (`grow/auth/check_subscription`).
///
...
mod tests {
    ...
    fn partial_tier_name_is_not_qualifying() {
        ...
        assert!(!QUALIFYING_TIERS.contains(&"Grow"));
