...
//! `.grow/config.toml` including `[permission]` / `[mcp_servers]` /
//! `[plugins].paths`, `~/.claude.json` `projects.<cwd>`, project `.grow/lsp.json`).
...
//! interactive prompt, and the durable [`grow_workspace::trust::TrustStore`]
//! reads/writes — lives in `grow-workspace` (client-side); this module keeps
...
//! on [`grow_workspace::folder_trust::decide`]; the consume-side nuance is
...
use grow_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};
use parking_lot::Mutex;

// Decision-side (scan/decide/prompt/store) relocated to `grow-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `grow_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
// (`grow_agent::plugins::TrustStore` at `~/.grow/trusted-plugins`, plus the
...
/// [`grow_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = grow_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: grow_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != grow_agent::config::AgentScope::Project || trusted()
}
...
///
/// Sources: project `.grow/config.toml [mcp_servers]` (NOT the user-tier global
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            grow_tools::implementations::lsp::config::LspServerConfig,
            grow_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, grow_tools::implementations::lsp::config::LspServerConfig> {
    grow_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use grow_workspace::folder_trust::repo_configs_present;
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; GROW_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because GROW_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. GROW_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because GROW_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. GROW_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because GROW_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; GROW_HOME-isolated store; GROW_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".grow").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. GROW_HOME-isolated so the trust store is empty;
        // GROW_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = grow_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. GROW_HOME-isolated so the trust store is empty;
        // GROW_FOLDER_TRUST unset so the default-on feature flag applies.
        use grow_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). GROW_HOME-isolated so the trust store is empty.
        use grow_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. GROW_HOME-isolated (empty store).
        use grow_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".grow").join("agents");
        ...
        .unwrap();
        let def = grow_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.grow/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). GROW_HOME-isolated (empty
        // store); GROW_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + GROW_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. GROW_HOME-isolated so the seeded store is the temp one;
        // GROW_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // GROW_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(grow_version::TEST_VERSION_ENV);
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.grow/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. GROW_HOME-isolated (empty store);
        // GROW_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".grow").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_denies_untrusted_permission_only_repo() {
        // Bridge: a clone whose ONLY repo-local config is `.grow/config.toml`
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // GROW_HOME-isolated (empty store); GROW_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".grow").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // GROW_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); GROW_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use grow_agent::plugins::discovery::DiscoveryConfig;
        use grow_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".grow").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // GROW_HOME-isolated so the grant writes to a temp store; GROW_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        let _flag = EnvGuard::unset("GROW_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".grow").join("hooks");
        ...
        let has_project_hook = |reg: &grow_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = grow_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &grow_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        use grow_tools::implementations::lsp::config::LspServerConfig;
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use grow_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use grow_tools::types::config_source::ConfigSource;

        // A `<cwd>/.grow/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.grow/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let grow = tmp.path().join(".grow");
        std::fs::create_dir_all(&grow).unwrap();
        std::fs::write(grow.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.grow/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let grow = tmp.path().join(".grow");
        std::fs::create_dir_all(&grow).unwrap();
        std::fs::write(grow.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.grow/config.toml [mcp_servers]`).
    ...
    /// `.grow/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `grow --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(grow_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("GROW_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no GROW_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(grow_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(grow_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("GROW_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // GROW_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("GROW_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROW_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".grow").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".grow").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with GROW_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. GROW_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(grow_version::TEST_VERSION_ENV);
        if option_env!("GROW_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
