...
pub(crate) async fn run_shell_child(
    ...
    let mut effective_runtime = grow_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = grow_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    effective_runtime.capability_mode = grow_subagent_resolution::intersect_capability_modes(
    ...
    grow_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = grow_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = grow_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        grow_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    if let Some(scope) = agent_memory_scope {
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                grow_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == grow_hooks::event::HookEventName::Stop {
                            s.event = grow_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    grow_agent::config::McpServerRef::Named(name) => {
                    ...
                    grow_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            grow_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    grow_diagnostics::session_ctx::log_event(grow_diagnostics::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            grow_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        false,
        ...
        if inherit_skills {
            ...
            grow_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
    ...
    let outcome = if result.success {
        grow_diagnostics::events::Outcome::Completed
    } else if result.cancelled {
        grow_diagnostics::events::Outcome::Cancelled
    } else {
        grow_diagnostics::events::Outcome::Error
    };
    grow_diagnostics::session_ctx::log_event(grow_diagnostics::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
