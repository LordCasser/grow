...
use grow_subagent_resolution::resolve_effective_overrides;
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn grow_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
    let mut ctx = ctx_with_toggle(HashMap::new());
...
fn explicit_override_takes_precedence_over_role() {
    ...
    let role = grow_subagent_resolution::config::SubagentRole {
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            grow_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = grow_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = grow_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use grow_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
fn test_sampling_config(model_slug: &str) -> grow_sampling_types::SamplingConfig {
