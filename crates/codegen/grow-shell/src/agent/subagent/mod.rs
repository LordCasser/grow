...
//! `grow-tools` coordinator actor. This module keeps shell-specific
...
use grow_subagent_resolution::ResumeSourceData;
use grow_tools::implementations::grow_build::monitor::types::MonitorEventBuffer;
use grow_tools::implementations::grow_build::task::coordinator::{
...
use grow_workspace::file_system::AsyncFileSystem;
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn grow_tools::implementations::lsp::LspBackend>>,
    ...
    /// (not an intermediate parent's) because grow-tools task/coordinator.rs
    ...
    pub sampling_config: grow_sampler::SamplerConfig,
    ...
    pub parent_terminal_backend: Option<Arc<dyn grow_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle: Option<grow_tools::notification::types::ToolNotificationHandle>,
    ...
    pub parent_scheduler_handle:
        Option<grow_tools::implementations::grow_build::scheduler::types::SchedulerHandle>,
    ...
    pub subagent_roles:
        std::collections::HashMap<String, grow_subagent_resolution::config::SubagentRole>,
    ...
    pub plugin_registry: Option<std::sync::Arc<grow_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<grow_tools::registry::types::ToolConfig>>,
    ...
    pub parent_tool_definitions: Option<Vec<grow_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<grow_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: grow_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: grow_tools::types::compat::CompatConfig,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut grow_agent::config::AgentDefinition) {
...
) -> (grow_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &grow_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &grow_sampler::SamplerConfig,
) {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(grow_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[grow_sampling_types::conversation::ConversationItem],
) -> usize {
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<grow_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<grow_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        grow_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[grow_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<grow_sampling_types::conversation::ConversationItem>,
    ...
    let (conversation, prefix_len) =
        grow_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &grow_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<grow_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = grow_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def =
        grow_subagent_resolution::discover_agent_definition(subagent_type, &resolution_context)?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    grow_subagent_resolution::available_agent_names(
        &grow_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) fn validate_subagent_type(
    ...
    match grow_subagent_resolution::validate_agent_name(subagent_type, &context) {
        ...
        Err(
            grow_subagent_resolution::ResolutionError::PersonaResolution(_)
            | grow_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match grow_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(grow_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(
            grow_subagent_resolution::ResolutionError::Unknown { .. }
            | grow_subagent_resolution::ResolutionError::PersonaResolution(_)
            | grow_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    grow_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut grow_agent::config::AgentDefinition,
) {
    let resolution_context = grow_subagent_resolution::HarnessToolsetContext {
    ...
    grow_subagent_resolution::apply_harness_toolset(subagent_type, &resolution_context, definition);
}
...
fn summarize_tool_config(
    config: &grow_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    grow_workspace::session::git::find_main_repo_root_from_path(&source_cwd).unwrap_or(source_cwd)
}
...
fn inject_subagent_completed_prompt(
    ...
    let summary =
        grow_tools::implementations::grow_build::task::completion_summary(request, result);
    let message = grow_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = grow_tools::reminders::wrap_reminder(&message);
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &grow_tools::implementations::grow_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
