...
//! connection to the service.example.com relay server with automatic reconnection.
...
/// Config for the service.example.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    ...
    pub(crate) fn for_session(
        ...
        if !session.is_service_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.service_ws_url.clone(),
            ws_origin: ctx.service_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to service.example.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `grow login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Grow WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(grow_version::VERSION),
    );
    req.headers_mut().insert(
...
mod tests {
    ...
    /// Helper to create a test ProviderAuth with the given key.
    fn test_auth(key: &str) -> ProviderAuth {
        ProviderAuth {
            key: key.to_string(),
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        assert!(
            service_auth.is_service_auth(),
            "precondition: is_service_auth"
        );
    ...
    /// Helper: write a ProviderAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &ProviderAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        let cfg = crate::auth::ServiceAuthConfig::default();
        let scope = cfg.auth_scope();
        let am = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_session = ProviderAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(TEST_OAUTH2_ISSUER.to_string()),
    ...
    async fn attempt_auth_recovery_same_key_backs_off_without_cancel() {
        ...
        let fresh_session = ProviderAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(TEST_OAUTH2_ISSUER.to_string()),
