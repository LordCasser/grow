//! Tracing layer for `target: "sampling_log"` → `~/.grow/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `GROW_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "GROW_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = grow_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
