...
//! - PerSession (`GROW_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.grow/debug/<session_id>.txt` (one file per session), with a
...
//! - SingleFile (explicit path via `GROW_LOG_FILE` or `GROW_DEBUG_LOG=<path>`):
...
///
/// Lives here (not in `grow-pager`) so the firehose directives below and
...
/// safe. See `grow-pager/src/tracing.rs` for the consumer side.
...
// Broad firehose filter for the routing/GROW_DEBUG_LOG sources: capture our
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,grow_pager=debug,grow_shell=debug,grow_tools=debug,grow_diagnostics=debug,grow_agent=debug,grow_mcp=debug,xai_acp_lib=debug,sampling_log=off";
...
// GROW_DEBUG_LOG single-file source (mirrors `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the GROW_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves GROW_LOG_FILE back-compat).
...
/// filter by source (GROW_LOG_FILE respects RUST_LOG). Open failures warn AFTER
...
pub(crate) enum DebugTarget {
    /// `GROW_DEBUG_LOG=1` → route per session into `<dir>` (`~/.grow/debug`).
...
/// (single file, RUST_LOG filter); else GROW_DEBUG_LOG — a truthy bool routes
/// per session into `~/.grow/debug`, an explicit path writes a single file.
///
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for GROW_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for GROW_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace GROW_LOG_FILE is treated as unset (mirrors GROW_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 GROW_DEBUG_LOG value is a path, not a bool keyword, and its
        ...
        match target {
            DebugTarget::SingleFile { path, src } => {
                assert_eq!(src, DebugSource::GrowDebugLog);
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "grow_shell", "inside session");
            });
            tracing::info!(target: "grow_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `grow_diagnostics::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "grow_diagnostics::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "grow_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "grow_shell", "one first");
                tracing::info!(target: "grow_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "grow_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn prune_old_logs_missing_dir_is_noop() {
        ...
        prune_old_logs(
            Path::new("/no/such/grow/debug/dir"),
