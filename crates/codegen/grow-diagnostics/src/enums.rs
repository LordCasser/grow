//! Shared diagnostics/config enums extracted from shell.
//!
//! These were originally defined inside `grow-shell` (in
//! `session::mcp_servers` and `util::config`) but are referenced by
//! diagnostics payload structs in this crate, so they live here and shell
...
/// signals (`turn_result.json`) and the `pr_created` diagnostics event.
