...
//! OS-level sandboxing for Grow via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use grow_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__GROW_INSIDE_BWRAP";
...
impl SandboxManager {
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if requires_hook_write_deny(&self.profile, workspace) {
            grow_config::ensure_grow_hook_slots(paths::grow_home().as_path())
...
/// Zero-permission placeholder (file or dir) under `grow_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent grow
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::grow_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    fn bwrap_hook_plan_binds_ancestors_then_leaves() {
        ...
        let sources = [grow_config::GlobalHookSource {
            path: leaf.clone(),
            kind: grow_config::GlobalHookSourceKind::ConfiguredSource,
        }];
    ...
    /// Create a temp workspace whose `.grow/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("grow-{tag}-{}-{nanos}", std::process::id()));
        let grow = ws.join(".grow");
        std::fs::create_dir_all(&grow).unwrap();
        std::fs::write(grow.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("grow-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::grow_home()
