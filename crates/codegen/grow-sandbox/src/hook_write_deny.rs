//! Grow-owned hook write-deny: plan, identity revalidation, and post-reexec checks.
//! Namespace lockdown is in [`crate::child_net`].

use std::path::{Path, PathBuf};

use grow_config::{
    GlobalHookSource, ensure_grow_hook_slots, missing_configured_sources,
    resolve_global_hook_sources,
};

#[cfg(target_os = "linux")]
use grow_config::unique_ancestors_rootward;
#[cfg(unix)]
use grow_config::validated_hook_json_files_for_sources;
...
impl From<grow_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: grow_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use grow_config::GlobalHookSourceKind;
    use std::os::unix::fs::MetadataExt;
    for s in sources {
...
pub fn resolve_hook_write_deny_snapshot() -> Result<Vec<GlobalHookSource>, HookWriteDenyError> {
    let grow = grow_home();
    let resolved =
        resolve_global_hook_sources(Some(grow.as_path()), /* reject_symlinks */ true)?;
...
pub fn prepare_hook_write_deny(
    ...
    let grow = grow_home();
    ensure_grow_hook_slots(grow.as_path())?;
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use grow_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
