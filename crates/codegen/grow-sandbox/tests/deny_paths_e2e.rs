//! E2E path-deny and Grow hook write-deny (subprocess; arm64-tagged).
...
const GROW_HOME_ENV: &str = "SANDBOX_E2E_GROW_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, grow_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        .env(HOME_ENV, home.as_os_str())
        .env(GROW_HOME_ENV, grow_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("GROW_HOME", grow_home.as_os_str());
}
...
fn run_scenario(
    ...
    apply_fixture_env(&mut cmd, home, grow_home, workspace);
    let output = cmd
...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = grow_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> grow_sandbox::ProfileName {
    grow_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    // Isolate HOME/GROW_HOME before any config OnceLock init.
    let home = PathBuf::from(std::env::var(HOME_ENV).expect(HOME_ENV));
    let grow_home = PathBuf::from(std::env::var(GROW_HOME_ENV).expect(GROW_HOME_ENV));
    ...
    match scenario.as_str() {
        ...
        "hook_write_deny_marker_spoof" => subprocess_hook_write_deny_marker_spoof(&grow_home),
        other => {
...
fn subprocess_profile_and_bwrap_reexec(profile: &grow_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !grow_sandbox::is_inside_bwrap() {
            ...
            match grow_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = grow_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_grow_home: &Path) {
    #[cfg(not(target_os = "linux"))]
    {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe {
            std::env::set_var("__GROW_INSIDE_BWRAP", "1");
        }
        match grow_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
/// Workspace-profile Grow-owned hook write-deny probes (existing sources + first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(GROW_HOME_ENV).expect(GROW_HOME_ENV));

    let profile = grow_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    ...
    if first_run {
        ...
        eprintln!("OK: first-run Grow hook slots denied");
    } else {
    ...
    // Parent grants remain creatable: Grow runtime sibling, workspace, temp.
    assert_write_ok(
        "grow runtime sibling",
...
/// Create isolated HOME + GROW_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    let grow = unique_temp_dir(&format!("{tag}-grow"));
    let workspace = unique_temp_dir(&format!("{tag}-ws"));
    // Empty global sandbox config under fixture GROW_HOME so generic tests do
    // not inherit the developer/runner's ~/.grow/sandbox.toml.
    fs::write(grow.join("sandbox.toml"), "").expect("empty global sandbox.toml");
    (
        home.clone(),
        grow.clone(),
        workspace.clone(),
...
fn run_deny_case(
    ...
    let (home, grow, tmp, _ch, _cg, _cw) = fixture_homes(tag);
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".grow")).expect("mkdir .grow");
    fs::write(
        tmp.join(".grow").join("sandbox.toml"),
        format!("[profiles.{profile}]\nextends = \"workspace\"\ndeny = [{deny_list}]\n"),
    )
    .expect("write sandbox.toml");

    // Ensure Grow fixed slots exist so workspace-based custom profiles can
    // resolve hook write-deny without depending on the real user tree.
    fs::create_dir_all(grow.join("hooks")).expect("mkdir fixture hooks");
    fs::write(grow.join("hooks-paths"), b"").expect("write fixture hooks-paths");
    ...
    let (status, stderr) = run_scenario(&home, &grow, &tmp, profile, targets, controls, postlaunch);
    assert!(
        status.success(),
...
fn hardlinked_hooks_paths_refuses_startup() {
    ...
    let (home, grow, workspace, _ch, _cg, _cw) = fixture_homes("hook-hl");
    fs::create_dir_all(grow.join("hooks")).unwrap();
    let reg = grow.join("hooks-paths");
    let alias = grow.join("hooks-paths-alias");
    fs::write(&reg, b"").unwrap();
    fs::hard_link(&reg, &alias).unwrap();

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &grow, &workspace, "hook_write_deny");
    assert!(
        !status.success(),
...
/// Workspace profile: Grow-owned direct hook sources are write-denied but readable;
/// create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths
/// targets are denied; parent rename is blocked; Grow/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    ...
    let (home, grow, workspace, _ch, _cg, _cw) = fixture_homes("hook");

    fs::create_dir_all(grow.join("hooks")).expect("mkdir hooks");
    fs::write(grow.join("hooks").join("keep.json"), r#"{"keep-me":true}"#)
        .expect("write keep.json");
    let dynamic = grow.join("sessions").join("extra-hooks");
    ...
    for needle in [
        ...
        "OK: grow runtime sibling writable",
...
fn hardlinked_hooks_json_refuses_startup() {
    ...
    let (home, grow, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-hl");
    fs::create_dir_all(grow.join("hooks")).unwrap();
    fs::write(grow.join("hooks-paths"), b"").unwrap();
    let active = grow.join("hooks").join("active.json");
    let alias = grow.join("hooks").join("active-alias.json");
...
fn symlinked_hooks_json_refuses_startup() {
    ...
    let (home, grow, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-sym");
    fs::create_dir_all(grow.join("hooks")).unwrap();
    fs::write(grow.join("hooks-paths"), b"").unwrap();
    let real = grow.join("real-active.json");
    let active = grow.join("hooks").join("active.json");
...
/// First-run: missing fixed slots are created as real Grow state before apply,
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    let (home, grow, workspace, _ch, _cg, _cw) = fixture_homes("hook-fr");
    // Intentionally leave hooks/ and hooks-paths absent (first-run ensure path).

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &grow, &workspace, "hook_write_deny_first_run");
    assert!(
        status.success(),
    ...
    for needle in [
        "OK: first-run Grow hook slots denied",
    ...
    // Post-exit host: Grow slots exist and are valid; no vendor artifacts.
    assert!(
        grow.join("hooks").is_dir(),
        "post-exit: hooks dir must exist as a real directory"
    );
    assert!(
        grow.join("hooks-paths").is_file(),
        "post-exit: hooks-paths must exist as a real file"
    );
    assert_eq!(
        fs::read(grow.join("hooks-paths")).expect("read hooks-paths"),
        b"",
...
fn hook_write_deny_refuses_marker_spoof() {
    ...
    #[cfg(target_os = "linux")]
    {
        let (home, grow, workspace, _ch, _cg, _cw) = fixture_homes("hook-spoof");
        fs::create_dir_all(grow.join("hooks")).unwrap();
        fs::write(grow.join("hooks").join("x.json"), b"{}").unwrap();
        fs::write(grow.join("hooks-paths"), b"").unwrap();
        let (status, stderr) =
            run_hook_write_deny_scenario(&home, &grow, &workspace, "hook_write_deny_marker_spoof");
        assert!(
            status.success(),
