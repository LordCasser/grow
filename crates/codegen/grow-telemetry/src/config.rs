...
//!
//! Extracted from `grow-shell::agent::config` so the data-collector
...
impl TelemetryConfig {
    pub fn apply_env_overrides(&mut self) {
        self.normalize();
        if let Some(value) = Self::env_override("GROW_TELEMETRY_EVENTS_URL") {
...
///
/// Local copy of `grow_shell::agent::config::env_bool` so this crate
