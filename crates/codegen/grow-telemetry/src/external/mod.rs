...
//!
//! Enterprise customers point the Grow CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `GROW_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `GROW_EXTERNAL_OTEL` unset (or no exporter
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &grow_auth::CredentialSnapshot) -> Self {
        Self {
