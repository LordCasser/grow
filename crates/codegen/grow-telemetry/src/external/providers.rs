...
//! `Authorization`/`X-Grow-Token-Auth`/`x-userid`;
...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    let mut attrs = vec![
        ...
        opentelemetry::KeyValue::new("grow_code.schema.version", super::schema::SCHEMA_VERSION),
    ];
    ...
    opentelemetry_sdk::Resource::builder_empty()
        // RQ6 (final): `grow-cli`, a wire commitment.
...
mod tests {
    ...
    /// `X-Grow-Token-Auth`, `x-userid`, or `x-teamid` unless customer-supplied
    ...
    fn exporter_headers_are_exactly_customer_headers() {
        ...
        for forbidden in ["Authorization", "X-Grow-Token-Auth", "x-userid", "x-teamid"] {
            assert!(
    ...
    fn exporter_metadata_is_customer_headers_only() {
        ...
        for forbidden in ["x-grow-token-auth", "x-userid", "x-teamid"] {
