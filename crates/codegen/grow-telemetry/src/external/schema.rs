...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "grow_code.session_start",
            Self::SessionEnd => "grow_code.session_end",
            Self::UserPrompt => "grow_code.user_prompt",
            Self::TurnCompleted => "grow_code.turn_completed",
            Self::ApiRequest => "grow_code.api_request",
            Self::ApiError => "grow_code.api_error",
            Self::ToolResult => "grow_code.tool_result",
            Self::ToolDecision => "grow_code.tool_decision",
            Self::McpServerConnection => "grow_code.mcp_server_connection",
            Self::PermissionModeChanged => "grow_code.permission_mode_changed",
            Self::SkillActivated => "grow_code.skill_activated",
            Self::PluginLoaded => "grow_code.plugin_loaded",
            Self::Compaction => "grow_code.compaction",
            Self::Subagent => "grow_code.subagent",
            Self::Auth => "grow_code.auth",
            Self::InternalError => "grow_code.internal_error",
            Self::ModelSwitched => "grow_code.model_switched",
            Self::ContextualTip => "grow_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `grow_code.session.count`.
    SessionCount,
    /// `grow_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `grow_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `grow_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `grow_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `grow_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "grow_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "grow_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "grow_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "grow_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "grow_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "grow_code.error.count";
...
/// Built-in tool names (closed enum surface in `grow-tools`) that pass
...
fn tool_outcome_label(o: &crate::events::ToolOutcome) -> &'static str {
...
/// `SessionHarness` → `grow_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `grow_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `grow_code.session_end`.
...
/// `PromptSubmitted` → `grow_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `grow_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `grow_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `grow_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `grow_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `grow_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `grow_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `grow_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `grow_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `grow_code.permission_mode_changed`.
...
/// `ContextualTip` → `grow_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `grow_code.permission_mode_changed`.
...
/// `SkillDispatched` → `grow_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `grow_code.plugin_loaded`.
...
/// `PluginUsed` → `grow_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `grow_code.compaction`.
...
/// `CompactionTriggered` → `grow_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `grow_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `grow_code.subagent` (`phase=completed`).
...
/// `InternalError` → `grow_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `grow_code.model_switched`.
