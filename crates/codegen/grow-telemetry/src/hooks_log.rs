...
//! file logger that writes to `~/.grow/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `grow_hooks` or `grow_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! GROW_HOOKS_LOG=1 grow              # enable, write to ~/.grow/logs/hooks.log
//! GROW_HOOKS_LOG=/tmp/h.log grow     # write to custom path
//! GROW_HOOKS_LOG=0 grow              # explicitly disable
//! tail -f ~/.grow/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "GROW_HOOKS_LOG";
...
///
/// Writes to `~/.grow/logs/hooks.log` (or custom path via `GROW_HOOKS_LOG`).
/// Filters to hooks (`grow_hooks`) and plugins (`grow_agent::plugins`) targets.
/// Set `GROW_HOOKS_LOG=0` to disable, `GROW_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter =
        tracing_subscriber::filter::EnvFilter::new("grow_hooks=debug,grow_agent::plugins=debug");
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || grow_home().join("logs").join("hooks.log");
