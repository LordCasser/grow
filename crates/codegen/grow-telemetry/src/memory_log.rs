...
//!
//! Provides a dedicated tracing target (`grow_memory`) with an optional
//! file logger that writes to `~/.grow/logs/memory.log`.
//!
//! ## When to use
//!
...
//! GROW_MEMORY_LOG=0 grow                # disable even when enabled
//! tail -f ~/.grow/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "grow_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "GROW_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.grow/logs/memory.log`. Filters to `grow_memory=trace`.
    /// Set `GROW_MEMORY_LOG=0` to disable, `GROW_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || grow_home().join("logs").join("memory.log");
