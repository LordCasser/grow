...
//!
//! Extracted from `grow-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `grow-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `grow-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "grow-shell-",
            EmitterOrigin::Workspace => "grow-workspace-",
        }
    }
}
...
/// event-name prefix (`grow-shell-*` vs `grow-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "grow-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "grow-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("grow-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("grow-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "grow-workspace-turn");
    }
