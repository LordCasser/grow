...
//!
//! Extracted from `grow-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`grow-shell-` /
/// `grow-workspace-`). Unprefixed names pass through unchanged. Kept in
...
fn normalize_tier(tier: &str) -> String {
    match tier {
        ...
        "Provider Plan" | "provider_plan" => "provider_plan",
        "Provider Plan Lite" | "provider_plan_lite" => "provider_plan_lite",
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("grow-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("grow-shell-turn"), "turn");
        assert_eq!(
            event_value("grow-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("grow-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("Provider Plan"), "provider_plan");
        assert_eq!(normalize_tier("Provider Plan High"), "provider_plan_high");
        assert_eq!(normalize_tier("provider_plan_high"), "provider_plan_high");
        assert_eq!(normalize_tier("Basic plan"), "x_basic");
    ...
    /// (e.g. a future `"grow-shell-ext-"` origin), an earlier `ALL` entry could
