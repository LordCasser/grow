...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,grow_pager=debug,grow_shell=debug,grow_tools=debug,grow_telemetry=debug,grow_agent=debug,grow_mcp=debug,xai_acp_lib=debug,sampling_log=off";
...
mod tests {
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `grow_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "grow_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
