...
//!
//! `grow-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "GROW_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `ServiceAuthConfig`).
///
/// Built by the binaries (`grow-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `grow_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-Grow-Token-Auth` header (typically `"grow-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`grow-pager`) -> `client.name`.
...
/// OTLP trace-export transport settings for Grow's internal stream.
#[derive(Debug, Default, Clone)]
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL. `None` means no internal destination.
    pub traces_url: Option<String>,
    /// `GROW_INTERNAL_OTLP_HEADERS` pairs.
...
///
/// - `client_name`: binary name (e.g. `"grow-tui"`, `"grow-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-grow-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("grow-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid GROW_OTEL_FILTER '{}': {}. Using default '{}'.",
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-Grow-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &grow_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-Grow-Token-Auth".to_string(), value.to_string());
    }
...
mod tests {
    ...
    use grow_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    use std::sync::Mutex;
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("grow-cli"),
