...
use grow_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "grow-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    let mut cfg = external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "GROW_EXTERNAL_OTEL" => Some("1".into()),
    ...
    assert!(!grow_telemetry::is_enabled());

    grow_telemetry::log_event(grow_telemetry::events::SessionHarness {
        session_id: "sess-gates-on".into(),
        client_identifier: Some("grow-pager".into()),
        model_id: "grow-4".into(),
        agent_name: "grow-build-plan".into(),
        permission_mode: grow_telemetry::enums::PermissionMode::Ask,
    ...
    grow_telemetry::log_event(grow_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "grow-4".into(),
    ...
    grow_telemetry::log_event(grow_telemetry::events::ModelResponseReceived {
    ...
    grow_telemetry::log_event(grow_telemetry::events::ToolCallCompleted {
        tool_name: "github__create_issue".into(),
        outcome: grow_telemetry::events::ToolOutcome::Success,
        duration_ms: 12,
    ...
    let harness = col::find_event(&collected, "grow_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.grow");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("grow-cli"),
        "service.name=grow-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("grow_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "grow_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "grow_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "grow_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "grow_code.session.count");
