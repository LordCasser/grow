...
fn external_stream_end_to_end() {
    ...
    let mut cfg = grow_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = grow_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    grow_telemetry::external::init(Some(cfg));
    assert!(grow_telemetry::external::is_active());
    ...
    grow_telemetry::log_event(grow_telemetry::events::SessionNew {
    ...
    for req in &logs {
        for rl in &req.resource_logs {
            ...
            for sl in &rl.scope_logs {
                assert_eq!(sl.scope.as_ref().map(|s| s.name.as_str()), Some("ai.grow"));
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("grow-cli")),
        "service.name=grow-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "grow_code.session_start",
        "grow_code.user_prompt",
        "grow_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "grow_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "grow_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "grow_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "grow_code.token.usage"));
    assert_eq!(
    ...
    grow_telemetry::external::shutdown();
    assert!(
