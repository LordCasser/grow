...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_grow_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use grow_shell::util::config;
use grow_shell::util::grow_home::{grow_application, grow_home};
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_grow_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_grow_version();
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_grow_version);
    if needs_update(
        &effective_current,
        &target,
        &update_config.channel,
    ...
    let running = get_installed_grow_version();
...
/// Remove the stale `grow-pager` symlink/binary from `~/.grow/bin/` left by
...
async fn refresh_deployment_config() {
    if !grow_shell::managed_config::has_principal() {
        return;
    }
    if !grow_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    grow_shell::managed_config::clear_orphan();
    if !grow_shell::config::is_managed_config_stale_for(
        &grow_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match grow_shell::managed_config::sync().await {
...
mod tests {
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        // grow and grow-pager should not interfere with each other.
    ...
    async fn test_cleanup_old_downloads_pager_multi_version() {
        // Verify cleanup works for grow-pager with multiple old versions.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
    ...
    async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
        ...
        std::fs::write(d.join("grow-pager-0.1.141-macos-aarch64"), "pager").unwrap();
        std::fs::write(d.join("grow-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("grow-0.1.141-macos-aarch64"), "current").unwrap();
