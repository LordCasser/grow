...
//! `grow-config`) and `GROW_TEST_VERSION` — so they
...
use grow_update::write_version_cache;
...
async fn get_installed_version_uses_env_var_override() {
    ...
    let v = grow_update::version::get_installed_grow_version();
    assert_eq!(v, "9.9.9");
    unsafe {
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        grow_update::version::get_installed_grow_version()
    };

    unsafe {
        std::env::set_var("GROW_TEST_VERSION", "0.0.0-test");
    }
    let overridden = grow_update::version::get_installed_grow_version();
