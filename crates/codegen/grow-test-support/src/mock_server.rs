...
/// `/v1/models` + `/v1/settings` + `/v1/privacy/coding-data-retention` server.
...
mod tests {
    ...
    async fn post_foreground(
        ...
        reqwest::Client::new()
            ...
            .header("x-grow-req-id", request_id)
            .header("x-grow-turn-idx", "1")
    ...
    fn explicit_request_headers_override_tool_count_heuristic() {
        ...
        headers.insert("x-grow-req-id", "title-request".parse().unwrap());
        assert!(
            !overrides
        ...
        headers.insert("x-grow-turn-idx", "1".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
        ...
        headers.insert("x-grow-req-id", "".parse().unwrap());
        headers.insert("x-grow-turn-idx", "".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
    ...
    async fn cancelling_replay_waits_for_primary_before_satisfaction() {
        ...
        let replay = tokio::spawn({
            ...
            async move {
                reqwest::Client::new()
                    .post(url)
                    .header("x-grow-req-id", "cancel-replay")
                    .header("x-grow-turn-idx", "1")
                    .json(&body)
                    .send()
                    .await
