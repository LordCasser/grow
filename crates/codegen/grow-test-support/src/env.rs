...
fn local_grow_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("grow{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_grow_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    cmd.current_dir(workspace_root())
        .args(["build", "-p", "grow-pager-bin", "--bin", "grow"])
        .stdin(std::process::Stdio::null())
        .envs(xai_tty_utils::pager_env());
    xai_tty_utils::detach_std_command(&mut cmd);
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build grow: {e}"));

    assert!(
        output.status.success(),
        "failed to build grow for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "grow build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve grow binary: `GROW_BINARY` env (CI) or a locally built `grow` binary.
pub fn grow_binary() -> PathBuf {
    if let Ok(path) = std::env::var("GROW_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "GROW_BINARY does not exist: {}", p.display());
        // Bazel's GROW_BINARY is runfiles-relative; the harness spawns the child
        // with a different cwd, so absolutize against the (runfiles-root) cwd now.
        return std::path::absolute(&p).unwrap_or(p);
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_grow") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_grow_binary_path();
    ensure_local_grow_binary(&binary);
    binary
}
