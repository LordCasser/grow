//! Hermetic filesystem and child-environment owner for grow integration tests.
...
/// grow network kill switches, and explicit overrides.
...
impl TestSandbox {
    ...
    /// Explicit grow state root.
    pub fn grow_home(&self) -> &Path {
        &self.grow_home
    }
    ...
    pub fn diagnostic_summary(&self) -> String {
        let mut summary = format!(
            "root={} home={} grow_home={} workspace={} temp={}",
            self.root().display(),
            self.home.display(),
            self.grow_home.display(),
...
impl TestSandboxBuilder {
    /// Wire grow API, models, feedback, trace, conversation, and web traffic to
    ...
    pub fn build(self) -> TestSandbox {
        ...
        for path in [&home, &grow_home, &workspace, &temp] {
        ...
        let mut env = baseline_env(&home, &grow_home, &temp, &parent_cwd);
...
impl TestSandbox {
    fn init_git_workspace(&self) {
        ...
        run_git(self, &["config", "user.name", "Grow Test"]);
...
fn apply_mock_url(env: &mut BTreeMap<OsString, OsString>, url: String) {
    ...
    env.insert("GROW_API_KEY".into(), TEST_API_KEY.into());
}
...
fn baseline_env(
    ...
    baseline_env_from_parent(home, grow_home, temp, parent_cwd, &parent_env)
}
...
fn baseline_env_from_parent(
    ...
    env.insert(
        ...
        grow_home.join("gitconfig").into_os_string(),
    );
    env
}
...
fn diagnostic_value_is_sensitive(key: &OsStr) -> bool {
    ...
    is_secret_key(&key)
        ...
        || matches!(
            ...
            "HOME" | "USERPROFILE" | "GROW_HOME" | "TMPDIR" | "TMP" | "TEMP" | "GIT_CONFIG_GLOBAL"
        )
}
...
mod tests {
    ...
    fn owns_distinct_isolated_paths() {
        ...
        assert_eq!(sandbox.grow_home(), sandbox.home().join(".grow"));
    }
    ...
    fn git_command_uses_sandbox_state_without_process_global_mutation() {
        ...
        let sandbox = TestSandbox {
            ...
            grow_home: root.path().join("home/.grow"),
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "GROW_HOME"),
            Some(sandbox.grow_home().into())
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&sandbox, "GROW_API_KEY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "GROW_LEADER_SOCKET"), None);
        assert_eq!(env_value(&sandbox, "GROW_DISABLE_WEB_FETCH"), None);
        assert_eq!(env_value(&sandbox, "GROW_WEB_FETCH"), None);
    }
    ...
    fn command_application_clears_ambient_env_and_command_override_wins() {
        ...
        cmd.env("AMBIENT_SECRET", "must-disappear")
            .env("GROW_PROMPT_SUGGESTIONS", "ambient");
        sandbox.apply_to_std_command(&mut cmd);
        cmd.env("GROW_PROMPT_SUGGESTIONS", "command");
        ...
        assert_eq!(
            env.get(OsStr::new("GROW_PROMPT_SUGGESTIONS"))
                .map(OsString::as_os_str),
    ...
    fn explicit_overrides_win_and_can_remove_baseline_entries() {
        ...
        sandbox
            ...
            .set_env("GROW_PROMPT_SUGGESTIONS", "true")
            .set_env("NO_PROXY", "override.invalid")
            .remove_env("GROW_DISABLE_AUTOUPDATER");
        assert_eq!(
            env_value(&sandbox, "TERM_PROGRAM").as_deref(),
            Some(OsStr::new("vscode"))
        );
        assert_eq!(
            env_value(&sandbox, "GROW_PROMPT_SUGGESTIONS").as_deref(),
            Some(OsStr::new("true"))
        );
        assert_eq!(
            env_value(&sandbox, "NO_PROXY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "GROW_DISABLE_AUTOUPDATER"), None);
    }
    ...
    fn diagnostics_fail_closed_for_credential_keys() {
        ...
        for (key, value) in [
            ...
            ("GROW_DEPLOYMENT_KEY", "deployment-key-do-not-print"),
            ("GROW_EXTRA_AUTH_KEY", "alpha-test-key-do-not-print"),
