//! Headless mode (`grow -p`) test runner.
//!
//! Runs the grow binary as a subprocess with the mock server, captures output.
...
/// Timeout for one headless grow invocation: 60 seconds, multiplied by
/// [`crate::scaled`]'s `GROW_TEST_TIMEOUT_SCALE`.
...
mod tests {
    ...
    async fn custom_headless_env_is_explicit_and_wins_after_sandbox_baseline() {
        ...
        cmd.args([
            "-c",
            "printf '%s|%s|%s|%s' \"${AMBIENT_ONLY-unset}\" \"$GROW_PROMPT_SUGGESTIONS\" \"$FEATURE_TEST_VAR\" \"$HOME\"",
        ])
        .env("AMBIENT_ONLY", "discarded")
        .env("GROW_PROMPT_SUGGESTIONS", "command-level-discarded");

        let result = run_headless_in_sandbox_borrowed_with_env(
            cmd,
            &sandbox,
            &[
                ("GROW_PROMPT_SUGGESTIONS", "explicit-override"),
