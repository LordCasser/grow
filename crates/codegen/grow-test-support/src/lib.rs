...
//! Shared test utilities for grow-build crates: mock inference server, SSE
...
//! - [`GrowStdioClient`] — ACP client that drives `grow agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `grow agent --leader stdio` (unix)
...
//! - [`run_headless`] — Run `grow -p` against the mock server and capture output
//! - [`git_workdir`] — Create a git-initialized [`TestSandbox`]
//! - [`grow_binary`] — Resolve the grow binary path (GROW_BINARY env or cargo_bin)
...
/// Multiply a harness timeout by `GROW_TEST_TIMEOUT_SCALE` (positive integer,
/// default 1). CI lanes on shared runner pools raise it so pool load slows
/// tests instead of failing them (see the Grow merge CI workflow).
pub fn scaled(base: std::time::Duration) -> std::time::Duration {
    let scale = std::env::var("GROW_TEST_TIMEOUT_SCALE")
        .ok()
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{GrowStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, grow_binary};
pub use headless::{
