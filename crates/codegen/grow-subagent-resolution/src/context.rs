...
//!
//! Extracted from `grow-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `grow-chat-state::compaction_utils::strip_system_tags` which
...
/// `grow-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use grow_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(grow_sampling_types::synthesized_reasoning_item(text))
    }
