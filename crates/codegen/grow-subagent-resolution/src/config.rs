...
//! `grow_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `grow-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(persona.model.as_deref(), Some("grow-3-fast"));
