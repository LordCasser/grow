...
use grow_agent::config::{AgentDefinition, IsolationMode};
use grow_agent::plugins::PluginRegistry;
use grow_agent::prompt::context::{PromptAudience, PromptContext};
...
pub fn discover_agent_definition(
    ...
    grow_agent::discovery::by_name_in_cwd_with_plugins(subagent_type, context.cwd, context.plugins)
        .or_else(|| {
            context
                .cli_agents
                .iter()
...
pub fn available_agent_names(context: &DefinitionResolutionContext<'_>) -> Vec<String> {
    let mut available: Vec<String> = grow_agent::discovery::all_subagents_with_plugins(
        context.cwd,
        context.toggles,
        context.plugins,
    )
...
pub fn validate_agent_name(
    ...
    let resolves = context
        ...
        || grow_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            context.cwd,
            context.plugins,
        )
...
pub async fn render_subagent_initial_user_message(
    ...
    let agents_md_files = grow_agent::prompt::agents_md::read_agents_config_with_paths(
