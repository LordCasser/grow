//! ACP stdio clients for testing grow sessions end-to-end: the typed
//! [`GrowStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
/// Spawn `grow agent stdio` with the sandbox's canonical hermetic environment.
...
/// Drives `grow agent stdio` via the ACP protocol over pipes.
///
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init_resp
            ...
            .unwrap_or_else(|| {
                ...
                panic!(
                    "expected auth method 'provider.api_key' but got: {ids:?}\n\
...
/// Drives `grow agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`GrowStdioClient`] (`ClientSideConnection`,
