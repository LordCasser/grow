//! Leader-mode (`grow agent --leader stdio`) test harness.
//!
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&grow_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_base_url_timeout(
        ...
        let socket = sandbox.grow_home().join("leader.sock");
        let lock = sandbox.grow_home().join("leader.lock");
        ...
        cmd.envs(tty_utils::pager_env())
            .env("GROW_CLI_CHAT_PROXY_BASE_URL", base_url)
        ...
        let log_path = sandbox.grow_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent grow test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
...
impl LeaderStdioClient {
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                .label("grow leader stdio client")
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            .auth_methods
            .iter()
            .find(|method| &*method.id().0 == "provider.api_key")
            .expect("provider.api_key auth method");
        self.conn
            .authenticate(
    ...
    /// Count of `grow/models/update` notifications received (catalog self-heal).
    pub fn models_update_count(&self) -> u32 {
        self.capture.models_update_count.load(Ordering::SeqCst)
    }

    /// Count of `grow/settings/update` notifications received (settings self-heal).
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".grow").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".grow").join("leader.log")).unwrap_or_default()
}
