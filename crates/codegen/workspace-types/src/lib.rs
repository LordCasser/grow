...
//! and gives the JSON wire format an explicit discriminator.
//!
//! Wire-format struct fields use **snake_case** as the canonical Serde
//! contract.
//!
//! # Wire integer types
//!
...
//! something different. `u64` pins the wire width regardless of host. The existing
...
///
/// Lives in `workspace-types` (instead of `workspace` or
/// `mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `workspace::permission`
