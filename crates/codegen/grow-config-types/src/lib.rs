...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-grow-doom-loop-check` header and parse the reported
...
pub struct RemoteSettings {
    ...
    /// Controlled via remote settings (`non_git_warning` in `grow_build_settings`).
    ...
    /// ghost text), from the `grow_build_settings` remote settings flag. Sits below
    /// env (`GROW_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `grow-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `grow-imagine-image`) instead of the default quality model
    /// (`grow-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// `grow_build_settings`. Per-model override on `ModelInfo`
    /// (`grow_build_models`) takes precedence; user config and env var
    /// further override per the resolver chain.
    #[serde(default)]
    pub auto_compact_threshold_percent: Option<u8>,
    /// Max subagent nesting depth (`grow_build_settings.subagents_max_depth`).
    #[serde(default)]
    ...
    /// `0` disables. Env (`GROW_COMPACTION_WALL_CLOCK_SECS`) overrides it.
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "grow-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("grow-build"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "grow-build-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("grow-build-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
