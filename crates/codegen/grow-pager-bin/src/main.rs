...
use grow_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = grow_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !grow_shell::util::is_grow_process(pid) {
        ...
        if let Err(e) = grow_shell::util::kill_process_by_pid(pid) {
...
) -> Result<(LeaderDescriptor, grow_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = grow_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = grow_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&grow_shell::leader::ControlPayload>,
...
/// Env override for the `grow workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "GROW_WORKSPACE_COMMAND";
/// Resolution of the `grow workspace` gate. `Unknown` is kept separate from
...
/// The `GROW_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&grow_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for grow on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<grow_shell::util::config::RemoteSettings> {
    join_early_prefetch(grow_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = grow_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
            "`grow workspace` start/restart/resume is unavailable under sandbox profile '{profile}': \
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`grow workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `grow workspace`. Check your \
             network connection (run `grow login` if you are signed out), then \
             try again."
            )
        }
    }
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a grow session, or run `grow workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = grow_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use grow_shell::auth::ensure_authenticated;
    ...
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        grow_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
            "`grow workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.grow/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.auth,
        false,
        Some("No cached credentials found. Run `grow login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.auth);
...
async fn run_agent_command(
    agent_args: Box<grow_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            use grow_pager::app::signal_handler::next_signal_code;
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Grow - v{}",
            grow_version::display_version_with_commit(
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(grow_shell::sampling::types::parse_canonical_effort_token);
    ...
    agent_config.default_auto_mode = grow_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(grow_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = grow_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = grow_shell::managed_config::sync().await {
                                            tracing::warn!(
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !grow_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<grow_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(grow_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<grow_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(grow_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    grow_shell::heap_profile::install(grow_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "grow {}\n",
        grow_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), channel_label,)
    )
}
...
fn dispatch_version_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = write_version(&mut std::io::stdout().lock(), grow_update::channel_label()) {
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = grow_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    if let Some(code) = grow_pager::app::mermaid_worker::maybe_run_render_subprocess() {
    ...
    grow_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    grow_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        grow_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        grow_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    grow_pager::memory_trace::start(grow_shell::util::grow_home::grow_home().join("memtrace"));
    raise_fd_limit();
    if let Err(e) = grow_config::validate_requirements() {
    ...
    if grow_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = grow_shell::util::grow_home::grow_home().join("crash");
    ...
    let crashed = grow_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(Command::Completions { shell }) = &args.command {
        grow_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        grow_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        grow_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    grow_shell::config::apply_sandbox(None, sandbox_profile_arg.as_deref(), args.cwd.as_deref());
    ...
    grow_shell::http::set_client_name(if is_interactive {
        grow_workspace::permission::ClientType::GrowPager
    } else {
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": grow_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(&mut std::io::stdout().lock(), grow_update::channel_label())?;
                }
                return Ok(());
            }
            ...
            Command::Inspect { json } => {
                ...
                grow_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return grow_pager::mcp_cmd::run(mcp_args).await;
            }
            ...
            Command::Models => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return grow_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return grow_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
    ...
    let headless_prompt = grow_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(grow_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() && args.output_format == grow_pager::headless::OutputFormat::Plain
        {
            args.output_format = grow_pager::headless::OutputFormat::Json;
        }
        return grow_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            grow_pager::headless::HeadlessOptions {
    ...
    let result = grow_pager::app::run(args, bg_update_rx).await;
    grow_sandbox::flush();
    match result {
        Ok(true) => {
...
fn is_managed_install(exe: Option<std::path::PathBuf>, grow_home: &std::path::Path) -> bool {
    ...
    let managed = grow_config::grow_application_in(grow_home);
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in grow_shell::leader::discover_leaders().await {
        if d.classification != grow_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match grow_shell::leader::LeaderClient::connect(
            socket_path,
            "grow-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(grow_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(grow_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            grow_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: grow_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(grow_shell::heap_profile::stats().expect("shell stats after install"));
        if !require_opt_prof() {
            assert!(!grow_shell::heap_profile::prof_available());
            return;
        }
        assert!(grow_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(grow_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(grow_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        grow_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    ...
    #[serial_test::serial(GROW_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("GROW_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("GROW_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("GROW_WORKSPACE_COMMAND", "off") };
