...
//! Backend environment presets for the Grow CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Grow ships without a hosted service backend. Values are populated only by
//! explicit `GROW_*` environment overrides.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct GrowEndpoints {
...
const PRODUCTION_ENDPOINTS: GrowEndpoints = GrowEndpoints {
...
pub enum GrowEnvironment {
    #[default]
    Production,
}
impl GrowEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        GrowEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            GrowEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, GrowEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            GrowEnvironment::Production => "GROW_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> GrowEndpoints {
        match self {
            GrowEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `service.example.com/code` driving a
    ...
    /// `GROW_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for GrowEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            GrowEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_env_prefix() {
        assert_eq!(GrowEnvironment::Production.env_prefix(), "GROW_PRODUCTION");
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_GROW_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    /// loop mistakenly connecting to `wss://service.example.com/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            GrowEnvironment::Production.relay_ws_url(),
            GrowEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            GrowEnvironment::from_flags(false, false),
            GrowEnvironment::Production
        );
    }
}
