//! Headless markdown analysis sharing Grow's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `grow-markdown` so analysis matches what Grow
//! Build actually renders 1:1.
//!
//! After parsing, Grow applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Grow uses to render markdown.
///
...
/// Offset event stream from Grow's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Grow's options; count elements and flag structural issues.
