...
pub use grow_config::grow_home;

/// Path to `$GROW_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    grow_home().join("pager.toml")
}

/// User-facing label for the user grow directory (``~/.grow`` or ``$GROW_HOME``).
///
/// Derived from resolved [`grow_home()`] vs `grow_config::default_grow_home()`,
/// not from whether `GROW_HOME` is set in the environment.
pub fn display_grow_home_prefix() -> String {
    display_grow_home_prefix_for(&grow_home())
}

fn display_grow_home_prefix_for(home: &Path) -> String {
    if home == grow_config::default_grow_home() {
        "~/.grow".to_string()
    } else {
        "$GROW_HOME".to_string()
    }
...
/// User-facing path under [`grow_home()`], e.g. ``~/.grow/config.toml``.
pub fn display_user_grow_path(relative: impl AsRef<Path>) -> String {
    display_user_grow_path_for(&grow_home(), relative)
}

fn display_user_grow_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_grow_home_prefix_for(home);
...
/// Abbreviate an absolute path for display: prefer [`grow_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let grow = grow_home();
    if let Ok(rest) = path_buf.strip_prefix(&grow) {
        let prefix = display_grow_home_prefix();
...
/// True when `path` is under user [`grow_home()`] (not project `{cwd}/.grow`).
pub fn is_under_user_grow_home(path: &Path) -> bool {
...
mod tests {
    ...
    #[test]
    fn display_grow_home_prefix_default_install() {
        if std::env::var("GROW_HOME").is_ok() {
            return;
        }
        assert_eq!(display_grow_home_prefix(), "~/.grow");
    }

    #[test]
    fn display_user_grow_path_joins_relative() {
        let path = display_user_grow_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".grow") || path.contains("$GROW_HOME"));
    }

    #[test]
    fn display_user_grow_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("grow-home-display-regression");
        assert_eq!(
            display_user_grow_path_for(&custom, "config.toml"),
            "$GROW_HOME/config.toml"
        );
        assert_eq!(
            display_user_grow_path_for(&custom, "sandbox.toml"),
            "$GROW_HOME/sandbox.toml"
        );
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_grow() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.grow/memory/MEMORY.md");
