...
/// Documented in `grow-pager/docs/internal/22-environment-variables.md`.
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(grow_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(grow_shared::clipboard::is_containerized_without_display)
}
...
/// clipboard (see `grow-pager`'s `pty_wrap` module). It advertises this to
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if grow_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = grow_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = grow_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`grow_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        grow_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = grow_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match grow_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
/// Write text and return a toast; emits `grow-shell-clipboard_copy` when enabled.
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    ...
    grow_config::user_grow_home().map(|grow_home| grow_home.join("last-copy.txt"))
}
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    grow_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !grow_shared::clipboard::x11_display_env_present() {
        return None;
    }
    grow_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && grow_shared::clipboard::x11_display_env_present()
    }
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match grow_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use grow_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    grow_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    grow_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    grow_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(grow_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match grow_shared::clipboard::get_image() {
        Ok(img) => {
...
mod tests {
    ...
    fn default_copy_fallback_path_is_grow_home() {
        ...
        let expected = grow_config::user_grow_home()
    ...
    /// the `GROW_HOME`-override integration test in `grow-pager`).
    #[test]
