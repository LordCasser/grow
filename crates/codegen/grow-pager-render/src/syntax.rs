...
//! Dark themes (GrowNight, TokyoNight) share `grow-night.tmTheme`;
//! GrowDay uses `grow-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `GrowNight` (so leftover kind-keyed paths
...
static SYNTECT_GROWNIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_GROWDAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `GrowNight`, so this returns the night theme. Token
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_GROWNIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/grow-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::GrowDay => SYNTECT_GROWDAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/grow-day.tmTheme"))),
    }
}
