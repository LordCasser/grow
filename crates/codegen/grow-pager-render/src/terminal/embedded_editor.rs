//! Detects whether grow is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` grow is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `grow-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
