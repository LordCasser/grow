...
//! lookup must be cheaper than re-loading from `~/.grow/config.toml`.
...
//! `grow_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::GrowNight as u8);
...
/// back to `GrowNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::GrowNight as u8 => ThemeKind::GrowNight,
        x if x == ThemeKind::GrowDay as u8 => ThemeKind::GrowDay,
...
/// When `None`, `to_theme_kind()` defaults to `GrowNight` / `GrowDay`.
...
///
/// On the first call, reads from `~/.grow/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    grow_markdown::set_color_level_cap(if locked {
        grow_markdown::ColorLevel::Basic
    } else {
        grow_markdown::ColorLevel::TrueColor
    });
    grow_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`GROW_THEME`)
...
/// and the user's dark/light theme mapping. Falls back to `GrowNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `grow_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = grow_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = grow_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::GrowNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::GrowDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::GrowNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::GrowDay);
            assert_eq!(applied, ThemeKind::GrowNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::GrowNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::growday().text_primary,
                "must not serve the cached (GrowDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!grow_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                grow_markdown::polarity_safe_syntax(),
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // growday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::GrowDay), false),
                ThemeKind::GrowDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::GrowNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_grownight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::GrowNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_growday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::GrowDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_grownight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_grownight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::GrowDay), true);
            assert_eq!(result, ThemeKind::GrowDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::GrowDay);
            assert_eq!(current_kind(), ThemeKind::GrowDay);
        });
    }
