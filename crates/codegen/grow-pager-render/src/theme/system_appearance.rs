...
/// in `config.toml`. When `None`, defaults to `GrowNight` / `GrowDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::GrowDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::GrowNight),
    }
}
