...
/// matches dot-directories and `%` matches percent-encoded segments — grow
/// session media lives under `~/.grow/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.grow/x.txt`) — one or more `/`-joined
...
mod tests {
    ...
    fn scan_content_x_offset_applied() {
        let line = make_line("https://example.com");
        let mut overlay = LinkOverlay::new();
    ...
    fn scan_detects_grow_session_media_path() {
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.grow/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.grow/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.grow/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.grow/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/grow-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/grow-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/grow-pager/src/render` should
        // NOT produce a link for the `/grow-pager/src/render` substring.
        let line = make_line("find crates/codegen/grow-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
