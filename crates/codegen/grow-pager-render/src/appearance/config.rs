...
pub struct AnimationConfig {
    ...
    /// Also enabled by the `GROW_FPS=1` env var. Default: false.
    pub show_fps: bool,
}
...
pub struct RawAnimationConfig {
    ...
    /// Requires a debug build. Also enabled by GROW_FPS=1 env var. Default: false.
    pub show_fps: bool,
}
...
fn lookup_named_color(name: &str) -> Result<Color, String> {
    // Named colors use the GrowNight RGB palette. They are quantized via
...
impl RawAppearanceConfig {
    pub fn to_toml_with_comments() -> String {
        ...
        let pager_path = crate::util::display_user_grow_path("pager.toml");
        let header = format!(
            "\
# Grow Pager Appearance Configuration ({pager_path})
...
pub fn persist_respect_manual_folds(enabled: bool) -> std::io::Result<()> {
    ...
    if grow_config::user_grow_home().is_none() {
        return Err(Error::new(
            ErrorKind::NotFound,
            "no user grow home resolved; refusing to write a cwd-relative pager.toml \
