...
/// Scrub stale grow-owned registrations from each known source repo,
/// scoped to worktrees under the grow home to prove ownership (see
...
fn prune_stale_git_worktree_registrations(repos: &BTreeSet<PathBuf>) -> u64 {
    let Ok(grow_home) = resolve_grow_home() else {
        tracing::warn!("auto worktree registration scrub skipped: grow home unresolved");
        return 0;
    };
    let cleaned: u64 = repos
        .iter()
        .filter(|repo| repo.is_dir())
        .map(|repo| crate::git::remove_stale_worktree_registrations_under(repo, &grow_home))
...
mod tests {
    ...
    #[test]
    fn prune_keeps_foreign_registrations_outside_grow_home() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::GrowHomeFixture::new();
        let db = WorktreeDb::open(&fx.home).unwrap();
        ...
        assert_eq!(
            count_regs(&repo),
            before,
            "user registrations outside grow home must survive"
        );
    }
