...
use pager::views::prompt_widget::{PromptBg, PromptStyle, PromptWidget};
...
fn active(prompt: &PromptWidget, items_width: u16) -> Option<(Kind, u16)> {
    use pager::views::completion_dropdown::MAX_VISIBLE_ROWS;
    use pager::views::file_search::dropdown::MAX_DROPDOWN_ROWS as FILE_MAX;
    use pager::views::slash_dropdown::desired_item_rows;
...
pub fn render(
    ...
    match kind {
        Kind::FileSearch => {
            pager::views::file_search::dropdown::render_dropdown(
                buf,
                items_rect,
                &prompt.file_search,
                theme,
        ...
        Kind::Slash => {
            ...
            pager::views::slash_dropdown::render_dropdown(buf, items_rect, &snap, hovered, theme);
        }
        Kind::Completion => {
            pager::views::completion_dropdown::render_dropdown(
                buf,
                items_rect,
...
pub fn modal_height(modal: Modal, agent: &mut AgentView, screen_h: u16, content_w: usize) -> u16 {
    match modal {
        Modal::Permission => agent
            .permission_queue
            .front()
            .map(|p| pager::views::permission_view::permission_view_height(p, screen_h, content_w))
            .unwrap_or(0),
        Modal::Question => {
            let input_mode = minimal_api::question_view(agent).is_some_and(|qv| {
                qv.focus == pager::views::question_view::QuestionFocus::InputMode
            });
            ...
            minimal_api::question_view_mut(agent)
                .map(|qv| {
                    pager::views::question_view::question_view_height(qv, screen_h, content_w)
                        .saturating_add(editor_extra)
                })
                .unwrap_or(0)
        }
        Modal::Rewind => minimal_api::rewind_state(agent)
            .map(|rw| pager::views::rewind::rewind_overlay_height(&rw.phase, screen_h))
            .unwrap_or(0),
        Modal::Cancel => {
            if minimal_api::cancel_turn_view(agent).is_some() {
                pager::views::modal::cancel_turn_panel_height(screen_h)
            } else {
                0
            }
        }
    ...
    match modal {
        ...
        Modal::Rewind => {
            if let Some(rw) = minimal_api::rewind_state(agent) {
                pager::views::rewind::render_rewind_overlay(buf, area, &rw.phase, true);
            }
            None
        }
        Modal::Cancel => {
            ...
            let drawn = if let Some(ctv) = minimal_api::cancel_turn_view(agent) {
                pager::views::modal::render_cancel_turn_panel(buf, area, ctv, true, &mut buttons);
                true
            } else {
                false
            };
...
fn render_question(
    ...
    use pager::views::question_view::{QUESTION_VIEW_HPAD, QuestionFocus};
    ...
    if let Some(qv) = minimal_api::question_view_mut(agent) {
        let vis = qv.questions.get(qv.active_tab).map(|question| {
            pager::views::question_view::visible_options_height(
                question,
                q_area.height,
                content_w,
                qv.focused_preview(),
    ...
    if let Some(qv) = minimal_api::question_view(agent) {
        pager::views::question_view::render_question_view(
            buf,
            q_area,
            qv,
    ...
    if input_mode {
        ...
        let freeform_sel = minimal_api::question_view(agent)
            ...
            .copied()
            .unwrap_or(false);
        let embed = pager::views::modal_window::embedded_row_style(theme, true);
        ...
        let marker = if is_multi {
            ...
            format!("({})", pager::glyphs::filled_dot())
        } else {
            "(\u{25cb})".to_string()
        };
        ...
        buf.set_span_safe(
            area.x + 9,
            row_y,
            &ratatui::text::Span::styled(pager::glyphs::prompt_arrow(), accent),
            2,
        );
        ...
        let editor = Rect {
            x: area.x + 11,
            y: row_y,
            width: pager::views::question_view::inline_text_width(area.width),
            height: input_h,
        };
        return agent
            .prompt
...
fn question_editor_h(agent: &AgentView, area_w: u16, cap: u16, theme: &Theme) -> u16 {
    let text_w = pager::views::question_view::inline_text_width(area_w);
    agent
        .prompt
...
mod tests {
    ...
    fn question_input_agent(text: &str) -> AgentView {
        use pager::views::prompt_widget::StashedPrompt;
        use pager::views::question_view::{Question, QuestionOption, QuestionViewState};
