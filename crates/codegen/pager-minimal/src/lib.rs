...
//! terminal's *native* scrollback (via `ratatui_inline::Terminal::insert_before_rows`,
...
//! # Wiring
//!
//! `pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`cli`) calls [`install`] once at
