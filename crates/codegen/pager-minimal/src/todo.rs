...
pub(super) fn todo_panel_height(agent: &pager::app::agent_view::AgentView, force: bool) -> u16 {
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    pager::glyphs::ballot_x(),
