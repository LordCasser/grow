...
mod imp {
    ...
    pub fn install(crash_dir: &Path, app_version: &str) -> bool {
        ...
        unsafe {
            ...
            version_buffer[..copy_len].copy_from_slice(&app_version.as_bytes()[..copy_len]);
        }
...
mod win {
    ...
    pub fn install(crash_dir: &Path, version: &str) -> bool {
...
mod tests {
    ...
    fn full_install_replaces_minimal_handler() {
        ...
        let dir = std::env::temp_dir().join("crash-handler-test-replace");
    ...
    fn install_creates_owner_only_crash_blob() {
        ...
        let _guard = SIGNAL_STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir =
            std::env::temp_dir().join(format!("crash-handler-test-0600-{}", std::process::id()));
    ...
    fn install_tightens_preexisting_0644_crash_blob() {
        ...
        let _guard = SIGNAL_STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir =
            std::env::temp_dir().join(format!("crash-handler-test-tighten-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("create crash dir");
