...
//! `sampling_types::SamplingConfig` so that the sampler crate
...
/// `session::actor::SessionActor::reconstruct_full_config`) by
/// composing chat-state's `sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers (e.g. `X-Grow-Token-Auth` for the
/// cli-chat-proxy) are
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-grow-doom-loop-check` header on
