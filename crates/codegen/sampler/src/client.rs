...
use sampling_types::{
    ...
    Result, SamplingError, SentCredential, build_messages_request, is_check_event, messages, rs,
};
...
pub use sampling_types::ApiBackend;
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = sampling_types::reported_cost_ticks(
        value
...
impl SamplingClient {
    ...
    pub async fn create_response(
        ...
        sampling_types::patch_reasoning_text_types(&mut request_body);
        let SentRequest {
            builder,
            sent_bearer,
    ...
    /// gate that adds the opt-in `x-grow-doom-loop-check` request header, so
    ...
    pub async fn conversation_collect(
        ...
        result
            ...
            .map_err(|info| crate::events::sampling_error_from_info(&info))
    }
}
...
mod tests {
    use super::*;
    use indexmap::IndexMap;
    use sampling_types::types::ChatRequestMessage;
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the shell agent product.
        assert!(ua.starts_with("my-client grow-shell/"));
    }
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "grow-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
