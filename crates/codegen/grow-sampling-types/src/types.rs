...
// Re-export ToolDefinition and FunctionTool from grow-tools.
...
pub use grow_tools::types::definition::{FunctionTool, ToolDefinition};
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "grow" }).as_object().unwrap()))
                .is_none()
        );
    }
