...
// engine in `crates/common/grow-compaction` operate over grow-build's
...
impl grow_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> grow_compaction::CompactionRole {
        use grow_compaction::CompactionRole;
    ...
    fn attachment_refs(&self) -> Vec<grow_compaction::CompactionFileRef> {
...
impl grow_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/grow-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`grow-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/grow-sampler/src/stream/chat_completions.rs])
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The grow-shell agent does NOT use this path — a
...
/// ([crates/codegen/grow-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use grow_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
