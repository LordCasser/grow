...
//!
//! TODO: Move from grow-shell/src/sampling/error.rs
...
pub fn status_user_message(status: StatusCode) -> String {
    match status.as_u16() {
        code @ 502..=504 => {
            format!("Grow is temporarily unavailable. Please try again in a moment. (HTTP {code}).")
        }
        // Cloudflare edge codes (origin down / connect fail / timeout / …).
        code @ 520..=524 => {
            format!(
                "Connection to Grow timed out or was interrupted. Please try again. (HTTP {code})."
            )
        }
...
<html lang="en-US">
<head><title>service.example.com | 524: A timeout occurred</title></head>
