//! Pure resolution logic for `grow plugin install <name>` marketplace refs.
...
pub struct BareNameSelection {
    ...
    pub chosen: usize,
    /// How many other copies of the name exist (non-zero only when featured
...
pub enum BareNameError {
    NotFound,
    /// Several sources provide the name and none is uniquely featured; payload
...
///
/// One match wins outright. With several matches, a single featured-source copy
...
pub fn select_bare_name(
    ...
    match matched.as_slice() {
        ...
        _ => {
            let featured: Vec<usize> = matched
                .iter()
                .copied()
                .filter(|&index| scanned[index].source.featured)
                .collect();
            match featured.as_slice() {
                [index] => Ok(BareNameSelection {
                    chosen: *index,
...
mod tests {
    ...
    fn featured_git_source(name: &str, url: &str) -> MarketplaceSource {
        MarketplaceSource {
            featured: true,
            ..git_source(name, url)
        }
    ...
    fn slugify_lowercases_and_hyphenates_spaces() {
        ...
        assert_eq!(slugify("Featured Source"), "featured-source");
    }
    ...
    fn addressable_qualifier_git_and_local() {
        assert_eq!(
            addressable_qualifier(&git_source(
                "x",
                "https://github.com/example/plugin-marketplace.git"
            )),
    ...
    #[test]
    fn bare_name_featured_priority_when_duplicate() {
        let pairs = [
            (
            ...
            (
                featured_git_source("Preferred", "https://github.com/example/preferred.git"),
                entry("sentry"),
            ),
        ];
        let scanned = scanned_entries(&pairs);
    ...
    #[test]
    fn bare_name_ambiguous_when_no_featured_match() {
        let pairs = [
            (
    ...
    #[test]
    fn bare_name_ambiguous_when_more_than_one_featured_match() {
        let pairs = [
            (
                featured_git_source("Preferred A", "https://github.com/example/a.git"),
                entry("sentry"),
            ),
            (
                featured_git_source("Preferred B", "https://github.com/example/b.git"),
                entry("sentry"),
            ),
        ];
        let scanned = scanned_entries(&pairs);
