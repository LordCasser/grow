...
//! field rename/retype in `grow-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use grow_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "Grow:grep".to_owned(),
...
fn finalize_request_minimal() -> grow_tools_api::FinalizeToolServerConfigRequest {
    grow_tools_api::FinalizeToolServerConfigRequest {
        tools: vec![],
        truncation: None,
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry = serde_json::from_value(serde_json::json!({"id": "Grow:read_file"}))
        .expect("deserialize minimal");
    assert_eq!(back.id, "Grow:read_file");
...
fn finalize_request_callback_fields_default_when_absent() {
    let back: grow_tools_api::FinalizeToolServerConfigRequest =
