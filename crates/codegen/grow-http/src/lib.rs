...
//! first built, and `GROW_SAMPLER_SHARED_CLIENT=0` falls back to
...
/// Process-level client identifier (`GROW_CLIENT_NAME` env var, default `"grow-shell"`).
...
/// (`grow -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-grow-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `grow -p`, `run_headless_inner` for
/// `grow agent [headless]`), `"interactive"` otherwise.
...
mod tests {
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(without_version.starts_with("grow-web grow-shell/"));
        assert!(!without_version.starts_with("grow-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "grow-shell/0.1.171 (macos; aarch64)");
    }
}
