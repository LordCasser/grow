...
use grow_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (grow_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    grow_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use grow_shell::tools::{TodoItem, TodoPriority};
