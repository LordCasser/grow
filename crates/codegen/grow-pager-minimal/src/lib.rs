//! Minimal (scrollback-native) render mode — `grow --minimal`.
//!
...
//! # Wiring
//!
//! `grow-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`grow_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`grow-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `grow-pager`.
///
...
/// subsequent calls are ignored (see [`grow_pager::minimal_hook`]).
pub fn install() {
    grow_pager::minimal_hook::install(grow_pager::minimal_hook::MinimalHooks { draw });
}
