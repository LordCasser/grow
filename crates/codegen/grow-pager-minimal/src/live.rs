...
use grow_pager::app::PagerTerminal;
use grow_pager::app::app_view::{ActiveView, AppView};
use grow_pager::minimal_api;
use grow_pager::render::Renderable;
use grow_pager::scrollback::state::ScrollbackState;
use grow_pager::scrollback::wrappers::EntryRenderer;
use grow_pager::theme::Theme;
use grow_pager::views::prompt_widget::{PromptBg, PromptStyle};
use grow_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &grow_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut grow_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        grow_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub(super) fn prompt_style(
    appearance: &grow_pager::appearance::AppearanceConfig,
    input_mode: grow_pager::app::agent_view::PromptInputMode,
    theme: &Theme,
    multiline: bool,
) -> PromptStyle {
    PromptStyle {
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    grow_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                grow_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
            grow_pager::views::btw_overlay::btw_panel_height(agent.btw_state.as_ref(), area.width)
        } else {
            0
        };
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            grow_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(grow_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a grow_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &grow_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_advance_phase_timer(
    agent: &mut grow_pager::app::agent_view::AgentView,
) -> Option<grow_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &grow_pager::app::agent_view::AgentView,
    activity: &Option<grow_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = grow_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = grow_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use grow_pager::views::context_bar::fmt_tokens;
...
fn minimal_pending_hint(
    pending: &Option<grow_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> grow_pager::app::agent_view::AgentView {
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use grow_pager::app::agent::AgentState;
        use grow_pager::scrollback::RenderBlock;
        use grow_pager::scrollback::entry::ScrollbackEntry;
        use grow_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &grow_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use grow_pager::acp::tracker::TurnActivity;
        use grow_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        grow_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        grow_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            grow_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_style_bash_mode_shows_bang_prefix() {
        use grow_pager::app::agent_view::PromptInputMode;
        use grow_pager::appearance::AppearanceConfig;
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(grow_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &grow_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use grow_pager::app::app_view::PendingAction;
        use grow_pager::input::key::KeyShortcut;
