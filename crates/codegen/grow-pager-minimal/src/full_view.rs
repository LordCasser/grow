...
use grow_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = grow_pager::appearance::cache::load_show_thinking_blocks();
        ...
        grow_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: grow_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(grow_pager::scrollback::block::RenderBlock::system(
    ...
    match std::fs::write(&path, out) {
        ...
        Err(e) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                agent
                    .scrollback
                    .push_block(grow_pager::scrollback::block::RenderBlock::system(format!(
...
mod tests {
    use super::*;
    use grow_pager::scrollback::block::RenderBlock;
    ...
    fn transcript_expands_thinking_committed_collapsed() {
        let theme = Theme::current();
        let appearance =
            super::super::commit::committed_appearance(&grow_pager::appearance::AppearanceConfig {
