...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> sandbox::ProfileName {
    sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_profile_and_bwrap_reexec(profile: &sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !sandbox::is_inside_bwrap() {
            ...
            match sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_grow_home: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    ...
    let profile = sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
