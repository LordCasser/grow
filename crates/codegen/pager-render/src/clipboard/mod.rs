...
//! `ratatui-textarea`, and adds [`SystemClipboard`] backed by `arboard`.
//!
...
/// Documented in `pager/docs/internal/22-environment-variables.md`.
pub const GROW_COPY_FILE_ENV: &str = "GROW_COPY_FILE";

/// Cached result of the remote-session check (env vars don't change at runtime).
fn is_remote() -> bool {
    static REMOTE: OnceLock<bool> = OnceLock::new();
    *REMOTE.get_or_init(client_support::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(client_support::clipboard::is_containerized_without_display)
}

/// Cached result of the "an upstream OSC 52 sink is capturing our output" check.
///
/// `grow wrap` runs a command inside a local PTY, scans its output for OSC 52
/// clipboard sequences, and writes their payload to the *real* (local) system
/// clipboard (see `pager`'s `pty_wrap` module). It advertises this to
/// the wrapped program via an environment variable so the
/// inner `grow` knows its OSC 52 writes are reliably intercepted and copied,
...
///
/// Two names are accepted: the canonical `GROW_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_GROW_OSC52_SINK`, which the default
/// OpenSSH client/server configs forward (`SendEnv LANG LC_*` /
/// `AcceptEnv LANG LC_*`) so the signal survives the hop into a remote `grow`.
pub fn osc52_sink_active() -> bool {
    static SINK: OnceLock<bool> = OnceLock::new();
    *SINK.get_or_init(|| {
        std::env::var_os("GROW_OSC52_SINK").is_some()
            || std::env::var_os("LC_GROW_OSC52_SINK").is_some()
    })
}

/// Kill switch: never emit OSC 52 clipboard sequences.
///
/// Set `GROW_CLIPBOARD_NO_OSC52` (any value) before starting Grow. Presence
/// forces the OSC 52 leg off for the whole process — including Linux "always
/// emit", tmux, SSH, container, and `GROW_OSC52_SINK` paths. Use this when the
/// host terminal paints OSC 52 payloads as visible garbage (e.g. OpenText
/// Exceed and other non-supporting emulators).
///
/// Same convention as `GROW_CLIPBOARD_NO_DATA_CONTROL`: env presence enables
/// the kill switch; resolved once and cached for the process lifetime.
pub fn osc52_disabled() -> bool {
    static DISABLED: OnceLock<bool> = OnceLock::new();
    *DISABLED.get_or_init(|| std::env::var_os("GROW_CLIPBOARD_NO_OSC52").is_some())
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if client_support::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
/// matters for non-tmux contexts — unless `GROW_CLIPBOARD_NO_OSC52` is set,
...
fn resolve_clipboard_route_with(ctx: &TerminalContext, no_osc52: bool) -> ClipboardRoute {
    ...
    // upstream `grow wrap` sink is capturing our output and will forward
    // the sequence to the real clipboard.
    // `GROW_CLIPBOARD_NO_OSC52` wins over every automatic path.
    let osc52 = !no_osc52
        && (cfg!(target_os = "linux")
            || is_tmux
            || is_remote()
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = client_support::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = client_support::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`client_support::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        client_support::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = client_support::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match client_support::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
impl ClipboardFeedback {
    ...
    fn message(self) -> &'static str {
        match self {
            ...
            Self::UnverifiedOscRemote | Self::UnverifiedOscContainer => {
                "Copy sent. If paste fails, use grow wrap or /minimal."
            }
...
/// Write text and return a toast; emits `shell-clipboard_copy` when enabled.
...
///
/// Override with [`GROW_COPY_FILE_ENV`] (supports `~`). Otherwise
/// `~/.grow/last-copy.txt` (grow's per-user home — short, stable, and
/// readable in a toast, unlike macOS's `/var/folders/...` temp dir).
///
/// `None` when no grow home resolves and the env var is unset: rather than
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    if let Ok(raw) = std::env::var(GROW_COPY_FILE_ENV) {
    ...
    config::user_grow_home().map(|grow_home| grow_home.join("last-copy.txt"))
}

/// Render a backup-file path for user-facing messages using the codebase-wide
/// abbreviation convention ([`crate::util::abbreviate_path`]): a grow-home
/// prefix collapses to `~/.grow` (or `$GROW_HOME` when overridden), and a
...
/// `GROW_COPY_FILE`) — the backup file is skipped rather than written to a
/// predictable temp location.
///
/// On Unix a missing parent directory is created `0700` (a custom
/// `GROW_COPY_FILE` may point at a not-yet-created private directory;
/// `~/.grow` normally already exists).
pub fn write_copy_fallback(text: &str) -> std::io::Result<std::path::PathBuf> {
    let Some(path) = default_copy_fallback_path() else {
        return Err(std::io::Error::new(
            ...
            "no home directory resolves; set GROW_COPY_FILE to enable the copy backup file",
        ));
    };
    #[cfg(unix)]
...
/// clipboard over SSH (notably Apple Terminal without `grow wrap`); a failed
...
fn log_clipboard_copy_event(
    ...
    diagnostics::session_ctx::log_event(diagnostics::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    client_support::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !client_support::clipboard::x11_display_env_present() {
        return None;
    }
    client_support::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && client_support::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    diagnostics::session_ctx::log_event(diagnostics::events::PasteKeyEmptyHostClipboard {
        terminal,
...
fn log_clipboard_paste_event(
    ...
    diagnostics::session_ctx::log_event(diagnostics::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().diagnostics_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match client_support::clipboard::get_attachments() {
        Ok(att) => {
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(client_support::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match client_support::clipboard::get_image() {
        Ok(img) => {
...
mod tests {
    ...
    fn clipboard_route_matrix() {
        ...
        for case in cases {
            // Pure helper with kill switch off so ambient GROW_CLIPBOARD_NO_OSC52
    ...
    fn clipboard_route_no_osc52_kill_switch_forces_off() {
        // GROW_CLIPBOARD_NO_OSC52 must win over Linux/tmux/SSH automatic emit.
        for ctx in [
            plain_terminal_ctx(),
            plain_tmux_ctx(),
            byobu_tmux_ctx(),
    ...
    /// the file must be owner-only (`0600`) — including when an older grow
    ...
    #[test]
    #[serial_test::serial(grow_copy_file)]
    fn default_copy_fallback_path_respects_grow_copy_file() {
        let dir = tempfile::tempdir().expect("tempdir");
        let custom = dir.path().join("custom-copy.txt");
        // SAFETY: test-only env mutation; serialized on the grow_copy_file key.
        unsafe {
            std::env::set_var(GROW_COPY_FILE_ENV, &custom);
        }
        let resolved = default_copy_fallback_path();
        unsafe {
            std::env::remove_var(GROW_COPY_FILE_ENV);
        }
    ...
    /// Without `GROW_COPY_FILE`, the default is `~/.grow/last-copy.txt`
    /// (grow home) — short and toast-friendly, unlike macOS's temp dir.
    #[test]
    #[serial_test::serial(grow_copy_file)]
    fn default_copy_fallback_path_is_grow_home() {
        unsafe {
            std::env::remove_var(GROW_COPY_FILE_ENV);
        }
        let path = default_copy_fallback_path();
        // Test envs always resolve a home (or set GROW_HOME).
        let expected = config::user_grow_home()
            .expect("home resolves in tests")
            .join("last-copy.txt");
        assert_eq!(path, Some(expected));
    }

    /// Toast paths collapse the home prefix to `~` (grow-home paths go
    /// through the shared `abbreviate_path` convention, covered further by
    /// the `GROW_HOME`-override integration test in `pager`).
    #[test]
    fn display_copy_path_abbreviates_home() {
        if std::env::var_os("GROW_HOME").is_none() {
            let home = dirs::home_dir().expect("home resolves in tests");
            assert_eq!(
                display_copy_path(&home.join(".grow").join("last-copy.txt")),
                "~/.grow/last-copy.txt"
            );
        }
        ...
        assert_eq!(
            display_copy_path(std::path::Path::new("/tmp/grow-0/last-copy.txt")),
            "/tmp/grow-0/last-copy.txt"
        );
        assert_eq!(
    ...
    fn delivery_clipboard_success_carries_backup_file() {
        let path = std::path::PathBuf::from("/tmp/grow-1/last-copy.txt");
    ...
    fn toast_message_names_backup_only_for_unverified_or_file_fallback() {
        ...
        assert_eq!(
            ...
            "Copy sent — saved to /tmp/grow-1/last-copy.txt"
        );
        ...
        assert_eq!(
            ...
            "Clipboard unreachable — wrote /tmp/grow-1/last-copy.txt"
        );
