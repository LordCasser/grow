...
mod tests {
    ...
    /// Persistence retains the exact source bytes and MIME identity.
    #[test]
    fn persisted_image_retains_bytes_and_mime() {
        let dir = tempfile::tempdir().unwrap();
        let payload = b"\x89PNG\r\n\x1a\nround-trip-bytes-12345".to_vec();
        let mut img = make_persistable_image(payload.clone());
        persist_to_session(&mut img, dir.path()).expect("persist");
        let bytes = std::fs::read(img.session_image_path.as_ref().expect("saved path")).unwrap();
