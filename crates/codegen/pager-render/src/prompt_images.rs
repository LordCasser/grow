...
use client_support::clipboard::mime_to_extension;
...
///
/// Delegates to [`client_support::clipboard::mime_to_extension`].
...
pub fn session_images_dir(
    ...
    let sid = session_id?;
    let info = client_support::session::info::Info {
        id: sid.clone(),
        cwd: cwd.to_string_lossy().into_owned(),
    };
    Some(client_support::session::session_dir(&info).join("images"))
}
...
pub fn session_mermaid_dir(
    ...
    Some(client_support::session::session_dir(&info).join("mermaid"))
}
...
///   [`shell::session::placeholder_images::load_placeholder_image`]
...
pub fn build_content_blocks_with_prefixes(
    ...
    build_content_blocks_with_prefixes_and_caps(
        ...
        images,
        allowed_prefixes,
        client_support::placeholder_images::MAX_PLACEHOLDER_AGGREGATE_BYTES,
    )
}
...
/// [`shell::session::placeholder_images::recover_orphan_placeholders_with_prefixes_and_caps`].
...
pub fn build_content_blocks_with_prefixes_and_caps(
    ...
    // attachment.
    let rewritten_text =
        client_support::placeholder_images::strip_paths_from_image_placeholders(rewritten_text);
    ...
    for img in &images {
        ...
        blocks.push(ContentBlock::Image(
            ImageContent::new(data, mime_type)
                ...
                .meta(Some(
                    client_support::placeholder_images::display_number_meta(img.display_number),
                )),
        ));
    }
...
fn resolve_orphan_placeholders(
    ...
    let placeholders = client_support::placeholder_images::extract_placeholders(&text);
    ...
    for ph in &placeholders {
        ...
        match client_support::placeholder_images::load_placeholder_image(&ph.path, allowed) {
            Ok(loaded) => {
                ...
                recovered.push(
                    ImageContent::new(data, loaded.mime_type)
                        ...
                        .meta(Some(
                            client_support::placeholder_images::display_number_meta(
                                ph.display_number,
                            ),
                        )),
                );
...
mod tests {
    ...
    fn file_url_single_slash_rejected_at_anchor_gate() {
        ...
        // URL parsing runs.
        let pasted = "file:/tmp/should_not_exist_abc_pager.png";
    ...
    fn dropped_path_nonexistent_bare_path_not_intercepted() {
        ...
        let entries = dropped_paths("/tmp/definitely_does_not_exist_xyz_pager.txt");
        assert!(
            entries.is_empty(),
    ...
    fn build_blocks_sets_display_number_meta() {
        ...
        assert_eq!(
            client_support::placeholder_images::display_number_from_meta(ic.meta.as_ref()),
            Some(3),
