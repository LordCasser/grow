...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("GROW_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "GROW_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
...
///
/// The returned command is TTY-guarded via [`tty_utils::detach_std_command`]
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/grow session/image 1.jpg");
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://service.example.com/provider_plan?referrer=grow-build";
