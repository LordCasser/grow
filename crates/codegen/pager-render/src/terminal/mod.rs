...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` grow is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches grow, which
    ...
    pub fn diagnostics_snapshot(&self) -> diagnostics::events::TerminalDiagnostic {
        ...
        diagnostics::events::TerminalDiagnostic {
            ...
            clipboard_native_tool: client_support::clipboard::native_tool_name().to_owned(),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `grow ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// Parsed from `[terminal] alt_screen` in `~/.grow/pager.toml` and
