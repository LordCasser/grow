...
mod tests {
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/pager/src/render` should
        // NOT produce a link for the `/pager/src/render` substring.
        let line = make_line("find crates/codegen/pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
