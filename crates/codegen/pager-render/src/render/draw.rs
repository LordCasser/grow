...
//! [`ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
pub fn spawn_writer_thread() -> (
    ...
    let test_delay = std::env::var("GROW_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || -> std::io::Result<()> {
            ...
            while let Ok(payload) = rx.recv() {
                ...
                let result = {
                    let _guard = client_support::stderr::stderr_lock();
