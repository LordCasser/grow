...
//! The default theme is GrowNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! GrowNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::GrowNight, ThemeKind::GrowDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::GrowNight => "grownight",
            Self::TokyoNight => "tokyonight",
            Self::GrowDay => "growday",
    ...
    /// when quantized to 256 or 16 colors. GrowNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::GrowNight => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "grownight" | "grow-night" | "dark" => Some(Self::GrowNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "growday" | "grow-day" | "light" | "day" => Some(Self::GrowDay),
...
/// `"grownight"` → `"Grow Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "grownight" => "Grow Night",
        "growday" => "Grow Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.grow/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::GrowNight => Self::grownight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::GrowDay => Self::growday(),
            ...
            // if reached, fall back to GrowNight.
            ThemeKind::Auto => Self::grownight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `GROW_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on grownight, erasing every state signal). We pin each
    ...
    /// theme (e.g., GrowNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // GrowNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    client_support::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::GrowNight.is_auto());
        assert!(!ThemeKind::GrowDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::growday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::grownight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::growday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. GrowNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::GrowNight => Theme::grownight(),
                ThemeKind::GrowDay => Theme::growday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_grownight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark GrowNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::grownight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("grownight"),
            Some(ThemeKind::GrowNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::GrowNight));
        assert_eq!(ThemeKind::from_name("growday"), Some(ThemeKind::GrowDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::GrowDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("grownight", ThemeKind::GrowNight),
            ("grow-night", ThemeKind::GrowNight),
            ("dark", ThemeKind::GrowNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("growday", ThemeKind::GrowDay),
            ("grow-day", ThemeKind::GrowDay),
            ("light", ThemeKind::GrowDay),
            ("day", ThemeKind::GrowDay),
