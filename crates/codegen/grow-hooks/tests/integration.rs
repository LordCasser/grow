//! Integration tests for grow-hooks.
//!
...
use grow_hooks::discovery::load_hooks;
use grow_hooks::dispatcher;
use grow_hooks::event::*;
use grow_hooks::result::HookDecision;
use grow_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        grow_hooks::result::HookRunResult::Success { .. }
    ));
}
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: grow_hooks::event::StopFailureKind::RateLimit,
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    assert!(
        ...
        "GROW_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed GROW_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "GROW_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed GROW_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "GROW_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
