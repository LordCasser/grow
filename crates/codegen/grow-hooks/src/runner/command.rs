...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("GROW_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = grow_config::shell::shell_command_argv(&command_str);
    ...
    let mut child = match cmd
        ...
        .env("GROW_HOOK_EVENT", envelope.hook_event_name.to_string())
...
///   hooks),
/// * the Grow process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from(
                "/project/.grow/hooks/bin/check.sh"
            ))
        );
        assert_eq!(
    ...
    fn make_scoped_ctx(scope: grow_tools::util::ProcessScope) -> RunContext<'static> {
        RunContext {
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.grow-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".grow-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.grow-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.grow-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.grow-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
    ...
    async fn command_hook_session_close_reaps_whole_group() {
        ...
        let envelope = make_envelope();
        let scope = grow_tools::util::ProcessScope::new();
