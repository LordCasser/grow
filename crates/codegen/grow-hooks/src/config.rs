...
pub use grow_config::HookProvenance;
...
pub fn parse_hooks_from_config_layers(
    layers: &[grow_config::HookConfigLayer],
) -> (Vec<HookSpec>, Vec<HookError>) {
    let home = grow_config::user_grow_home();
...
mod tests {
    ...
    fn config_layer(source_name: &str, toml_src: &str) -> grow_config::HookConfigLayer {
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let hooks = value.get("hooks").cloned().unwrap();
        grow_config::HookConfigLayer::new(grow_config::HookProvenance::Managed, source_name, hooks)
    }
    ...
    fn config_layers_additive_and_dedup_keeps_higher_authority() {
        let mk = |src: &str, prov, cmd: &str| {
            ...
            grow_config::HookConfigLayer::new(
                prov,
                src,
        ...
        use grow_config::HookProvenance::{Managed, User};
