...
fn load_hooks_from_directory(dir: &Path) -> (Vec<HookSpec>, Vec<HookError>) {
    ...
    // hooks still load. (Sandbox fail-closed listing lives in grow_config.)
    ...
    for entry in entries {
        ...
        if !grow_config::is_direct_hook_json_name(name) || !path.is_file() {
            continue;
        }
...
fn is_valid_hook_file(path: &Path) -> bool {
    ...
    grow_config::is_direct_hook_json_name(name) && path.is_file()
}
