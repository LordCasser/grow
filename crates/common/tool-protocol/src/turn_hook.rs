...
/// Default `session_relationship` value used by local session events.
...
pub struct AfterTurnPayload {
    ...
    /// the workspace decodes it into its `CancellationCategory` at emit time.
    /// `None` for non-cancelled turns.
