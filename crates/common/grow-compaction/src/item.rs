...
//! harness implements [`CompactionItem`] for its `GrowTurn`;
//! grow-build implements it for `grow_sampling_types::ConversationItem`.
//!
...
//! `<grow_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `GrowRole` (Grow chat) and the
/// `ConversationItem` variants (grow-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Grow chat) — maps to System on harnesses without a
...
/// extraction (`<grow_file id=".." name=".." />` lines in the
/// `<grow_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Grow chat's
    /// `GrowTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Grow chat: a
    ...
    /// strips their `<grow_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<grow_file>` lines in the
    /// `<grow_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Grow chat: a `Developer` turn with category
    ...
    /// requests/results (Grow chat: keep only `Channel` contents of an
    ...
    /// `max_tokens` (grow-build style: `max_bytes = max_tokens * 4`, prefix
...
/// constructs the typed harness items that make up grow-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Grow chat's
/// `GrowTurn`; adding these constructors to it as required methods would break
/// that impl. They are also grow-build-specific (Grow chat's tail-keep path
...
///
/// The grow-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Grow chat stores turns as `Arc<GrowTurn>`).
