...
//! stay in each product host (for example `grow-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `grow-sampling-types`. It is decoupled from both Grow chat and
//! grow-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — grow-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Grow chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Grow chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Grow chat and
//! grow-build; hosts still own snapshotting and host-only sections).
...
// grow-build's full-replace subsystem now lives under `code_compaction`;
