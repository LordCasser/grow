//! grow-build's full-replace compaction pass.
//!
//! grow-build does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `grow-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). grow-build keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (grow-build's
...
/// Run grow-build's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam grow-build's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // grow-build appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce grow-build's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_grow_build_history_shape() {
        let llm_turns = vec![
