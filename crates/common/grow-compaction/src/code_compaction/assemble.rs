//! Compacted-history assembly (grow-build's rebuild structure, generic).
//!
//! Moved from `grow-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! grow-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // grow-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn grow_build_order_recent_before_summary() {
