...
///
/// - `FullReplace` (default): grow-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from grow-shell's
...
/// time (token threshold only, matching grow-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as grow-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<grow_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "grow-4.20";
