...
//!                          that alone exceed budget (grow-build style:
...
//!   (one-shot, same `tokens * 4` budget as grow-build)
...
/// One-shot payload shrink (grow-build: `max_bytes = max_tokens * 4`).
///
...
///
/// Matches grow-build's focus: in-place truncate is for oversized tool
...
pub fn fit_turns_for_summarizer<T: CompactionItemBuilder>(
    ...
    // One-shot prefix clip on tool results only (grow-build style).
    ...
    // grow-build: when even the newest unit alone exceeds budget, keep it
...
///
/// Mirrors grow-build `truncate_text_to_bytes` / `truncate_item_to_tokens`:
