...
pub enum CompactionTarget {
    ...
    /// with a single summary — grow-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Grow chat's
/// `StreamProcessor` (`Item = Arc<GrowTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<GrowTurn>`) already satisfies it.
