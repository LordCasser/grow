...
//!
//! - Grow chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `GrowTurn::get_num_tokens`.
//! - grow-build estimates with `bytes / 4`.
//!
...
//! (Grow chat: `ItemTokenCounter<Arc<GrowTurn>>`), so exactly one mechanism
//! is in play.
