...
- 状态：已获批并删除；图片模块 142/142 回归通过；仓库外公共API使用未知。证据见 audit-image-viewer-entrypoints。
...
- 状态：已获批并删除；诊断库 57/57 回归通过，锁文件无依赖升级。公共库仓库外使用未知；证据见 audit-diagnostic-device-id-reachability。
...
- 状态：未启用缓存及断开开关已删除，保留取消安全的后台计算，正规化回归 40/40 通过，见 remove-inactive-image-normalization-cache。按 provider/model 选择原图或描述、视觉辅助/OCR 降级及可恢复失败提示已完成；完整核心回归 12313 项通过，最终图片回归 100/100，见 implement-session-image-fallback。
