...
GitHub Actions / ubuntu-24.04-arm
    ...
    ├── Python/C 两份实现分别自签，产物必须字节级一致
    └── .codesign/大小门禁 → grow-{version}-ohos-aarch64.tar.gz
```
...
Grow 2.0.1 只维护 GitHub Release 二进制渠道。OHOS 资产不嵌入 `rg`：运行时从 `PATH` 解析，
...
Grow 2.0.1 不维护 Harmonybrew、Homebrew 或其他包管理器 formula。仓库内旧 formula 已删除，避免与
