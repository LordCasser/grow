...
Goal detail exposes cache-hit input, cache-miss input and output from the same durable settlement as the budget. Total and budget both use full input plus output; reasoning is already included in output. Historical aggregate-only usage cannot recover cache reads or categories, so it remains an explicitly unclassified lower bound. Exact budget enforcement requires complete usage; unbudgeted work continues with incomplete usage. See the [Goal contract](../../openspec/specs/behavior-goal/spec.md).
