...
| `hook` | 每个真实 occurrence 先冻结事件、因果引用、配置 generation/provenance 与有序 handler plan；每个 run 恰好一个 finished/skipped 终态，最后以 gate-specific typed aggregate decision 闭合；恢复只记录 interrupted/outcome-unknown，绝不重跑外部副作用 |
| `input` | `Submitted` 只保存 content-addressed payload 引用；HumanIntent 必须经过一次 Hook admission，`AdmissionResolved` 原子提交 Allow/Block、初始 FIFO/exact-Turn route 和 queue-edit supersession；FIFO 的 `TurnStarted` 只预留 identity，`Consumed` 与 canonical user item 原子进入 Surface，host-plane 输入以 `Handled` 闭合；未消费预留在中断 Turn 关闭时释放，blocked input 永不进入 Surface |
...
| `image_projection` | Surface 投影事实；精确绑定触发 runtime、提交前 Surface revision、live SurfaceId、图片 fingerprint/count、逐 group 的 typed disposition、关联 Assistant tool call 以及同 response 的有序 `Reasoning/BackendToolCall` carriers；provider description 引用可验证的 ImageDescription Sideband，local OCR 声明其 engine，unsupported-model removal 要求 replacement 恰为 `当前模型不支持多模态，图片已经被删除`。description/OCR 保留原图并附加可复用文本，removal 把该 group 换成标准文本并只改变 materialized Surface，原 payload 仍是不可变 Timeline 证据；接受后推进 Surface revision 并不可逆改变所有模型可见载体 |
...
### Hook occurrence 与执行边界

Hook 的执行计划在 `Triggered` 时一次冻结。计划按确定顺序保存 handler identity、provenance、kind、failure policy 和 matcher/policy 结论；配置 reload 只能影响下一个 occurrence。每个计划项预先取得唯一 `run_id`：执行项必须经过 `RunStarted → RunFinished`，不执行项必须以 `RunSkipped` 闭合，最后才允许写入 gate-specific `Completed`。外部进程、HTTP 请求和 client callback 都不能早于各自 durable `RunStarted`；模型 admission、工具继续、路由和 stop 决定都不能越过 durable `Completed`。恢复只把未闭合 run 终止为 `InterruptedOutcomeUnknown`，不会重放可能已经产生副作用的 handler。

file 与 client handler 共同组成一条冻结的有序策略链。file handler 保持注册顺序串行执行，因为前一项可能改变后一项可见的项目状态；每个 project/agent handler 又必须在自己的执行边界重新验证 frozen source 与当前 Folder Trust identity。client callback 同样只有轮到自身时才写入 `RunStarted` 并发起请求。任一 handler 首次给出决定性 block 或 force-stop 后，所有后序 handler 都以 `PriorBlock` 跳过，不能再启动进程、HTTP、client request 或 observe notification；因此不存在“已经启动、再用 `Cancelled` 假装未执行”的外部副作用窗口。Observe occurrence 没有决定性控制结果，仍按冻结顺序执行全部匹配项。
...
- `rewind_surface(target)`：展开压缩和修剪的 shadow，沿最后一次 `Rewind` 选择出的分支重建未压缩历史，再在 `target` prompt 前截断。已接受的图片 shadow 会在新 Surface 生成前再次投影：description/OCR 仍保留原图，unsupported-model removal 仍然删除该 group。模型不能通过 rewind、fork、resume、切换模型或压缩恢复被文字描述取代或被删除的图片；durably 删除的图片只能由删除前的显式 rewind 或重新附图找回。原件只保留为不可变 Timeline 证据。
...
压缩只有一条准备、生成、提交管线。自动压缩主阈值默认是 80%，预压缩阈值从最终解析的主阈值减去 10 个百分点；主阈值不大于 10% 时只关闭预压缩，不增加独立配置。准备和提交都沿用 Step 结束、待处理控制生效之后、下一次采样之前的边界，不在响应流或工具执行中替换上下文。达到预压缩阈值先做 pre-prune，降到阈值以下就结束；仍有压力才记录 `Compaction::Started {mode: background}` 并冻结输入、目标范围、模型路由和 Control context 身份。每个 session 最多保留一个生成中或待提交的任务。后台只生成摘要并完成 Sideband，前台 Step/Turn 可以继续。结果就绪后在下一个安全边界提交；空闲期间就绪则保留到下一次采样前。达到主阈值时，已有任务记录 `Promoted` 并转前台等待，复用同一请求、重试进度和剩余 wall-clock 预算，不再准备、执行 PreCompact hook 或 memory flush；没有可用任务才走同步流程。
...
手动压缩、rewind、模型/Agent/Goal/Behavior 权威变更先取消旧任务，迟到结果不再发布。Stop、session 关闭和异常退出同时关闭后台任务、Sideband 与 Goal 结算；重启不恢复后台计算，也不自动应用尚未提交的摘要。崩溃恢复仍只把“恰有一条 summary 且恰有一条匹配 replacement”的事务补成 completed，其余补成 failed。后台普通失败不打断正在运行的前台，本 Turn 不反复预压缩；主阈值上的同步恢复仍走原错误分类，持久化、预算和权限不能 fail open。替换只遮蔽 Surface，原消息仍在 Timeline、branch transcript、session search 与 Trajectory 中。

Goal 结算屏障只放行同一有效执行代里仍在运行的后台摘要，已返回但未确认的结算、旧执行代、转前台或取消的请求仍必须等待。每次 provider attempt 继续归属启动时捕获的 Goal，逐次且唯一计费；额度耗尽或 usage 不完整仍关闭 admission。后台启动和结果就绪不发送前台 compacting 通知。未提升的成功提交先保留待发布提示，等下一次普通请求完成 request projection 后，才以该投影显示一次 `async compact` 生效提示并更新 token 用量；没有后继请求时不把仍带旧 request envelope 的 Surface-only 估算展示为最终值。提升后沿用同步开始、完成、失败和取消通知，前台等待耗时从提升开始计算。Trajectory 直接展示 Timeline 的后台 Started、可选 Promoted、摘要引用和终态，总耗时仍从最初启动计算，不新增事实日志。
...
`/btw` 在一个 ChatState 命令中冻结当前已提交 Surface 与输入引用，使用 `sampling-types::project_portable_history` 构造旁路历史：保留合法配对的工具调用、结果、附件和 Assistant 正文；连续工具尾部与部分完成批次中的已完成调用也保留。未返回的调用不生成悬空协议或虚构结果，reasoning 与 provider-native continuation 不进入该独立请求。旁路问题只追加到请求副本，overload 重试复用同一快照；冻结之后的新结果不会自动进入本次回答。见 [Side question 上下文契约](../../openspec/specs/model-sampling/spec.md#requirement-side-questions-preserve-completed-tool-evidence-from-their-frozen-context)。
...
Recap 请求复用 portable 历史投影保留完整工具尾部和部分批次中已完成的结果；过预算路径继续去掉 reasoning，并在裁剪前后检查配对，避免把已完成工具证据当作未完成尾部删除。契约见 [Recap 工具证据](../../openspec/specs/model-sampling/spec.md#requirement-recap-retains-completed-tool-evidence)。
...
### Active child follow-up 审计边界

当前 Grow 没有“父 Agent 向已经存活的 child 追加消息”的协议或工具。`SubagentEvent` 的闭集只有 spawn、query、cancel、inspection 与生命周期管理；child 创建时发送的单次 `SessionCommand::QueuePrompt` 是 spawn seed admission，不是 active-child follow-up，也不能据此声称后续消息已经被 child 接受。

| 契约 | 当前事实 | 结论 |
| --- | --- | --- |
| ownership 与 liveness 分开判断 | query/cancel 先用 `belongs_to_session` 过滤 owner，snapshot 再以 `is_running` 表达 liveness；但没有 follow-up admission 可以复用这两个判断 | follow-up 未覆盖 |
| `Accepted` 以 child durable admission 为屏障 | 没有 follow-up request、child Timeline message receipt 或 durable acknowledgement | 未覆盖 |
| 稳定 message id；区分 accepted/rejected/unconfirmed | 只有 spawn prompt 的一次性 UUID，没有 follow-up identity 或 delivery outcome | 未覆盖 |
| saturation、deadline、payload limit、channel closed 为 typed outcome | query/cancel 的 timeout/channel 行为不构成消息投递结果；没有 follow-up outcome 类型 | 未覆盖 |
| 不增加全局通道、不复用父 FIFO | 当前因为功能不存在而没有旁路；这是未来实现必须保持的边界，不是功能覆盖 | 边界成立 |

因此 v2.1.0 的稳定结论是：只保留现有 spawn seed 和查询/取消生命周期，不把 coordinator channel 的 `send`、父 Timeline 事件或父 FIFO 当成 child admission。若未来启动 follow-up，唯一可接受的 `Accepted` 屏障是带稳定 `message_id` 的消息已经原子进入 child 自己的 Timeline/admission；设计债务与 typed outcome 验收条件记录在 ROADMAP。
...
跨任务意见复用 Notification inbox：Received 保存原文引用，Consumed 同时携带 `ConversationItem::AgentMessage`，消费和模型输入不可分开提交。该 batch 保留 receipt/source/target/message/reply_to 与 prompt index；Timeline 校验正文 hash 和原收件身份。Surface 中是一个事实，三个 provider adapter 在请求时展开为完整的 `receive_agent_message` 调用/结果对，不注册执行工具，也不附人类权限证据。普通工具统计仍只计算模型发起的调用，压缩和 input_ref 继续使用 canonical 坐标。协议与恢复边界见 [消息契约](../../openspec/specs/session-timeline/spec.md#requirement-agent-opinion-consumption-preserves-source-and-exact-context) 和 [协调架构](local-coordination.md#父子消息与通信展示)。
