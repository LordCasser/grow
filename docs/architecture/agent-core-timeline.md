...
所有辅助模型调用统一为 Sideband 实体。request 必须在 provider emission 前 durable append，记录 purpose prompt、冻结的 `input_refs`、route、executor 与 output schema；每次调用前追加 attempt，成功追加 result+end，失败追加带错误的 end。输入引用只保存 Timeline 区间，不复制内容；当前实现只接受发起 session 自身已经存在的区间，跨实体引用必须先经过显式 resolver，不能伪装成本地范围。Sideband schema v2 使用严格嵌套 kind，自己写出的完整生命周期必须能够无损 replay；新建目录层级也属于 durability barrier。Trajectory 合并视图会校验独立 request 与父 spawn 的 purpose、input refs 和 initiator ref 一致。
