...
file handler 保持冻结的注册顺序串行执行，因为前一项可能改变后一项可见的项目状态；每个 project/agent handler 又必须在自己的执行边界重新验证 frozen source 与当前 Folder Trust identity。首次决定性 block 之后的 file 项以 `PriorBlock` 跳过。client decision callback 相互独立：先为整个 cohort 持久化 `RunStarted`，再并发请求；Prompt/Tool gate 收到首个决定性 block 后取消仍未完成的已启动 sibling，并以 `Cancelled` 逐项闭合，Stop gate 则等待全部响应并按原注册顺序聚合。file 的 force-stop 不再请求 client decision，但 Stop observer 仍收到通知；非 Stop sibling 以 `PriorBlock` 闭合。

`HookLifecycleProjection` 是 TUI、Trajectory 与 diagnostics 的共同读取模型。实时 `HookExecution`/`HookAnnotation` 只是在 durable `Completed` 之后按 `occurrence_id` 查询该投影得到的瞬态 transport，不写入 `updates.jsonl`，也不能再次触发 Notification Hook。handler 名称、typed outcome/control、skip reason 和准确 `elapsed_ms` 都来自 Timeline；实时投影丢失时可以重新查询，不能把 transport 缓存提升为第二事实源。child 内发生的 occurrence 只写 child Timeline，parent 只保留其自身的 subagent 生命周期，不复制 child Hook 细节。
