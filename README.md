...
当前源码版本为 `1.1.1`。完整配置参考 [config.example.toml](config.example.toml)，分主题文档见
...
版本级变更见 [1.1.1 release notes](crates/codegen/shell/changelogs/1.1.1.md)。
...
| 平台 | Release 资产 |
| --- | --- |
| macOS Apple Silicon | `grow-1.1.1-macos-aarch64.tar.gz` |
| macOS Intel | `grow-1.1.1-macos-x86_64.tar.gz` |
| Linux x86_64 | `grow-1.1.1-linux-x86_64.tar.gz` |
| Linux arm64 | `grow-1.1.1-linux-aarch64.tar.gz` |
| Linux riscv64 | `grow-1.1.1-linux-riscv64.tar.gz` |
| Linux x86_64（musl） | `grow-1.1.1-linux-x86_64-musl.tar.gz` |
| Linux arm64（musl） | `grow-1.1.1-linux-aarch64-musl.tar.gz` |
| Windows x86_64 | `grow-1.1.1-windows-x86_64.tar.gz` |
| Windows arm64 | `grow-1.1.1-windows-aarch64.tar.gz` |
...
GROW_VERSION=1.1.1 GROW_TOOLS_BUNDLE_RG_PATH="$(command -v rg)" \
  cargo build --locked --profile release-dist -p cli --bin grow
./target/release-dist/grow --version
```

release commit 完成且工作区干净后，可以用 `scripts/act-release.sh validate` 在本地校验 tag/version
契约；脚本会在缺少 `v1.1.1` 时只创建本地 tag，不会推送。

正式发布流程：

1. 提交版本、lockfile、release notes 和文档。
2. 创建并推送 annotated tag `v1.1.1`，不要提前创建公开 Release。
