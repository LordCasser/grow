...
- [Timing and timestamp macros](draft-deltas/developer-support/spec.md#requirement-timing-and-timestamp-macros)：timed 宏 SHALL 保留代码块返回值，裸调用返回 value 与毫秒耗时；log 分支在指定或默认 debug 级别记录时间，sync/async try 分支保留 Result 并为 Err 附加错误字段。
