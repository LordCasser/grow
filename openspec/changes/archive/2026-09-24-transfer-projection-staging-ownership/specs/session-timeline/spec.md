...
#### Scenario: Projection commit failure restores the staged window
- **WHEN** exact appends or the response projection fail through the retry boundary
- **THEN** the same staged untagged notifications and first-candidate anchor remain available for retry, and a later successful exact-key retry produces one FIFO-ordered projection without duplicates.
