...
- [ ] 4.3 按 E01–E15 执行可重复的场景评估，保存输入、工具轨迹、证据和未执行限制；核对无效等待、前提失效、交接、Sideband 阻塞和验收结果。
- [ ] 4.4 固定模型/effort/权限/工具/初始状态进行 baseline/candidate 配对比较，记录正确率、交付耗时、token、重复劳动与返工；根据证据接纳或修订候选，不虚报效率收益。

## 5. Archive after implementation

- [ ] 5.1 将所有 delta 场景与验证记录逐项对应，确认实现与文档已完成后执行 `openspec validate --all --strict --no-interactive`；未验证事项保持未完成。
- [ ] 5.2 运行 `openspec archive improve-dependency-driven-agent-prompts --yes` 合入规范，再执行全量 strict 与 archived 校验；保存结果，确认未提前合入未实施契约。
