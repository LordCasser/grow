...
本 change 最初按用户要求交付方案；2026-09-22 用户进一步授权专项实施 prompt，并要求与正在修改代码的 Agent 协调。当前实施范围为下述文案、相关测试和开发者说明，具体证据见 `verification.md`。保留 delta specs，不设置 `skip_specs: true`；真实模型行为与性能对照仍需独立验证，未完成前不归档、不提前合入主规范。
